    ...
    if (substr_count($url, 'youtube.com/watch?v=') == 1 || substr_count($url, 'youtu.be/') == 1 || $is_embed) {
        ...
        if ($video_id) {
            ...
            if ($start_sec || $end_sec) {
                $embed_html_code .= '<div class="read-topic"><span class="icon-block"><i class="fas fa-film"></i></span>' . (($start_sec && $end_sec) ? '<b>' . echo_time_minutes(($end_sec - $start_sec)) . '</b> VIDEO SLICE' : 'FROM <b>' . ($start_sec ? echo_time_minutes($start_sec) : 'START') . '</b> TO <b>' . ($end_sec ? echo_time_minutes($end_sec) : 'END') . '</b>') . ':</div>';
            }
