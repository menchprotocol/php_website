...
function echo_in_read($in, $common_prefix = null, $show_editor = false, $completion_rate = null, $recipient_en = false)
{
    ...
    if($has_time_estimate || $idea_count){
        $ui .= '<div class="pull-right montserrat show-max" style="width:144px;"><span style="width:55px; display: inline-block;">'.( $idea_count ? '<i class="fas fa-circle idea"></i><span style="padding-left:2px;">'.$idea_count.'</span>' : '' ).'</span>'.( $has_time_estimate ? '<i class="fas fa-clock"></i>'.echo_time_range($metadata) : '' ).'</div>';
    }
