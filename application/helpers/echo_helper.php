...
function echo_in_message_manage($ln)
{
    ...
    $ui .= '<li style="min-width:48px; display:inline-block;" class="'.require_activate_superpower(10989 /* PEGASUS */).'"><a class="btn btn-read edit-off" style="border:2px solid #FC1B44 !important;" href="/read/history/' . $ln['ln_id'] . '" target="_parent"><i class="fas fa-link iswhite"></i> '.echo_number($count_msg_trs[0]['totals']).'</a></li>';
...
function echo_radio_entities($parent_en_id, $child_en_id, $enable_mulitiselect){
    ...
    if(count($CI->config->item('en_ids_'.$parent_en_id))){
        ...
        foreach($CI->config->item('en_all_'.$parent_en_id) as $en_id => $m) {
            $ui .= '<a href="javascript:void(0);" onclick="radio_update('.$parent_en_id.','.$en_id.','.$enable_mulitiselect.')" class="list-group-item item-'.$en_id.' '.( $count>=$show_max ? 'extra-items-'.$parent_en_id.' hidden ' : '' ).( count($CI->READ_model->ln_fetch(array(
        ...
        foreach($CI->READ_model->ln_fetch(array(
            ...
            $ui .= '<a href="javascript:void(0);" onclick="radio_update('.$parent_en_id.','.$item['en_id'].','.$enable_mulitiselect.')" class="list-group-item item-'.$item['en_id'].' '.( $count>=$show_max ? 'extra-items-'.$parent_en_id.' hidden ' : '' ).( count($CI->READ_model->ln_fetch(array(
...
function echo_2level_entities($main_obj, $all_link_types, $link_types_counts, $all_shown, $link_field, $details_en, $addup_total_count, $display_field){
    ...
    foreach($all_link_types as $en_id => $m){
        ...
        if(!isset($en_all_detail[$en_id])){
            ...
            $rows .= '<td style="text-align: left;" class="'.( $show_in_advance_only ? require_activate_superpower(10989 /* PEGASUS */) : '' ).'">';
            $rows .= '<span class="icon-block" style="margin-left:8px;">'.$m['m_icon'].'</span>';
            $rows .= '<a href="/play/'.$en_id.'">'.$m['m_name'].'</a>';
            $rows .= '</td>';


            $rows .= '<td style="text-align: right;" class="'.( $show_in_advance_only ? require_activate_superpower(10989 /* PEGASUS */) : '' ).'">';
            if($display_field=='total_count'){
...
function echo_in($in, $in_linked_id = 0, $is_parent = false)
{
    ...
    //LINK TYPE
    $ui .= '<span class="icon-block-sm ln_type_' . $ln_id . require_activate_superpower(10989 /* PEGASUS */) . '"><span data-toggle="tooltip" data-placement="right" title="' . $en_all_4486[$in['ln_type_entity_id']]['m_name'] . ': ' . $en_all_4486[$in['ln_type_entity_id']]['m_desc'] . ' @'.$in['ln_type_entity_id'].'">' . $en_all_4486[$in['ln_type_entity_id']]['m_icon'] . '</span></span>';

    //LINK STATUS
    $ui .= '<span class="icon-block-sm ln_status_entity_id_' . $ln_id . ( in_array($in['ln_status_entity_id'], $CI->config->item('en_ids_7359')) ? require_activate_superpower(10989 /* PEGASUS */) : '' ) . '"><span data-toggle="tooltip" data-placement="right" title="'.$en_all_6186[$in['ln_status_entity_id']]['m_name'].' @'.$in['ln_status_entity_id'].': '.$en_all_6186[$in['ln_status_entity_id']]['m_desc'].'">' . $en_all_6186[$in['ln_status_entity_id']]['m_icon'] . '</span></span>';


    //Show Completion Marks based on Intent Link Type:
    $ui .= '<span class="in_assessment_' . $ln_id . require_activate_superpower(10989 /* PEGASUS */) . '" data-toggle="tooltip" data-placement="right" title="Completion Marks">'. echo_in_marks($in) .'</span>';
    ...
    //BLOG STATUS
    $ui .= '<span class="icon-block-sm in_status_entity_id_' . $in['in_id'] . ( in_array($in['in_status_entity_id'], $CI->config->item('en_ids_7355')) ? require_activate_superpower(10989 /* PEGASUS */) : '' ) . '"><span data-toggle="tooltip" data-placement="right" title="'.$en_all_4737[$in['in_status_entity_id']]['m_name'].': '.$en_all_4737[$in['in_status_entity_id']]['m_desc'].'">' . $en_all_4737[$in['in_status_entity_id']]['m_icon'] . '</span></span>';
    ...
    $ui .= '<div style="padding-left:5px;" class="pull-right inline-block '. require_activate_superpower(10989 /* PEGASUS */) .'">';
    ...
    $ui .= '&nbsp;<a href="/intents/' . $in['in_id'] . '" class="tree-badge-' . $in['in_id'] . ' badge badge-primary is_not_bg is_hard_link" style="display:inline-block; margin-right:-2px; width:40px; border:2px solid #FFDF0F !important;">' . $tree_count . '<i class="fas fa-angle-right"></i></a>';
...
function echo_caret($en_id, $m, $url_append){
    ...
    $ui = '<li class="nav-item dropdown '.require_activate_superpower(find_matching_superpowers($m['m_parents'])).'">';
...
function echo_en($en, $is_parent = false)
{
    ...
    if ($ln_id > 0) {
        ...
        if($en['ln_external_id'] > 0){
            if($en['ln_parent_entity_id']==6196){
                ...
                $ui .= '<span class="icon-block-sm '.require_activate_superpower(10989 /* PEGASUS */).'" data-toggle="tooltip" data-placement="right" title="Link External ID = '.$en['ln_external_id'].' [Messenger Profile]"><a href="/read/messenger_fetch_profile/'.$en['ln_external_id'].'" target="_blank"><i class="fas fa-project-diagram"></i></a></span>';
            } else {
                $ui .= '<span class="icon-block-sm '.require_activate_superpower(10989 /* PEGASUS */).'" data-toggle="tooltip" data-placement="right" title="Link External ID = '.$en['ln_external_id'].'"><i class="fas fa-project-diagram"></i></span>';
            }
        }
    }
    ...
    //STATUS
    $ui .= '<span class="icon-block-sm en_status_entity_id_' . $en['en_id'] . ( in_array($en['en_status_entity_id'], $CI->config->item('en_ids_7357')) ? require_activate_superpower(10989 /* PEGASUS */) : '' ).'"><span data-toggle="tooltip" data-placement="right" title="'.$en_all_6177[$en['en_status_entity_id']]['m_name'].' @'.$en['en_status_entity_id'].': '.$en_all_6177[$en['en_status_entity_id']]['m_desc'].'">' . $en_all_6177[$en['en_status_entity_id']]['m_icon'] . '</span></span>';
    ...
    //ICON SET
    $ui .= '<div class="pull-right inline-block '. require_activate_superpower(10989 /* PEGASUS */) .'">';
...
//data-toggle="tooltip" data-placement="top" title="'.$en_all_4527[$cache_en_id]['m_name'].'"
    ...
    foreach ($en_all_this as $en_id => $m) {
        ...
        $ui .= '<a class="dropdown-item '.( $is_set ? ' active ' : require_activate_superpower(find_matching_superpowers($m['m_parents'])) ).'" href="javascript:void();" '.( !$is_set ? 'onclick="update_dropdown('.$cache_en_id.','.$en_id.')' : '' ).'"><span class="icon-block-sm">'.$m['m_icon'].'</span>'.$m['m_name'].'</a>';
    }
