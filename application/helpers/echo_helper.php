...
function echo_en_stats_overview($cached_list, $report_name){
    ...
    foreach($cached_list as $group_en_id=>$people_group) {
        ...
        $inner_ui .= '<td style="text-align: right;"><a href="/links?ln_status_entity_id='.join(',', $CI->config->item('en_ids_7360')) /* Link Statuses Active */.'&ln_type_entity_id='.join(',', $CI->config->item('en_ids_4592')).'&ln_parent_entity_id=' . $group_en_id . '">' . number_format($child_links[0]['en__child_count'], 0) . '</a><i class="far fa-info-circle icon-block" data-toggle="tooltip" data-placement="top" title="'.$people_group['m_desc'].'"></i></td>';
        $inner_ui .= '</tr>';
...
function echo_in_setting($in_setting_en_id, $in_field_name){
    ...
    foreach ($CI->config->item('en_all_'.$in_setting_en_id) as $type_en_id => $in_type) {
        ...
        $ui .= '<td style="text-align: right;"><a href="/links?ln_type_entity_id=4250&in_status_entity_id=' . join(',', $CI->config->item('en_ids_7356')) . '&'.$in_field_name.'='.$type_en_id.'">'.number_format($in_count[0]['total_active_intents'], 0).'</a><i class="far fa-info-circle icon-block" data-toggle="tooltip" title="'.$in_type['m_desc'].'" data-placement="top"></i></td>';
        $ui .= '</tr>';
