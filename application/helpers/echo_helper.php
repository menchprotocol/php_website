...
function echo_in_read($in, $common_prefix = null, $show_editor = false, $completion_rate = null, $recipient_en = false)
{
    ...
    if($has_time_estimate || $idea_count){
        $ui .= '<div class="pull-right montserrat" style="width:136px;"><span style="width:53px; display: inline-block;">'.( $idea_count ? '<i class="fas fa-circle idea"></i><span style="padding-left:3px;" class="idea">'.$idea_count.'</span>' : '' ).'</span>'.( $has_time_estimate ? '<span class="grey">'.echo_time_range($metadata).'</span>': '' ).'</div>';
    }
...
function echo_time_range($metadata){
    ...
    return '<span title="Estimated Read Time Of '.( $metadata['in__metadata_min_seconds'] < $metadata['in__metadata_max_seconds'] ? echo_time_hours($metadata['in__metadata_min_seconds']).' - ' : '' ).echo_time_hours($metadata['in__metadata_max_seconds']).'" data-toggle="tooltip" data-placement="top">'.echo_time_hours(round(($metadata['in__metadata_min_seconds']+$metadata['in__metadata_max_seconds'])/2)).'</span>';
}
...
function echo_en($en, $is_parent = false, $extra_class = null, $control_enabled = false, $is_source = false)
{
    ...
    //ROW
    $ui = '<div class="list-group-item no-side-padding itemsource en-item object_saved saved_en_'.$en['en_id'].' en___' . $en['en_id'] . ( $ln_id > 0 ? ' tr_' . $en['ln_id'].' ' : '' ) . ( $is_parent ? ' parent-source ' : '' ) . ' '. $extra_class  . '" source-id="' . $en['en_id'] . '" en-status="' . $en['en_status_source_id'] . '" tr-id="'.$ln_id.'" ln-status="'.( $ln_id ? $en['ln_status_source_id'] : 0 ).'" is-parent="' . ($is_parent ? 1 : 0) . '">';
