    ...
    if(is_https_url($url)){
        ...
        if ((substr_count($url, 'youtube.com/watch') == 1) || substr_count($url, 'youtu.be/') == 1 || $is_embed) {
            ...
            if ($video_id) {
                ...
                //Header For Time
                if($end_time){
                    $embed_html_code .= '<div class="read-topic"><span class="img-block icon-block-xs"><i class="fas fa-clock"></i></span>WATCH SHORT CLIP FROM '.echo_time_hours($start_time, true).' to '.echo_time_hours($end_time, true).':</div>';
                }
