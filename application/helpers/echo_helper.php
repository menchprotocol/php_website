...
function echo_en($en, $is_parent = false, $extra_class = null, $control_enabled = false, $is_source = false)
{
    ...
    if(superpower_active(10939, true)){
        ...
        if($en__portfolios_count[0]['totals'] > 0){
            $child_counter .= '<span class="'.superpower_active(10939).' pull-right" '.( $show_toolbar ? 'style="margin-top: -23px;"' : '' ).'><span class="icon-block doright montserrat source" title="'.number_format($en__portfolios_count[0]['totals'], 0).' PORTFOLIO SOURCES">'.echo_number($en__portfolios_count[0]['totals']).'</span></span>';
            $child_counter .= '<div class="doclear">&nbsp;</div>';
        }
    }
