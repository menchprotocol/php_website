...
function echo_read_breadcrumbs($in_id){
    ...
    foreach ($recursive_parents as $grand_parent_ids) {
        foreach(array_intersect($grand_parent_ids, $list_ids) as $intersect) {
            ...
            $ui .= '<div class="previous_reads"><a href="javascript:void(0);" onclick="$(\'.previous_reads\').toggleClass(\'hidden\');" class="montserrat grey"><span class="icon-block">' . $en_all_2738[6205]['m_icon'] . '</span>'.count($breadcrumb_items).' Previous Reads</a></div>';
...
function echo_mench_stats(){
    ...
    $ui .= '<tr>';
    $ui .= '<td class="play MENCHcolumn1"><span class="play"><span class="icon-block">' . $en_all_2738[4536]['m_icon'] . '</span><span class="montserrat" title="'.number_format($play_coins[0]['total_coins'], 0).'">'.echo_number($play_coins[0]['total_coins']).'</span></span></td>';
    $ui .= '<td class="idea MENCHcolumn2"><span class="idea"><span class="icon-block">' . $en_all_2738[4535]['m_icon'] . '</span><span class="montserrat" title="'.number_format($idea_coins[0]['total_coins'], 0).'">'.echo_number($idea_coins[0]['total_coins']).'</span></span></td>';
    $ui .= '<td class="read MENCHcolumn3"><span class="read"><span class="icon-block">' . $en_all_2738[6205]['m_icon'] . '</span><span class="montserrat" title="'.number_format($read_coins[0]['total_coins'], 0).'">'.echo_number($read_coins[0]['total_coins']).'</span></span></td>';
    $ui .= '</tr>';
    $ui .= '<tr>';
    $ui .= '<td class="play MENCHcolumn1"><span class="icon-block show-max" style="margin-right: -8px;">&nbsp;</span><span class="montserrat" style="padding-left: 8px;">PLAYERS</span></td>';
    $ui .= '<td class="idea MENCHcolumn2"><span class="icon-block show-max" style="margin-right: -8px;">&nbsp;</span><span class="montserrat" style="padding-left: 8px;">IDEAS</span></td>';
    $ui .= '<td class="read MENCHcolumn3"><span class="icon-block show-max" style="margin-right: -8px;">&nbsp;</span><span class="montserrat" style="padding-left: 8px;">READS</span></td>';
    $ui .= '</tr>';
    $ui .= '</table>';
...
function echo_en($en, $is_parent = false)
{
    ...
    //PLAYER NAME
    $ui .= '<a href="/play/'.$en['en_id'] . '" class="montserrat '.extract_icon_color($en['en_icon']).' en_name_' . $en['en_id'] . '">' . $en['en_name'] . '</a>';
    ...
    foreach ($en__parents as $en_parent) {
        $ui .= '<span class="en_child_icon_' . $en_parent['en_id'] . '"><a href="/play/' . $en_parent['en_id'] . '" data-toggle="tooltip" title="' . $en_parent['en_name'] . (strlen($en_parent['ln_content']) > 0 ? ' = ' . $en_parent['ln_content'] : '') . '" data-placement="bottom">' . echo_en_icon($en_parent['en_icon']) . '</a></span>&nbsp;';
    }
    $ui .= ' </div>';
