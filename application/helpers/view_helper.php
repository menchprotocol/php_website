...
function view_i_time($i_stats, $give_right_space = false, $micro_sign = false){
    ...
    if(!$micro_sign && $i_stats['i___6170']>0){
        $ui .= ( $has_any_diff && !$micro_sign ? number_format(intval($i_stats['i___6169']), 0).'<span class="mid-range">-</span>' : '' ).number_format(intval($i_stats['i___6170']), 0).' idea'.view__s(intval($i_stats['i___6170'])).'<span class="mid-range">&middot;</span>';
    }
...
function view_i($x__type, $top_i__id = 0, $previous_i = null, $i, $control_enabled = false, $message_input = null, $focus_e = false, $completion_rate = null, $extra_class = null){
    ...
    $ui  = '<div '.( isset($i['x__id']) ? ' x__id="'.$i['x__id'].'" ' : '' ).' class="coin_cover '.( $focus_coin ? ' focus-coin col-md-8 col-11 ' : ' edge-coin col-md-4 col-6 ' ).' no-padding coin-12273 coin___12273_'.$i['i__id'].' '.( $has_sortable ? ' cover_sort ' : '' ).( isset($i['x__id']) ? ' cover_x_'.$i['x__id'].' ' : '' ).( $has_soft_lock ? ' not-allowed ' : '' ).' '.$extra_class.'" '.( $has_hard_lock ? ' title="'.$e___11035[$x__type]['m__title'].'" data-toggle="tooltip" data-placement="bottom" ' : ( $has_soft_lock ? ' title="'.$e___11035[$lock_notice]['m__title'].'" data-toggle="tooltip" data-placement="top" ' : '' ) ).'>';
