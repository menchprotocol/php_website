...
function view_i_tree_stats($i_stats, $noshow_idea, $hide_time, $hide_idea = true){
    ...
    //IDEAS
    if(!$noshow_idea){
        $ui .= '<span class="how_it_works '.( $hide_idea ? ' i_x_stats hidden ' : '' ).' inline-block '.extract_icon_color($e___13544[13629]['m__icon']).'" '.( !$cover_show ? 'style="min-width:100px;"' : '' ).' title="'.$e___13544[13629]['m__title'].'" data-toggle="tooltip" data-placement="top">'.( $i_stats['i___6169'] > 0 ? ( $has_idea ? ( $cover_show ? $e___13544[13629]['m__icon'].'&nbsp;' : '<span class="icon-block">'.$e___13544[13629]['m__icon'].'</span>' ) : '' ).( $has_idea ? view_number($i_stats['i___6169']).( $is_interactive ? '<span class="mid-range">-</span>'.view_number($i_stats['i___6170']) : '' ) : '&nbsp;' ) : '&nbsp;' ).'</span>';
    }
...
function view_i_note($x__type, $x, $note_e = false)
{
    ...
    if($note_e){
        ...
        //EMOJI
        $ui .= '<td class="table-btn"><span class="btn btn-grey" id="emoji_pick_id'.$x['x__id'].'" title="'.$e___11035[14038]['m__title'].'"><span class="icon-block">'.$e___11035[14038]['m__icon'].'</span></span></td>';
...
function view_i_x($i, $can_click, $common_prefix = null, $show_editor = false, $completion_rate = null)
{
    ...
    $ui .= '<div class="row">';
        $ui .= '<div class="col-sm col-md" title="'.$e___12467[$x_mode]['m__title'].' '.$completion_rate['completion_percentage'].'% COMPLETE">';
...
function view_i_note_list($x__type, $i_notes, $e_of_i, $show_empty_error = false, $show_headline = true){
    ...
    if($show_empty_error && !count($i_notes) && $e_of_i){
        ...
        $ui .= '<div class="msg alert alert-danger" role="alert"><span class="icon-block">&nbsp;</span>No '.ucwords(strtolower($e___4485[$x__type]['m__title'])).', be the first to post one</div>';
        $ui .= '</div>';
    }
    ...
    if(!in_array($x__type, $CI->config->item('n___12677')) && $e_of_i){
        ...
        //EMOJI
        $ui .= '<td class="table-btn"><span class="btn btn-grey" id="emoji_pick_type'.$x__type.'" data-toggle="tooltip" title="'.$e___11035[14038]['m__title'].' '.$e___11035[14038]['m__message'].'" data-placement="top"><span class="icon-block">'.$e___11035[14038]['m__icon'].'</span></span></td>';
