...
function view_coins(){
    ...
    foreach($CI->config->item('e___14874') as $e__id => $m) {
        ...
        $ui .= '<div class="col '.( $e__id==6255 ? ' col-12 col-lg-4 ' : ' col-6 col-lg-4 ' ).'" style="text-align: center; padding: 0 5px;">'.$m['m__cover'].'<div class="css__title '.extract_icon_color($m['m__cover']).'">'.number_format($count, 0).' '.$m['m__title'].'</div><div>'.$m['m__message'].'</div></div>';
    }
    $ui .= '</div>';
