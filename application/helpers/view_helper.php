    ...
    if(is_https_url($url)){
        ...
        if ((substr_count($url, 'youtube.com/watch') == 1) || substr_count($url, 'youtu.be/') == 1 || $is_embed) {
            ...
            if ($video_id) {
                ...
                //Header For Time
                if($end_time){
                    $embed_html_code .= '<div class="headline" style="padding-bottom: 0;"><span class="icon-block-xs"><i class="fas fa-cut"></i></span>'.round_minutes($end_time-$start_time).' MIN CLIP ('.view_time_hours($start_time, true).'<span class="mid-range">-</span>'.view_time_hours($end_time, true).')</div>';
                }
...
function view_i_list($i, $is_next, $user_e, $prefix_statement = null){
    ...
    $ui = '<div class="pull-left headline"><span class="icon-block">'.$e___12467[12273]['m_icon'].'</span>'.( strlen($prefix_statement) > 0 ? $prefix_statement : 'UP NEXT:' ).( $common_prefix ? ' '.$common_prefix : '' ).'</div>';
