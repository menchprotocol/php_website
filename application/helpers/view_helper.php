...
function view_i($x__type, $top_i__id = 0, $previous_i = null, $i, $message_input = null, $focus_e = false, $completion_rate = null, $extra_class = null, $is_first_incomplete = false){
    ...
    $clickable = !$has_any_lock && !$focus_coin && !$editing_enabled;
    $can_click = $clickable && $top_i__id && ($discovery_mode || $top_i__id);
    ...
    $ui .= '<td width="20%"><div>'.( $clickable && !$can_click ? '<a href="'.$href.'"><i class="fas fa-arrow-right"></i></a>' : '' ).'</div></td>';
    $ui .= '</tr></table>';
...
function view_e($x__type, $e, $extra_class = null, $source_of_e = false)
{
    ...
    //Source UI
    $ui  = '<div e__id="' . $e['e__id'] . '" '.( isset($e['x__id']) ? ' x__id="'.$e['x__id'].'" ' : '' ).' class="coin_cover no-padding coin___12274_'.$e['e__id'].' '.$extra_class.( $discovery_mode ? ' coinface-6255 coin-6255 coinface-12274 coin-12274 ' : ' coinface-12274 coin-12274  ' ).( $focus_coin ? ' focus-coin col-xl-4 col-lg-6 col-md-8 col-10 ' : ' edge-coin col-xl-2 col-lg-3 col-md-4 col-6 ' ).( $show_text_editor ? ' doedit ' : '' ).( $has_sortable ? ' cover_sort ' : '' ).( isset($e['x__id']) ? ' cover_x_'.$e['x__id'].' ' : '' ).( $has_soft_lock ? ' not-allowed ' : '' ).'">';
