...
function view_list_source_items($i, $x__creator, $x, $plain_no_html = false, $append_m = array()){
    ...
    if($plain_no_html){
        ...
        return '<div class="source-info"><span data-toggle="tooltip" data-placement="top" title="'.( count($append_m) ? $append_m['m__title'].( strlen($append_m['m__message']) ? ': '.$append_m['m__message'] : '' ) : '' ).'">'
            ...
            . '<span>'.( $x['x__type']==41949 ? '<div class="payment_box"><a href="https://www.google.com/maps/search/'.urlencode($x['e__title']).'" target="_blank" style="text-decoration:underline;" class="sub_note main__title">'.$x['e__title'].' <i class="far fa-external-link"></i></a></div>' : $x['e__title'] ) . ( strlen($x['x__message']) ? ':' : '' ) .'</span>'
