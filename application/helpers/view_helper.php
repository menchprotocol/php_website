...
function view_x($x, $is_parent_tr = false)
{
    ...
    //Transaction ID
    $ui .= '<div class="simple-line"><a href="/ledger?x__id='.$x['x__id'].'" data-toggle="tooltip" data-placement="top" title="'.$e___4341[4367]['m_name'].'" class="montserrat"><span class="icon-block">'.$e___4341[4367]['m_icon']. '</span>'.$x['x__id'].'</a></div>';
...
function view_i_x($i, $common_prefix = null, $show_editor = false, $completion_rate = null, $recipient_e = false, $extra_class = null)
{
    ...
    //Extra Stats
    $ui .= '<div class="montserrat handler_13509 hidden hideIfEmpty" style="padding:5px 0 5px 34px;">';

    if($i_stats['e_count']){
        $ui .= '<span style="width: 34px;" class="source inline-block">'.$e___12467[12274]['m_icon'].' '.$i_stats['e_count'].'</span>';
    }

    if($i_stats['i___13443']){
        $ui .= '<span style="width: 34px;" class="idea inline-block">'.$e___12467[12273]['m_icon'].' '.$i_stats['i___13443'].'</span>';
    }
