...
function view_i($x__type, $top_i__id = 0, $previous_i = null, $i, $control_enabled = false, $message_input = null, $focus_e = false, $completion_rate = null, $extra_class = null, $is_first_incomplete = false){
    ...
    if(!$discovery_mode && $superpower_10939){
        ...
        $message_tooltip = ' data-toggle="tooltip" data-placement="bottom" title="'.$messages.'" ';
    }
    ...
    //Title Cover
    $ui .= '<div class="cover-content" '.$message_tooltip.'>';
    ...
    //IDEAs and Time
    $view_i_time = view_i_time($i_stats);
    $ui .= '<div class="cover-text"><div class="'.( !$linkbar_visible ? ' coin-hover ' : '' ).' grey">' . ( $view_i_time ? $view_i_time : '&nbsp;' ) . '</div>'.( count($minter) ? '<div class="coin-hover grey mini-font">Minted <span title="'.$minter[0]['x__time'].' PST">'.view_time_difference(strtotime($minter[0]['x__time'])).' ago</span> by <a href="/@'.$minter[0]['e__id'].'"><u>'.$minter[0]['e__title'].'</u></a></div>' : '' ).'</div>';
