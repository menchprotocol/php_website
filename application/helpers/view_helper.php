...
function view_card_x($x, $has_x__reference = false)
{
    ...
    foreach ($CI->config->item('e___4341') as $e__id => $m) {
        ...
        if (in_array(6160, $m['m__following']) && intval($x[$e___32088[$e__id]['m__message']]) > 0) {
            ...
            //SOURCE
            foreach ($CI->Sources->read(array('e__id' => $x[$e___32088[$e__id]['m__message']])) as $focus_e) {
            ...
            //IDEA
            foreach ($CI->Ideas->read(array('i__id' => $x[$e___32088[$e__id]['m__message']])) as $focus_i) {
            ...
            //TRANSACTION
            if (!$has_x__reference) {
                foreach ($CI->Ledger->read(array('x__id' => $x[$e___32088[$e__id]['m__message']])) as $ref_x) {
...
function view_e_covers($x__type, $e__id, $page_num = 0, $append_card_icon = true)
{
    ...
    if (in_array($x__type, $CI->config->item('n___42377'))) {
        ...
        */
        ...
        $query_filters = array(
            'x__player' => $e__id,
            'x__type IN (' . njoin($x__type) . ')' => null, //DISCOVERY GROUP
        );
    ...
    if ($page_num > 0) {
        ...
        $limit = view_memory(6404, 11064);
        $query = $CI->Ledger->read($query_filters, $joins_objects, $limit, ($page_num - 1) * $limit, $order_columns);
        return $query;
        ...
        $query = $CI->Ledger->read($query_filters, $joins_objects, 1, 0, array(), 'COUNT(x__id) as totals');
...
function view_i_covers($x__type, $i__id, $page_num = 0, $append_card_icon = true, $headline_authors = array())
{
    ...
    if (in_array($x__type, $CI->config->item('n___42261'))) {
        ...
        $joins_objects = array('x__previous');
        $query_filters = array(
            'x__type IN (' . njoin($x__type) . ')' => null, //IDEA LINKS
            'x__next' => $i__id,
        );
        ...
        $query_filters = array(
            'x__type IN (' . njoin($x__type) . ')' => null, //DISCOVERIES
            'x__previous' => $i__id,
        );
    ...
    if ($page_num > 0) {
        ...
        $limit = view_memory(6404, 11064);
        return $CI->Ledger->read($query_filters, $joins_objects, $limit, ($page_num - 1) * $limit, $order_columns);
...
function view_instant_select($focus__id, $down_e__id = 0, $right_i__id = 0)
{
    ...
    $selection_ids = array();
    $selection_options = $CI->Ledger->read(array(
        'x__following' => $focus__id,
        'x__type IN (' . njoin(33337) . ')' => null, //SOURCE LINKS
...
function view_single_select_instant($cache_e__id, $selected_e__id, $access_level_i = 0, $show_title = true, $o__id = 0, $x__id = 0)
{
    ...
    if ($selected_e__id && !isset($e___this[$selected_e__id])) {
        ...
        foreach($CI->Ledger->read(array(
            'x__following IN (' . njoin($cache_e__id) . ')' => null, //SOURCE LINKS
...
function view_sync_links($str, $return_array = false, $save_i__id = 0)
{
    ...
    if (intval($save_i__id) > 0) {
        ...
        $player_e = superpower_unlocked();
        foreach ($CI->Ledger->read(array(
            'x__type IN (' . njoin(4736) . ')' => null, //Idea Message Links 3x
            'x__next' => $save_i__id,
        )) as $x) {
        ...
        $CI->Ideas->edit($save_i__id, array(
            'i__message' => trim($str),
            'i__cache' => $i__cache,
        ), true, $player_e['e__id']);
...
function view_i_nav($discovery_mode, $focus_i)
{
    ...
    foreach ($CI->config->item('e___' . ($discovery_mode ? 42877 : 31890)) as $x__type => $m) {
        ...
        if (!$coins_count[$x__type] && ($discovery_mode || in_array($x__type, $CI->config->item('n___12144')))) {
            continue;
        }
...
function view_card_i($x__type, $i, $previous_i = null, $target_i__hashtag = null, $focus_e__id = 0)
{
    ...
    $discovery_seg = (strtolower($CI->uri->segment(1)) != 'apps' && strlen($CI->uri->segment(2)) ? $CI->uri->segment(1) : false);
    ...
    if ($discovery_mode && $focus_i__hashtag && !$focus__node && $x__player) {
        foreach ($CI->Ideas->read(array(
            ...
            'i__type IN (' . njoin(7712) . ')' => null, //Input Choice
        )) as $focus_i) {
            $focus_i__or = $focus_i;
        }
    }
    ...
    if (!$is_cache && $x__player) {
        $discoveries = $CI->Ledger->read(array(
    ...
    if ($focus_i__or) {
        $ui .= '<div class="this_selector this_selector_' . $i['i__id'] . '" selection_i__id="' . $i['i__id'] . '"><span class="icon-block-sm">' . (count($CI->Ledger->read(array(
    ...
    foreach ($CI->Ledger->read(array(
        ...
        if ($focus__node && $x__player && $x__player != $creator['e__id']) {
            $followings = $CI->Ledger->read(array(
    ...
    if ($x__player) {
        ...
        //Fetch discovery
        $x_completes = $CI->Ledger->read(array(
            'x__type IN (' . njoin(6255) . ')' => null, //DISCOVERIES
            'x__player' => $x__player,
            'x__previous' => $i['i__id'],
        ), array('x__next'));
        ...
        if (in_array($i['i__type'], $CI->config->item('n___7712'))) {
            ...
            if (isset($_GET['process_pay']) && !count($x_completes)) {
                ...
                $cart_max = $CI->Ledger->read(array(
                    'x__type IN (' . njoin(42991) . ')' => null, //Active Writes
                    'x__next' => $i['i__id'],
                    'x__following' => 29651, //Cart Max Quantity
                ));
                $cart_min = $CI->Ledger->read(array(
                    'x__type IN (' . njoin(42991) . ')' => null, //Active Writes
                    'x__next' => $i['i__id'],
                ...
                if (filter_var($paypal_email, FILTER_VALIDATE_EMAIL) && count($total_dues) && $total_dues[0]['x__message'] > 0 && count($currency_types) == 1) {
                    ...
                    $digest_fees = count($CI->Ledger->read(array(
                        'x__type IN (' . njoin(42991) . ')' => null, //Active Writes
                        'x__next' => $i['i__id'],
            ...
            $x_responses = $CI->Ledger->read(array(
            ...
            if (in_array($i['i__type'], $CI->config->item('n___43002'))) {
                ...
                if ($i['i__type'] == 31794) {
                    ...
                    $has_time = count($CI->Ledger->read(array(
                        'x__type IN (' . njoin(42991) . ')' => null, //Active Writes
                        'x__next' => $i['i__id'],
    ...
    foreach ($CI->config->item('e___31904') as $x__type_target_bar => $m_target_bar) {
        ...
        if ($x__type_target_bar == 31770 && !$discovery_mode && $link_type_ui && $superpower_10939) {
            ...
            $reactions = $CI->Ledger->read(array(
...
function view_list_e($i, $plain_no_html = false)
{
    ...
    foreach ($CI->Ledger->read(array(
        'x__type IN (' . njoin(33602) . ')' => null, //Writer Links Active
        'x__next' => $i['i__id'],
        'x__following IN (' . njoin(42421) . ')' => null, //Featured Inputs
...
function view_card_e($x__type, $e, $extra_class = null)
{
    ...
    foreach ($CI->Ledger->read(array(
        'x__type IN (' . njoin(42777) . ')' => null, //Featured Profile
        'x__follower' => $e['e__id'],
    ...
    if (!$is_app && $access_level_e >= 1) {
        ...
        foreach ($CI->config->item('e___31963') as $x__type_target_bar => $m_target_bar) {
            ...
            } elseif ($x__type_target_bar == 42795 && $player_e && $player_e['e__id'] != $e['e__id'] && count($CI->Ledger->read(array(
                    'x__follower' => $e['e__id'],
    ...
    foreach ($CI->Ledger->read(array(
        'x__following IN (' . njoin(14036) . ')' => null, //Featured sources
        'x__follower' => $e['e__id'],
        'x__type IN (' . njoin(32292) . ')' => null, //SOURCE LINKS
