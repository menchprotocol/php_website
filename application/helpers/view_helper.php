...
function view_list_sources($x__creator, $x){
    ...
    return '<div class="source-info">'
        ...
        . '<div class="payment_box">'. ( in_array($x['e__id'], $this->config->item('n___33349')) ? '<a href="https://www.google.com/maps/search/'.urlencode($x['x__message']).'" target="_blank" style="text-decoration:underline;" class="sub_note css__title">'.$x['x__message'].'</a>' : '<div class="sub_note css__title">'.nl2br($x['x__message']).'</div>' ) . '</div>'
        . '</div>';
...
function view_card_e($x__type, $e, $extra_class = null)
{
    ...
    //Coin Cover
    $ui .= ( !$focus_card ? '<a href="'.$href.'"' : '<div' ).' class="'.($discovery_mode ? 'coinType12274 coinType6255' : 'coinType12274' ) . ' card_privacy_'.$e['e__access'].( !$e_of_e ? ' ready-only ' : '' ).' black-background-obs cover-link" '.( $has_valid_url ? 'style="background-image:url(\''.$e['e__cover'].'\');"' : '' ).'>';
