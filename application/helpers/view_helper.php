...
function view_card_i($x__type, $top_i__hashtag = 0, $previous_i = null, $i, $focus_e = false){
    ...
    foreach($CI->X_model->fetch(array(
        ...
        $ui .= '<div class="creator_headline"><a href="/@'.$creator['e__handle'].'"><span class="icon-block icon-block-img">'.view_cover($creator['e__cover']).'</span>'.$creator['e__title'].'<span class="grey mini-font">@'.$creator['e__handle'].'</span><span class="grey mini-font" title="'.date("Y-m-d H:i:s", strtotime($creator['x__time'])).'">'.view_time_difference($creator['x__time'], true).'</span></a></div>';
    }
