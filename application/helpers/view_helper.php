...
function view_radio_e($focus_id, $down_e__id = 0, $right_i__id = 0){
    ...
    foreach($CI->config->item('e___'.$focus_id) as $e__id => $m) {
        $ui .= '<div class="list-group-item main__title custom_ui_'.$focus_id.'_'.$e__id.' item-'.$e__id.' '.( in_array($e__id, $already_selected) ? ' active ' : '' ). '"><a href="javascript:void(0);" class="itemsetting" onclick="e_radio('.$focus_id.','.$e__id.','.( $multi_select ? 1 : 0 ).','.$down_e__id.','.$right_i__id.')">'.( strlen($m['m__cover']) ? '<span class="icon-block-xs change-results">'.$m['m__cover'].'</span>' : '' ).$m['m__title'].'</a></div>';
        $count++;
    }
