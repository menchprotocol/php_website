...
function view_card_i($x__type, $top_i__hashtag = 0, $previous_i = null, $i, $focus_e = false){
    ...
    if(!$discovery_mode && $x__id && isset($i['x__creator'])){
        ...
        foreach($CI->E_model->fetch(array(
            ...
            $ui .= '<div class="creator_headline"><a href="/@'.$creator['e__handle'].'"><span class="icon-block icon-block-img">'.view_cover($creator['e__cover']).'</span><b>'.$creator['e__title'].'</b><span class="grey mini-font mini-padded mini-frame">@'.$creator['e__handle'].'</span></a><span class="grey mini-font mini-padded" title="'.date("Y-m-d H:i:s", strtotime($i['x__time'])).' PST">'.view_time_difference($i['x__time'], true).'</span></div>';
        }
    }
