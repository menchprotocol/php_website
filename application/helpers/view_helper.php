    ...
    if(is_https_url($url)){
        ...
        if ((substr_count($url, 'youtube.com/watch') == 1) || substr_count($url, 'youtu.be/') == 1 || $is_embed) {
            ...
            if ($video_id) {
                ...
                //Header For Time
                if($end_time){
                    $embed_html_code .= '<div class="headline" style="padding-bottom: 0;"><span class="icon-block-xs"><i class="fas fa-cut"></i></span>'.view_time_hours($start_time, true).' TO '.view_time_hours($end_time, true).' = '.round_minutes($end_time-$start_time).' MIN</div>';
                }
...
function view_i_list($i, $is_next, $user_e, $prefix_statement = null){
    ...
    $ui .= '<a href="javascript:void(0);" onclick="$(\'.i_x_stats\').toggleClass(\'hidden\')" class="icon-block pull-right grey" title="'.$e___11035[13689]['m_title'].'" style="margin:-24px 5px 0 0;">'.$e___11035[13689]['m_icon'].'</a>';
