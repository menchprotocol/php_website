...
function view_show_more($see_more_type, $class, $href_link = null){
    ...
    return '<div class="coin_cover coin_reverse col-xl-2 col-lg-3 col-md-4 col-sm-6 col-12 no-padding '.$class.'">
...
function view_load_page_i($x__type, $page, $limit, $list_count, $extra_class = null)
{
    ...
    return '<div class="coin_cover coin_reverse col-xl-2 col-lg-3 col-md-4 col-sm-6 col-12 no-padding load-more '.$extra_class.'">
...
function view_coins(){
    ...
    foreach($CI->config->item('e___14874') as $e__id => $m) {
        $count++;
        $ui .= '<div class="coin_cover coin_cover_'.$count.' no-padding col-xl-2 col-lg-3 col-md-4 col-sm-6 col-12"><div class="cover-wrapper"><div class="bg'.$e__id.' coinType'.$e__id.' cover-link"><div class="cover-btn"><div class="the_counter css__title"><div class="min-height"><b class="css__title coin_count_'.$e__id.'">'.number_format(count_unique_coins($e__id), 0).'</b></div>'.$m['m__title'].'</div></div></div></div></div>';
    }
    $ui .= '</div>';
...
function view_i($x__type, $top_i__id = 0, $previous_i = null, $i, $message_input = null, $focus_e = false, $completion_rate = null, $extra_class = null, $is_first_incomplete = false){
    ...
    $ui  = '<div '.( isset($i['x__id']) ? ' x__id="'.$i['x__id'].'" ' : '' ).' class="coin_cover '.( $focus_coin ? ' focus-coin col-xl-4 col-lg-6 col-md-8 col-10 ' : ' edge-coin col-xl-2 col-lg-3 col-md-4 col-sm-6 col-12 ' ).( $parent_is_or ? ' doborderless ' : '' ).( $has_soft_lock ? ' soft_lock ' : '' ).' no-padding '.( $is_completed ? ' coin-6255 ' : ' coin-12273 ' ).' coin___12273_'.$i['i__id'].' '.( $has_sortable ? ' cover_sort ' : '' ).( isset($i['x__id']) ? ' cover_x_'.$i['x__id'].' ' : '' ).( $has_soft_lock ? ' not-allowed ' : '' ).' '.$extra_class.'" '.( $has_hard_lock ? ' title="'.$e___11035[$x__type]['m__title'].'" data-toggle="tooltip" data-placement="top" ' : ( $has_soft_lock ? ' title="'.$e___11035[$lock_notice]['m__title'].'" data-toggle="tooltip" data-placement="top" ' : '' ) ).'>';
...
function view_e($x__type, $e, $extra_class = null, $source_of_e = false)
{
    ...
    //Source UI
    $ui  = '<div e__id="' . $e['e__id'] . '" '.( isset($e['x__id']) ? ' x__id="'.$e['x__id'].'" ' : '' ).' class="coin_cover no-padding coin___12274_'.$e['e__id'].' '.$extra_class.( $discovery_mode ? ' coinface-6255 coin-6255 coinface-12274 coin-12274 ' : ' coinface-12274 coin-12274  ' ).( $focus_coin ? ' focus-coin col-xl-4 col-lg-6 col-md-8 col-10 ' : ' edge-coin col-xl-2 col-lg-3 col-md-4 col-sm-6 col-12 ' ).( $show_text_editor ? ' doedit ' : '' ).( $has_sortable ? ' cover_sort ' : '' ).( isset($e['x__id']) ? ' cover_x_'.$e['x__id'].' ' : '' ).( $has_soft_lock ? ' not-allowed ' : '' ).'">';
