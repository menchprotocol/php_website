...
function view_coin_line($href, $is_current, $o__type, $o__cover, $o__title, $x__message){
    return '<a href="'.( $is_current ? 'javascript:alert(\'You are here already!\');' : $href ).'" class="dropdown-item move_away css__title '.( $is_current ? ' active ' : '' ).'"><span class="icon-block">'.$o__type.'</span><span class="icon-block">'.$o__cover.'</span>'.$o__title.'<span class="pull-right inline-block">'.( $is_current ? '<i class="fas fa-map-marker"></i>' : '<i class="far fa-arrow-right"></i>' ).'</span>'.( strlen($x__message) ? '<div style="padding:5px 0 0 41px; font-size:0.89em; white-space: normal;">'.$x__message.'</div>' : '' ).'</a>';
}
        ...
        if($page_num == -1){
            ...
            if($x__type==12274){
                ...
                foreach($CI->X_model->fetch($query_filters, $join_objects, 10, 0, array(
                    ...
                    $ui .= view_coin_line('/@'.$source_e['e__id'], $source_e['e__id']==$current_e, view_cover(12274,$e___4593[$source_e['x__type']]['m__cover']), view_cover(12274,$source_e['e__cover']), $source_e['e__title'].view_coins_e(12273,  $source_e['e__id'], -1).view_coins_e(12274,  $source_e['e__id'], -1).view_coins_e(6255,  $source_e['e__id'], -1), view_x__message($source_e['x__message'],$source_e['x__type']));
                }
            } elseif($x__type==12273){
                //IDEAS
                ...
                foreach($CI->X_model->fetch($query_filters, $join_objects, 10, 0, array('x__spectrum' => 'DESC')) as $next_i) {
                    $ui .= view_coin_line('/i/i_go/'.$next_i['i__id'], $next_i['i__id']==$current_i, view_cover(12274,$e___4737[$next_i['i__type']]['m__cover']), view_cover(12273,$next_i['i__cover']), view_i_title($next_i).view_coins_i(12273,  $next_i, -1).view_coins_i(12274,  $next_i, -1).view_coins_i(6255,  $next_i, -1), view_x__message($next_i['x__message'],$next_i['x__type']));
                }
            } elseif($x__type==6255){
                //DISCOVERIES / IDEAS
                $e___4593 = $CI->config->item('e___4593'); //Transaction Types
                ...
                foreach($CI->X_model->fetch($query_filters, $join_objects, 10, 0, array('x__id' => 'DESC')) as $x_i) {
                    $ui .= view_coin_line('/i/i_go/'.$x_i['i__id'], $x_i['i__id']==$current_i, view_cover(12274,$e___4593[$x_i['x__type']]['m__cover']), view_cover(12273,$x_i['i__cover']), view_i_title($x_i).view_coins_i(12273,  $x_i, -1).view_coins_i(12274,  $x_i, -1).view_coins_i(6255,  $x_i, -1), view_x__message($x_i['x__message'],$x_i['x__type']));
                }
            }
            $ui .= '</div>';
            $ui .= '</div>';
    ...
    if($page_num == -1){
        ...
        if($x__type==12274){
            ...
            foreach($CI->X_model->fetch($query_filters, array('x__right'), 0, 0, array('x__spectrum' => 'ASC')) as $next_i) {
                ...
                $ui .= view_coin_line('/~'.$next_i['i__id'], $next_i['i__id']==$current_i, view_cover(12274,$e___4737[$next_i['i__type']]['m__cover']), view_cover(12273,$next_i['i__cover']), view_i_title($next_i).$message_tooltip.view_coins_i(12273,  $next_i, -1).view_coins_i(12274,  $next_i, -1).view_coins_i(6255,  $next_i, -1), view_x__message($next_i['x__message'],$next_i['x__type']));
            }
        } elseif($x__type==6255){
            //DISCOVERIES / SOURCES
            $e___4593 = $CI->config->item('e___4593'); //Transaction Types
