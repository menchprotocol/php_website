...
function view_single_select_instant($cache_e__id, $selected_e__id, $write_privacy_i = true, $show_full_name = true, $o__id = 0, $x__id = 0){
    ...
    */
    ...
    $ui .= '<span class="current_content">'.( isset($e___this[$selected_e__id]['m__cover']) ? '<span class="icon-block">'.$e___this[$selected_e__id]['m__cover'].'</span>'.( $show_full_name ?  $e___this[$selected_e__id]['m__title'] : '' ) : '' ).'</span>'.( $show_full_name ? '<span class="icon-block-xs"><i class="fal fa-angle-down"></i></span>' : '' );
    ...
    if($write_privacy_i){
        ...
        foreach($e___this as $e__id => $m) {
            ...
            $superpowers_required = array_intersect($CI->config->item('n___10957'), $m['m__following']);
            if(!count($superpowers_required) || superpower_unlocked(end($superpowers_required))){
                $ui .= '<a class="dropdown-item drop_item_instant_'.$cache_e__id.'_'.$o__id.'_'.$x__id.' main__title optiond_'.$e__id.'_'.$o__id.'_'.$x__id.' '.( $e__id==$selected_e__id ? ' active ' : '' ).'" href="javascript:void();" this_id="'.$e__id.'" onclick="update_instant_select('.$cache_e__id.', '.$e__id.', '.$o__id.', '.$x__id.', '.intval($show_full_name).')"><span class="icon-block">'.$m['m__cover'].'</span>'.$m['m__title'].( isset($e___11035[$e__id]) && strlen($e___11035[$e__id]['m__message']) ? '<span class="icon-block-xs" title="'.$e___11035[$e__id]['m__message'].'" data-toggle="tooltip" data-placement="top">'.$e___11035[11035]['m__cover'].'</span>' : '' ).'</a>';
            }
...
function view_media($media_url, $link){
    ...
    return '<div class="card_cover card_i_cover col-sm-4 col-6 no-padding"><a href="'.$link.'"><div class="square">'.$view_links['i__cache'].'</div></a></div>';
}
...
function view_card_e($x__type, $e, $extra_class = null)
{
    ...
    foreach($CI->X_model->fetch(array(
        ...
        if($focus_card && in_array($social_link['x__up'], $CI->config->item('n___32172'))){
