    ...
    if(is_https_url($url)){
        ...
        if ((substr_count($url, 'youtube.com/watch') == 1) || substr_count($url, 'youtu.be/') == 1 || $is_embed) {
            ...
            if ($video_id) {
                ...
                //Header For Time
                if($end_time){
                    $embed_html_code .= '<div class="headline" style="padding-bottom: 0;"><span class="icon-block-xs"><i class="fas fa-cut"></i></span>'.view_time_hours($start_time, true).'<span class="mid-range">-</span>'.view_time_hours($end_time, true).' = '.round_minutes($end_time-$start_time).' MIN</div>';
                }
