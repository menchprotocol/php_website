...
function view_coins_i($x__type, $i, $page_num = 0, $append_coin_icon = true){
    ...
    if($page_num < 0){
        ...
        $ui = '<div class="dropdown inline-block">';
        $ui .= '<button type="button" class="btn no-left-padding no-right-padding css__title" id="coingroup'.$x__type.'_'.$i['i__id'].'" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="'.number_format($count_query, 0).'">'.$coin_icon.$coin_count.'</button>';
...
function view_i($x__type, $top_i__id = 0, $previous_i = null, $i, $control_enabled = false, $message_input = null, $focus_e = false, $completion_rate = null, $extra_class = null, $is_first_incomplete = false){
    ...
    if($focus_coin){
        ...
        $ui .= '<div class="'.( !$linkbar_visible ? ' coin-hover ' : '' ).'">';
        $ui .= '<table class="coin_coins"><tr>';
        $ui .= '<td width="25%" class="center push_down"><div>'.view_coins_i(11019,  $i).'</div></td>';
        $ui .= '<td width="25%" class="center">'.view_coins_i(12273,  $i).'</td>';
        $ui .= '<td width="25%" class="center"><div>'.view_coins_i(12274,  $i).'</div></td>';
        $ui .= '<td width="25%" class="center push_down"><div class="i_reset_discoveries_'.$i['i__id'].'">'.view_coins_i(6255,  $i).'</div></td>';
        $ui .= '</tr></table>';
        $ui .= '</div>';
