...
function view_show_more($see_more_type, $class, $href_link = null){
    ...
    return '<div class="card_cover card_reverse col-xl-2 col-lg-3 col-md-2 col-sm-3 col-6 no-padding '.$class.'">
...
function view_e_covers($x__type, $e__id, $page_num = 0, $append_card_icon = true){
    ...
    $write_access_e = write_access_e($e__id);
    ...
    if($x__type==12274){
        ...
        $query_filters = array(
            ...
            'x__type IN (' . join(',', $CI->config->item('n___32292')) . ')' => null, //SOURCE LINKS
            'x__access IN (' . join(',', $CI->config->item('n___7359')) . ')' => null, //PUBLIC
            'e__access IN (' . join(',', $CI->config->item(( $write_access_e ? 'n___7358' /* ACTIVE */ : 'n___7357' /* PUBLIC/OWNER */  ))) . ')' => null,
        );
...
function view_card_i($x__type, $top_i__id = 0, $previous_i = null, $i, $focus_e = false){
    ...
    $member_e = superpower_unlocked();
    $write_access_i = ( $cache_app || $access_locked ? false : write_access_i($i['i__id']) );
    $user_input = $focus_e;
    ...
    $ui = '<div i__id="'.$i['i__id'].'" '.( $x__id ? ' x__id="'.$x__id.'" ' : '' ).' class="card_cover card_i_cover contrast_bg '.( $focus_card ? ' focus-cover slim_flat col-md-6 col-sm-8 col-10
     ' : ' edge-cover card_click_i col-md-2 col-sm-3 col-6 ' ).( $cache_app ? ' is-cache ' : '' ).( $followings_is_or ? ' doborderless ' : '' ).' no-padding '.( $discovery_mode ? ' coin-6255 ' : ' coin-12273 ' ).' card___12273_'.$i['i__id'].' '.( $has_sortable ? ' sort_draggable ' : '' ).( $x__id ? ' cover_x_'.$x__id.' ' : '' ).'">';
            ...
            if($x__type_top_bar==31770 && $link_type_ui && ($write_access_i || $link_creator)){
                ...
                if(is_array($CI->config->item('e___'.$focus_dropdown))){
                    foreach($CI->config->item('e___'.$focus_dropdown) as $e__id_dropdown => $m_dropdown) {
                        ...
                        } elseif($e__id_dropdown==13571 && $x__id > 0 && $write_access_i){
                        ...
                        } elseif($e__id_dropdown==28636 && $write_access_i && $x__id){
                        ...
                        } elseif($e__id_dropdown==26560 && isset($i['x__type']) && in_array($i['x__type'], $CI->config->item('n___32014')) && ($link_creator || $write_access_i)){
    ...
    $ui .= ( $click_locked ? '<div' . $locked_info : '<a href="'.$href.'"' ).' class="'.( $has_discovered && $discovery_mode ? 'coinType6255' : 'coinType12273' ).' black-background-obs cover-link">';
...
function view_card_e($x__type, $e, $extra_class = null)
{
    ...
    $write_access_e = ( $access_locked ? false :  write_access_e($e['e__id']) );
    $member_e = superpower_unlocked();
    $e___11035 = $CI->config->item('e___11035'); //NAVIGATION
    $superpower_10939 = superpower_active(10939, true);
    ...
    //Source UI
    $ui  = '<div e__id="' . $e['e__id'] . '" '.( isset($e['x__id']) ? ' x__id="'.$e['x__id'].'" ' : '' ).' class="card_cover card_e_cover contrast_bg no-padding card___12274_'.$e['e__id'].' '.$extra_class.( $is_app ? ' coin-6287 ' : '' ).( $has_sortable ? ' sort_draggable ' : '' ).( $discovery_mode ? ' coinface-6255 coin-6255 coinface-12274 coin-12274 ' : ' coinface-12274 coin-12274  ' ).( $focus_card ? ' focus-cover slim_flat col-md-6 col-sm-8 col-10 ' : ' edge-cover card_e_click col-md-2 col-sm-3 col-6 ' ).( $show_text_editor ? ' doedit ' : '' ).( isset($e['x__id']) ? ' cover_x_'.$e['x__id'].' ' : '' ).( $has_soft_lock ? ' not-allowed ' : '' ).'">';
        ...
        foreach($CI->config->item('e___31770') as $x__type1 => $m1){
            if(in_array($e['x__type'], $CI->config->item('n___'.$x__type1))){
                foreach($CI->X_model->fetch(array(
                    ...
                    $link_type_ui .= view_dropdown($x__type1, $e['x__type'], null, $write_access_e, false, $e['e__id'], $x__id);
                    $link_type_ui .= '</div></td>';
                }
                $link_type_id = $x__type1;
                break;
    ...
    if(!$cache_app && !$is_app) {
        ...
        foreach($CI->config->item('e___31963') as $x__type_top_bar => $m_top_bar) {
            ...
            } elseif($x__type_top_bar==6177 && ($write_access_e || $access_locked || $always_see || in_array($e['e__access'], $CI->config->item('n___32172')))){
...
function view_input($cache_e__id, $current_value, $s__id, $write_access_i, $tabindex = 0, $extra_large = false){

    $CI =& get_instance();
    $e___12112 = $CI->config->item('e___12112');
    $current_value = htmlentities($current_value);
    $name = 'input'.substr(md5($cache_e__id.$current_value.$s__id.$write_access_i.$tabindex), 0, 8);

    //Define element attributes:
    $attributes = ( $write_access_i ? '' : 'disabled' ).' spellcheck="false" tabindex="'.$tabindex.'" old-value="'.$current_value.'" id="input_'.$cache_e__id.'_'.$s__id.'" class="form-control 
    ...
    return '<span class="span__'.$cache_e__id.' '.( !$write_access_i ? ' edit-locked ' : '' ).'">'.$focus_element.'</span>';
...
function view_dropdown($cache_e__id, $selected_e__id, $btn_class = null, $write_access_i = true, $show_full_name = true, $o__id = 0, $x__id = 0){
