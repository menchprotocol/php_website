...
function view_select_single($cache_e__id, $selected_e__id, $write_privacy_i = true, $show_full_name = true, $o__id = 0, $x__id = 0){
    ...
    if(!$cache_app && !$discovery_mode && superpower_unlocked(10939)){
        ...
        if($x__id){
            foreach($CI->config->item('e___31770') as $x__type1 => $m1){
                if(in_array($i['x__type'], $CI->config->item('n___'.$x__type1))){
                    foreach($CI->X_model->fetch(array(
                        ...
                        $link_type_ui .= view_select_single($x__type1, $i['x__type'], $write_privacy_i, false, $i['i__id'], $x__id);
                        $link_type_ui .= '</div></td>';
                    }
            ...
            if(!$link_type_ui){
                ...
                $link_type_ui .= view_select_single(4593, $i['x__type'], false, false, $i['i__id'], $x__id);
                $link_type_ui .= '</div></td>';
            }
        }
        ...
        foreach($CI->config->item('e___31904') as $x__type_top_bar => $m_top_bar) {
            ...
            if($x__type_top_bar==31770 && $link_type_ui && (!$discovery_mode || $write_privacy_i)){
                ...
                $top_bar_ui .= view_select_single(4737, $i['i__type'], $write_privacy_i, false, $i['i__id']);
                $top_bar_ui .= '</div></td>';
                ...
                $top_bar_ui .= view_select_single(31004, $i['i__privacy'], $write_privacy_i, false, $i['i__id']);
                $top_bar_ui .= '</div></td>';
                ...
                $active_bars++;
                $top_bar_ui .= '<td><div class="'.( $always_see || in_array($i['i__privacy'], $CI->config->item('n___32172')) ? '' : 'show-on-hover' ).'">';
                $top_bar_ui .= view_select_single(42260, 0, $member_e, false, $i['i__id']);
                $top_bar_ui .= '</div></td>';
...
function view_card_e($x__type, $e, $extra_class = null)
{
    ...
    //Source UI
    $ui  = '<div e__id="' . $e['e__id'] . '" e__handle="' . $e['e__handle'] . '" e__privacy="' . $e['e__privacy'] . '" '.( isset($e['x__id']) ? ' x__id="'.$e['x__id'].'" x__privacy="'.$e['x__privacy'].'" ' : '' ).' class="card_cover card_e_cover contrast_bg no-padding s__12274_'.$e['e__id'].' '.$extra_class.( $is_app ? ' coin-6287 ' : '' ).( $has_sortable ? ' sort_draggable ' : '' ).( $discovery_mode ? ' coinface-6255 coin-6255 coinface-12274 coin-12274 ' : ' coinface-12274 coin-12274  ' ).( $focus_card ? ' focus-cover slim_flat col-sm-8 col-11 ' : ' edge-cover card_click_e col-md-4 col-6 ' ).( $show_text_editor ? ' doedit ' : '' ).( isset($e['x__id']) ? ' cover_x_'.$e['x__id'].' ' : '' ).( $has_soft_lock ? ' not-allowed ' : '' ).'">';
    ...
    if(!$cache_app && !$is_app && superpower_unlocked(10939)) {
            ...
            foreach($CI->config->item('e___31770') as $x__type1 => $m1){
                if(in_array($e['x__type'], $CI->config->item('n___'.$x__type1))){
                    foreach($CI->X_model->fetch(array(
                        ...
                        $link_type_ui .= view_select_single($x__type1, $e['x__type'], $write_privacy_e, false, $e['e__id'], $x__id);
                        $link_type_ui .= '</div></td>';
                    }
                    $link_type_id = $x__type1;
                    break;
        ...
        foreach($CI->config->item('e___31963') as $x__type_top_bar => $m_top_bar) {
            ...
            if($x__type_top_bar==31770 && $x__id && superpower_unlocked(13422)){
                ...
                $top_bar_ui .= view_select_single(6177, $e['e__privacy'], $write_privacy_e, false, $e['e__id']);
                $top_bar_ui .= '</div></td>';
    ...
    foreach($CI->X_model->fetch(array(
        ...
        if(in_array(4256 , $e___14036[$social_link['x__up']]['m__following'])){
            //We made sure not the current website:
            $social_url = 'href="'.$social_link['x__message'].'" target="_blank"';
        } elseif(in_array(32097 , $e___14036[$social_link['x__up']]['m__following'])){
            $social_url = 'href="mailto:'.$social_link['x__message'].'"';
        } elseif(in_array(42181 , $e___14036[$social_link['x__up']]['m__following'])){
            //Phone Number
        ...
        //Append to links:
        $featured_sources .= '<span class="'.( $focus_card ? 'icon-block-xs' : 'icon-block-xx' ).'">'.( $social_url && $focus_card ? '<a '.$social_url.' data-toggle="tooltip" data-placement="top" title="'.$info.'">'.$e___14036[$social_link['x__up']]['m__cover'].'</a>' : ( $focus_card ? '<a href="/@'.$e___14036[$social_link['x__up']]['m__handle'].'" data-toggle="tooltip" data-placement="top" title="'.$info.'">'.$e___14036[$social_link['x__up']]['m__cover'].'</a>' : '<span data-toggle="tooltip" data-placement="top" title="'.$info.'">'.$e___14036[$social_link['x__up']]['m__cover'].'</span>' ) ).'</span>';
