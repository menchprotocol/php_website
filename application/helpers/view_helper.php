    ...
    if(in_array($x__type, $CI->config->item('n___14311'))){
        ...
        foreach($CI->config->item('e___14418') as $x__type2 => $m) {
            ...
            if($x__type2==14468){
                ...
                $tab_ui .= '<textarea class="form-control msg note-textarea indifferent algolia_search new-note power_editor editing-mode emoji-input input_note_'.$x__type.'" note_type_id="' . $x__type . '" placeholder="'.$e___4485[$x__type]['m__title'].'..." style="margin:0 82px 0 41px; width:calc(100% - 82px);">'.$textarea_content.'</textarea>';
                ...
                //PREVIEW
                $tab_ui .= '<div class="list-group '.( $e_of_i ? ' editor_preview doabsolute ' : '' ).' editor_preview_'.$x__type.'">';
                foreach($i_notes as $i_note) {
...
function view_e($x__type, $e, $extra_class = null, $source_of_e = false, $common_prefix = null, $message_input = null)
{
    ...
    //Source UI
    $ui  = '<div e__id="' . $e['e__id'] . '" '.( isset($e['x__id']) ? ' x__id="'.$e['x__id'].'" ' : '' ).' class="coin_cover en-item col-md-4 col-6 no-padding e_line_'.$e['e__id'].' '.( $has_sortable ? ' cover_sort ' : '' ).( isset($e['x__id']) ? ' cover_x_'.$e['x__id'].' ' : '' ).( $has_soft_lock ? ' not-allowed ' : '' ).' '.$extra_class.'">';
