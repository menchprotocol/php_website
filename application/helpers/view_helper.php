    ...
    if(is_https_url($url)){
        ...
        if ((substr_count($url, 'youtube.com/watch') == 1) || substr_count($url, 'youtu.be/') == 1 || $is_embed) {
            ...
            if ($video_id) {
                ...
                //Header For Time
                if($end_time){
                    $embed_html_code .= '<div class="headline" style="padding-bottom: 0;" title="FROM '.round_minutes($start_time).' TO '.round_minutes($end_time).' MIN."><span class="icon-block-xs"><i class="fas fa-film"></i></span>'.round_minutes($end_time-$start_time).' MIN. CLIP FROM '.view_time_hours($start_time, true).' TO '.view_time_hours($end_time, true).':</div>';
                }
