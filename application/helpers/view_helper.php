...
function view_coins(){
    ...
    foreach($CI->config->item('e___14874') as $e__id => $m) {
        ...
        $ui .= '<div class="col-4" style="text-align: center"><p>'.$m['m__message'].'</p><span class="css__title '.extract_icon_color($m['m__cover']).'">'.$m['m__cover'].'&nbsp;'.number_format($count, 0).' <span class="block-if-mini">'.$m['m__title'].'</span></span></div>';
    }
    $ui .= '</div>';
