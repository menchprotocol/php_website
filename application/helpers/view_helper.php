...
function view_i_time($i_stats, $show_icon = false){
    ...
    $ui = null;
    $ui .= '<div class="css__title doupper grey inline-block" data-toggle="tooltip" data-placement="top" title="'.( $has_time_diff ? $i_stats['i___6169'].' - '.$i_stats['i___6170'].' '.$e___13544[12273]['m__title'].' Discovered Interactively' : $i_stats['i___6169'].' '.$e___13544[12273]['m__title'] ).'">';
        ...
        if ((substr_count($url, 'youtube.com/watch') == 1) || substr_count($url, 'youtu.be/') == 1 || $is_embed) {
            ...
            if ($video_id) {
                ...
                if($end_time){
                    ...
                    $embed_html_code .= '<div class="css__title subtle-line"><span class="icon-block-xs">'.$e___11035[13292]['m__icon'].'</span>'.( $seconds<60 ? $seconds.' SEC.' : round_minutes($seconds).' MIN' ).' <span class="inline-block">FROM '.view_time_hours($start_time, true).' TO '.view_time_hours($end_time, true).'</span></div>';
                }
