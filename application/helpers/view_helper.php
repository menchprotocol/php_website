...
function view_i_tree_stats($i_stats, $show_min = false){
    ...
    //TIME STATS
    $ui .= '<span class="icon-block grey">'.$e___13544[13292]['m_icon'].'</span><span class="inline-block grey" style="min-width:89px;">'.round_minutes($i_stats['i___6161']).( round_minutes($i_stats['i___6161']) != round_minutes($i_stats['i___6162']) ? '<span style="padding: 0 1px;">-</span>'.round_minutes($i_stats['i___6162']) : '' ).'&nbsp;MIN</span>';
    ...
    if(is_https_url($url)){
        ...
        if ((substr_count($url, 'youtube.com/watch') == 1) || substr_count($url, 'youtu.be/') == 1 || $is_embed) {
            ...
            if ($video_id) {
                ...
                //Header For Time
                if($end_time){
                    $embed_html_code .= '<div class="headline" style="padding-bottom: 0;" title="FROM '.round_minutes($start_time).' TO '.round_minutes($end_time).' MIN."><span class="icon-block-xs"><i class="fas fa-cut"></i></span>'.round_minutes($end_time-$start_time).' MIN ('.view_time_hours($start_time, true).' TO '.view_time_hours($end_time, true).')</div>';
                }
