...
function view_coins(){
    ...
    $ui .= '<div class="row justify-content list-coins" style="font-size: 1.8em; padding-bottom: 55px; line-height: 130%;"><div style="min-height:40px; width: 100%; text-align: center;"><b class="coin_count_x css__title">'.number_format($query[0]['totals'], 0).'</b></div><div class="css__title" style="text-align: center;">Transactions</div></div>';
...
function view_i($x__type, $top_i__id = 0, $previous_i = null, $i, $focus_e = false, $tree_progress = null, $extra_class = null){
    ...
    //Top action menu:
    $ui = '<div i__id="'.$i['i__id'].'" '.( $x__id ? ' x__id="'.$x__id.'" ' : '' ).' class="coin_cover '.( $focus_coin ? ' focus-coin col-md-8 col-12 ' : ' edge-coin coin_i_click col-md-4 col-6 ' ).( $parent_is_or ? ' doborderless ' : '' ).' no-padding '.( $is_completed ? ' coin-6255 ' : ' coin-12273 ' ).' coin___12273_'.$i['i__id'].' '.( $has_sortable ? ' cover_sort x_sort ' : '' ).( $x__id ? ' cover_x_'.$x__id.' ' : '' ).' '.$extra_class.'">';
    ...
    if(isset($i['x__message']) && strlen($i['x__message'])>0){
        ...
        if($e_of_i && !$discovery_mode) {
            //Can edit:
            $message_tooltip = '<a href="javascript:void(0);" onclick="load_message_27963(' . $i['i__id'] . ')" class="mini-font messages_4231_' . $i['i__id'] . '">' . (strlen($messages) ? $messages : '<i class="no-message"> </i>') . '</a>';
        } elseif($can_click){
...
function view_featured_source($x__source, $x){
    ...
    return '<div class="source-info">'
        ...
        . '<div class="payment_box">'. ( $x['e__id']==30976 /* Hack: Location loads with Google Maps */ ? '<a href="https://www.google.com/maps/search/'.urlencode($x['x__message']).'" target="_blank" style="text-decoration:underline;" class="sub_note css__title">'.$x['x__message'].'</a><div id="load_map" style="width:100%;height:200px;"></div><script>
