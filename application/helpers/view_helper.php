    ...
    if(is_https_url($url)){
        ...
        if ((substr_count($url, 'youtube.com/watch') == 1) || substr_count($url, 'youtu.be/') == 1 || $is_embed) {
            ...
            if ($video_id) {
                ...
                //Header For Time
                if($end_time){
                    $embed_html_code .= '<div class="headline" style="padding-bottom: 0; font-size:0.84em;"><span class="icon-block">'.$e___11035[13292]['m_icon'].'</span>'.round_minutes($end_time-$start_time).' MIN CLIP ('.view_time_hours($start_time, true).' TO '.view_time_hours($end_time, true).')</div>';
                }
