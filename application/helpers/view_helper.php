        ...
        if ((substr_count($url, 'youtube.com/watch') == 1) || substr_count($url, 'youtu.be/') == 1 || $is_embed) {
            ...
            if ($video_id) {
                ...
                if($end_time){
                    ...
                    $embed_html_code .= '<div class="grey css__title ignore-click" style="padding:5px 0 0 0; font-size:0.84em;"><span class="icon-block-xs">'.$e___11035[13292]['m__icon'].'</span>'.( $seconds<60 ? $seconds.' SEC.' : round_minutes($seconds).' MIN' ).' CLIP <span class="inline-block">('.view_time_hours($start_time, true).' TO '.view_time_hours($end_time, true).')</span></div>';
                }
...
function view_radio_e($parent_e__id, $child_e__id, $enable_mulitiselect, $show_max = 25){
    ...
    foreach($CI->config->item('e___'.$parent_e__id) as $e__id => $m) {
        $ui .= '<a href="javascript:void(0);" onclick="e_radio('.$parent_e__id.','.$e__id.','.$enable_mulitiselect.')" class="item'.extract_icon_color($m['m__icon']).' list-group-item css__title itemsetting item-'.$e__id.' '.( $count>=$show_max ? 'extra-items-'.$parent_e__id.' hidden ' : '' ).( in_array($e__id, $already_selected) ? ' active ' : '' ). '"><span class="icon-block">'.$m['m__icon'].'</span>'.$m['m__title'].'<span class="change-results"></span></a>';
        $count++;
    }
    ...
    if(in_array($x__type, $CI->config->item('n___14311'))){
        ...
        foreach($CI->config->item('e___14418') as $x__type2 => $m) {
            ...
            if($x__type2==14468){
                ...
                $tab_ui .= '<div class="power-editor-'.$x__type.' indifferent">';
                $tab_ui .= '<textarea class="form-control msg note-textarea algolia_search new-note power_editor doabsolute emoji-input input_note_'.$x__type.'" note_type_id="' . $x__type . '" placeholder="DESCRIBE IDEA..." style="margin:0 82px 0 41px; width:calc(100% - 82px);" current_text="'.trim($textarea_content).'">'.$textarea_content.'</textarea>';
...
function view_e_settings($list_id, $show_accordion){
    ...
    foreach($CI->config->item('e___'.$list_id) as $acc_e__id => $acc_detail) {
        ...
        if(strlen($acc_detail['m__message']) > 0){
            $tab_ui .= '<div class="msg">' . $acc_detail['m__message'] . '</div>';
        }
        ...
        if ($acc_e__id == 12289) {
            ...
            foreach($CI->config->item('e___12279') as $x__type3 => $m3) {
                ...
                $tab_ui .= '<a href="javascript:void(0);" onclick="e_avatar(\'' . $avatar_icon_parts[0] . '\', \'' . $avatar_icon_parts[1] . '\')" icon-css="' . $avatar_icon_parts[1] . '" class="col-4 col-sm-3 col-md-2 list-group-item avatar-item avatar-type-'.$avatar_icon_parts[0].' avatar-name-'.$avatar_icon_parts[1].' ' .( $avatar_type_match ? '' : ' hidden ' ). ( $avatar_type_match && $e__icon_parts[1] == $avatar_icon_parts[1] ? ' active ' : '') . '"><div class="avatar-icon">' . $m3['m__icon'] . '</div></a>';
