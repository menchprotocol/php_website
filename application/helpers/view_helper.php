...
function view_i_list($in_my_x, $i, $is_next, $user_e, $list_title = null, $index_id = 99 /* Locked */){
    ...
    foreach($is_next as $key => $next_i){
        ...
        $ui .= view_i_x($next_i, ( $in_my_x ? $counter : $index_id ), ($is_featured || $completion_rate['completion_percentage'] > 0), $common_prefix, false, $completion_rate);
...
function view_x_progress($completion_rate, $i, $show_micro){
    ...
    if($completion_rate['steps_total'] < 55 && !$show_micro){
        ...
        $ui .= '<div class="progress-bg-list '.( $show_micro ? ' progress-micro ' : '' ).'"><div class="progress-done" style="width:'.$completion_rate['completion_percentage'].'%"></div></div>';
