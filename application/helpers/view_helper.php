...
function view_e_covers($x__type, $e__id, $page_num = 0, $append_card_icon = true){
    ...
    if($page_num > 0){
        ...
        if($append_card_icon){
            ...
            $ui .= '<button type="button" class=" no-left-padding no-right-padding main__title load_e_covers button_of_'.$e__id.'_'.$x__type.'" id="card_e_group_'.$x__type.'_'.$e__id.'" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" load_x__type="'.$x__type.'" load_e__id="'.$e__id.'" load_counter="'.$count_query.'" load_first_segment="'.$first_segment.'"><span title="'.$title_desc.'" data-toggle="tooltip" data-placement="top">'.$card_icon.$visual_counter.'</span></button>';
...
function view_card_i($x__type, $top_i__hashtag = 0, $previous_i = null, $i, $focus_e = false){
    ...
    if(!$cache_app && !$discovery_mode){
        foreach($CI->config->item('e___31904') as $x__type_top_bar => $m_top_bar) {
            ...
            if($x__type_top_bar==31770 && $link_type_ui && (!$discovery_mode || $write_privacy_i)){
                ...
                $top_bar_ui .= view_dropdown(4737, $i['i__type'], $write_privacy_i, false, $i['i__id']);
                $top_bar_ui .= '</div></td>';
                ...
                $top_bar_ui .= view_dropdown(31004, $i['i__privacy'], $write_privacy_i, false, $i['i__id']);
                $top_bar_ui .= '</div></td>';
                ...
                $active_bars++;
                $top_bar_ui .= '<td><div class="'.( $always_see || in_array($i['i__privacy'], $CI->config->item('n___32172')) ? '' : 'show-on-hover' ).'">';
                $top_bar_ui .= view_dropdown(42260, 0, $member_e, false, $i['i__id']);
                $top_bar_ui .= '</div></td>';
                ...
                if(is_array($CI->config->item('e___'.$focus_dropdown))){
                    foreach($CI->config->item('e___'.$focus_dropdown) as $e__id_dropdown => $m_dropdown) {
                        ...
                        if($e__id_dropdown==12589 && $write_privacy_i){
                            ...
                            $action_buttons .= '<a href="javascript:void();" this_id="'.$i['i__privacy'].'" onclick="update_dropdown(31004, 6182, '.$i['i__id'].', '.$x__id.', 0)" class="dropdown-item dropi_31004_'.$i['i__id'].'_'.$x__id.' main__title optiond_6182_'.$i['i__id'].'_'.$x__id.'">'.$anchor.'</a>';
                ...
                if($action_buttons && $focus_dropdown>0){
                    ...
                    $top_bar_ui .= '<button type="button" class=" no-left-padding no-right-padding main__title" id="action_menu_i_'.$i['i__id'].'" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="'.$e___14980[$focus_dropdown]['m__title'].'">'.$e___14980[$focus_dropdown]['m__cover'].'</button>';
    ...
    if(!$discovery_mode && $x__id && isset($i['x__creator'])){
        ...
        foreach($CI->E_model->fetch(array(
            ...
            $ui .= '<div class="creator_headline"><a href="/@'.$creator['e__handle'].'"><span class="icon-block icon-block-img">'.view_cover($creator['e__cover']).'</span><b>'.$creator['e__title'].'</b><span class="grey mini-font mini-padded mini-frame">@'.$creator['e__handle'].'</span></a><span class="grey mini-font mini-padded mini-frame" title="'.date("Y-m-d H:i:s", strtotime($i['x__time'])).' PST">'.view_time_difference($i['x__time'], true).'</span></div>';
        }
    }
...
function view_card_e($x__type, $e, $extra_class = null)
{
    ...
    if(!$cache_app && !$is_app) {
        ...
        foreach($CI->config->item('e___31963') as $x__type_top_bar => $m_top_bar) {
            ...
            if($x__type_top_bar==31770 && $x__id && superpower_unlocked(13422)){
                ...
                $top_bar_ui .= view_dropdown(6177, $e['e__privacy'], $write_privacy_e, false, $e['e__id']);
                $top_bar_ui .= '</div></td>';
                ...
                if($focus_dropdown>0 && is_array($CI->config->item('e___'.$focus_dropdown))){
                    foreach($CI->config->item('e___'.$focus_dropdown) as $e__id_dropdown => $m_dropdown) {
                        ...
                        if($e__id_dropdown==4997){
                            ...
                            //Delete Permanently
                            $action_buttons .= '<li><hr class="dropdown-divider"></li>';
                            $action_buttons .= '<a href="javascript:void();" this_id="'.$e['e__privacy'].'" onclick="update_dropdown(6177, 6178, '.$e['e__id'].', '.$x__id.', 0)" class="dropdown-item dropi_6177_'.$e['e__id'].'_'.$x__id.' main__title optiond_6178_'.$e['e__id'].'_'.$x__id.'">'.$anchor.'</a>';
                ...
                if($action_buttons && $focus_dropdown>0){
                    ...
                    $top_bar_ui .= '<td><div class="'.( $always_see ? '' : 'show-on-hover' ).'">';
                    $top_bar_ui .= '<div class="dropdown inline-block">';
                    $top_bar_ui .= '<button type="button" class=" no-left-padding no-right-padding main__title" id="action_menu_e_'.$e['e__id'].'" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="'.$e___14980[$focus_dropdown]['m__title'].'">'.$e___14980[$focus_dropdown]['m__cover'].'</button>';
  ...
  <button type="button" class=" dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-expanded="true">
...
function view_dropdown($cache_e__id, $selected_e__id, $write_privacy_i = true, $show_full_name = true, $o__id = 0, $x__id = 0){
    ...
    */
    ...
    if($write_privacy_i){
        ...
        foreach($e___this as $e__id => $m) {
            ...
            $superpowers_required = array_intersect($CI->config->item('n___10957'), $m['m__following']);
            if(!count($superpowers_required) || superpower_unlocked(end($superpowers_required))){
                $ui .= '<a class="dropdown-item dropi_'.$cache_e__id.'_'.$o__id.'_'.$x__id.' main__title optiond_'.$e__id.'_'.$o__id.'_'.$x__id.' '.( $e__id==$selected_e__id ? ' active ' : '' ).'" href="javascript:void();" this_id="'.$e__id.'" onclick="update_dropdown('.$cache_e__id.', '.$e__id.', '.$o__id.', '.$x__id.', '.intval($show_full_name).')" title="'.$m['m__message'].'"><span class="icon-block">'.$m['m__cover'].'</span>'.$m['m__title'].'</a>';
            }
