...
function view_i_tree_stats($i_stats, $noshow_idea, $hide_time, $hide_idea = true){
    ...
    $cover_show = (!$hide_time && !$hide_idea && !$noshow_idea);
    ...
    //IDEAS
    if(!$noshow_idea){
        $ui .= '<span class="how_it_works '.( $hide_idea ? ' i_x_stats hidden ' : '' ).' inline-block '.extract_icon_color($e___13544[13629]['m_icon']).'" '.( !$cover_show && !$hide_idea ? 'style="min-width:100px;"' : '' ).' title="'.$e___13544[13629]['m_title'].'" data-toggle="tooltip" data-placement="top">'.( $i_stats['i___6169'] > 0 ? ( $has_idea ? ( $cover_show ? $e___13544[13629]['m_icon'].'&nbsp;' : '<span class="icon-block">'.$e___13544[13629]['m_icon'].'</span>' ) : '' ).( $has_idea ? view_number($i_stats['i___6169']).( $is_interactive ? '<span class="mid-range">-</span>'.view_number($i_stats['i___6170']) : '' ) : '&nbsp;' ) : '&nbsp;' ).'</span>';
    }


    //TIME STATS
    if($i_stats['i___6161'] > 0){
        $ui .= '<span class="inline-block grey '.( $hide_time ? ' i_x_stats hidden ' : '' ).'">';
        $ui .= ( $cover_show ? '&nbsp;&nbsp;'.$e___13544[13292]['m_icon'].' ' : '<span class="icon-block">'.$e___13544[13292]['m_icon'].'</span>' );
...
function view_mench_coins(){
    ...
    foreach($CI->config->item('e___12467') as $e__id => $m) {
        ...
        $ui .= '<div class="col-4"><span class="montserrat '.extract_icon_color($m['m_icon']).'" title="'.number_format($count, 0).' '.$m['m_title'].': '.$m['m_message'].'" data-toggle="tooltip" data-placement="top">'.$m['m_icon'].'&nbsp;'.view_number($count).'</span></div>';
    }
    $ui .= '</div>';
    $ui .= '</div>';
    $ui .= '</div>';
...
function view_coins_e($x__type, $e__id, $page_num = 0, $append_coin_icon = true){
    ...
    if($page_num > 0){
        ...
        if($append_coin_icon){
            ...
            return ( $count_query[0]['totals'] > 0 ? '<span class="montserrat '.extract_icon_color($e___12467[$x__type]['m_icon']).'" title="'.number_format($count_query[0]['totals'], 0).' '.$e___12467[$x__type]['m_title'].'" data-toggle="tooltip" data-placement="top">'.$e___12467[$x__type]['m_icon'].'&nbsp;'.view_number($count_query[0]['totals']).'</span>' : null);
        } else {
...
function view_i_list($in_my_x, $i, $is_next, $user_e, $list_title = null, $index_id = 99 /* Locked */){
    ...
    foreach($is_next as $key => $next_i){
        ...
        $ui .= view_i_x($next_i, ( $in_my_x ? $counter : $index_id ), ( $completion_rate['completion_percentage'] > 0 ), $common_prefix, false, $completion_rate);
...
function view_i_note_list($x__type, $i_notes, $e_of_i, $show_empty_error = false){
