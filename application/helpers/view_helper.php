...
function view_coin_line($href, $is_current, $o__type, $o__cover, $o__title, $x__message){
    return '<a href="'.( $is_current ? 'javascript:alert(\'You are here already!\');' : $href ).'" class="dropdown-item move_away css__title '.( $is_current ? ' active ' : '' ).'">'.$o__type.'<span class="icon-block">'.$o__cover.'</span>'.$o__title.'<span class="pull-right inline-block">'.( $is_current ? '<i class="fas fa-map-marker"></i>' : '<i class="far fa-arrow-right"></i>' ).'</span>'.( strlen($x__message) ? '<div style="padding:5px 0 0 41px; font-size:0.89em; white-space: normal;">'.$x__message.'</div>' : '' ).'</a>';
}
