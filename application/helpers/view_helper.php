...
function view_coins(){
    ...
    foreach($CI->config->item('e___14874') as $e__id => $m) {
        ...
        $ui .= '<b class="css__title large_cover zq'.$e__id.' coin_count_'.$e__id.'">'.number_format(count_unique_coins($e__id), 0).'</b>';
        $ui .= '<div class="css__title zq'.$e__id.'">'.$m['m__title'].'</div>';
        $ui .= '</div>';
    }
    $ui .= '</div>';
