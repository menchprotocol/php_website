...
function view_i_tree_stats($i_stats, $show_min = false){
    ...
    //Variable time range:
    $ui = '<span class="montserrat doupper '.( $is_interactive ? ' underdot ' : '' ).'" style="padding-right:8px;" '.( $is_interactive ? ' data-toggle="tooltip" data-placement="right" title="Number of ideas discovered & total duration depends on the interactive choices you make along the way 🔀" ': '' ).'>';

    //IDEA STATS
    $ui .= '<span class="icon-block">'.( $has_idea ? ( $is_interactive ? '<i class="fas fa-random idea"></i>' : $e___13544[13629]['m_icon'] ) : '&nbsp;' ).'</span><span class="inline-block '.extract_icon_color($e___13544[13629]['m_icon']).'" style="'.( $show_min ? 'min-width:70px;' : 'min-width:89px;' ).'">'.( $has_idea ? view_number($i_stats['i___6169']).( $is_interactive ? '<span style="padding: 0 2px;">-</span>'.view_number($i_stats['i___6170']) : '' ).( $show_min ? '' : '&nbsp;IDEA'.view__s($i_stats['i___6170']) ) : '&nbsp;' ).'</span>';


    //TIME STATS
    $ui .= '<span class="icon-block">'.$e___13544[13292]['m_icon'].'</span><span class="inline-block '.extract_icon_color($e___13544[13292]['m_icon']).'">'.round_minutes($i_stats['i___6161']).( round_minutes($i_stats['i___6161']) != round_minutes($i_stats['i___6162']) ? '<span style="padding: 0 2px;">-</span>'.round_minutes($i_stats['i___6162']) : '' ).'&nbsp;MIN</span>';
...
function view_i_cover($x__type, $i, $show_editor, $extra_class = null, $message_input = null){
    ...
    $ui  = '<div '.( isset($i['x__id']) ? ' x__id="'.$i['x__id'].'" ' : '' ).' class="i_class_'.$x__type.'_'.$i['i__id'].' no-padding big-cover '.( $show_editor ? ' home_sort ' : '' ).' '.$extra_class.'">';
        ...
        if($show_editor){
            ...
            //RIGHT EDITING:
            $ui .= '<div class="note-editor cover-editor edit-off '.superpower_active(10939).'">';
            $ui .= '<span class="show-on-hover">';
        ...
        $ui .= '<div class="row">';
            $ui .= '<div class="col-9 feature-content">';
                ...
                //Title
                $ui .= '<div>';
                $ui .= '<span class="icon-block icon-title"><a href="'.$href.'">'.( $x__type == 6255 ? view_icon_i_x($completion_rate['completion_percentage'], $i) : view_cache(4737 /* Idea Status */, $i['i__status'], true, null, $i['i__id']) ).'</a></span>';
