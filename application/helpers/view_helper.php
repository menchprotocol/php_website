...
function view_card_i($x__type, $top_i__id = 0, $previous_i = null, $i, $focus_e = false, $tree_progress = null, $extra_class = null){
    ...
    foreach($CI->config->item('e___31904') as $x__type_top_bar => $m_top_bar) {
        ...
        if($x__type_top_bar==31770 && $link_type_ui){
        ...
        } elseif($x__type_top_bar==4737 && $e_of_i){

            $active_bars++;
            $top_bar_ui .= '<td><div class="show-on-hover">';
            $top_bar_ui .= view_input_dropdown(4737, $i['i__type'], null, $e_of_i, false, $i['i__id']);
            $top_bar_ui .= '</div></td>';

        } elseif($x__type_top_bar==31004 && $e_of_i){

            $active_bars++;
            $top_bar_ui .= '<td><div class="show-on-hover">';
            $top_bar_ui .= view_input_dropdown(31004, $i['i__privacy'], null, $e_of_i, false, $i['i__id']);
            $top_bar_ui .= '</div></td>';

        } elseif($x__type_top_bar==31911 && $e_of_i){
            ...
            if(is_array($CI->config->item('e___'.$focus_dropdown))){
                foreach($CI->config->item('e___'.$focus_dropdown) as $e__id_dropdown => $m_dropdown) {
                    ...
                    } elseif($e__id_dropdown==6182 && $e_of_i){
                        //Delete
                    ...
                    } elseif(substr($m_dropdown['m__message'], 0, 1)=='/'){
                        //Standard button
...
function view_card_e($x__type, $e, $extra_class = null)
{
    ...
    //Coin Cover
    $ui .= ( !$focus_cover ? '<a href="'.$href.'"' : '<div' ).' class="'.($discovery_mode ? 'coinType12274 coinType6255' : 'coinType12274' ) . ' card_privacy_'.$e['e__privacy'].( !$source_of_e ? ' ready-only ' : '' ).' black-background-obs cover-link" '.( $has_valid_url ? 'style="background-image:url(\''.$e['e__cover'].'\');"' : '' ).'>';
