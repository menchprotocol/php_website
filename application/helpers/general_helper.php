...
function load_app($app_userid = 14563 /* Error if none provided */, $focus_user = 0, $focus_post = 0, $target_post = 0, $inframe = false)
{
    ...
    if ($memory_detected && in_array($app_userid, $CI->config->item('userids___42920'))) {
        boost_power();
    }
            ...
            if ($_GET['hash'] == view_hash($_GET['time'] . $focus_e['userhandle'])) {
                ...
                if ($focus_i) {
                    if (post_is_startable($focus_i)) {
                        ...
                        $CI->Chains->post_discovered(4559, $focus_e['userid'], ($target_i ? $target_i['postid'] : 0), $focus_i);
                ...
                if (!$user_session) {
                    $session_data = $CI->Users->activate($user_session, true);
                }
    ...
    $skip_post_privacy_check = !$memory_detected || in_array($app_userid, $CI->config->item('userids___43388'));
    ...
    if ($memory_detected) {
        ...
        if (in_array($app_userid, $CI->config->item('userids___14599')) && !in_array($app_userid, $CI->config->item('userids___12741'))) {
            ...
            if (!isset($_GET['reset_cache'])) {
                ...
                foreach ($CI->Chains->read(array(
                    ...
                    if (strtotime($latest_cache['chaintime']) <= (time() - view_memory(6404, 14599))) {
                        //Its expired, void it:
                        $CI->Chains->delete($latest_cache['chainid']);
                    } else {
    ...
    if (!$ui) {
        ...
        $raw_app = $CI->load->view($app_userr, $view_input, true);
        $ui .= $raw_app;
    }


    if ($new_cache) {
        $cache_x = $CI->Chains->create(array(
            'chainuserdomain' => website_setting(0),
            'chainusertype' => 44176, //User View
            'chainuserinput' => 14599, //Cache App
            'chainuseroutput' => $app_userid,
    ...
    //App title?
    if ($memory_detected && in_array($app_userid, $CI->config->item('userids___42928'))) {
    ...
    if (!$inframe && $app_userid == 30795 && $target_i && $focus_i && $user_session && $target_i['posthashtag'] == $focus_i['posthashtag']) {
        ...
        if (!post_is_startable($target_i)) {
            ...
            $completion_status = $CI->Chains->post_discovered(4235, $user_session['userid'], 0, $target_i);

            //Now return next post:
            $next__url = $CI->Chains->next_posts($user_session['userid'], $target_i['posthashtag'], $target_i);
    ...
    if (!$memory_detected) {
        ...
        if (in_array($app_userid, $CI->config->item('userids___12741'))) {
            ...
            //Regular UI:
            //Load App:
            echo $CI->load->view('websiteheader', $view_input, true);
            echo $ui;
            echo $cache_chaintime;
            echo $CI->load->view('websitefooter', array(), true);
