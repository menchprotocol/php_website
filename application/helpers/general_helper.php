...
function view_tree($i, $open_by_default = true)
{
    ...
    if(isset($_GET['expand'])){
        ...
        foreach ($CI->Chains->read(array(
            ...
            echo '<div class="maxwidth cover_x_'.$creator['chainid'].'">'.( strlen($_GET['expand'])>1 ? '<a href="' . view_app_chain(44328) . '/'.$_GET['expand'].'@' . $creator['sourcehandle'] . '" target="_blank" title="' . $sources___11035[44328]['m__title'] . '">' : '' ).'<span class="icon-block-sm">' . $sources___11035[44328]['m__cover'] . '</span></a> <a href="'.view_memory(42903,42902).$creator['sourcehandle'].'"><span class="icon-block">'.view_cover($creator['sourcecover']).'</span><b class="main__title">'.$creator['sourcevalue'].'</b> <span class="grey">@'.$creator['sourcehandle'].'</span></a> <span class="grey"><span class="icon-block-sm">' . $sources___11035[4362]['m__cover'] . '</span>'.view_time_difference($creator['chaintime'], false).'</span> <a href="javascript:void(0);" onclick="chain_delete(' . $creator['chainid'] . ', ' . $creator['chainid'] . ',\'' . $i['ideahashtag'] . '\')" title="'.$sources___11035[10673]['m__title'].'" class="grey">' . $sources___11035[10673]['m__cover'] . '</a></div>';
