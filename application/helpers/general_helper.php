...
function post_view($chainusertype, $i, $previous_i = null, $target_posthashtag = null, $focus__userid = 0, $x_completes = false)
{
    ...
    foreach ($CI->Chains->read(array(
        ...
        $ui .= '<div class="creator_headline"><a href="' . view_memory(42903, 42902) . $creator['userhandle'] . '"><span class="icon-block">' . view_cover($creator['usercover']) . '</span><b class="hidden">' . $creator['username'] . '</b><span class="grey mini-font mini-frame">@' . $creator['userhandle'] . '</span></a><span class="grey mini-font mini-padded mini-frame mini_time" title="' . date("Y-m-d H:i:s", strtotime($creator['chaintime'])) . ' PST">' . view_time_difference($creator['chaintime'], true) . '</span></div>';
...
function user_view($chainusertype, $e, $extra_class = null, $extra_value = null)
{
    ...
    $ui .= '<div class="creator_headline grey"><span class="ignore-click ui_userhandle_' . $e['userid'] . '" title="ID ' . $e['userid'] . '">'.( isset($e['chainusertype']) && $users___4593[$e['chainusertype']]['m__cover']!='@' ? ( substr_count($users___4593[$e['chainusertype']]['m__cover'], '@') ? '<b class="underdot" title="'.$users___4593[$e['chainusertype']]['m__name'].'">'.$users___4593[$e['chainusertype']]['m__cover'].'</b>' : '<span title="'.$users___4593[$e['chainusertype']]['m__name'].'">'.$users___4593[$e['chainusertype']]['m__cover'].'</span> @' ) : '@' ) . $e['userhandle'] . '</span><span class="grey mini-font mini-padded mini-frame mini_time" title="' . date("Y-m-d H:i:s", strtotime($creator['chaintime'])) . ' PST">' . view_time_difference($creator['chaintime'], true) . '</span>'.( !$focus__node && strlen($e['userbio']) ? '<span class="icon-block-sm" data-toggle="tooltip" data-placement="top" title="' . $e['userbio'] . '"><i class="far fa-info-circle"></i></span>' : '' ).'</div>';
    if($focus__node){
