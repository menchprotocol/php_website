...
function get_domain_setting($setting_id = 0, $initiator_e__id = 0){
    ...
    if(!$setting_id){
        ...
        $skip_first_word = in_array($setting_id, $CI->config->item('n___26090')) || in_array($setting_id, $CI->config->item('n___26155')) || substr($e___domain_sett[$source_id]['m__message'], 0, 1)=='@' || substr($e___domain_sett[$source_id]['m__message'], 0, 1)=='/';
...
function message_list($i__id, $e__id, $exclude_e, $include_e, $continue_x__id = 0){
    ...
    foreach($query as $subscriber){
        ...
        if (count($CI->X_model->fetch(array(
            'x__up IN (' . join(',', $CI->config->item('n___28914')) . ')' => null, //Fully Unsubscribed
