    ...
    */
    ...
    foreach ($CI->config->item('users___44257') as $chainusertype_target_bar => $m_target_bar) {
        ...
        if ($chainusertype_target_bar == 33532 && !$is_cache && $user_session && $post_access >= 2 && !$is_locked) {
            ...
            $bottom_menu_ui .= '<a href="javascript:void(0);" class="btn btn-sm" onclick="post_edit(0,0,' . $i['postid'] . ')"><span class="icon-block-sm">' . $m_target_bar['m__cover'] . '</span>' . ($focus__node && 0 ? $m_target_bar['m__title'] : '') . '</a>';
            $bottom_menu_ui .= '</span>';
...
function user_view($chainusertype, $e, $extra_class = null, $extra_value = null)
{
    ...
    $ui .= '<div class="creator_headline grey">' . ( isset($e['chainusertype']) && $users___4593[$e['chainusertype']]['m__cover']!='@' ? '<span title="'.$users___4593[$e['chainusertype']]['m__message'].'">' . $users___4593[$e['chainusertype']]['m__title'] . '</span>&nbsp;' : '' ) . '<span class="ignore-click ui_userhandle_' . $e['userid'] . '" title="ID ' . $e['userid'] . '">@' . $e['userhandle'] . '</span></div>';
