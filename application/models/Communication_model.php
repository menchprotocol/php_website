...
class Communication_model extends CI_Model
{
    ...
    function dispatch_subscription_email($related_intents){
        ...
        foreach($this->Links_model->ln_fetch(array(
            ...
            foreach($this->Links_model->ln_fetch(array(
                ...
                foreach ($this->config->item('tr_object_links') as $ln_field => $obj_type) {
                    if (intval($insert_columns[$ln_field]) > 0) {
                        ...
                        if ($obj_type == 'in') {
                            ...
                            $personalized_intro .= '<div>' . echo_clean_db_name($ln_field) . ': <a href="https://mench.com/intents/' . $ins[0]['in_id'] . '" target="_parent">#'.$ins[0]['in_id'].' '.$ins[0]['in_outcome'].'</a></div>';
                            ...
                            //Include link:
                            $personalized_intro .= '<div>' . echo_clean_db_name($ln_field) . ' ID: <a href="https://mench.com/links?ln_id=' . $insert_columns[$ln_field] . '" target="_parent">'.$insert_columns[$ln_field].'</a></div>';
                ...
                $personalized_intro .= '<div>Link ID: <a href="https://mench.com/links?ln_id=' . $insert_columns['ln_id'] . '" target="_blank">' . $insert_columns['ln_id'] . '</a></div>';
    ...
    function digest_received_payload($en, $quick_reply_payload)
    {
        ...
        if (strlen($quick_reply_payload) < 1) {
            ...
            $in_completion_method_entity_id = one_two_explode('TRYANOTHERRESPONSE_', '', $quick_reply_payload);
            $this->Communication_model->dispatch_message(
                'Ok, so try again by sending me another '.$en_all_6144[$in_completion_method_entity_id]['m_name'].' to continue.',
                $en,
                true
            );
    ...
    function digest_received_text($en, $fb_received_message)
    {
        ...
        if (in_array($fb_received_message, array('stats', 'stat', 'statistics'))) {
            ...
            $res = $search_index->search($master_command, [
                ...
                'filters' => 'alg_obj_is_in=1 AND (alg_in_type_entity_id = '.join(' OR alg_in_type_entity_id = ', $this->config->item('en_ids_7582')).')', //Intent Action Plan Addable
            ]);
