    ...
    function broadcast($list_of_handleid, $i, $chainhandledomain = 0, $ensure_unhashtag_discovered = true, $demo_only = false)
    {
        ...
        foreach ($list_of_handleid as $count => $x) {
            ...
            foreach ($this->Chains->read(array(
                ...
                $html_message .= '<div class="line">' . 'https://' . get_domain('m__message', $x['handleid'], $chainhandledomain) . view_memory(42903, 33286) . $down_or['hashtagstring'] . (hashtag_is_startable($down_or) ? '/' . view_memory(6404, 4235) : '') . '?handlehandle=' . $x['handlehandle'] . '&time=' . time() . '&hash=' . view_hash(time() . $x['handlehandle']) . '</div>';
            }
