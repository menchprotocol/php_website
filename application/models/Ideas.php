...
class Ideas extends CI_Model
{
    ...
    function write($add_fields, $x__player = 14068)
    {
        ...
        if (!$add_fields['i__id']) {
            ...
            $this->Ledger->write(array(
                'x__message' => 'i->write() failed to create a new idea',
        ...
        $pinned_followers = $this->Ledger->read(array(
        ...
        foreach ($pinned_followers as $x_pinned) {
            if (!in_array($x_pinned['e__id'], $e_appended) && !count($this->Ledger->read(array(
    ...
    function edit($id, $update_columns, $external_sync = false, $x__player = 0, $x__type = 0)
    {
        ...
        if ($x__player > 0) {
            $before_data = $this->Ideas->read(array('i__id' => $id));
        }

        //Update:
        $this->db->where('i__id', $id);
        $this->db->edit('cache_ideas', $update_columns);
            ...
            if (count($is)) {
                ...
                foreach ($this->Ledger->read(array( //Idea Transactions
                    ...
                    if (0 && count($this->Ledger->read($filters))) {
                        ...
                        $x_adjusted += $this->Ledger->edit($x['x__id'], $update_filter, $x__player, 26785 /* Idea Link Migrated */);
    ...
    function duplicate($i, $copy_to__id, $x__player)
    {
        ...
        foreach ($this->Ledger->read(array(
            'x__type IN (' . njoin(27240) . ')' => null, //COPY Transactions
    ...
    function i_link($i, $x__type, $next_i, $x__player)
    {
        ...
        if (0 && $x__type == 4228 && count($this->Ledger->find_previous(0, $next_i['i__hashtag'], $i['i__id']))) {
            return array(
                'status' => 0,
                'message' => 'Idea already added in the inverse direction, so it cannot be added here',
            );
        } elseif (count($this->Ledger->read(array(
            'x__previous' => $i['i__id'],
            'x__type' => $x__type,
    ...
    function recursive_down_ids($i, $scope, $loop_breaker_ids = array())
    {
        ...
        foreach ($this->Ledger->read(array(
            ...
            $recursive_down_ids = $this->Ideas->recursive_down_ids($next_i, $scope, $loop_breaker_ids);
    ...
    function recursive_clone($i__id, $do_recursive, $x__player, $previous_i = null, $clone_title = null)
    {
        ...
        foreach ($this->Ledger->read($filters, array(), 0) as $x) {
            $this->Ledger->write(array(
                'x__player' => $x__player,
                'x__type' => $x['x__type'],
                'x__next' => $i_new['i__id'],
        ...
        foreach ($this->Ledger->read(array(
            'x__type IN (' . njoin(41301) . ')' => null, //Duplicate Links
            'x__next' => $i__id,
    ...
    function mass_edit($i__id, $action_e__id, $action_command1, $action_command2, $x__player)
    {

        //Alert: Has a twin function called e_mass_edit()
        ...
        foreach ($is_next as $next_i) {
            ...
            if (in_array($action_e__id, array(12591, 12592, 27080, 27985, 27081, 27986, 27082, 27083, 27084, 27085, 27086, 27087)) && view_valid_handle_e($action_command1)) {
                ...
                foreach ($this->Sources->read(array(
                    ...
                    $i_has_e = $this->Ledger->read(array(
                        'x__type IN (' . njoin(33602) . ')' => null, //Idea/Source Links Active
                        'x__next' => $next_i['i__id'],
                    ...
                    if (in_array($action_e__id, array(12591, 27080, 27985, 27082, 27084, 27086)) && !count($i_has_e)) {
                        ...
                        $this->Ledger->edit($i_has_e[0]['x__id'], array(
                            'x__privacy' => 6173,
                ...
                foreach ($this->Ideas->read(array(
                    ...
                    if ($action_e__id == 27240) {
                        ...
                        //Copy
                        $link_count = $this->Ideas->duplicate($next_i, $i['i__id'], $x__player);
                        ...
                        $is_previous = $this->Ledger->read(array(
                        ...
                        if (in_array($action_e__id, array(12611, 28801)) && !count($is_previous)) {
                            ...
                            //Link
                            $status = $this->Ideas->i_link($i, 4228, $next_i, $x__player);

                            if ($status['status']) {

                                if ($action_e__id == 28801) {
                                    //Also remove old link:
                                    $this->Ledger->edit($next_i['x__id'], array(
                                        'x__privacy' => 6173, //Transaction Deleted
                        ...
                        if ($action_e__id == 12612 && count($is_previous)) {
                            //Unlink
                            $this->Ledger->edit($is_previous[0]['x__id'], array(
                                'x__privacy' => 6173,
