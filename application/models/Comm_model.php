    ...
    function fb_ref_process($u, $fb_ref){
	    ...
	    if(!$fb_ref || strlen($fb_ref)<1){
            ...
            if($unsub_value=='CANCEL'){
                ...
                $this->db->query("UPDATE v5_subscriptions SET w_status=-1 WHERE w_status>=0 AND w_child_u_id=".$u['u_id']);
                ...
                //Log engagement:
                $this->Db_model->e_create(array(
                    'e_parent_u_id' => $u['u_id'], //Initiated by PA
                    'e_child_u_id' => $u['u_id'],
                    'e_value' => 'Student unsubscribed from all '.$total_unsubscribed.' subscriptions',
    ...
    function fb_message_process($u, $fb_message_received){
        ...
        if(substr_count($fb_message_received, 'unsubscribe')>0 || substr_count($fb_message_received, 'quit')>0){
            ...
            if($res['nbHits']>0){
                ...
                $this->Db_model->e_create(array(
                    'e_parent_u_id' => $u['u_id'],
                    'e_parent_c_id' => 7431, //Suggest new intent
                    'e_value' => $c_target_outcome, //The suggested intent
                ));
    ...
    function compose_messages($e, $skip_messages=false){
        ...
        if(!$error_message){
            ...
            if(!isset($cs[0])){
                ...
                $requirement_notes = echo_c_requirements($cs[0]);
                if($requirement_notes){
                    //Make it Bot friendly with more clear instructions:
                    $requirement_notes = $requirement_notes.'. You can just type in your response...';
                }
            }
        }
            ...
            if(count($k_outs)<=1){
                ...
                if(intval($cs[0]['c_is_any'])){
                    ...
                    if($requirement_notes){
                        ...
                        //Now informing them that we'll go with the first option:
                        $message_body .= "\n\n".'We will start with option /1 and work our way through this list.';
                    }
