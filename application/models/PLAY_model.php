...
class PLAY_model extends CI_Model
{
    ...
    function en_activate_session($en, $messenger_signin = 0){
        ...
        $this->READ_model->ln_create(array(
            'ln_creator_player_id' => $en['en_id'],
            'ln_type_player_id' => 7564, //PLAYER Signin on Website Success
        ));
    ...
    function en_create($insert_columns, $external_sync = false, $ln_creator_player_id = 0)
    {

        //What is required to create a new blog?
        if (detect_missing_columns($insert_columns, array('en_status_player_id', 'en_name'), $ln_creator_player_id)) {
            return false;
        }
    ...
    function en_update($id, $update_columns, $external_sync = false, $ln_creator_player_id = 0)
    {
        ...
        if ($affected_rows > 0 && $ln_creator_player_id > 0) {
            ...
            foreach ($update_columns as $key => $value) {
                ...
                $this->READ_model->ln_create(array(
                    'ln_creator_player_id' => ($ln_creator_player_id > 0 ? $ln_creator_player_id : $id),
                    'ln_type_player_id' => $ln_type_player_id,
                    'ln_child_player_id' => $id,
    ...
    function en_radio_set($en_parent_bucket_id, $set_en_child_id, $ln_creator_player_id)
    {
         ...
         *  $ln_creator_player_id is the user player ID that one of the children of $en_parent_bucket_id should be assigned (like a drop down)
        ...
        foreach ($this->READ_model->ln_fetch(array(
            ...
            'ln_parent_player_id IN (' . join(',', $children) . ')' => null, //Current children
            'ln_status_player_id IN (' . join(',', $this->config->item('en_ids_7360')) . ')' => null, //Link Statuses Active
        ), array(), config_var(11064)) as $ln) {

            if (!$already_assigned && $ln['ln_parent_player_id'] == $set_en_child_id) {
                ...
                ), $ln_creator_player_id, 6224 /* User Account Updated */);
            }
    ...
    function en_unlink($en_id, $ln_creator_player_id = 0, $merger_en_id = 0){
        ...
        foreach(array_merge(
            ...
            if($merger_en_id > 0 && $adjust_tr['ln_parent_player_id']!=$merger_en_id && $adjust_tr['ln_child_player_id']!=$merger_en_id){

                //Update core field:
                $target_field = ($adjust_tr['ln_child_player_id'] == $en_id ? 'ln_child_player_id' : 'ln_parent_player_id');
    ...
    function en_sync_domain($url, $ln_creator_player_id = 0, $page_title = null)
    {
        /*
         *
        ...
        } elseif ($ln_creator_player_id) {

            //Yes, let's add a new player:
            $added_en = $this->PLAY_model->en_verify_create(( $page_title ? $page_title : $domain_analysis['url_domain_name'] ), $ln_creator_player_id, 6181, detect_fav_icon($domain_analysis['url_clean_domain']));
    ...
    function en_sync_url($url, $ln_creator_player_id = 0, $link_parent_en_ids = array(), $add_to_child_en_id = 0, $page_title = null)
    {
         ...
         *
         * - $url:                  Input URL
         * - $ln_creator_player_id:       IF > 0 will save URL (if not already there) and give credit to this player as the trainer
            ...
            if ($embed_code['status']) {
                ...
                foreach($this->config->item('en_all_11080') as $en_id => $m){
                    if(in_array($domain_analysis['url_file_extension'], explode('|' , $m['m_desc']))){
                        $ln_type_player_id = $en_id;
                        break;
                    }
                }

                if(!$ln_type_player_id){
                    ...
                    $this->READ_model->ln_create(array(
                        ...
                        'ln_type_player_id' => 4246, //Platform Bug Reports
                        'ln_parent_player_id' => 11080,
                        'ln_metadata' => $domain_analysis,
                    ));

                    $ln_type_player_id = 4261; //Assign FILE as default
                }
            }
        }
        ...
        $url_content = null;
        if(!in_array($ln_type_player_id, $this->config->item('en_ids_11059')) /* not a direct file type */ && !(substr_count($url,'youtube.com/embed/')>0 && (substr_count($url,'start=')>0 || substr_count($url,'end=')>0))){
        ...
        if (!$name_was_passed) {
            ...
            if (strlen($page_title) > 0) {
                ...
                $page_title = $en_all_4537[$ln_type_player_id]['m_name'] . ' ' . $url_identified;
        ...
        return array_merge(
            ...
            array(
                'status' => ($url_already_existed && !$ln_creator_player_id ? 0 : 1),
                'message' => ($url_already_existed && !$ln_creator_player_id ? 'URL is already linked to @' . $en_url['en_id'] . ' ' . $en_url['en_name'].' [READ ID '.$en_url['ln_id'].']' : 'Success'),
    ...
    function en_mass_update($en_id, $action_en_id, $action_command1, $action_command2, $ln_creator_player_id)
    {
        ...
        foreach ($children as $en) {
            ...
            } elseif ($action_en_id == 5865 && ($action_command1=='*' || $en['ln_status_player_id']==$action_command1) && in_array($action_command2, $this->config->item('en_ids_6186') /* Link Statuses */)) { //Update Matching Link Status
    ...
    function en_messenger_add($psid, $quick_reply_payload = null)
    {
        ...
        if (!$fetch_result ) {
            ...
            if(isset($fb_profile['locale'])){
                ...
                foreach (array(
                         ...
                         ) as $ln_parent_player_id) {

                    //Did we find a relation? Create the link:
                    if ($ln_parent_player_id > 0) {

                        //Create new link:
                        $this->READ_model->ln_create(array(
                            'ln_type_player_id' => 4230, //Raw link
                            'ln_creator_player_id' => $added_en['en']['en_id'], //User gets credit as trainer
        ...
        $this->READ_model->ln_create(array(
            'ln_parent_player_id' => 6196, //Mench Messenger
            'ln_type_player_id' => 4230, //Raw link
            'ln_creator_player_id' => $added_en['en']['en_id'],
            'ln_child_player_id' => $added_en['en']['en_id'],
            'ln_external_id' => $psid,
        ));
        ...
        $this->READ_model->ln_create(array(
            'ln_parent_player_id' => 4456, //Receive Regular Notifications (User can change later on...)
            'ln_type_player_id' => 4230, //Raw link
            'ln_creator_player_id' => $added_en['en']['en_id'],
            'ln_child_player_id' => $added_en['en']['en_id'],
        ));
