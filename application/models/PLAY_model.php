...
class PLAY_model extends CI_Model
{
    ...
    function en_sync_url($url, $ln_creator_entity_id = 0, $link_parent_en_ids = array(), $add_to_child_en_id = 0, $page_title = null)
    {
        ...
        if ($domain_analysis['url_is_root']) {
            ...
            if ($embed_code['status']) {
                ...
                if(!$ln_type_entity_id){
                    ...
                    $ln_type_entity_id = 4261; //Assign FILE as default
                }
            }
        }
        ...
        $url_content = null;
        if(!in_array($ln_type_entity_id, $this->config->item('en_ids_11059')) /* not a direct file type */ && !(substr_count($url,'youtube.com/embed/')>0 && (substr_count($url,'start=')>0 || substr_count($url,'end=')>0))){
        ...
        return array_merge(
            ...
            array(
                ...
                'message' => ($url_already_existed && !$ln_creator_entity_id ? 'URL is already linked to @' . $en_url['en_id'] . ' ' . $en_url['en_name'].' [READ ID '.$en_url['ln_id'].']' : 'Success'),
