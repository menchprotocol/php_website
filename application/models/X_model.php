        ...
        foreach($string_references['ref_e'] as $referenced_e){
            ...
            if(($current_mench['x_name']=='discover' && !superpower_active(10939, true)) || $is_current_e || $simple_version){
                ...
                if((!$has_text && $is_current_e) || ($current_mench['x_name']=='discover' && $e_count==1 && $e_media_count==$e_count /* All media */)){
                    ...
                    //TEXT ONLY
                    $output_body_message = str_replace($identifier_string, '<span '.$text_tooltip.'>' . ( !substr_count($es[0]['e__title'], ' ') ? '<span class="icon-block-xs e__icon_'.$es[0]['e__id'].'">'.view_e__icon($es[0]['e__icon']).'</span>' : '' ) . '<span class="text__6197_'.$es[0]['e__id'].'">' . $es[0]['e__title'] . '</span></span>'.' ', $output_body_message);
                ...
                //FULL SOURCE LINK
                $output_body_message = str_replace($identifier_string, '<span '.$text_tooltip.'><a class="montserrat" href="/@' . $es[0]['e__id'] . '">'.( !in_array($es[0]['e__status'], $this->config->item('n___7357')) ? '<span class="icon-block-xs">'.$e___6177[$es[0]['e__status']]['m_icon'].'</span> ' : '' ).'<span class="icon-block-xs e__icon_'.$es[0]['e__id'].'">'.view_e__icon($es[0]['e__icon']).'</span><span class="text__6197_'.$es[0]['e__id'].'">' . $es[0]['e__title'] . '</span></a></span>'.' ', $output_body_message);
