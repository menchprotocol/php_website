...
class X_model extends CI_Model
{
            ...
            if(count($e_links)){
                ...
                if($simple_version){
                    ...
                    foreach($e_links as $e_link){
                        $e_dropdown .= '<a href="'.$e_link['x__message'].'" target="_blank" class="dropdown-item css__title ignore-click"><span class="icon-block">'.view_cover(12274,$e_link['e__cover'], true).'</span>'.$e_link['e__title'].'</a>';
                    }
                    $e_dropdown .= '</div></div>';
    ...
    function mark_complete($top_i__id, $i, $add_fields) {
        ...
        if(!isset($search_fields['x__privacy'])){
            //Only search active:
            $search_fields['x__privacy IN (' . join(',', $this->config->item('n___7360')) . ')'] = null;
        }
        ...
        if ($detected_x_type['status'] && $member_e && !in_array($detected_x_type['x__type'] , $this->config->item('n___32248'))) {
