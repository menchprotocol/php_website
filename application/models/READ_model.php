        ...
        if($note_in_id > 0){
            ...
            foreach($in_query as $count => $in_read){
                ...
                if(strlen($in_read['ln_content'])){
                    ...
                    $infobar_details .= '</div>';
                    $string_references = extract_source_references($in_read['ln_content'], true);
                }

                $do_hide = (!$string_references['ref_time_found'] && (($bold_upto_weight && $bold_upto_weight>=$in_read['in_weight']) || ($count >= $show_max)));
