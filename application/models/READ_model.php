...
class READ_model extends CI_Model
{
    ...
    function ln_update($id, $update_columns, $ln_creator_play_id = 0, $ln_type_play_id = 0, $ln_content = '')
    {
        ...
        if ($affected_rows > 0 && $ln_creator_play_id > 0 && $ln_type_play_id > 0) {
    ...
    function ln_fetch($match_columns = array(), $join_objects = array(), $limit = 100, $limit_offset = 0, $order_columns = array('ln_id' => 'DESC'), $select = '*', $group_by = null)
    {
        ...
        if (in_array('en_parent', $join_objects)) {
            ...
            $this->db->join('table_play', 'ln_creator_play_id=en_id','left');
        }
    ...
    function ln_create($insert_columns, $external_sync = false)
    {
        ...
        //Set some defaults:
        if (!isset($insert_columns['ln_creator_play_id']) || intval($insert_columns['ln_creator_play_id']) < 1) {
            $insert_columns['ln_creator_play_id'] = 0;
        }

        //Only require link type:
        if (detect_missing_columns($insert_columns, array('ln_type_play_id'), $insert_columns['ln_creator_play_id'])) {
            return false;
        }
        ...
        if(in_array($insert_columns['ln_type_play_id'] , $this->config->item('en_ids_6255')) && in_array($insert_columns['ln_status_play_id'] , $this->config->item('en_ids_7359')) && $insert_columns['ln_parent_blog_id'] > 0 && $insert_columns['ln_creator_play_id'] > 0){
        ...
        if(in_array($insert_columns['ln_type_play_id'] , $this->config->item('en_ids_5967')) && $insert_columns['ln_type_play_id']!=5967 /* Email Sent causes endless loop */ && !is_dev_environment()){
            ...
            if(count($sub_en_ids) > 0){
                ...
                if($insert_columns['ln_creator_play_id'] > 0){

                    //Fetch trainer details:
                    $trainer_ens = $this->PLAY_model->en_fetch(array(
                        'en_id' => $insert_columns['ln_creator_play_id'],
                    ));
                ...
                foreach($sub_en_ids as $to_en_id){
                    $this->READ_model->ln_create(array(
                        ...
                        'ln_creator_play_id' => $to_en_id, //Sent to this user
    ...
    function read_coin($in_id, $recipient_en, $push_message = false){
        ...
        if (count($ins) < 1) {
            $this->READ_model->ln_create(array(
                'ln_type_play_id' => 4246, //Platform Bug Reports
                'ln_creator_play_id' => ( isset($recipient_en['en_id']) ? $recipient_en['en_id'] : 0 ),
        ...
        foreach($read_coins as $read_history){
            echo '<span class="info-item" style="margin-right:0;" data-toggle="tooltip" data-placement="top" title="READ COIN Transaction '.$read_history['ln_id'].' ['.$en_all_6255[$read_history['ln_type_play_id']]['m_name'].'] ON ['.$read_history['ln_timestamp'].']"><span class="icon-block-sm">'.$en_all_6255[$read_history['ln_type_play_id']]['m_icon'].'</span>'.$read_history['ln_content'].'</span>';
        }

        //Show all incomplete:
        $en_all_6146 = $this->config->item('en_all_6146');
        foreach($read_incompletes as $read_history){
            echo '<span class="info-item" style="margin-right:0;" data-toggle="tooltip" data-placement="top" title="INCOMPLETE Transaction '.$read_history['ln_id'].' ['.$en_all_6146[$read_history['ln_type_play_id']]['m_name'].'] ON ['.$read_history['ln_timestamp'].']"><span class="icon-block-sm">'.$en_all_6146[$read_history['ln_type_play_id']]['m_icon'].'</span>'.$read_history['ln_content'].'</span>';
        }
    ...
    function read_answer($en_id, $question_in_id, $answer_in_ids){
        ...
        if($answers_newly_added>0 || $answers_newly_removed>0){
            ...
            return array(
                'status' => 1,
                'message' => ($answers_newly_added>0 ? $answers_newly_added.' answer'.echo__s($answers_newly_added).' saved' : '').($answers_newly_removed>0 ? ($answers_newly_added>0 ? ' & ' : '').$answers_newly_removed.' removed' : '').'. Going next...',
            );
        } else {
    ...
    function digest_received_payload($en, $quick_reply_payload)
    {
        ...
        if (strlen($quick_reply_payload) < 1) {
            ...
            if ($action_unsubscribe == 'CANCEL') {
                ...
                foreach($this->READ_model->ln_fetch(array(
                    'ln_type_play_id IN (' . join(',', $this->config->item('en_ids_7347')) . ')' => null, //🔴 READING LIST Blog Set
                    'ln_creator_play_id' => $en['en_id'], //Belongs to this User
                    'ln_status_play_id IN (' . join(',', $this->config->item('en_ids_7359')) . ')' => null, //Link Statuses Public
    ...
    function digest_received_text($en, $fb_received_message)
    {
        ...
        if (in_array($fb_received_message, array('stats', 'stat', 'statistics'))) {
            ...
            $this->READ_model->ln_create(array(
                ...
                'ln_creator_play_id' => $en['en_id'], //user who searched
            ...
            //Log link:
            $this->READ_model->ln_create(array(
                'ln_creator_play_id' => $en['en_id'], //User who initiated this message
                'ln_content' => $fb_received_message,
