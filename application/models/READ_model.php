...
class READ_model extends CI_Model
{
    ...
    function read_coin($in_id, $recipient_en, $push_message = false){
        ...
        foreach($read_coins as $read_history){
            echo '<span class="info-item" style="margin-right:0;" data-toggle="tooltip" data-placement="top" title="READ COIN ID '.$read_history['ln_id'].' ['.$en_all_6255[$read_history['ln_type_play_id']]['m_name'].'] ON ['.$read_history['ln_timestamp'].']"><span class="icon-block-sm">'.$en_all_6255[$read_history['ln_type_play_id']]['m_icon'].'</span>'.$read_history['ln_content'].'</span>';
        }

        //Show all incomplete:
        $en_all_6146 = $this->config->item('en_all_6146');
        foreach($read_incompletes as $read_history){
            echo '<span class="info-item" style="margin-right:0;" data-toggle="tooltip" data-placement="top" title="INCOMPLETE ID '.$read_history['ln_id'].' ['.$en_all_6146[$read_history['ln_type_play_id']]['m_name'].'] ON ['.$read_history['ln_timestamp'].']"><span class="icon-block-sm">'.$en_all_6146[$read_history['ln_type_play_id']]['m_icon'].'</span>'.$read_history['ln_content'].'</span>';
        }
