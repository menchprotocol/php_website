...
class READ_model extends CI_Model
{
        ...
        if (in_array($ins[0]['in_type_play_id'], $this->config->item('en_ids_7712'))){
            ...
            if(!count($in__children)){
                ...
                if ($push_message) {
                    ...
                    echo '<div class="margin-top-down"><a class="btn btn-read" href="javascript:void(0)" onclick="read_answer()">'.( count($previously_answered)>0 ? 'UPDATE' : 'SAVE' ).' & CONTINUE <i class="fas fa-angle-right"></i></a>'.( count($previously_answered)>0 ? ' or <a href="javascript:void(0);" onclick="$(\'.selected_before\').toggleClass(\'hidden\');"><span class="icon-block"><i class="fas fa-times-square"></i></span><u>Cancel</u></a>' : '' ).' <span class="result-update"></span></div>';
    ...
    function read_answer($en_id, $question_in_id, $answer_in_ids){
        ...
        if($answers_newly_added>0 || $answers_newly_removed>0){
            ...
            return array(
                'status' => 1,
                'message' => ($answers_newly_added>0 ? $answers_newly_added.' added' : '').($answers_newly_removed>0 ? ($answers_newly_added>0 ? ' & ' : '').$answers_newly_removed.' removed' : ''),
