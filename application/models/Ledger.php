...
class Ledger extends CI_Model
{
    ...
    function read($query_filters = array(), $joins_objects = array(), $limit = 100, $limit_offset = 0, $order_columns = array('x__id' => 'DESC'), $select = '*', $group_by = null)
    {
        ...
        //Universal Privacy?
        if (!substr_count(join('', array_keys($query_filters)), 'x__privacy')) {
            //Append the universal privacy access rule:
            $query_filters['x__privacy IN (' . join(',', dynamic_privacy_x()) . ')'] = null;
        }
        ...
        if (in_array('x__previous', $joins_objects)) {
            ...
            if (!substr_count(join('', array_keys($query_filters)), 'i__privacy')) {
                //Append the universal privacy access rule:
                $query_filters['i__privacy IN (' . join(',', dynamic_privacy_i(null, $query_filters['x__previous'])) . ')'] = null;
            }
        } elseif (in_array('x__next', $joins_objects)) {
            $this->db->join('cache_ideas', 'x__next=i__id', 'left');
            if (!substr_count(join('', array_keys($query_filters)), 'i__privacy')) {
                //Append the universal privacy access rule:
                $query_filters['i__privacy IN (' . join(',', dynamic_privacy_i(null, $query_filters['x__next'])) . ')'] = null;
            }
        }
        ...
        if (in_array('x__following', $joins_objects)) {
            ...
            if (!substr_count(join('', array_keys($query_filters)), 'e__privacy')) {
                //Append the universal privacy access rule:
                $query_filters['e__privacy IN (' . join(',', dynamic_privacy_e(null, $query_filters['x__following'])) . ')'] = null;
            }
        } elseif (in_array('x__follower', $joins_objects)) {
            $this->db->join('cache_sources', 'x__follower=e__id', 'left');
            if (!substr_count(join('', array_keys($query_filters)), 'e__privacy')) {
                //Append the universal privacy access rule:
                $query_filters['e__privacy IN (' . join(',', dynamic_privacy_e(null, $query_filters['x__follower'])) . ')'] = null;
            }
        } elseif (in_array('x__type', $joins_objects)) {
            $this->db->join('cache_sources', 'x__type=e__id', 'left');
            if (!substr_count(join('', array_keys($query_filters)), 'e__privacy')) {
                //Append the universal privacy access rule:
                $query_filters['e__privacy IN (' . join(',', dynamic_privacy_e(null, $query_filters['x__type'])) . ')'] = null;
            }
        } elseif (in_array('x__player', $joins_objects)) {
            $this->db->join('cache_sources', 'x__player=e__id', 'left');
            if (!substr_count(join('', array_keys($query_filters)), 'e__privacy')) {
                //Append the universal privacy access rule:
                $query_filters['e__privacy IN (' . join(',', dynamic_privacy_e(null, $query_filters['x__player'])) . ')'] = null;
            }
        }
    ...
    function write($add_fields, $external_sync = false)
    {
        ...
        if (!in_array($add_fields['x__type'], $this->config->item('n___4593'))) {
            $this->Ledger->write(array(
                'x__message' => 'x->write() failed to create because of invalid transaction type @' . $add_fields['x__type'],
        ...
        if ($add_fields['x__id'] < 1) {
            ...
            $this->Ledger->write(array(
                'x__type' => 4246, //Platform Bug Reports
                'x__player' => $add_fields['x__player'],
                'x__message' => 'write() Failed to create',
                'x__metadata' => array(
                    'input' => $add_fields,
                ),
            ));
        ...
        $tr_watchers = $this->Sources->fetch_recursive(42381, $add_fields['x__type'], $this->config->item('n___30820'), array(), 1);
        if (is_array($tr_watchers) && count($tr_watchers)) {
            ...
            foreach ($this->config->item('e___4341') as $e__id => $m) {
                ...
                if (in_array(6202, $m['m__following'])) {
                    ...
                    //IDEA
                    foreach ($this->Ideas->read(array('i__id' => $add_fields[$e___32088[$e__id]['m__message']])) as $this_i) {
                    ...
                    //SOURCE
                    foreach ($this->Sources->read(array('e__id' => $add_fields[$e___32088[$e__id]['m__message']])) as $this_e) {
            ...
            foreach ($tr_watchers as $tr_watcher) {
                ...
                if ($tr_watcher['e__id'] != $add_fields['x__player']) {
                    $this->Ledger->send_dm($tr_watcher['e__id'], $subject, $html_message, array(
    ...
    function edit($id, $update_columns, $x__player = 0, $x__type = 0, $x__message = '')
    {
        ...
        if (count($update_columns) == 0) {
            ...
            $this->Ledger->write(array(
                'x__message' => 'x->edit() failed to update because of invalid transaction type @' . $x__type,
                'x__type' => 4246, //Platform Bug Reports
                'x__player' => $x__player,
        ...
        $before_data = $this->Ledger->read(array(
            'x__id' => $id,
        ));
        ...
        $this->db->edit('mench_ledger', $update_columns);
            ...
            if (strlen($x__message) == 0) {
                ...
                foreach ($update_columns as $key => $value) {
                    ...
                    if ($key == 'x__privacy') {
                        ...
                        $befores = $this->Sources->read(array(
                            'e__id' => $before_data[0][$key],
                        ));
                        $after_e = $this->Sources->read(array(
                            'e__id' => $value,
                        ));
                        ...
                        $before_i = $this->Ideas->read(array(
                            'i__id' => $before_data[0][$key],
                        ));
                        $after_i = $this->Ideas->read(array(
                            'i__id' => $value,
                        ));
    ...
    function x_update_instant_select($focus__id, $o__id, $element_id, $new_e__id, $migrate_s__handle, $x__id = 0)
    {
        ...
        if ($element_id == 4486 && $x__id > 0) {
            ...
            //IDEA LINK TYPE
            $status = $this->Ledger->edit($x__id, array(
                'x__type' => $new_e__id,
            ...
            if (!$x__id) {
                ...
                foreach ($this->Ledger->read(array(
                    ...
                    'x__type IN (' . njoin(42795) . ')' => null, //Follow
                ), array(), 1) as $found_x) {
                    $x__id = $found_x['x__id'];
                }
            }
            ...
            if ($x__id > 0) {
                ...
                $status = count($this->Ledger->write(array(
            ...
            if (!$x__id) {
                ...
                foreach ($this->Ledger->read(array(
                    ...
                    'x__next' => $o__id,
                    'x__type IN (' . njoin(42260) . ')' => null, //Reactions
                ), array(), 1) as $found_x) {
                    $x__id = $found_x['x__id'];
                }
            }
            ...
            if (!in_array($new_e__id, $this->config->item('n___7358'))) {
                ...
                if ($o__id == $focus__id) {
                    ...
                    if (!$deletion_redirect) {
                        foreach ($this->Sources->read(array('e__id' => $o__id)) as $e2) {
                ...
                $links_removed = $this->Sources->remove($o__id, $player_e['e__id'], $migrate_s__handle);

            }

            //Update:
            if (strlen($migrate_s__handle) < 2 || count($this->Sources->read(array(
                    'LOWER(e__handle)' => strtolower($migrate_s__handle),
                )))) {
                $status = $this->Sources->edit($o__id, array(
            ...
            if (!in_array($new_e__id, $this->config->item('n___31871'))) {
                ...
                if ($o__id == $focus__id) {
                    ...
                    //Find Published Followings:
                    foreach ($this->Ledger->read(array(
                        'x__type IN (' . njoin(42268) . ')' => null, //IDEA LINKS
                        'x__next' => $o__id,
                ...
                //Delete all transactions:
                $links_removed = $this->Ideas->remove($o__id, $player_e['e__id'], $migrate_s__handle);

            }

            //Update Idea:
            $status = $this->Ideas->edit($o__id, array(
            ...
            foreach (array_intersect($this->config->item('n___' . $new_e__id), $this->config->item('n___42179')) as $dynamic_e__id) {
                ...
                if (in_array($data_type, $this->config->item('n___42188'))) {
                    ...
                    $already_responded = count($this->Ledger->read(array(
                        'x__following IN (' . njoin($dynamic_e__id) . ')' => null, //All possible answers
                        'x__next' => $o__id,
    ...
    function send_dm($e__id, $subject, $html_message, $x_data = array(), $template_i__id = 0, $x__website = 0, $log_tr = true, $demo_only = false)
    {
        ...
        $bypass_notifications = count($this->Ledger->read(array(
        ...
        if (!$bypass_notifications) {
            $notification_levels = $this->Ledger->read(array(
                'x__following IN (' . njoin(30820) . ')' => null, //Active Subscriber
                'x__follower' => $e__id,
                'x__type IN (' . njoin(32292) . ')' => null, //SOURCE LINKS
            ));
        ...
        foreach ($this->Ledger->read(array(
            ...
            if (!filter_var($e_data['x__message'], FILTER_VALIDATE_EMAIL)) {
                $this->Ledger->edit($e_data['x__id'], array(
    ...
    function send_i_mass_dm($list_of_e__id, $i, $x__website = 0, $ensure_undiscovered = true, $demo_only = false)
    {
        ...
        foreach ($list_of_e__id as $count => $x) {
            ...
            if (!isset($x['e__id'])) {
                ...
                continue;
            } elseif ($ensure_undiscovered && count($this->Ledger->read(array(
                    'x__previous' => $i['i__id'],
                    'x__player' => $x['e__id'],
            ...
            $send_dm = $this->Ledger->send_dm($x['e__id'], $subject_line, $content_message, array(
                'x__previous' => $i['i__id'],
            ), $i['i__id'], $x__website, true, $demo_only);

            //Mark as discovered:
            if ($send_dm['status'] && !$demo_only) {
                $this->Ledger->mark_complete(43142, $x['e__id'], 0, $i);
                $total_sent++;
            }
    ...
    function find_previous($e__id, $target_i__hashtag, $focus_i__id, $loop_breaker_ids = array())
    {
        ...
        //Fetch followings:
        foreach ($this->Ledger->read(array(
            'x__type IN (' . njoin(42268) . ')' => null, //Active Sequence Up
            'x__next' => $focus_i__id,
        ), array('x__previous')) as $i_previous) {

            //Validate Selection:
            $input__selection = in_array($i_previous['i__type'], $this->config->item('n___7712'));
            $is_selected = count($this->Ledger->read(array(
                'x__type IN (' . njoin(7704) . ')' => null, //Discovery Expansion
            ...
            $website_finder = $this->Ledger->find_previous($e__id, $target_i__hashtag, $i_previous['i__id'], $loop_breaker_ids);
    ...
    function find_previous_discovered($focus_i__id, $x__player, $loop_breaker_ids = array())
    {
        ...
        foreach ($this->Ledger->read(array(
            ...
            return $this->Ledger->find_previous_discovered($prev_i['i__id'], $x__player, $loop_breaker_ids);
        }
    ...
    function find_next($e__id, $target_i__hashtag, $i, $find_after_i__id = 0, $search_up = true, $target_completed = false, $loop_breaker_ids = array())
    {
        ...
        foreach ($this->Ledger->read(array(
            'x__previous' => $i['i__id'],
            'x__type IN (' . njoin(42267) . ')' => null, //Active Sequence Down
            ...
            if ($target_completed || !count($this->Ledger->read(array(
                    'x__type IN (' . njoin(6255) . ')' => null, //DISCOVERIES
                    'x__player' => $e__id,
            ...
            //Keep looking deeper:
            $next__url = $this->Ledger->find_next($e__id, $target_i__hashtag, $next_i, 0, false, $target_completed, $loop_breaker_ids);
            if ($next__url) {
                return $next__url;
            }
        ...
        if ($search_up && $target_i__hashtag != $i['i__hashtag']) {
            ...
            foreach (array_reverse($this->Ledger->find_previous($e__id, $target_i__hashtag, $i['i__id'])) as $p_i) {
                //Find the next siblings:
                $next__url = $this->Ledger->find_next($e__id, $target_i__hashtag, $p_i, $current_previous, false, $target_completed);
                if ($next__url) {
                    return $next__url;
                }
    ...
    function mark_complete($x__type, $x__player, $target_i__id = 0, $i, $focus_i_data = array(), $x_data = array())
    {
        ...
        if ($input__upload || $input__text) {
            ...
            $x_responses = $this->Ledger->read(array(
            ...
            if (strlen($focus_i_data['i__text']) || count($focus_i_data['uploaded_media'])) {
                ...
                //Delete Links
                $this->Ideas->remove($x_responses[0]['i__id'], $x__player);

                //Delete Idea:
                $this->Ideas->edit($x_responses[0]['i__id'], array(
        ...
        //Make sure not already discovered:
        foreach ($this->Ledger->read(array(
        ...
        $new_x = $this->Ledger->write($x_data);
        ...
        if ($input__selection) {
            foreach ($this->Ledger->read(array(
                ...
                if (!in_array($next_i['i__type'], $this->config->item('n___43039')) && !count($this->Ledger->read(array(
                        'x__type IN (' . njoin(42267) . ')' => null, //IDEA LINKS
                        'x__previous' => $next_i['i__id'],
                    ), array('x__next'), 0, 0))) {
                    //Mark as complete:
                    $this->Ledger->mark_complete(i__discovery_link($next_i), $x_data['x__player'], $target_i__id, $next_i, $x_data);
                }
            }
        }
        ...
        if ($x_data['x__player'] && in_array($x_data['x__type'], $this->config->item('n___40986'))) {
            ...
            $clone_urls = '';
            foreach ($this->Ledger->read(array(
                'x__type IN (' . njoin(32275) . ')' => null, //DISCOVERY TRIGGERS
                'x__previous' => $i['i__id'],
                ...
                if ($clone_i['x__type'] == 32247) {
                    ...
                    $result = $this->Ideas->recursive_clone($clone_i['i__id'], 0, $x_data['x__player'], null, $new_title);
                    if ($result['status']) {
                    ...
                    foreach ($this->Ledger->read(array(
                        ...
                        $this->Ledger->edit($remove_x['x__id'], array(
            ...
            if (strlen($clone_urls)) {
                ...
                $this->Ledger->send_dm($x_data['x__player'], $i_title, $clone_urls);
                //Also DM all watchers of the idea:
                foreach ($this->Ledger->read(array(
                    'x__type' => 10573, //WATCHERS
                    'x__next' => $i['i__id'],
                ), array(), 0) as $watcher) {
                    $this->Ledger->send_dm($watcher['x__following'], $i_title, $clone_urls);
                }
            }
            ...
            foreach ($this->Ledger->read(array(
                ...
                if (in_array($x_tag['x__following'], $this->config->item('n___43048'))) {
                    ...
                    if ($x_tag['x__following'] == 6197 && strlen(trim($x_data['x__message'])) >= 2) {
                        ...
                        $this->Sources->edit($x__player, array(
            ...
            foreach ($this->Ledger->read(array(
                ...
                foreach ($this->Ledger->read(array(
                    ...
                    $this->Ledger->edit($existing_x['x__id'], array(
                        'x__privacy' => 6173,
            ...
            $watchers = $this->Ledger->read(array(
                'x__type' => 10573, //WATCHERS
                'x__next' => $i['i__id'],
            ), array(), 0);
            if (count($watchers)) {
                ...
                if (count($es_discoverer)) {
                    ...
                    foreach ($watchers as $watcher) {
                        if (!in_array(intval($watcher['x__following']), $sent_watchers)) {
                            ...
                            $this->Ledger->send_dm($watcher['x__following'], $es_discoverer[0]['e__title'] . ' Discovered: ' . view_i_title($i, true),
                                //Message Body:
    ...
    function tree_progress($e__id, $i, $current_level = 0, $loop_breaker_ids = array())
    {
        ...
        $recursive_down_ids = $this->Ideas->recursive_down_ids($i, 'AND');
        ...
        if (count($recursive_down_ids['recursive_i_ids'])) {
            foreach ($this->Ledger->read(array(
                ...
                //Fetch recursive:
                $tree_progress = $this->Ledger->tree_progress($e__id, $expansion_in, $current_level, $loop_breaker_ids);

                if (!$tree_progress && !count($this->Ledger->read(array(
                        'x__type IN (' . njoin(6255) . ')' => null, //DISCOVERIES
                        'x__player' => $e__id, //Belongs to this Member
    ...
    {
        return count($this->Ledger->read(array(
