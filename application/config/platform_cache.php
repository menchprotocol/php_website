<?php

defined('BASEPATH') OR exit('No direct script access allowed');

/*
* Keep a cache of certain parts of the idea for faster processing
* See source @4527 for more details
*
*/

//Generated 2020-09-22 21:57:28 PST

//SOURCE TAB SHOW EVEN IF ZERO:
$config['n___12574'] = array(6225,4997);
$config['e___12574'] = array(
    6225 => array(
        'm_title' => 'MY ACCOUNT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cog source" aria-hidden="true"></i>',
        'm_profile' => array(12574,13425,11089,4527),
    ),
    4997 => array(
        'm_title' => 'SOURCE LIST EDITOR',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-list source" aria-hidden="true"></i>',
        'm_profile' => array(12574,13429,11035,12703,12590,4527),
    ),
);

//MONTHLY SUBSCRIPTION:
$config['n___13518'] = array(13038,13042);
$config['e___13518'] = array(
    13038 => array(
        'm_title' => '80 DISCOVERIES PER MONTH FOR FREE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-hand-holding-heart discover"></i>',
        'm_profile' => array(13889,13518,4755),
    ),
    13042 => array(
        'm_title' => 'UNLIMITED DISCOVERIES $8 PER MONTH',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-hand-holding-usd discover"></i>',
        'm_profile' => array(13518,4755),
    ),
);

//SOURCE LINK DISPLAY TOOLTIP-ONLY:
$config['n___13899'] = array(4255);
$config['e___13899'] = array(
    4255 => array(
        'm_title' => 'SOURCE LINK TEXT',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-align-left source"></i>',
        'm_profile' => array(13899,12822,10593,4593,4592),
    ),
);

//TRANSACTIONS UNLINK:
$config['n___6173'] = array(6182,6178);
$config['e___6173'] = array(
    6182 => array(
        'm_title' => 'IDEA DELETE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-trash-alt idea" aria-hidden="true"></i>',
        'm_profile' => array(6173,12400,4593,4737),
    ),
    6178 => array(
        'm_title' => 'SOURCES DELETE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-trash-alt source" aria-hidden="true"></i>',
        'm_profile' => array(6173,4593,12401,6177),
    ),
);

//TRANSACTIONS PUBLISH:
$config['n___6176'] = array(6184,6181);
$config['e___6176'] = array(
    6184 => array(
        'm_title' => 'IDEA PUBLISH',
        'm_message' => '',
        'm_icon' => '<i class="far fa-globe idea" aria-hidden="true"></i>',
        'm_profile' => array(6176,10648,4737,7355,7356),
    ),
    6181 => array(
        'm_title' => 'SOURCES PUBLISH',
        'm_message' => '',
        'm_icon' => '<i class="far fa-globe source" aria-hidden="true"></i>',
        'm_profile' => array(6176,10654,6177,7358,7357),
    ),
);

//TRANSACTIONS DRAFT:
$config['n___6175'] = array(6183,6180);
$config['e___6175'] = array(
    6183 => array(
        'm_title' => 'IDEA DRAF',
        'm_message' => '',
        'm_icon' => '<i class="far fa-spinner fa-spin idea" aria-hidden="true"></i>',
        'm_profile' => array(6175,4737,10648,7356),
    ),
    6180 => array(
        'm_title' => 'SOURCES DRAFT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-spinner fa-spin source" aria-hidden="true"></i>',
        'm_profile' => array(6175,10654,6177,7358),
    ),
);

//TRANSACTIONS FEATURE:
$config['n___12399'] = array(12137,12563);
$config['e___12399'] = array(
    12137 => array(
        'm_title' => 'IDEA FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star idea" aria-hidden="true"></i>',
        'm_profile' => array(12399,13813,6404,11035,13420,10648,12138,7356,7355,4737),
    ),
    12563 => array(
        'm_title' => 'SOURCES FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star source" aria-hidden="true"></i>',
        'm_profile' => array(12399,4527,12575,10654,7358,7357,6177),
    ),
);

//MENCH SOCIAL LINKS:
$config['n___13894'] = array(13019,13017,13018,13021,13016,13015,13418);
$config['e___13894'] = array(
    13019 => array(
        'm_title' => 'INSTAGRAM',
        'm_message' => '',
        'm_icon' => '<i class="fab fa-instagram" aria-hidden="true"></i>',
        'm_profile' => array(13894,3301,7146),
    ),
    13017 => array(
        'm_title' => 'TWITTER',
        'm_message' => '',
        'm_icon' => '<i class="fab fa-twitter" aria-hidden="true"></i>',
        'm_profile' => array(13894,3300,7146),
    ),
    13018 => array(
        'm_title' => 'FACEBOOK',
        'm_message' => '',
        'm_icon' => '<i class="fab fa-facebook" aria-hidden="true"></i>',
        'm_profile' => array(13894,2793,7146),
    ),
    13021 => array(
        'm_title' => 'GITHUB',
        'm_message' => '',
        'm_icon' => '<i class="fab fa-github" aria-hidden="true"></i>',
        'm_profile' => array(13894,3303,7146),
    ),
    13016 => array(
        'm_title' => 'LINKEDIN',
        'm_message' => '',
        'm_icon' => '<i class="fab fa-linkedin" aria-hidden="true"></i>',
        'm_profile' => array(13904,13894,3302,7146),
    ),
    13015 => array(
        'm_title' => 'YOUTUBE',
        'm_message' => '',
        'm_icon' => '<i class="fab fa-youtube"></i>',
        'm_profile' => array(13894,3308,7146),
    ),
    13418 => array(
        'm_title' => 'PRODUCT HUNT',
        'm_message' => '',
        'm_icon' => '<i class="fab fa-product-hunt"></i>',
        'm_profile' => array(13894,13417,7146),
    ),
);

//CUSTOMIZABLE UI:
$config['n___13890'] = array(13491,13884,13518);
$config['e___13890'] = array(
    13491 => array(
        'm_title' => 'FONT SIZE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-text-size"></i>',
        'm_profile' => array(11054,13890,6225,4527,6204),
    ),
    13884 => array(
        'm_title' => 'DISPLAY MODE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-eclipse"></i>',
        'm_profile' => array(6204,11054,13890,4527,6225),
    ),
    13518 => array(
        'm_title' => 'MONTHLY SUBSCRIPTION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-usd-circle discover"></i>',
        'm_profile' => array(11054,13890,6204,4527,6225,13450),
    ),
);

//DISPLAY MODE:
$config['n___13884'] = array(13885,13886);
$config['e___13884'] = array(
    13885 => array(
        'm_title' => 'LIGHT MODE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-sun"></i>',
        'm_profile' => array(13884),
    ),
    13886 => array(
        'm_title' => 'DARK MODE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-moon"></i>',
        'm_profile' => array(13889,13884),
    ),
);

//ACCOUNT DEFAULTS:
$config['n___13889'] = array(13038,13886,13493);
$config['e___13889'] = array(
    13038 => array(
        'm_title' => '80 DISCOVERIES PER MONTH FOR FREE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-hand-holding-heart discover"></i>',
        'm_profile' => array(13889,13518,4755),
    ),
    13886 => array(
        'm_title' => 'DARK MODE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-moon"></i>',
        'm_profile' => array(13889,13884),
    ),
    13493 => array(
        'm_title' => 'MEDIUM FONT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-h2"></i>',
        'm_profile' => array(13889,4755,13491),
    ),
);

//REQUIREMENTS:
$config['n___13865'] = array(13875,13876);
$config['e___13865'] = array(
    13875 => array(
        'm_title' => 'MEETS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-lock-open"></i>',
        'm_profile' => array(13865),
    ),
    13876 => array(
        'm_title' => 'MISSING',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-lock"></i>',
        'm_profile' => array(13865),
    ),
);

//ANNOUNCEMENTS:
$config['n___13783'] = array(13784,13785,13786);
$config['e___13783'] = array(
    13784 => array(
        'm_title' => 'ALL USERS',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(13783),
    ),
    13785 => array(
        'm_title' => 'COMPLETED USERS',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(13783),
    ),
    13786 => array(
        'm_title' => 'INCOMPLETE USERS',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(13783),
    ),
);

//PRIMARY COIN:
$config['n___13776'] = array(6255);
$config['e___13776'] = array(
    6255 => array(
        'm_title' => 'DISCOVERIES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,13776,13450,11089,12677,11018,12467,12228,4527),
    ),
);

//SHOW URL:
$config['n___13672'] = array(10876);
$config['e___13672'] = array(
    10876 => array(
        'm_title' => 'MENCH URL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link"></i>',
        'm_profile' => array(13672,13023,6403,4527,1326),
    ),
);

//SOURCE ICON:
$config['n___6198'] = array(4260,13577,13578);
$config['e___6198'] = array(
    4260 => array(
        'm_title' => 'SOURCE LINK IMAGE',
        'm_message' => 'Insert HTML image code & paste image URL',
        'm_icon' => '<i class="fad fa-image source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,6198,11059,10593,4593,4592,4537),
    ),
    13577 => array(
        'm_title' => 'ICON CODE',
        'm_message' => 'Insert Font Awesome HTML code & add icon code',
        'm_icon' => '<i class="fas fa-smile-plus"></i>',
        'm_profile' => array(6198),
    ),
    13578 => array(
        'm_title' => 'ICON LIBRARY',
        'm_message' => 'Visit Font Awesome to browse all icons',
        'm_icon' => '<i class="fas fa-icons"></i>',
        'm_profile' => array(6198),
    ),
);

//EDIT SOURCE:
$config['n___13571'] = array(6177,6197,6198,6186,4372,4593,13572);
$config['e___13571'] = array(
    6177 => array(
        'm_title' => 'SOURCE STATUS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sliders-h source" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,12766,4527,11054,6204,5003,6160,6206),
    ),
    6197 => array(
        'm_title' => 'SOURCE TITLE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-fingerprint source" aria-hidden="true"></i>',
        'm_profile' => array(13571,13296,13025,6404,12112,12232,10646,5000,4998,4999,6206),
    ),
    6198 => array(
        'm_title' => 'SOURCE ICON',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-image-polaroid source"></i>',
        'm_profile' => array(4527,13571,12605,10653,5943,10625,6206),
    ),
    6186 => array(
        'm_title' => 'TRANSACTION STATUS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-sliders-h" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,4527,11054,10677,10656,6204,5865,6160,7304,4341),
    ),
    4372 => array(
        'm_title' => 'TRANSACTION MESSAGE',
        'm_message' => 'Write, Drop a File or Paste URL',
        'm_icon' => '<i class="fas fa-comment-lines" aria-hidden="true"></i>',
        'm_profile' => array(13571,7578,10679,10657,5001,4341),
    ),
    4593 => array(
        'm_title' => 'TRANSACTION TYPE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,6194,6204,11081,10659,6160,4341,4527),
    ),
    13572 => array(
        'm_title' => 'UPLOAD MEDIA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-image"></i>',
        'm_profile' => array(6404,11035,13571),
    ),
);

//REFERENCED IDEAS:
$config['n___13550'] = array(4983,4231);
$config['e___13550'] = array(
    4983 => array(
        'm_title' => 'CITATIONS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link source rotate90" aria-hidden="true"></i>',
        'm_profile' => array(12359,7551,13550,13543,13407,4485,12141,10939,12450,12228,4593,10593),
    ),
    4231 => array(
        'm_title' => 'MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comment-lines idea" aria-hidden="true"></i>',
        'm_profile' => array(12994,13550,13294,12359,4485,10593,4986,4603,4593),
    ),
);

//UNIQUE SOURCES:
$config['n___13548'] = array(4251);
$config['e___13548'] = array(
    4251 => array(
        'm_title' => 'SOURCE CREATED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle source" aria-hidden="true"></i>',
        'm_profile' => array(13548,12401,12149,12141,10593,4593),
    ),
);

//MAX IDEAS:
$config['n___6170'] = array(6162);
$config['e___6170'] = array(
    6162 => array(
        'm_title' => 'MAXIMUM TIME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-stopwatch"></i>',
        'm_profile' => array(6170,13292,4739,6159),
    ),
);

//MIN IDEAS:
$config['n___6169'] = array(6161);
$config['e___6169'] = array(
    6161 => array(
        'm_title' => 'MINIMUM TIME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-stopwatch"></i>',
        'm_profile' => array(6169,13292,4735,6159),
    ),
);

//IDEA MAP STATS:
$config['n___13544'] = array(13629,13292);
$config['e___13544'] = array(
    13629 => array(
        'm_title' => 'IDEA RANGE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea"></i>',
        'm_profile' => array(13544),
    ),
    13292 => array(
        'm_title' => 'DISCOVERY TIME',
        'm_message' => '',
        'm_icon' => '<i class="far fa-stopwatch"></i>',
        'm_profile' => array(11035,13544,4356),
    ),
);

//AVATARS TYPES:
$config['n___13533'] = array(13535,13536,13534);
$config['e___13533'] = array(
    13535 => array(
        'm_title' => 'THIN',
        'm_message' => 'far',
        'm_icon' => '<i class="far fa-paw-claws source"></i>',
        'm_profile' => array(13533),
    ),
    13536 => array(
        'm_title' => 'DUOTO',
        'm_message' => 'fad',
        'm_icon' => '<i class="fad fa-paw-claws source"></i>',
        'm_profile' => array(13533),
    ),
    13534 => array(
        'm_title' => 'BOLD',
        'm_message' => 'fas',
        'm_icon' => '<i class="fas fa-paw-claws source"></i>',
        'm_profile' => array(13533),
    ),
);

//QUICK SHARE:
$config['n___13531'] = array(3300,12889,12890,13904,3288,3099,13026);
$config['e___13531'] = array(
    3300 => array(
        'm_title' => 'TWITTER',
        'm_message' => 'twitter',
        'm_icon' => '<i class="fab fa-twitter source" aria-hidden="true"></i>',
        'm_profile' => array(13531,12891,1326),
    ),
    12889 => array(
        'm_title' => 'FACEBOOK',
        'm_message' => 'facebook',
        'm_icon' => '<i class="fab fa-facebook source"></i>',
        'm_profile' => array(13531,12891),
    ),
    12890 => array(
        'm_title' => 'MESSENGER',
        'm_message' => 'messenger',
        'm_icon' => '<i class="fab fa-facebook-messenger source" aria-hidden="true"></i>',
        'm_profile' => array(13531,12889),
    ),
    13904 => array(
        'm_title' => 'LINKEDIN',
        'm_message' => 'linkedin',
        'm_icon' => '<i class="fab fa-linkedin" aria-hidden="true"></i>',
        'm_profile' => array(13531,12891),
    ),
    3288 => array(
        'm_title' => 'EMAIL',
        'm_message' => 'email',
        'm_icon' => '<i class="fas fa-envelope-open" aria-hidden="true"></i>',
        'm_profile' => array(13861,13531,6404,4269,6225,4755),
    ),
    3099 => array(
        'm_title' => 'REDDIT',
        'm_message' => 'reddit',
        'm_icon' => '<i class="fab fa-reddit"></i>',
        'm_profile' => array(13531,12891,1326),
    ),
    13026 => array(
        'm_title' => 'WHATSAPP',
        'm_message' => 'whatsapp',
        'm_icon' => '<i class="fab fa-whatsapp-square isgreen"></i>',
        'm_profile' => array(13531,12891),
    ),
);

//IDEA TAB SHOW EVEN IF ZERO:
$config['n___13530'] = array(6255,10573,13865,12419,4601,7545,12589,11047,12273,12274);
$config['e___13530'] = array(
    6255 => array(
        'm_title' => 'DISCOVERIES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,13776,13450,11089,12677,11018,12467,12228,4527),
    ),
    10573 => array(
        'm_title' => 'MY IDEAS MAP',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sitemap idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,11035,11054,4527,10984,11018,4485,4593,7551),
    ),
    13865 => array(
        'm_title' => 'REQUIREMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shield-check source"></i>',
        'm_profile' => array(4527,11035,13421,13298,7551,13530,4593,11018,11089,4485),
    ),
    12419 => array(
        'm_title' => 'COMMENTS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-comments discover" aria-hidden="true"></i>',
        'm_profile' => array(12701,11035,13871,12994,11089,13530,11018,12359,5967,10593,4986,4593,4485),
    ),
    4601 => array(
        'm_title' => 'KEYWORDS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-tags idea" aria-hidden="true"></i>',
        'm_profile' => array(12700,13023,13530,11018,4485,10593,4593),
    ),
    7545 => array(
        'm_title' => 'AWARDS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-file-certificate source" aria-hidden="true"></i>',
        'm_profile' => array(11089,13530,6159,13421,13298,12197,11018,4485,7551,4593),
    ),
    12589 => array(
        'm_title' => 'IDEA LIST EDITOR',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-list idea" aria-hidden="true"></i>',
        'm_profile' => array(13530,13403,11018,4527,12590),
    ),
    11047 => array(
        'm_title' => 'IDEA MENU',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-caret-down" aria-hidden="true"></i>',
        'm_profile' => array(13530,12700,11018,4527,11040),
    ),
    12273 => array(
        'm_title' => 'IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13300,13291,13450,12675,13530,11018,11089,12761,12112,12467,12228),
    ),
    12274 => array(
        'm_title' => 'SOURCES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle source" aria-hidden="true"></i>',
        'm_profile' => array(13920,13450,13530,11018,11089,12761,13291,12467,12228),
    ),
);

//SOURCES FEATURE:
$config['n___12563'] = array(2997,4446,3005,13339,3147,13218,13350,3192,2998);
$config['e___12563'] = array(
    2997 => array(
        'm_title' => 'ARTICLES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-newspaper source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    4446 => array(
        'm_title' => 'ASSESSMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    3005 => array(
        'm_title' => 'BOOKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-book source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    13339 => array(
        'm_title' => 'INDUSTRY EXPERTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-badge-check source"></i>',
        'm_profile' => array(13450,12523,12563,6159,13207),
    ),
    3147 => array(
        'm_title' => 'ONLINE COURSES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-presentation source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    13218 => array(
        'm_title' => 'PODCAST EPISODES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-microphone source"></i>',
        'm_profile' => array(12563,3000),
    ),
    13350 => array(
        'm_title' => 'REPORTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-file-chart-line source"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    3192 => array(
        'm_title' => 'TOOLS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-tools source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    2998 => array(
        'm_title' => 'VIDEOS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle source"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
);

//NEXT:
$config['n___12211'] = array(6677);
$config['e___12211'] = array(
    6677 => array(
        'm_title' => 'READ & NEXT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12211,13022,12330,7585,6192,4559),
    ),
);

//DISCOVERY CONTROLLER:
$config['n___13289'] = array(12991,13877,13563,13871,12211);
$config['e___13289'] = array(
    12991 => array(
        'm_title' => 'BACK',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chevron-left" aria-hidden="true"></i>',
        'm_profile' => array(13289,11035),
    ),
    13877 => array(
        'm_title' => 'SAVE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-bookmark"></i>',
        'm_profile' => array(13289),
    ),
    13563 => array(
        'm_title' => 'EDIT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-pen"></i>',
        'm_profile' => array(10939,13289),
    ),
    13871 => array(
        'm_title' => 'COMMENT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-comment" aria-hidden="true"></i>',
        'm_profile' => array(13289),
    ),
    12211 => array(
        'm_title' => 'NEXT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chevron-right" aria-hidden="true"></i>',
        'm_profile' => array(4527,13289,11035),
    ),
);

//FONT SIZE:
$config['n___13491'] = array(13494,13493,13492);
$config['e___13491'] = array(
    13494 => array(
        'm_title' => 'LARGE FONT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-h1"></i>',
        'm_profile' => array(4755,13491),
    ),
    13493 => array(
        'm_title' => 'MEDIUM FONT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-h2"></i>',
        'm_profile' => array(13889,4755,13491),
    ),
    13492 => array(
        'm_title' => 'SMALL FONT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-h3"></i>',
        'm_profile' => array(4755,13491),
    ),
);

//UNIQUE IDEAS:
$config['n___13480'] = array(4250);
$config['e___13480'] = array(
    4250 => array(
        'm_title' => 'IDEA CREATED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13480,5967,12400,12149,10593,4593),
    ),
);

//HEADER NAVIGATION:
$config['n___13479'] = array(4269,12500);
$config['e___13479'] = array(
    4269 => array(
        'm_title' => 'SIGN IN/UP',
        'm_message' => 'For guests',
        'm_icon' => '<i class="fas fa-sign-in-alt" aria-hidden="true"></i>',
        'm_profile' => array(13479,10876,4527,11035),
    ),
    12500 => array(
        'm_title' => 'VERTICAL MENU',
        'm_message' => 'For signed-in members',
        'm_icon' => '<i class="fas fa-ellipsis-v"></i>',
        'm_profile' => array(13479,12079,12823,4527),
    ),
);

//METADATA CLEAN VARIABLES:
$config['n___7277'] = array(6159,6172);
$config['e___7277'] = array(
    6159 => array(
        'm_title' => 'IDEA METADATA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-lambda idea" aria-hidden="true"></i>',
        'm_profile' => array(7276,7277,4527,6212,11049,6201),
    ),
    6172 => array(
        'm_title' => 'SOURCE METADATA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-lambda source"></i>',
        'm_profile' => array(7277,4527,6212,6206),
    ),
);

//SOURCE METADATA:
$config['n___6172'] = array(6207);
$config['e___6172'] = array(
    6207 => array(
        'm_title' => 'SOURCE METADATA ALGOLIA ID',
        'm_message' => 'algolia__id',
        'm_icon' => '<i class="far fa-lambda"></i>',
        'm_profile' => array(3323,6172),
    ),
);

//TRANSACTION TYPE UPDATE:
$config['n___13442'] = array(4486);
$config['e___13442'] = array(
    4486 => array(
        'm_title' => 'IDEA LINKS',
        'm_message' => 'x__type',
        'm_icon' => '<i class="fas fa-link idea" aria-hidden="true"></i>',
        'm_profile' => array(13442,13408,12700,11054,12079,10662,4527),
    ),
);

//IDEA METADATA:
$config['n___6159'] = array(7545,6208,6168,6283,12885,6228,13339,6170,6162,4430,6169,6161,13897);
$config['e___6159'] = array(
    7545 => array(
        'm_title' => 'AWARDS',
        'm_message' => 'i___7545',
        'm_icon' => '<i class="fas fa-file-certificate source" aria-hidden="true"></i>',
        'm_profile' => array(11089,13530,6159,13421,13298,12197,11018,4485,7551,4593),
    ),
    6208 => array(
        'm_title' => 'IDEA METADATA ALGOLIA ID',
        'm_message' => 'algolia__id',
        'm_icon' => '<i class="far fa-lambda"></i>',
        'm_profile' => array(6159,3323),
    ),
    6168 => array(
        'm_title' => 'IDEA METADATA COMMON STEPS',
        'm_message' => 'i___6168',
        'm_icon' => '<i class="far fa-lambda"></i>',
        'm_profile' => array(6159),
    ),
    6283 => array(
        'm_title' => 'IDEA METADATA EXPANSION CONDITIONAL',
        'm_message' => 'i___6283',
        'm_icon' => '<i class="far fa-lambda"></i>',
        'm_profile' => array(6159),
    ),
    12885 => array(
        'm_title' => 'IDEA METADATA EXPANSION SOME',
        'm_message' => 'i___12885',
        'm_icon' => '<i class="far fa-lambda"></i>',
        'm_profile' => array(6159),
    ),
    6228 => array(
        'm_title' => 'IDEA METADATA EXPANSION STEPS',
        'm_message' => 'i___6228',
        'm_icon' => '<i class="far fa-lambda"></i>',
        'm_profile' => array(6159),
    ),
    13339 => array(
        'm_title' => 'INDUSTRY EXPERTS',
        'm_message' => 'i___13339',
        'm_icon' => '<i class="fas fa-badge-check source"></i>',
        'm_profile' => array(13450,12523,12563,6159,13207),
    ),
    6170 => array(
        'm_title' => 'MAX IDEAS',
        'm_message' => 'i___6170',
        'm_icon' => '<i class="fas fa-circle idea"></i>',
        'm_profile' => array(13629,4527,6159),
    ),
    6162 => array(
        'm_title' => 'MAXIMUM TIME',
        'm_message' => 'i___6162',
        'm_icon' => '<i class="fas fa-stopwatch"></i>',
        'm_profile' => array(6170,13292,4739,6159),
    ),
    4430 => array(
        'm_title' => 'MENCH MEMBERS',
        'm_message' => 'i___4430',
        'm_icon' => '<i class="fas fa-users source" aria-hidden="true"></i>',
        'm_profile' => array(13450,13207,6159,4364,10573),
    ),
    6169 => array(
        'm_title' => 'MIN IDEAS',
        'm_message' => 'i___6169',
        'm_icon' => '<i class="fas fa-circle idea"></i>',
        'm_profile' => array(13629,4527,6159),
    ),
    6161 => array(
        'm_title' => 'MINIMUM TIME',
        'm_message' => 'i___6161',
        'm_icon' => '<i class="fas fa-stopwatch"></i>',
        'm_profile' => array(6169,13292,4735,6159),
    ),
    13897 => array(
        'm_title' => 'NONFICTION CONTENT',
        'm_message' => 'i___13897',
        'm_icon' => '<i class="fas fa-file-certificate source"></i>',
        'm_profile' => array(6159,13450,13207),
    ),
);

//MENCH VARIABLES:
$config['n___6212'] = array(6201,6159,6206,6172,4341,6103,13442);
$config['e___6212'] = array(
    6201 => array(
        'm_title' => 'MENCH IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-table idea" aria-hidden="true"></i>',
        'm_profile' => array(6212,11054,4527,7735),
    ),
    6159 => array(
        'm_title' => 'IDEA METADATA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-lambda idea" aria-hidden="true"></i>',
        'm_profile' => array(7276,7277,4527,6212,11049,6201),
    ),
    6206 => array(
        'm_title' => 'MENCH SOURCES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-table source" aria-hidden="true"></i>',
        'm_profile' => array(6212,4527,7735),
    ),
    6172 => array(
        'm_title' => 'SOURCE METADATA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-lambda source"></i>',
        'm_profile' => array(7277,4527,6212,6206),
    ),
    4341 => array(
        'm_title' => 'MENCH LEDGER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-atlas discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13450,12701,12500,6212,12887,11047,12707,10876,11035,4527,7735),
    ),
    6103 => array(
        'm_title' => 'TRANSACTION METADATA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-lambda"></i>',
        'm_profile' => array(6212,4527,4341),
    ),
    13442 => array(
        'm_title' => 'TRANSACTION TYPE UPDATE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes" aria-hidden="true"></i>',
        'm_profile' => array(4527,6212),
    ),
);

//SOURCE TAB SHOW IF SOURCE:
$config['n___13425'] = array(6225);
$config['e___13425'] = array(
    6225 => array(
        'm_title' => 'MY ACCOUNT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cog source" aria-hidden="true"></i>',
        'm_profile' => array(12574,13425,11089,4527),
    ),
);

//SORTABLE:
$config['n___13413'] = array(6132,13412);
$config['e___13413'] = array(
    6132 => array(
        'm_title' => 'DISCOVERY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-arrows"></i>',
        'm_profile' => array(11035,13413,6153,4593,4755),
    ),
    13412 => array(
        'm_title' => 'IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-arrows"></i>',
        'm_profile' => array(11035,13413,4755,4593),
    ),
);

//IDEA TOOLBAR:
$config['n___13408'] = array(7585,4737,4486,4358,6402);
$config['e___13408'] = array(
    7585 => array(
        'm_title' => 'IDEA TYPES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes idea" aria-hidden="true"></i>',
        'm_profile' => array(12701,13408,13295,11054,12079,6201,4527,6204,10651,6160),
    ),
    4737 => array(
        'm_title' => 'IDEA STATUS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sliders-h idea" aria-hidden="true"></i>',
        'm_profile' => array(12701,13408,13295,11054,12079,4527,6204,6226,6160,6201),
    ),
    4486 => array(
        'm_title' => 'IDEA LINKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link idea" aria-hidden="true"></i>',
        'm_profile' => array(13442,13408,12700,11054,12079,10662,4527),
    ),
    4358 => array(
        'm_title' => 'DISCOVERY MARKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comment-alt-check discover" aria-hidden="true"></i>',
        'm_profile' => array(13408,12700,12112,10663,6103,6410),
    ),
    6402 => array(
        'm_title' => 'CONDITION SCORE RANGE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-temperature-high idea" aria-hidden="true"></i>',
        'm_profile' => array(13408,12700,10664,6410),
    ),
);

//DISCOVER LAYOUT:
$config['n___12994'] = array(4736,4231,4235,13291,12419,13289);
$config['e___12994'] = array(
    4736 => array(
        'm_title' => 'IDEA TITLE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-h1 idea" aria-hidden="true"></i>',
        'm_profile' => array(13407,13294,12994,6404,12112,10644,6201),
    ),
    4231 => array(
        'm_title' => 'MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comment-lines idea" aria-hidden="true"></i>',
        'm_profile' => array(12994,13550,13294,12359,4485,10593,4986,4603,4593),
    ),
    4235 => array(
        'm_title' => 'GET STARTED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chevron-right discover" aria-hidden="true"></i>',
        'm_profile' => array(12994,11035,12969,12227,4593,5967,4755),
    ),
    13291 => array(
        'm_title' => 'DISCOVERY TABS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bars discover"></i>',
        'm_profile' => array(10939,4527,12994),
    ),
    12419 => array(
        'm_title' => 'COMMENTS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-comments discover" aria-hidden="true"></i>',
        'm_profile' => array(12701,11035,13871,12994,11089,13530,11018,12359,5967,10593,4986,4593,4485),
    ),
    13289 => array(
        'm_title' => 'DISCOVERY CONTROLLER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-rectangle-wide discover"></i>',
        'm_profile' => array(4527,12994),
    ),
);

//IDEA NOTES SORTING ALLOWED:
$config['n___4603'] = array(4231);
$config['e___4603'] = array(
    4231 => array(
        'm_title' => 'MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comment-lines idea" aria-hidden="true"></i>',
        'm_profile' => array(12994,13550,13294,12359,4485,10593,4986,4603,4593),
    ),
);

//DISCOVER TAB SHOW IF LOGGED-IN:
$config['n___13304'] = array();
$config['e___13304'] = array(
);

//DISCOVER TAB HIDE IF ZERO:
$config['n___13298'] = array(7545,13865);
$config['e___13298'] = array(
    7545 => array(
        'm_title' => 'AWARDS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-file-certificate source" aria-hidden="true"></i>',
        'm_profile' => array(11089,13530,6159,13421,13298,12197,11018,4485,7551,4593),
    ),
    13865 => array(
        'm_title' => 'REQUIREMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shield-check source"></i>',
        'm_profile' => array(4527,11035,13421,13298,7551,13530,4593,11018,11089,4485),
    ),
);

//DISCOVER TAB OPEN BY DEFAULT:
$config['n___13300'] = array(12273);
$config['e___13300'] = array(
    12273 => array(
        'm_title' => 'IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13300,13291,13450,12675,13530,11018,11089,12761,12112,12467,12228),
    ),
);

//DISCOVERY TABS:
$config['n___13291'] = array(12273,12274,13023);
$config['e___13291'] = array(
    12273 => array(
        'm_title' => 'IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13300,13291,13450,12675,13530,11018,11089,12761,12112,12467,12228),
    ),
    12274 => array(
        'm_title' => 'SOURCES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle source" aria-hidden="true"></i>',
        'm_profile' => array(13920,13450,13530,11018,11089,12761,13291,12467,12228),
    ),
    13023 => array(
        'm_title' => 'SHARE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-share"></i>',
        'm_profile' => array(13291,4527,13024),
    ),
);

//LEADERBOARD:
$config['n___13207'] = array(4430,13339,13897);
$config['e___13207'] = array(
    4430 => array(
        'm_title' => 'MENCH MEMBERS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-users source" aria-hidden="true"></i>',
        'm_profile' => array(13450,13207,6159,4364,10573),
    ),
    13339 => array(
        'm_title' => 'INDUSTRY EXPERTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-badge-check source"></i>',
        'm_profile' => array(13450,12523,12563,6159,13207),
    ),
    13897 => array(
        'm_title' => 'NONFICTION CONTENT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-file-certificate source"></i>',
        'm_profile' => array(6159,13450,13207),
    ),
);

//SEARCH INDEX:
$config['n___12761'] = array(12273,12274);
$config['e___12761'] = array(
    12273 => array(
        'm_title' => 'IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13300,13291,13450,12675,13530,11018,11089,12761,12112,12467,12228),
    ),
    12274 => array(
        'm_title' => 'SOURCES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle source" aria-hidden="true"></i>',
        'm_profile' => array(13920,13450,13530,11018,11089,12761,13291,12467,12228),
    ),
);

//SHARE:
$config['n___13023'] = array(10876,13531,4601);
$config['e___13023'] = array(
    10876 => array(
        'm_title' => 'MENCH URL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link"></i>',
        'm_profile' => array(13672,13023,6403,4527,1326),
    ),
    13531 => array(
        'm_title' => 'QUICK SHARE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-share-alt"></i>',
        'm_profile' => array(13023,4527),
    ),
    4601 => array(
        'm_title' => 'KEYWORDS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-tags idea" aria-hidden="true"></i>',
        'm_profile' => array(12700,13023,13530,11018,4485,10593,4593),
    ),
);

//IDEA TYPE ALL NEXT:
$config['n___13022'] = array(6677,6683,7637);
$config['e___13022'] = array(
    6677 => array(
        'm_title' => 'READ & NEXT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12211,13022,12330,7585,6192,4559),
    ),
    6683 => array(
        'm_title' => 'REPLY & NEXT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-keyboard idea" aria-hidden="true"></i>',
        'm_profile' => array(13022,12955,6144,7585,6192),
    ),
    7637 => array(
        'm_title' => 'UPLOAD & NEXT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cloud-upload idea" aria-hidden="true"></i>',
        'm_profile' => array(13022,12955,12117,7585,6192),
    ),
);

//MY DISCOVERIES:
$config['n___12969'] = array(4235,7495);
$config['e___12969'] = array(
    4235 => array(
        'm_title' => 'GET STARTED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chevron-right discover" aria-hidden="true"></i>',
        'm_profile' => array(12994,11035,12969,12227,4593,5967,4755),
    ),
    7495 => array(
        'm_title' => 'RECOMMENDED DISCOVERY',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-megaphone discover" aria-hidden="true"></i>',
        'm_profile' => array(12969,12227,4593,4755),
    ),
);

//IDEA TYPE TAKES COMPLETION TIME:
$config['n___12955'] = array(6683,6684,7231,7637);
$config['e___12955'] = array(
    6683 => array(
        'm_title' => 'REPLY & NEXT',
        'm_message' => '60',
        'm_icon' => '<i class="fas fa-keyboard idea" aria-hidden="true"></i>',
        'm_profile' => array(13022,12955,6144,7585,6192),
    ),
    6684 => array(
        'm_title' => 'SELECT ONE',
        'm_message' => '15',
        'm_icon' => '<i class="fas fa-check-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12955,12883,12336,12129,7585,7712,6193,6157),
    ),
    7231 => array(
        'm_title' => 'SELECT SOME',
        'm_message' => '20',
        'm_icon' => '<i class="fas fa-check-square idea" aria-hidden="true"></i>',
        'm_profile' => array(12955,12884,12334,12129,7585,7712,6193,7489),
    ),
    7637 => array(
        'm_title' => 'UPLOAD & NEXT',
        'm_message' => '30',
        'm_icon' => '<i class="fas fa-cloud-upload idea" aria-hidden="true"></i>',
        'm_profile' => array(13022,12955,12117,7585,6192),
    ),
);

//SOURCE MENU:
$config['n___12887'] = array(12193,4341,12888,7267,12712,7279);
$config['e___12887'] = array(
    12193 => array(
        'm_title' => 'GOOGLE',
        'm_message' => '/e/search_google/',
        'm_icon' => '<i class="fab fa-google"></i>',
        'm_profile' => array(12891,12887),
    ),
    4341 => array(
        'm_title' => 'MENCH LEDGER',
        'm_message' => '/ledger?any_e__id=',
        'm_icon' => '<i class="fas fa-atlas discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13450,12701,12500,6212,12887,11047,12707,10876,11035,4527,7735),
    ),
    12888 => array(
        'm_title' => 'SOURCE EXPLORE EXPERTS',
        'm_message' => '/e/plugin/12888?e__id=',
        'm_icon' => '<i class="fas fa-shield-check source"></i>',
        'm_profile' => array(12741,6287,12887),
    ),
    7267 => array(
        'm_title' => 'SOURCE ICON SEARCH',
        'm_message' => '/e/search_icon/',
        'm_icon' => '🔍',
        'm_profile' => array(12887,6287),
    ),
    12712 => array(
        'm_title' => 'SOURCE REVIEW JSON',
        'm_message' => '/e/plugin/12712?e__id=',
        'm_icon' => '<i class="fad fa-lambda source" aria-hidden="true"></i>',
        'm_profile' => array(12887,12741,6287),
    ),
    7279 => array(
        'm_title' => 'SYNC SEARCH INDEX',
        'm_message' => '/e/plugin/7279?obj=4536&object__id=',
        'm_icon' => '<i class="fad fa-search"></i>',
        'm_profile' => array(6287,12741,12887,11047,3323,7287),
    ),
);

//IDEA TYPE SELECT ONE:
$config['n___12883'] = array(6907,6684);
$config['e___12883'] = array(
    6907 => array(
        'm_title' => 'MEET ANY REQUIREMENT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cube idea" aria-hidden="true"></i>',
        'm_profile' => array(12330,12883,12700,7486,7485,6140,7585,6193,7309,6997),
    ),
    6684 => array(
        'm_title' => 'SELECT ONE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12955,12883,12336,12129,7585,7712,6193,6157),
    ),
);

//IDEA TYPE SELECT SOME:
$config['n___12884'] = array(7231);
$config['e___12884'] = array(
    7231 => array(
        'm_title' => 'SELECT SOME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square idea" aria-hidden="true"></i>',
        'm_profile' => array(12955,12884,12334,12129,7585,7712,6193,7489),
    ),
);

//IDEA LINK ONE-WAY:
$config['n___12842'] = array(4229);
$config['e___12842'] = array(
    4229 => array(
        'm_title' => 'IDEA LINK CONDITIONAL',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-question-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12842,4486,4527,6410,6283,4593),
    ),
);

//IDEA LINK TWO-WAYS:
$config['n___12840'] = array(4228);
$config['e___12840'] = array(
    4228 => array(
        'm_title' => 'IDEA LINK FIXED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12840,4486,6410,4593),
    ),
);

//SOURCE LINK DISPLAY MESSAGE:
$config['n___12822'] = array(4259,4257,4261,4260,4255,4256,4258);
$config['e___12822'] = array(
    4259 => array(
        'm_title' => 'AUDIO',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-volume-up source"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4257 => array(
        'm_title' => 'EMBED URL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4758,12822,12605,12524,12403,4593,4592,4537),
    ),
    4261 => array(
        'm_title' => 'FILE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-file-pdf source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4260 => array(
        'm_title' => 'IMAGE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-image source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,6198,11059,10593,4593,4592,4537),
    ),
    4255 => array(
        'm_title' => 'TEXT',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-align-left source"></i>',
        'm_profile' => array(13899,12822,10593,4593,4592),
    ),
    4256 => array(
        'm_title' => 'URL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-external-link source" aria-hidden="true"></i>',
        'm_profile' => array(13433,12822,11080,4593,4592,4537),
    ),
    4258 => array(
        'm_title' => 'VIDEO',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
);

//MY IDEAS MAP:
$config['n___10573'] = array(4430);
$config['e___10573'] = array(
    4430 => array(
        'm_title' => 'MENCH MEMBERS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-users source" aria-hidden="true"></i>',
        'm_profile' => array(13450,13207,6159,4364,10573),
    ),
);

//PLUGIN RETURN CODE ONLY:
$config['n___12741'] = array(12733,12722,7276,4356,11049,7275,4527,7277,12710,12709,12888,13790,12732,12712,7278,12967,7279,12114,12569);
$config['e___12741'] = array(
    12733 => array(
        'm_title' => 'DISCOVER REVIEW JSON',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-code discover"></i>',
        'm_profile' => array(11047,12741,12701,6287),
    ),
    12722 => array(
        'm_title' => 'DISCOVER REVIEW JSON',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(12741,6287),
    ),
    7276 => array(
        'm_title' => 'IDEA CACHED SOURCES',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sync idea mench-spin" aria-hidden="true"></i>',
        'm_profile' => array(6287,12741,11047,7286,7274),
    ),
    4356 => array(
        'm_title' => 'IDEA DURATION (SECONDS)',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-stopwatch idea" aria-hidden="true"></i>',
        'm_profile' => array(10986,13295,6287,12741,11047,7274,6404,12112,10650,6201),
    ),
    11049 => array(
        'm_title' => 'IDEA REVIEW JSON',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-lambda idea" aria-hidden="true"></i>',
        'm_profile' => array(12741,6287,11047),
    ),
    7275 => array(
        'm_title' => 'IDEA SYNC COMMON BASE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sync idea mench-spin"></i>',
        'm_profile' => array(6287,12741,7286,7274),
    ),
    4527 => array(
        'm_title' => 'MENCH MEMORY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-memory" aria-hidden="true"></i>',
        'm_profile' => array(4755,6403,12741,6287,4527),
    ),
    7277 => array(
        'm_title' => 'METADATA CLEAN VARIABLES',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(4527,6287,12741,7287,7274),
    ),
    12710 => array(
        'm_title' => 'MY SESSION VARIABLES',
        'm_message' => '',
        'm_icon' => '👤',
        'm_profile' => array(12741,6287),
    ),
    12709 => array(
        'm_title' => 'PHP INFO',
        'm_message' => '',
        'm_icon' => 'ℹ️',
        'm_profile' => array(12741,6287),
    ),
    12888 => array(
        'm_title' => 'SOURCE EXPLORE EXPERTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shield-check source"></i>',
        'm_profile' => array(12741,6287,12887),
    ),
    13790 => array(
        'm_title' => 'SOURCE IDEA SPREADSHEET',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-table idea"></i>',
        'm_profile' => array(12741,6287),
    ),
    12732 => array(
        'm_title' => 'SOURCE/IDEA SYNC STATUSES',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(12741,6287),
    ),
    12712 => array(
        'm_title' => 'SOURCE REVIEW JSON',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-lambda source" aria-hidden="true"></i>',
        'm_profile' => array(12887,12741,6287),
    ),
    7278 => array(
        'm_title' => 'SYNC GEPHI INDEX',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(6287,12741,7287,7274),
    ),
    12967 => array(
        'm_title' => 'SYNC ICON MODES',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sync source mench-spin" aria-hidden="true"></i>',
        'm_profile' => array(6287,12741,4758,7274),
    ),
    7279 => array(
        'm_title' => 'SYNC SEARCH INDEX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-search"></i>',
        'm_profile' => array(6287,12741,12887,11047,3323,7287),
    ),
    12114 => array(
        'm_title' => 'WEEKLY PAYOUTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sync discover"></i>',
        'm_profile' => array(6225,13450,4593,6287,12741,7274,7569),
    ),
    12569 => array(
        'm_title' => 'WEIGHT ALGORITHM',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-weight"></i>',
        'm_profile' => array(6287,12741,7274),
    ),
);

//MENCH RANDOM MESSAGES:
$config['n___12687'] = array(12691,12694,12695);
$config['e___12687'] = array(
    12691 => array(
        'm_title' => 'EMAIL YOURS TRULY LINE',
        'm_message' => 'Cheers, | Have an Awesome Day, | Have a Blast, | All The Best, | Enjoy, | Have Fun, | Many Thanks,',
        'm_icon' => '',
        'm_profile' => array(12687),
    ),
    12694 => array(
        'm_title' => 'LOADING MESSAGE',
        'm_message' => 'Are you having a good day today? | Be gentle with yourself today. | Congratulate yourself for the great job you\'re doing | Crunching the latest data, just for you. Hang tight... | Have a glass of water nearby? Time for a sip! | Offer hugs. Someone probably needs them. | You are unique! | Get a drink of water. Stay hydrated! | Have you danced today? | Have you listened to your favourite song recently? 🎵 | Have you stretched recently? | Have you recently told someone you\'re proud of them? | Help is out there. Don\'t be afraid to ask. | Hey! Life is tough, but so are you! 💪 | Hey, jump up for a sec and stretch, yeah? 👐 | I know it\'s cheesey but I hope you have a grate day! | Is there a window you can look through? The world is beautiful. 🌆 | Is your seat comfortable? Can you adjust your chair properly? | It can be hard to get started, can\'t it? That\'s okay, you got this. | It\'s so great to have you here today | Keep growing, keep learning, keep moving forward! | Learning new things is important - open your eyes to the world around you! | Making things awesome... | Novel, new, silly, & unusual activities can help lift your mood. | Play for a few minutes. Doodle, learn solitaire, fold a paper airplane, do something fun. | Don\'t take yourself for granted. You\'re important. | Rest your eyes for a moment. Look at something in the distance and count to five! 🌳 | Self care is important, look after and love yourself, you\'re amazing! | Set aside time for a hobby. Gardening, drone building, knitting, do something for the pure pleasure of it. | So often our power lies not in ourselves, but in how we help others find their own strength | Sometimes doing something nice for somebody else is the best way to feel good about yourself! 👭 | Stop. Breathe. Be here now. | Stop. Take three slow deep breaths. | Take 5 minutes to straighten the space around you. Set a timer. | Take a break before you need it. It will make it easier to prevent burnout. | Take a moment to send a message to someone you love 😻 | Take care of yourself. We need you. | Technology is a tool. Use it wisely. | The impact you leave on the universe can never be erased. | There are no impostors here | There\'s someone who is so so grateful that you exist together. | Today is a great day to let a friend know how much you appreciate them. | Water is good for you year round. If you\'re thirsty, you\'re dehydrated. | We all have superpowers. You included. I hope you are using yours to make your life a joyful one. | When\'s the last time you treated yourself? | With the dawning of a new day comes a clean slate and lots of opportunity. | You are fantastic | You are loved. <3 | You are so very important 💛💛💕 | You can do this! | You cannot compare your successes to the apparent achievements of others. 🌄 | You deserve to be safe and to have nice things happen to you. | You have the power to change the world. | You\'re allowed to start small. 🐞 | have you hugged anyone lately? | it\'s time to check your thirst level, human. | 💗: don\'t forget to take a little bit of time to say hi to a friend | 🌸: remember to let your eyes rest, maybe by looking at a plant... | 🙌: take a second to adjust your posture | 😎🌈💕',
        'm_icon' => '',
        'm_profile' => array(12687),
    ),
    12695 => array(
        'm_title' => 'SAVING MESSAGE',
        'm_message' => 'Learning everyday 😎 | Growing with you 🌸 | Getting smarter ^~^',
        'm_icon' => '',
        'm_profile' => array(12687),
    ),
);

//IDEA TAB OPEN BY DEFAULT:
$config['n___12675'] = array(12273);
$config['e___12675'] = array(
    12273 => array(
        'm_title' => 'IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13300,13291,13450,12675,13530,11018,11089,12761,12112,12467,12228),
    ),
);

//IDEA TAB NO MANUAL ADD:
$config['n___12677'] = array(6255,12896);
$config['e___12677'] = array(
    6255 => array(
        'm_title' => 'DISCOVERIES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,13776,13450,11089,12677,11018,12467,12228,4527),
    ),
    12896 => array(
        'm_title' => 'SAVED IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bookmark discover"></i>',
        'm_profile' => array(13877,12701,11035,11018,11089,12677,4485,7551,4755,4593),
    ),
);

//VERTICAL MENU:
$config['n___12500'] = array(13207,6287,4341,7291);
$config['e___12500'] = array(
    13207 => array(
        'm_title' => 'LEADERBOARD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-medal source"></i>',
        'm_profile' => array(13920,13907,13543,11035,12500,12489,10876,4527),
    ),
    6287 => array(
        'm_title' => 'MENCH PLUGINS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-plug" aria-hidden="true"></i>',
        'm_profile' => array(13920,12500,4367,12701,6405,12699,10876,11035,4527),
    ),
    4341 => array(
        'm_title' => 'MENCH LEDGER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-atlas discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13450,12701,12500,6212,12887,11047,12707,10876,11035,4527,7735),
    ),
    7291 => array(
        'm_title' => 'LOGOUT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-power-off"></i>',
        'm_profile' => array(13920,12500,10876),
    ),
);

//MENCH COINS:
$config['n___12467'] = array(12274,12273,6255);
$config['e___12467'] = array(
    12274 => array(
        'm_title' => 'SOURCES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle source" aria-hidden="true"></i>',
        'm_profile' => array(13920,13450,13530,11018,11089,12761,13291,12467,12228),
    ),
    12273 => array(
        'm_title' => 'IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13300,13291,13450,12675,13530,11018,11089,12761,12112,12467,12228),
    ),
    6255 => array(
        'm_title' => 'DISCOVERIES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,13776,13450,11089,12677,11018,12467,12228,4527),
    ),
);

//IDEA LIST EDITOR:
$config['n___12589'] = array(12591,12592,12611,12612);
$config['e___12589'] = array(
    12591 => array(
        'm_title' => 'SOURCE ADD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-plus-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4593,12589),
    ),
    12592 => array(
        'm_title' => 'SOURCE REMOVE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-minus-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4593,12589),
    ),
    12611 => array(
        'm_title' => 'PREVIOUS IDEA ADD',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-layer-plus idea" aria-hidden="true"></i>',
        'm_profile' => array(4593,12589),
    ),
    12612 => array(
        'm_title' => 'PREVIOUS IDEA REMOVE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-layer-minus idea" aria-hidden="true"></i>',
        'm_profile' => array(4593,12589),
    ),
);

//SIGN IN/UP:
$config['n___4269'] = array(3288,13025,3286);
$config['e___4269'] = array(
    3288 => array(
        'm_title' => 'EMAIL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-envelope-open" aria-hidden="true"></i>',
        'm_profile' => array(13861,13531,6404,4269,6225,4755),
    ),
    13025 => array(
        'm_title' => 'FULL NAME',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-fingerprint source" aria-hidden="true"></i>',
        'm_profile' => array(4269),
    ),
    3286 => array(
        'm_title' => 'PASSWORD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-key" aria-hidden="true"></i>',
        'm_profile' => array(13014,4269,6225,7578,4755),
    ),
);

//IDEA NOTES FILE UPLOADING ALLOWED:
$config['n___12359'] = array(4983,12419,4231);
$config['e___12359'] = array(
    4983 => array(
        'm_title' => 'CITATIONS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link source rotate90" aria-hidden="true"></i>',
        'm_profile' => array(12359,7551,13550,13543,13407,4485,12141,10939,12450,12228,4593,10593),
    ),
    12419 => array(
        'm_title' => 'COMMENTS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-comments discover" aria-hidden="true"></i>',
        'm_profile' => array(12701,11035,13871,12994,11089,13530,11018,12359,5967,10593,4986,4593,4485),
    ),
    4231 => array(
        'm_title' => 'MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comment-lines idea" aria-hidden="true"></i>',
        'm_profile' => array(12994,13550,13294,12359,4485,10593,4986,4603,4593),
    ),
);

//PORTFOLIO EDITOR UPPERCASE:
$config['n___12577'] = array(4999,4998,5000,11956,5981,13441,5982);
$config['e___12577'] = array(
    4999 => array(
        'm_title' => 'NAME POSTFIX',
        'm_message' => '',
        'm_icon' => '<i class="source fad fa-fingerprint"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    4998 => array(
        'm_title' => 'NAME PREFIX',
        'm_message' => '',
        'm_icon' => '<i class="source fad fa-fingerprint"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    5000 => array(
        'm_title' => 'NAME REPLACE',
        'm_message' => '',
        'm_icon' => '<i class="source fad fa-fingerprint"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    11956 => array(
        'm_title' => 'PROFILE COPY @ IF HAS @',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-layer-plus source"></i>',
        'm_profile' => array(12577,4593,4997),
    ),
    5981 => array(
        'm_title' => 'PROFILE COPY @ IF MISSING',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-layer-plus source"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    13441 => array(
        'm_title' => 'PROFILE MOVE @ IF MISSING',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-arrow-right source"></i>',
        'm_profile' => array(12577,4593,4997),
    ),
    5982 => array(
        'm_title' => 'PROFILE REMOVE @ IF THERE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-layer-minus source"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
);

//SOURCE STATUS FEATURED:
$config['n___12575'] = array(12563);
$config['e___12575'] = array(
    12563 => array(
        'm_title' => 'SOURCES FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star source" aria-hidden="true"></i>',
        'm_profile' => array(12399,4527,12575,10654,7358,7357,6177),
    ),
);

//SOURCE LINK VISUAL:
$config['n___12524'] = array(4259,4257,4261,4260,4258);
$config['e___12524'] = array(
    4259 => array(
        'm_title' => 'AUDIO',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-volume-up source"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4257 => array(
        'm_title' => 'EMBED URL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4758,12822,12605,12524,12403,4593,4592,4537),
    ),
    4261 => array(
        'm_title' => 'FILE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-file-pdf source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4260 => array(
        'm_title' => 'IMAGE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-image source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,6198,11059,10593,4593,4592,4537),
    ),
    4258 => array(
        'm_title' => 'VIDEO',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
);

//SYNC ICONS:
$config['n___12523'] = array(2997,4446,3005,6293,13339,3147,13350,3192,2998);
$config['e___12523'] = array(
    2997 => array(
        'm_title' => 'ARTICLES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-newspaper source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    4446 => array(
        'm_title' => 'ASSESSMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    3005 => array(
        'm_title' => 'BOOKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-book source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    6293 => array(
        'm_title' => 'GIPHY GIFS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-file-image source"></i>',
        'm_profile' => array(12523,12891,1326),
    ),
    13339 => array(
        'm_title' => 'INDUSTRY EXPERTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-badge-check source"></i>',
        'm_profile' => array(13450,12523,12563,6159,13207),
    ),
    3147 => array(
        'm_title' => 'ONLINE COURSES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-presentation source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    13350 => array(
        'm_title' => 'REPORTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-file-chart-line source"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    3192 => array(
        'm_title' => 'TOOLS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-tools source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    2998 => array(
        'm_title' => 'VIDEOS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle source"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
);

//DISCOVERY LEGEND:
$config['n___12446'] = array(13752,13757,13822,12447,13338);
$config['e___12446'] = array(
    13752 => array(
        'm_title' => 'LOCKED',
        'm_message' => '',
        'm_icon' => '<i class="far fa-lock"></i>',
        'm_profile' => array(12446),
    ),
    13757 => array(
        'm_title' => 'CURRENT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea"></i>',
        'm_profile' => array(12446),
    ),
    13822 => array(
        'm_title' => 'NEXT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-step-forward discover"></i>',
        'm_profile' => array(12446),
    ),
    12447 => array(
        'm_title' => 'IN PROGRESS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle discover"></i>',
        'm_profile' => array(12446),
    ),
    13338 => array(
        'm_title' => 'COMPLETED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover"></i>',
        'm_profile' => array(12446),
    ),
);

//MENCH URL:
$config['n___10876'] = array(13810,13207,7291,4341,6287,4269);
$config['e___10876'] = array(
    13810 => array(
        'm_title' => 'EVENT #1 REGISTRY',
        'm_message' => 'https://mench.com/e/plugin/13790?i__id=13980&i__tree_id=14289&e_sources_id=13810',
        'm_icon' => '🔥',
        'm_profile' => array(10876,13465),
    ),
    13207 => array(
        'm_title' => 'LEADERBOARD',
        'm_message' => 'https://mench.com/@',
        'm_icon' => '<i class="fas fa-medal source"></i>',
        'm_profile' => array(13920,13907,13543,11035,12500,12489,10876,4527),
    ),
    7291 => array(
        'm_title' => 'LOGOUT',
        'm_message' => 'https://mench.com/e/signout',
        'm_icon' => '<i class="fas fa-power-off"></i>',
        'm_profile' => array(13920,12500,10876),
    ),
    4341 => array(
        'm_title' => 'MENCH LEDGER',
        'm_message' => 'https://mench.com/ledger',
        'm_icon' => '<i class="fas fa-atlas discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13450,12701,12500,6212,12887,11047,12707,10876,11035,4527,7735),
    ),
    6287 => array(
        'm_title' => 'MENCH PLUGINS',
        'm_message' => 'https://mench.com/e/plugin',
        'm_icon' => '<i class="fas fa-plug" aria-hidden="true"></i>',
        'm_profile' => array(13920,12500,4367,12701,6405,12699,10876,11035,4527),
    ),
    4269 => array(
        'm_title' => 'SIGN IN/UP',
        'm_message' => 'https://mench.com/signin',
        'm_icon' => '<i class="fas fa-sign-in-alt" aria-hidden="true"></i>',
        'm_profile' => array(13479,10876,4527,11035),
    ),
);

//SOURCE LINK TYPE CUSTOM UI:
$config['n___12403'] = array(4257);
$config['e___12403'] = array(
    4257 => array(
        'm_title' => 'SOURCE LINK EMBED URL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4758,12822,12605,12524,12403,4593,4592,4537),
    ),
);

//SOURCE STATUS SYNC:
$config['n___12401'] = array(4251,6178,10654);
$config['e___12401'] = array(
    4251 => array(
        'm_title' => 'SOURCE CREATED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle source" aria-hidden="true"></i>',
        'm_profile' => array(13548,12401,12149,12141,10593,4593),
    ),
    6178 => array(
        'm_title' => 'SOURCES DELETE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-trash-alt source" aria-hidden="true"></i>',
        'm_profile' => array(6173,4593,12401,6177),
    ),
    10654 => array(
        'm_title' => 'SOURCE STATUS UPDATE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sliders-h source" aria-hidden="true"></i>',
        'm_profile' => array(12401,4593),
    ),
);

//IDEA STATUS SYNC:
$config['n___12400'] = array(4250,6182,10648);
$config['e___12400'] = array(
    4250 => array(
        'm_title' => 'CREATED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13480,5967,12400,12149,10593,4593),
    ),
    6182 => array(
        'm_title' => 'DELETE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-trash-alt idea" aria-hidden="true"></i>',
        'm_profile' => array(6173,12400,4593,4737),
    ),
    10648 => array(
        'm_title' => 'UPDATE STATUS',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sliders-h idea" aria-hidden="true"></i>',
        'm_profile' => array(12400,4593),
    ),
);

//IDEA TYPE COMPLETE IF EMPTY:
$config['n___12330'] = array(6914,6907,6677);
$config['e___12330'] = array(
    6914 => array(
        'm_title' => 'MEET ALL REQUIREMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cubes idea" aria-hidden="true"></i>',
        'm_profile' => array(12330,12700,7486,7485,6140,7585,6192,7309,6997),
    ),
    6907 => array(
        'm_title' => 'MEET ANY REQUIREMENT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cube idea" aria-hidden="true"></i>',
        'm_profile' => array(12330,12883,12700,7486,7485,6140,7585,6193,7309,6997),
    ),
    6677 => array(
        'm_title' => 'READ & NEXT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12211,13022,12330,7585,6192,4559),
    ),
);

//DISCOVER UNLOCKS:
$config['n___12327'] = array(7485,7486,6997);
$config['e___12327'] = array(
    7485 => array(
        'm_title' => 'ANSWER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12334,12336,7489,6157,12327,12229,12227,12141,4593,4755,6255),
    ),
    7486 => array(
        'm_title' => 'CHILDREN',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12327,12229,12227,12141,4755,4593,6255),
    ),
    6997 => array(
        'm_title' => 'CONDITION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(6140,12327,12229,12227,12141,4229,6255,4593,4755),
    ),
);

//DISCOVER EXPANSIONS:
$config['n___12326'] = array(12336,12334,6140);
$config['e___12326'] = array(
    12336 => array(
        'm_title' => 'ANSWER ONE LINK',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(7704,4755,4593,12326,12227),
    ),
    12334 => array(
        'm_title' => 'ANSWER SOME LINK',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square discover" aria-hidden="true"></i>',
        'm_profile' => array(7704,4755,4593,12326,12227),
    ),
    6140 => array(
        'm_title' => 'UNLOCK CONDITION LINK',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-lock-open discover" aria-hidden="true"></i>',
        'm_profile' => array(12326,12227,6410,4229,4755,4593),
    ),
);

//AVATARS SUPER:
$config['n___12279'] = array(12280,12281,12282,12286,12287,12288,12308,12309,12310,12234,12233,10965,12236,12235,10979,12295,12294,12293,12296,12297,12298,12300,12301,12299,12237,12238,10978,12314,12315,12316,12240,12239,10963,12241,12242,12207,12244,12243,10966,12245,12246,10976,12248,12247,10962,12249,12250,10975,12252,12251,10982,12253,12254,10970,12302,12303,12304,12256,12255,10972,12306,12307,12305,12257,12258,10969,12312,12313,12311,12260,12259,10960,12277,12276,12278,12439,12262,10981,12264,12263,10968,12265,12266,10974,12290,12291,12292,12268,12267,12206,12269,12270,10958,12285,12284,12283,12272,12271,12231);
$config['e___12279'] = array(
    12280 => array(
        'm_title' => 'ALICORN BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-alicorn source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12281 => array(
        'm_title' => 'ALICORN LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-alicorn source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12282 => array(
        'm_title' => 'ALICORN MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-alicorn source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12286 => array(
        'm_title' => 'BAT BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bat source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12287 => array(
        'm_title' => 'BAT LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-bat source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12288 => array(
        'm_title' => 'BAT MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-bat source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12308 => array(
        'm_title' => 'BLACK WIDOW BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-spider-black-widow source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12309 => array(
        'm_title' => 'BLACK WIDOW LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-spider-black-widow source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12310 => array(
        'm_title' => 'BLACK WIDOW MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-spider-black-widow source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12234 => array(
        'm_title' => 'DOGY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-dog source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12233 => array(
        'm_title' => 'DOGY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-dog source"></i>',
        'm_profile' => array(12279,10956),
    ),
    10965 => array(
        'm_title' => 'DOGY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-dog source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12236 => array(
        'm_title' => 'DONALD BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-duck source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12235 => array(
        'm_title' => 'DONALD LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-duck source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10979 => array(
        'm_title' => 'DONALD MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-duck source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12295 => array(
        'm_title' => 'DOVE BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-dove source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12294 => array(
        'm_title' => 'DOVE LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-dove source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12293 => array(
        'm_title' => 'DOVE MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-dove source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12296 => array(
        'm_title' => 'DRAGON BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-dragon source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12297 => array(
        'm_title' => 'DRAGON LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-dragon source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12298 => array(
        'm_title' => 'DRAGON MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-dragon source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12300 => array(
        'm_title' => 'ELEPHANT BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-elephant source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12301 => array(
        'm_title' => 'ELEPHANT LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-elephant source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12299 => array(
        'm_title' => 'ELEPHANT MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-elephant source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12237 => array(
        'm_title' => 'FISHY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-fish source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12238 => array(
        'm_title' => 'FISHY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-fish source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10978 => array(
        'm_title' => 'FISHY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-fish source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12314 => array(
        'm_title' => 'FROG BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-frog source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12315 => array(
        'm_title' => 'FROG LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-frog source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12316 => array(
        'm_title' => 'FROG MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-frog source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12240 => array(
        'm_title' => 'HIPPOY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-hippo source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12239 => array(
        'm_title' => 'HIPPOY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-hippo source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10963 => array(
        'm_title' => 'HIPPOY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-hippo source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12241 => array(
        'm_title' => 'HONEY BADGER BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-badger-honey source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12242 => array(
        'm_title' => 'HONEY BADGER LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-badger-honey source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12207 => array(
        'm_title' => 'HONEY BADGER MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-badger-honey source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12244 => array(
        'm_title' => 'HONEY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-deer source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12243 => array(
        'm_title' => 'HONEY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-deer source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10966 => array(
        'm_title' => 'HONEY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-deer source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12245 => array(
        'm_title' => 'HORSY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-horse source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12246 => array(
        'm_title' => 'HORSY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-horse source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10976 => array(
        'm_title' => 'HORSY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-horse source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12248 => array(
        'm_title' => 'HUMAN BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-monkey source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12247 => array(
        'm_title' => 'HUMAN LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-monkey source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10962 => array(
        'm_title' => 'HUMAN MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-monkey source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12249 => array(
        'm_title' => 'KIWI BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-kiwi-bird source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12250 => array(
        'm_title' => 'KIWI LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-kiwi-bird source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10975 => array(
        'm_title' => 'KIWI MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-kiwi-bird source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12252 => array(
        'm_title' => 'MIMY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cat source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12251 => array(
        'm_title' => 'MIMY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-cat source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10982 => array(
        'm_title' => 'MIMY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-cat source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12253 => array(
        'm_title' => 'MOMY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cow source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12254 => array(
        'm_title' => 'MOMY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-cow source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10970 => array(
        'm_title' => 'MOMY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-cow source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12302 => array(
        'm_title' => 'NARWHAL BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-narwhal source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12303 => array(
        'm_title' => 'NARWHAL LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-narwhal source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12304 => array(
        'm_title' => 'NARWHAL MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-narwhal source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12256 => array(
        'm_title' => 'NINJA BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-turtle source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12255 => array(
        'm_title' => 'NINJA LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-turtle source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10972 => array(
        'm_title' => 'NINJA MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-turtle source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12306 => array(
        'm_title' => 'PEGASUS BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-pegasus source"></i>',
        'm_profile' => array(10984,12700,12279),
    ),
    12307 => array(
        'm_title' => 'PEGASUS LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-pegasus source"></i>',
        'm_profile' => array(10984,12700,12279),
    ),
    12305 => array(
        'm_title' => 'PEGASUS MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-pegasus source" aria-hidden="true"></i>',
        'm_profile' => array(10984,12700,12279),
    ),
    12257 => array(
        'm_title' => 'PIGGY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-pig source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12258 => array(
        'm_title' => 'PIGGY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-pig source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10969 => array(
        'm_title' => 'PIGGY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-pig source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12312 => array(
        'm_title' => 'RAM BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-ram source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12313 => array(
        'm_title' => 'RAM LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-ram source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12311 => array(
        'm_title' => 'RAM MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-ram source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12260 => array(
        'm_title' => 'ROGER BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-rabbit source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12259 => array(
        'm_title' => 'ROGER LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-rabbit source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10960 => array(
        'm_title' => 'ROGER MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-rabbit source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12277 => array(
        'm_title' => 'RUDOLPH BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-deer-rudolph source" aria-hidden="true"></i>',
        'm_profile' => array(10984,12279),
    ),
    12276 => array(
        'm_title' => 'RUDOLPH LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-deer-rudolph source" aria-hidden="true"></i>',
        'm_profile' => array(10984,12279),
    ),
    12278 => array(
        'm_title' => 'RUDOLPH MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-deer-rudolph source" aria-hidden="true"></i>',
        'm_profile' => array(10984,12279),
    ),
    12439 => array(
        'm_title' => 'RUSSEL BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-crow source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12262 => array(
        'm_title' => 'RUSSEL LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-crow source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10981 => array(
        'm_title' => 'RUSSEL MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-crow source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12264 => array(
        'm_title' => 'SHEEPY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sheep source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12263 => array(
        'm_title' => 'SHEEPY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-sheep source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10968 => array(
        'm_title' => 'SHEEPY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sheep source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12265 => array(
        'm_title' => 'SNAKY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-snake source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12266 => array(
        'm_title' => 'SNAKY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-snake source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10974 => array(
        'm_title' => 'SNAKY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-snake source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12290 => array(
        'm_title' => 'SPACE CAT BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cat-space source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12291 => array(
        'm_title' => 'SPACE CAT LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-cat-space source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12292 => array(
        'm_title' => 'SPACE CAT MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-cat-space source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12268 => array(
        'm_title' => 'SPIDER BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-spider source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12267 => array(
        'm_title' => 'SPIDER LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-spider source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12206 => array(
        'm_title' => 'SPIDER MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-spider source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12269 => array(
        'm_title' => 'SQUIRRELY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-squirrel source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12270 => array(
        'm_title' => 'SQUIRRELY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-squirrel source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10958 => array(
        'm_title' => 'SQUIRRELY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-squirrel source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12285 => array(
        'm_title' => 'UNICORN BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-unicorn source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12284 => array(
        'm_title' => 'UNICORN LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-unicorn source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12283 => array(
        'm_title' => 'UNICORN MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-unicorn source"></i>',
        'm_profile' => array(10984,12279),
    ),
    12272 => array(
        'm_title' => 'WHALE BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-whale source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12271 => array(
        'm_title' => 'WHALE LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-whale source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12231 => array(
        'm_title' => 'WHALE MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-whale source"></i>',
        'm_profile' => array(12279,10956),
    ),
);

//DISCOVERY COMPLETED:
$config['n___12229'] = array(7492,6157,7489,12117,4559,6144,7485,7486,6997);
$config['e___12229'] = array(
    7492 => array(
        'm_title' => 'DISCOVER TERMINATE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-times-octagon discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,4755,4593,6146),
    ),
    6157 => array(
        'm_title' => 'SOURCE ANSWER ONE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4593,6255,4755),
    ),
    7489 => array(
        'm_title' => 'SOURCE ANSWER SOME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4755,6255,4593),
    ),
    12117 => array(
        'm_title' => 'SOURCE FILE UPLOAD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cloud-upload discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,4593,4755,6255),
    ),
    4559 => array(
        'm_title' => 'SOURCE READ MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="far fa-circle discover"></i>',
        'm_profile' => array(12229,12227,12141,6255,4755,4593),
    ),
    6144 => array(
        'm_title' => 'SOURCE TEXT REPLY',
        'm_message' => '',
        'm_icon' => '<i class="far fa-keyboard discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,6255,4755,4593),
    ),
    7485 => array(
        'm_title' => 'SOURCE UNLOCK ANSWER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12334,12336,7489,6157,12327,12229,12227,12141,4593,4755,6255),
    ),
    7486 => array(
        'm_title' => 'SOURCE UNLOCK CHILDREN',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12327,12229,12227,12141,4755,4593,6255),
    ),
    6997 => array(
        'm_title' => 'SOURCE UNLOCK CONDITION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(6140,12327,12229,12227,12141,4229,6255,4593,4755),
    ),
);

//DISCOVER PROGRESS:
$config['n___12227'] = array(12336,12334,7492,6140,4235,7495,6157,7489,12117,4559,6144,7485,7486,6997);
$config['e___12227'] = array(
    12336 => array(
        'm_title' => 'DISCOVER ANSWER ONE LINK',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(7704,4755,4593,12326,12227),
    ),
    12334 => array(
        'm_title' => 'DISCOVER ANSWER SOME LINK',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square discover" aria-hidden="true"></i>',
        'm_profile' => array(7704,4755,4593,12326,12227),
    ),
    7492 => array(
        'm_title' => 'DISCOVER TERMINATE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-times-octagon discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,4755,4593,6146),
    ),
    6140 => array(
        'm_title' => 'DISCOVER UNLOCK CONDITION LINK',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-lock-open discover" aria-hidden="true"></i>',
        'm_profile' => array(12326,12227,6410,4229,4755,4593),
    ),
    4235 => array(
        'm_title' => 'GET STARTED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chevron-right discover" aria-hidden="true"></i>',
        'm_profile' => array(12994,11035,12969,12227,4593,5967,4755),
    ),
    7495 => array(
        'm_title' => 'RECOMMENDED DISCOVERY',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-megaphone discover" aria-hidden="true"></i>',
        'm_profile' => array(12969,12227,4593,4755),
    ),
    6157 => array(
        'm_title' => 'SOURCE ANSWER ONE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4593,6255,4755),
    ),
    7489 => array(
        'm_title' => 'SOURCE ANSWER SOME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4755,6255,4593),
    ),
    12117 => array(
        'm_title' => 'SOURCE FILE UPLOAD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cloud-upload discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,4593,4755,6255),
    ),
    4559 => array(
        'm_title' => 'SOURCE READ MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="far fa-circle discover"></i>',
        'm_profile' => array(12229,12227,12141,6255,4755,4593),
    ),
    6144 => array(
        'm_title' => 'SOURCE TEXT REPLY',
        'm_message' => '',
        'm_icon' => '<i class="far fa-keyboard discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,6255,4755,4593),
    ),
    7485 => array(
        'm_title' => 'SOURCE UNLOCK ANSWER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12334,12336,7489,6157,12327,12229,12227,12141,4593,4755,6255),
    ),
    7486 => array(
        'm_title' => 'SOURCE UNLOCK CHILDREN',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12327,12229,12227,12141,4755,4593,6255),
    ),
    6997 => array(
        'm_title' => 'SOURCE UNLOCK CONDITION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(6140,12327,12229,12227,12141,4229,6255,4593,4755),
    ),
);

//DISCOVER TYPE COIN AWARD:
$config['n___12141'] = array(4983,6157,7489,4251,12117,4559,6144,7485,7486,6997);
$config['e___12141'] = array(
    4983 => array(
        'm_title' => 'CITATIONS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link source rotate90" aria-hidden="true"></i>',
        'm_profile' => array(12359,7551,13550,13543,13407,4485,12141,10939,12450,12228,4593,10593),
    ),
    6157 => array(
        'm_title' => 'SOURCE ANSWER ONE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4593,6255,4755),
    ),
    7489 => array(
        'm_title' => 'SOURCE ANSWER SOME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4755,6255,4593),
    ),
    4251 => array(
        'm_title' => 'SOURCE CREATED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle source" aria-hidden="true"></i>',
        'm_profile' => array(13548,12401,12149,12141,10593,4593),
    ),
    12117 => array(
        'm_title' => 'SOURCE FILE UPLOAD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cloud-upload discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,4593,4755,6255),
    ),
    4559 => array(
        'm_title' => 'SOURCE READ MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="far fa-circle discover"></i>',
        'm_profile' => array(12229,12227,12141,6255,4755,4593),
    ),
    6144 => array(
        'm_title' => 'SOURCE TEXT REPLY',
        'm_message' => '',
        'm_icon' => '<i class="far fa-keyboard discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,6255,4755,4593),
    ),
    7485 => array(
        'm_title' => 'SOURCE UNLOCK ANSWER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12334,12336,7489,6157,12327,12229,12227,12141,4593,4755,6255),
    ),
    7486 => array(
        'm_title' => 'SOURCE UNLOCK CHILDREN',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12327,12229,12227,12141,4755,4593,6255),
    ),
    6997 => array(
        'm_title' => 'SOURCE UNLOCK CONDITION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(6140,12327,12229,12227,12141,4229,6255,4593,4755),
    ),
);

//IDEA STATUS FEATURED:
$config['n___12138'] = array(12137);
$config['e___12138'] = array(
    12137 => array(
        'm_title' => 'IDEA FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star idea" aria-hidden="true"></i>',
        'm_profile' => array(12399,13813,6404,11035,13420,10648,12138,7356,7355,4737),
    ),
);

//MENCH TEXT INPUTS:
$config['n___12112'] = array(4358,4356,12273,4736,6197,4739,4735);
$config['e___12112'] = array(
    4358 => array(
        'm_title' => 'DISCOVERY MARKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comment-alt-check discover" aria-hidden="true"></i>',
        'm_profile' => array(13408,12700,12112,10663,6103,6410),
    ),
    4356 => array(
        'm_title' => 'IDEA DURATION (SECONDS)',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-stopwatch idea" aria-hidden="true"></i>',
        'm_profile' => array(10986,13295,6287,12741,11047,7274,6404,12112,10650,6201),
    ),
    12273 => array(
        'm_title' => 'IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13300,13291,13450,12675,13530,11018,11089,12761,12112,12467,12228),
    ),
    4736 => array(
        'm_title' => 'IDEA TITLE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-h1 idea" aria-hidden="true"></i>',
        'm_profile' => array(13407,13294,12994,6404,12112,10644,6201),
    ),
    6197 => array(
        'm_title' => 'SOURCE TITLE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-fingerprint source" aria-hidden="true"></i>',
        'm_profile' => array(13571,13296,13025,6404,12112,12232,10646,5000,4998,4999,6206),
    ),
    4739 => array(
        'm_title' => 'UNLOCK MAX PERCENTAGE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-temperature-up idea" aria-hidden="true"></i>',
        'm_profile' => array(6103,12112,6402),
    ),
    4735 => array(
        'm_title' => 'UNLOCK MIN PERCENTAGE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-temperature-down idea" aria-hidden="true"></i>',
        'm_profile' => array(6103,12112,6402),
    ),
);

//MENCH DROPDOWN MENUS:
$config['n___12079'] = array(3000,4486,4737,7585,12500);
$config['e___12079'] = array(
    3000 => array(
        'm_title' => 'NONFICTION TYPES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes source" aria-hidden="true"></i>',
        'm_profile' => array(12079,4527),
    ),
    4486 => array(
        'm_title' => 'IDEA LINKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link idea" aria-hidden="true"></i>',
        'm_profile' => array(13442,13408,12700,11054,12079,10662,4527),
    ),
    4737 => array(
        'm_title' => 'IDEA STATUS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sliders-h idea" aria-hidden="true"></i>',
        'm_profile' => array(12701,13408,13295,11054,12079,4527,6204,6226,6160,6201),
    ),
    7585 => array(
        'm_title' => 'IDEA TYPES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes idea" aria-hidden="true"></i>',
        'm_profile' => array(12701,13408,13295,11054,12079,6201,4527,6204,10651,6160),
    ),
    12500 => array(
        'm_title' => 'VERTICAL MENU',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-ellipsis-v"></i>',
        'm_profile' => array(13479,12079,12823,4527),
    ),
);

//SOURCE LIST EDITOR:
$config['n___4997'] = array(5000,4998,4999,5001,5003,5865,5943,12318,10625,13441,5981,11956,5982,12928,12930);
$config['e___4997'] = array(
    5000 => array(
        'm_title' => 'NAME REPLACE',
        'm_message' => 'Search for occurrence of string in child entity names and if found, updates it with a replacement string',
        'm_icon' => '<i class="source fad fa-fingerprint"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    4998 => array(
        'm_title' => 'NAME PREFIX',
        'm_message' => 'Adds string to the beginning of all child entities. Make sure to include a space for it to look good',
        'm_icon' => '<i class="source fad fa-fingerprint"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    4999 => array(
        'm_title' => 'NAME POSTFIX',
        'm_message' => 'Adds string to the end of all child entities',
        'm_icon' => '<i class="source fad fa-fingerprint"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    5001 => array(
        'm_title' => 'CONTENT REPLACE',
        'm_message' => 'Search for occurance of string in child entity link contents and if found, updates it with a replacement string',
        'm_icon' => '<i class="source fad fa-sticky-note"></i>',
        'm_profile' => array(4997,4593),
    ),
    5003 => array(
        'm_title' => 'STATUS',
        'm_message' => 'Updates all child entity statuses that match the initial entity status condition',
        'm_icon' => '<i class="source fad fa-sliders-h"></i>',
        'm_profile' => array(4997,4593),
    ),
    5865 => array(
        'm_title' => 'LINK STATUS',
        'm_message' => 'Updates all child entity link statuses that match the initial link status condition',
        'm_icon' => '<i class="source fad fa-sliders-h" aria-hidden="true"></i>',
        'm_profile' => array(4997,4593),
    ),
    5943 => array(
        'm_title' => 'ICON SET FOR ALL',
        'm_message' => 'Updates all child entity icons with string which needs to be a valid icon',
        'm_icon' => '<i class="fad fa-user-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4997,4593),
    ),
    12318 => array(
        'm_title' => 'ICON SET IF NONE',
        'm_message' => 'Updates all icons that are not set to the new value.',
        'm_icon' => '<i class="fad fa-user-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4593,4997),
    ),
    10625 => array(
        'm_title' => 'ICON REPLACE',
        'm_message' => 'Search for occurrence of string in child entity icons and if found, updates it with a replacement string',
        'm_icon' => '<i class="source fad fa-user-circle"></i>',
        'm_profile' => array(4593,4997),
    ),
    13441 => array(
        'm_title' => 'PROFILE MOVE @ IF MISSING',
        'm_message' => 'Migrates all portfolio sources to another source while preserves their message',
        'm_icon' => '<i class="fad fa-arrow-right source"></i>',
        'm_profile' => array(12577,4593,4997),
    ),
    5981 => array(
        'm_title' => 'PROFILE COPY @ IF MISSING',
        'm_message' => 'If not already done so, will add searched entity as the parent of all child entities',
        'm_icon' => '<i class="fad fa-layer-plus source"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    11956 => array(
        'm_title' => 'PROFILE COPY @ IF HAS @',
        'm_message' => 'Adds a parent entity only IF the entity has another parent entity.',
        'm_icon' => '<i class="fad fa-layer-plus source"></i>',
        'm_profile' => array(12577,4593,4997),
    ),
    5982 => array(
        'm_title' => 'PROFILE REMOVE @ IF THERE',
        'm_message' => 'If already added as the parent, this will remove searched entity as the parent of all child entities',
        'm_icon' => '<i class="fad fa-layer-minus source"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    12928 => array(
        'm_title' => 'PROFILE COPY @ IF HAS 1+ IDEAS',
        'm_message' => 'Adds a profile source if the source has 1 or more ideas',
        'm_icon' => '<i class="fad fa-layer-plus source"></i>',
        'm_profile' => array(4997),
    ),
    12930 => array(
        'm_title' => 'PROFILE COPY @ IF HAS 0 IDEAS',
        'm_message' => 'Adds a profile source if the source has 0 ideas',
        'm_icon' => '<i class="fad fa-layer-plus source"></i>',
        'm_profile' => array(4997),
    ),
);

//SOURCE LINK URLS:
$config['n___4537'] = array(4259,4257,4261,4260,4256,4258);
$config['e___4537'] = array(
    4259 => array(
        'm_title' => 'AUDIO',
        'm_message' => 'URL to a raw audio file',
        'm_icon' => '<i class="fad fa-volume-up source"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4257 => array(
        'm_title' => 'EMBED URL',
        'm_message' => 'Recognizable URL that offers an embed widget for a more engaging play-back experience',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4758,12822,12605,12524,12403,4593,4592,4537),
    ),
    4261 => array(
        'm_title' => 'FILE',
        'm_message' => 'URL to a raw file',
        'm_icon' => '<i class="fad fa-file-pdf source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4260 => array(
        'm_title' => 'IMAGE',
        'm_message' => 'URL to a raw image file',
        'm_icon' => '<i class="fad fa-image source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,6198,11059,10593,4593,4592,4537),
    ),
    4256 => array(
        'm_title' => 'URL',
        'm_message' => 'Link note contains a generic URL only',
        'm_icon' => '<i class="fas fa-external-link source" aria-hidden="true"></i>',
        'm_profile' => array(13433,12822,11080,4593,4592,4537),
    ),
    4258 => array(
        'm_title' => 'VIDEO',
        'm_message' => 'URL to a raw video file',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
);

//IDEA LINKS:
$config['n___4486'] = array(4228,4229);
$config['e___4486'] = array(
    4228 => array(
        'm_title' => 'FIXED',
        'm_message' => 'Follow each other',
        'm_icon' => '<i class="fas fa-play-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12840,4486,6410,4593),
    ),
    4229 => array(
        'm_title' => 'CONDITIONAL',
        'm_message' => 'May follow each other',
        'm_icon' => '<i class="fad fa-question-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12842,4486,4527,6410,6283,4593),
    ),
);

//IDEA TABS:
$config['n___11018'] = array(12273,4601,12589,10573,13898,12274,13865,7545,6255,13907,12419,12896,13783,11047);
$config['e___11018'] = array(
    12273 => array(
        'm_title' => 'IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13300,13291,13450,12675,13530,11018,11089,12761,12112,12467,12228),
    ),
    4601 => array(
        'm_title' => 'KEYWORDS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-tags idea" aria-hidden="true"></i>',
        'm_profile' => array(12700,13023,13530,11018,4485,10593,4593),
    ),
    12589 => array(
        'm_title' => 'IDEA LIST EDITOR',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-list idea" aria-hidden="true"></i>',
        'm_profile' => array(13530,13403,11018,4527,12590),
    ),
    10573 => array(
        'm_title' => 'MY IDEAS MAP',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sitemap idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,11035,11054,4527,10984,11018,4485,4593,7551),
    ),
    13898 => array(
        'm_title' => 'IDEA SHAREHOLDERS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chart-pie idea"></i>',
        'm_profile' => array(11018,13450),
    ),
    12274 => array(
        'm_title' => 'SOURCES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle source" aria-hidden="true"></i>',
        'm_profile' => array(13920,13450,13530,11018,11089,12761,13291,12467,12228),
    ),
    13865 => array(
        'm_title' => 'REQUIREMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shield-check source"></i>',
        'm_profile' => array(4527,11035,13421,13298,7551,13530,4593,11018,11089,4485),
    ),
    7545 => array(
        'm_title' => 'AWARDS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-file-certificate source" aria-hidden="true"></i>',
        'm_profile' => array(11089,13530,6159,13421,13298,12197,11018,4485,7551,4593),
    ),
    6255 => array(
        'm_title' => 'DISCOVERIES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,13776,13450,11089,12677,11018,12467,12228,4527),
    ),
    13907 => array(
        'm_title' => 'DISCOVERY CREDITS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-badge discover"></i>',
        'm_profile' => array(11018,13898,11089),
    ),
    12419 => array(
        'm_title' => 'COMMENTS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-comments discover" aria-hidden="true"></i>',
        'm_profile' => array(12701,11035,13871,12994,11089,13530,11018,12359,5967,10593,4986,4593,4485),
    ),
    12896 => array(
        'm_title' => 'SAVED IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bookmark discover"></i>',
        'm_profile' => array(13877,12701,11035,11018,11089,12677,4485,7551,4755,4593),
    ),
    13783 => array(
        'm_title' => 'ANNOUNCEMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-calendar-day discover"></i>',
        'm_profile' => array(4527,7569,12701,11018),
    ),
    11047 => array(
        'm_title' => 'IDEA MENU',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-caret-down" aria-hidden="true"></i>',
        'm_profile' => array(13530,12700,11018,4527,11040),
    ),
);

//SOURCE TABS:
$config['n___11089'] = array(6225,13865,7545,12274,12273,6255,13907,12896,12419,12887);
$config['e___11089'] = array(
    6225 => array(
        'm_title' => 'MY ACCOUNT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cog source" aria-hidden="true"></i>',
        'm_profile' => array(12574,13425,11089,4527),
    ),
    13865 => array(
        'm_title' => 'REQUIREMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shield-check source"></i>',
        'm_profile' => array(4527,11035,13421,13298,7551,13530,4593,11018,11089,4485),
    ),
    7545 => array(
        'm_title' => 'AWARDS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-file-certificate source" aria-hidden="true"></i>',
        'm_profile' => array(11089,13530,6159,13421,13298,12197,11018,4485,7551,4593),
    ),
    12274 => array(
        'm_title' => 'SOURCES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle source" aria-hidden="true"></i>',
        'm_profile' => array(13920,13450,13530,11018,11089,12761,13291,12467,12228),
    ),
    12273 => array(
        'm_title' => 'IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13300,13291,13450,12675,13530,11018,11089,12761,12112,12467,12228),
    ),
    6255 => array(
        'm_title' => 'DISCOVERIES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,13776,13450,11089,12677,11018,12467,12228,4527),
    ),
    13907 => array(
        'm_title' => 'DISCOVERY CREDITS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-badge discover"></i>',
        'm_profile' => array(11018,13898,11089),
    ),
    12896 => array(
        'm_title' => 'SAVED IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bookmark discover"></i>',
        'm_profile' => array(13877,12701,11035,11018,11089,12677,4485,7551,4755,4593),
    ),
    12419 => array(
        'm_title' => 'COMMENTS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-comments discover" aria-hidden="true"></i>',
        'm_profile' => array(12701,11035,13871,12994,11089,13530,11018,12359,5967,10593,4986,4593,4485),
    ),
    12887 => array(
        'm_title' => 'SOURCE MENU',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-caret-down" aria-hidden="true"></i>',
        'm_profile' => array(11089,12703,4527,11040),
    ),
);

//MY ACCOUNT:
$config['n___6225'] = array(12289,10957,13884,13491,3288,3286,13518,12114);
$config['e___6225'] = array(
    12289 => array(
        'm_title' => 'AVATAR',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-mask" aria-hidden="true"></i>',
        'm_profile' => array(6225),
    ),
    10957 => array(
        'm_title' => 'SUPERPOWERS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bolt" aria-hidden="true"></i>',
        'm_profile' => array(6225,11035,5007,4527),
    ),
    13884 => array(
        'm_title' => 'DISPLAY MODE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-eclipse"></i>',
        'm_profile' => array(6204,11054,13890,4527,6225),
    ),
    13491 => array(
        'm_title' => 'FONT SIZE',
        'm_message' => 'Here is the current font size...',
        'm_icon' => '<i class="fas fa-text-size"></i>',
        'm_profile' => array(11054,13890,6225,4527,6204),
    ),
    3288 => array(
        'm_title' => 'EMAIL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-envelope-open" aria-hidden="true"></i>',
        'm_profile' => array(13861,13531,6404,4269,6225,4755),
    ),
    3286 => array(
        'm_title' => 'PASSWORD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-key" aria-hidden="true"></i>',
        'm_profile' => array(13014,4269,6225,7578,4755),
    ),
    13518 => array(
        'm_title' => 'MONTHLY SUBSCRIPTION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-usd-circle discover"></i>',
        'm_profile' => array(11054,13890,6204,4527,6225,13450),
    ),
    12114 => array(
        'm_title' => 'WEEKLY PAYOUTS',
        'm_message' => 'No earnings yet. Map ideas to receive weekly payouts.',
        'm_icon' => '<i class="fas fa-sync discover"></i>',
        'm_profile' => array(6225,13450,4593,6287,12741,7274,7569),
    ),
);

//MENCH NAVIGATION:
$config['n___11035'] = array(13007,13914,13915,13865,4997,13813,13912,10939,13542,11019,13913,12137,10573,13919,13550,13502,12419,13670,4235,13495,13562,12969,13524,12896,12906,6415,4341,12467,13579,13401,13292,13573,13574,13571,12707,12991,12211,13689,13207,11068,6287,4755,6155,13415,10673,13671,7256,13692,13693,4269,6132,13412,13908,13909,13911,10957,13572);
$config['e___11035'] = array(
    13007 => array(
        'm_title' => 'SORT SOURCES ALPHABETICALLY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sort-alpha-down source"></i>',
        'm_profile' => array(13422,13429,11035,4593),
    ),
    13914 => array(
        'm_title' => 'NEW SOURCE TITLE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-plus-circle source"></i>',
        'm_profile' => array(11035),
    ),
    13915 => array(
        'm_title' => 'LIST ALL SOURCES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-search-plus source"></i>',
        'm_profile' => array(11035),
    ),
    13865 => array(
        'm_title' => 'REQUIREMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shield-check source"></i>',
        'm_profile' => array(4527,11035,13421,13298,7551,13530,4593,11018,11089,4485),
    ),
    4997 => array(
        'm_title' => 'SOURCE LIST EDITOR',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-list source" aria-hidden="true"></i>',
        'm_profile' => array(12574,13429,11035,12703,12590,4527),
    ),
    13813 => array(
        'm_title' => 'FEATURED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star idea" aria-hidden="true"></i>',
        'm_profile' => array(11035),
    ),
    13912 => array(
        'm_title' => 'NEW IDEA (START WITH VERB)',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-plus-circle rotate90 idea"></i>',
        'm_profile' => array(11035),
    ),
    10939 => array(
        'm_title' => 'SUPERPOWER OF IDEATION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-lightbulb-on idea" aria-hidden="true"></i>',
        'm_profile' => array(13450,11035,6404,10957),
    ),
    13542 => array(
        'm_title' => 'NEXT IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sitemap idea"></i>',
        'm_profile' => array(11035,12273,11020),
    ),
    11019 => array(
        'm_title' => 'PREVIOUS IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sitemap rotate180 idea"></i>',
        'm_profile' => array(11035,11020,13294),
    ),
    13913 => array(
        'm_title' => 'LIST ALL IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-search-plus idea"></i>',
        'm_profile' => array(11035),
    ),
    12137 => array(
        'm_title' => 'IDEA FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star idea" aria-hidden="true"></i>',
        'm_profile' => array(12399,13813,6404,11035,13420,10648,12138,7356,7355,4737),
    ),
    10573 => array(
        'm_title' => 'MY IDEAS MAP',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sitemap idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,11035,11054,4527,10984,11018,4485,4593,7551),
    ),
    13919 => array(
        'm_title' => 'MY IDEAS LIST',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-list idea"></i>',
        'm_profile' => array(13920,11035),
    ),
    13550 => array(
        'm_title' => 'REFERENCED IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea"></i>',
        'm_profile' => array(11035,4527,12273),
    ),
    13502 => array(
        'm_title' => 'CANCEL EDITING',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-times discover"></i>',
        'm_profile' => array(11035),
    ),
    12419 => array(
        'm_title' => 'COMMENTS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-comments discover" aria-hidden="true"></i>',
        'm_profile' => array(12701,11035,13871,12994,11089,13530,11018,12359,5967,10593,4986,4593,4485),
    ),
    13670 => array(
        'm_title' => 'FILTER DISCOVERIES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-filter discover" aria-hidden="true"></i>',
        'm_profile' => array(12701,11035),
    ),
    4235 => array(
        'm_title' => 'GET STARTED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chevron-right discover" aria-hidden="true"></i>',
        'm_profile' => array(12994,11035,12969,12227,4593,5967,4755),
    ),
    13495 => array(
        'm_title' => 'EDIT ANSWER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-pen discover"></i>',
        'm_profile' => array(11035),
    ),
    13562 => array(
        'm_title' => 'PREVIEW DISCOVERY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chevron-right discover"></i>',
        'm_profile' => array(11035),
    ),
    12969 => array(
        'm_title' => 'MY DISCOVERIES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-compass discover" aria-hidden="true"></i>',
        'm_profile' => array(11035,12228,4527),
    ),
    13524 => array(
        'm_title' => 'SAVE & NEXT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chevron-right discover" aria-hidden="true"></i>',
        'm_profile' => array(11035),
    ),
    12896 => array(
        'm_title' => 'SAVED IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bookmark discover"></i>',
        'm_profile' => array(13877,12701,11035,11018,11089,12677,4485,7551,4755,4593),
    ),
    12906 => array(
        'm_title' => 'UNSAVE DISCOVERY',
        'm_message' => '',
        'm_icon' => '<i class="far fa-bookmark discover"></i>',
        'm_profile' => array(12896,11035,4755,4593),
    ),
    6415 => array(
        'm_title' => 'DELETE ALL DISCOVERIES',
        'm_message' => 'Resetting your discoveries will remove all the discovery coins you mined so you can start from scratch again.',
        'm_icon' => '<i class="fas fa-trash-alt discover" aria-hidden="true"></i>',
        'm_profile' => array(11035,4593,4755),
    ),
    4341 => array(
        'm_title' => 'MENCH LEDGER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-atlas discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13450,12701,12500,6212,12887,11047,12707,10876,11035,4527,7735),
    ),
    12467 => array(
        'm_title' => 'MENCH COINS',
        'm_message' => '',
        'm_icon' => '<img src="https://mench.com/img/mench.png" class="mench-spin no-radius">',
        'm_profile' => array(11035,13450,11054,10939,12030,4527),
    ),
    13579 => array(
        'm_title' => 'DELETE NOTE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-trash-alt" aria-hidden="true"></i>',
        'm_profile' => array(4593,11035),
    ),
    13401 => array(
        'm_title' => 'CLOSE SEARCH',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-search-minus"></i>',
        'm_profile' => array(11035,7256),
    ),
    13292 => array(
        'm_title' => 'DISCOVERY TIME',
        'm_message' => '',
        'm_icon' => '<i class="far fa-stopwatch"></i>',
        'm_profile' => array(11035,13544,4356),
    ),
    13573 => array(
        'm_title' => 'DOWNLOAD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cloud-download"></i>',
        'm_profile' => array(11035),
    ),
    13574 => array(
        'm_title' => 'EDIT NOTE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-pen-square"></i>',
        'm_profile' => array(11035),
    ),
    13571 => array(
        'm_title' => 'EDIT SOURCE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-pen-square"></i>',
        'm_profile' => array(13297,11035,4527,13566),
    ),
    12707 => array(
        'm_title' => 'FILTER TRANSACTIONS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-filter" aria-hidden="true"></i>',
        'm_profile' => array(11035,12701),
    ),
    12991 => array(
        'm_title' => 'BACK',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chevron-left" aria-hidden="true"></i>',
        'm_profile' => array(13289,11035),
    ),
    12211 => array(
        'm_title' => 'NEXT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chevron-right" aria-hidden="true"></i>',
        'm_profile' => array(4527,13289,11035),
    ),
    13689 => array(
        'm_title' => 'INFO',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-info-circle"></i>',
        'm_profile' => array(11035),
    ),
    13207 => array(
        'm_title' => 'LEADERBOARD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-medal source"></i>',
        'm_profile' => array(13920,13907,13543,11035,12500,12489,10876,4527),
    ),
    11068 => array(
        'm_title' => 'MAGIC LOGIN',
        'm_message' => '',
        'm_icon' => '<i class="far fa-envelope-open" aria-hidden="true"></i>',
        'm_profile' => array(11035,11065),
    ),
    6287 => array(
        'm_title' => 'MENCH PLUGINS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-plug" aria-hidden="true"></i>',
        'm_profile' => array(13920,12500,4367,12701,6405,12699,10876,11035,4527),
    ),
    4755 => array(
        'm_title' => 'PRIVATE INFORMATION',
        'm_message' => '',
        'm_icon' => '<i class="far fa-eye-slash" aria-hidden="true"></i>',
        'm_profile' => array(11035,12701,4755,6771,4527),
    ),
    6155 => array(
        'm_title' => 'REMOVE DISCOVERY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-times"></i>',
        'm_profile' => array(11035,6150,4593,4755),
    ),
    13415 => array(
        'm_title' => 'UNLINK IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-unlink"></i>',
        'm_profile' => array(11035,4755,4593),
    ),
    10673 => array(
        'm_title' => 'UNLINK SOURCE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-unlink"></i>',
        'm_profile' => array(11035,4593),
    ),
    13671 => array(
        'm_title' => 'RESET FILTERS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-times"></i>',
        'm_profile' => array(11035,13670),
    ),
    7256 => array(
        'm_title' => 'SEARCH MENCH',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-search" aria-hidden="true"></i>',
        'm_profile' => array(6404,12678,13357,12701,11035,3323),
    ),
    13692 => array(
        'm_title' => 'SELECT ALL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle"></i>',
        'm_profile' => array(11035,7231),
    ),
    13693 => array(
        'm_title' => 'SELECT NONE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-circle"></i>',
        'm_profile' => array(11035,7231),
    ),
    4269 => array(
        'm_title' => 'SIGN IN/UP',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sign-in-alt" aria-hidden="true"></i>',
        'm_profile' => array(13479,10876,4527,11035),
    ),
    6132 => array(
        'm_title' => 'SORT DISCOVERY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-arrows"></i>',
        'm_profile' => array(11035,13413,6153,4593,4755),
    ),
    13412 => array(
        'm_title' => 'SORT IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-arrows"></i>',
        'm_profile' => array(11035,13413,4755,4593),
    ),
    13908 => array(
        'm_title' => 'SORT IDEA UP/DOWN',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sort"></i>',
        'm_profile' => array(11035),
    ),
    13909 => array(
        'm_title' => 'SORT NOTE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sort"></i>',
        'm_profile' => array(11035),
    ),
    13911 => array(
        'm_title' => 'SORT SOURCE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sort"></i>',
        'm_profile' => array(11035),
    ),
    10957 => array(
        'm_title' => 'SUPERPOWERS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bolt" aria-hidden="true"></i>',
        'm_profile' => array(6225,11035,5007,4527),
    ),
    13572 => array(
        'm_title' => 'UPLOAD MEDIA',
        'm_message' => 'Upload files up to 25 MB. Larger files can be uploaded to a 3rd-party platform & paste URL here.',
        'm_icon' => '<i class="fas fa-image"></i>',
        'm_profile' => array(6404,11035,13571),
    ),
);

//SUPERPOWERS:
$config['n___10957'] = array(10939,13354,12673,13403,10984,10986,13420,12700,13422,13402,12706,12703,13421,12699,12701,13758);
$config['e___10957'] = array(
    10939 => array(
        'm_title' => 'IDEATION',
        'm_message' => 'Collaboratively Publish Ideas',
        'm_icon' => '<i class="fas fa-lightbulb-on idea" aria-hidden="true"></i>',
        'm_profile' => array(13450,11035,6404,10957),
    ),
    13354 => array(
        'm_title' => 'INLINE EDITING',
        'm_message' => 'Rename Idea Titles Inline',
        'm_icon' => '<i class="far fa-money-check-edit idea"></i>',
        'm_profile' => array(10957),
    ),
    12673 => array(
        'm_title' => 'IDEA TOOLBAR',
        'm_message' => 'Edit next & previous ideas on the fly',
        'm_icon' => '<i class="far fa-ellipsis-h idea" aria-hidden="true"></i>',
        'm_profile' => array(10957),
    ),
    13403 => array(
        'm_title' => 'IDEA LIST EDIT',
        'm_message' => 'Mass Idea Edit',
        'm_icon' => '<i class="far fa-list idea" aria-hidden="true"></i>',
        'm_profile' => array(10957),
    ),
    10984 => array(
        'm_title' => 'WALKIE TALKIE',
        'm_message' => 'Collaborative Publishing Powers',
        'm_icon' => '<i class="far fa-walkie-talkie idea" aria-hidden="true"></i>',
        'm_profile' => array(10957),
    ),
    10986 => array(
        'm_title' => 'IDEA TIMING',
        'm_message' => 'Advance Source Tools',
        'm_icon' => '<i class="far fa-stopwatch idea" aria-hidden="true"></i>',
        'm_profile' => array(10957),
    ),
    13420 => array(
        'm_title' => 'FEATURED IDEAS',
        'm_message' => 'Feature Ideas',
        'm_icon' => '<i class="far fa-star idea" aria-hidden="true"></i>',
        'm_profile' => array(10957),
    ),
    12700 => array(
        'm_title' => 'ADVANCE IDEATION',
        'm_message' => 'Advance Idea Linking',
        'm_icon' => '<i class="far fa-user-shield idea" aria-hidden="true"></i>',
        'm_profile' => array(10957),
    ),
    13422 => array(
        'm_title' => 'SOURCING',
        'm_message' => 'Manage Sources with Advance Tools',
        'm_icon' => '<i class="far fa-circle source" aria-hidden="true"></i>',
        'm_profile' => array(10957),
    ),
    13402 => array(
        'm_title' => 'INLINE SOURCING',
        'm_message' => 'Rename Source Titles Inline',
        'm_icon' => '<i class="far fa-money-check-edit source"></i>',
        'm_profile' => array(10957),
    ),
    12706 => array(
        'm_title' => 'SOURCE TOOLBAR',
        'm_message' => 'List Parent Sources',
        'm_icon' => '<i class="far fa-ellipsis-h source" aria-hidden="true"></i>',
        'm_profile' => array(10957),
    ),
    12703 => array(
        'm_title' => 'SOURCE LIST EDIT',
        'm_message' => 'Mass Source Edit',
        'm_icon' => '<i class="far fa-list source" aria-hidden="true"></i>',
        'm_profile' => array(10957),
    ),
    13421 => array(
        'm_title' => 'REQUIREMENTS & CERTIFICATES',
        'm_message' => 'Manage Player Certificates',
        'm_icon' => '<i class="far fa-file-certificate source" aria-hidden="true"></i>',
        'm_profile' => array(10957),
    ),
    12699 => array(
        'm_title' => 'PLUGINS',
        'm_message' => 'Access Mench Plugins',
        'm_icon' => '<i class="far fa-plug source" aria-hidden="true"></i>',
        'm_profile' => array(10957),
    ),
    12701 => array(
        'm_title' => 'DISCOVERY GLASSES',
        'm_message' => 'Read info from all players',
        'm_icon' => '<i class="far fa-glasses discover" aria-hidden="true"></i>',
        'm_profile' => array(10957),
    ),
    13758 => array(
        'm_title' => 'DISCOVERY TOOLBAR',
        'm_message' => 'See Source Discovery Stats',
        'm_icon' => '<i class="far fa-ellipsis-h discover" aria-hidden="true"></i>',
        'm_profile' => array(10957),
    ),
);

//TRANSACTION STATUS:
$config['n___6186'] = array(12399,6176,6175,6173);
$config['e___6186'] = array(
    12399 => array(
        'm_title' => 'FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,7360,7359,6186),
    ),
    6176 => array(
        'm_title' => 'PUBLISH',
        'm_message' => '',
        'm_icon' => '<i class="far fa-globe discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,6186,7360,7359),
    ),
    6175 => array(
        'm_title' => 'DRAFT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-spinner-third fa-spin discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,7364,6186,7360),
    ),
    6173 => array(
        'm_title' => 'UNLINK',
        'm_message' => '',
        'm_icon' => '<i class="far fa-unlink discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,10686,10673,6186),
    ),
);

//SOURCE STATUS:
$config['n___6177'] = array(12563,6181,6180,6178);
$config['e___6177'] = array(
    12563 => array(
        'm_title' => 'FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star source" aria-hidden="true"></i>',
        'm_profile' => array(12399,4527,12575,10654,7358,7357,6177),
    ),
    6181 => array(
        'm_title' => 'PUBLISH',
        'm_message' => '',
        'm_icon' => '<i class="far fa-globe source" aria-hidden="true"></i>',
        'm_profile' => array(6176,10654,6177,7358,7357),
    ),
    6180 => array(
        'm_title' => 'DRAFT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-spinner fa-spin source" aria-hidden="true"></i>',
        'm_profile' => array(6175,10654,6177,7358),
    ),
    6178 => array(
        'm_title' => 'DELETE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-trash-alt source" aria-hidden="true"></i>',
        'm_profile' => array(6173,4593,12401,6177),
    ),
);

//IDEA STATUS:
$config['n___4737'] = array(12137,6184,6183,6182);
$config['e___4737'] = array(
    12137 => array(
        'm_title' => 'FEATURE',
        'm_message' => 'Searchable by all players',
        'm_icon' => '<i class="fas fa-star idea" aria-hidden="true"></i>',
        'm_profile' => array(12399,13813,6404,11035,13420,10648,12138,7356,7355,4737),
    ),
    6184 => array(
        'm_title' => 'PUBLISH',
        'm_message' => 'Published & Discoverable',
        'm_icon' => '<i class="far fa-globe idea" aria-hidden="true"></i>',
        'm_profile' => array(6176,10648,4737,7355,7356),
    ),
    6183 => array(
        'm_title' => 'DRAF',
        'm_message' => 'Not Discoverable',
        'm_icon' => '<i class="far fa-spinner fa-spin idea" aria-hidden="true"></i>',
        'm_profile' => array(6175,4737,10648,7356),
    ),
    6182 => array(
        'm_title' => 'DELETE',
        'm_message' => 'Archived',
        'm_icon' => '<i class="far fa-trash-alt idea" aria-hidden="true"></i>',
        'm_profile' => array(6173,12400,4593,4737),
    ),
);

//DISCOVER ALL CONNECTIONS:
$config['n___11081'] = array(4429,4368,4371,4369,4364,4593,4366);
$config['e___11081'] = array(
    4429 => array(
        'm_title' => 'DOWN',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-triangle rotate180 source" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,6160,4341),
    ),
    4368 => array(
        'm_title' => 'LEFT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-triangle rotate270 idea" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,6202,4341),
    ),
    4371 => array(
        'm_title' => 'REFERENCE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-atlas" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,4367,4341),
    ),
    4369 => array(
        'm_title' => 'RIGHT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-triangle rotate90 idea" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,6202,4341),
    ),
    4364 => array(
        'm_title' => 'SOURCE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-user-tag" aria-hidden="true"></i>',
        'm_profile' => array(6194,11081,6160,4341),
    ),
    4593 => array(
        'm_title' => 'TYPE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,6194,6204,11081,10659,6160,4341,4527),
    ),
    4366 => array(
        'm_title' => 'UP',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-triangle source" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,6160,4341),
    ),
);

//SOURCE LINK FILE EXTENSIONS:
$config['n___11080'] = array(4259,4261,4260,4256,4258);
$config['e___11080'] = array(
    4259 => array(
        'm_title' => 'AUDIO',
        'm_message' => 'pcm|wav|aiff|mp3|aac|ogg|wma|flac|alac|m4a|m4b|m4p',
        'm_icon' => '<i class="fad fa-volume-up source"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4261 => array(
        'm_title' => 'FILE',
        'm_message' => 'pdf|pdc|doc|docx|tex|txt|7z|rar|zip|csv|sql|tar|xml|exe',
        'm_icon' => '<i class="fad fa-file-pdf source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4260 => array(
        'm_title' => 'IMAGE',
        'm_message' => 'jpeg|jpg|png|gif|tiff|bmp|img|svg|ico|webp',
        'm_icon' => '<i class="fad fa-image source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,6198,11059,10593,4593,4592,4537),
    ),
    4256 => array(
        'm_title' => 'URL',
        'm_message' => 'htm|html',
        'm_icon' => '<i class="fas fa-external-link source" aria-hidden="true"></i>',
        'm_profile' => array(13433,12822,11080,4593,4592,4537),
    ),
    4258 => array(
        'm_title' => 'VIDEO',
        'm_message' => 'mp4|m4v|m4p|avi|mov|flv|f4v|f4p|f4a|f4b|wmv|webm|mkv|vob|ogv|ogg|3gp|mpg|mpeg|m2v',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
);

//SOURCE LINK UPLOAD FILE:
$config['n___11059'] = array(4259,4261,4260,4258);
$config['e___11059'] = array(
    4259 => array(
        'm_title' => 'AUDIO',
        'm_message' => 'audio',
        'm_icon' => '<i class="fad fa-volume-up source"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4261 => array(
        'm_title' => 'FILE',
        'm_message' => 'file',
        'm_icon' => '<i class="fad fa-file-pdf source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4260 => array(
        'm_title' => 'IMAGE',
        'm_message' => 'image',
        'm_icon' => '<i class="fad fa-image source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,6198,11059,10593,4593,4592,4537),
    ),
    4258 => array(
        'm_title' => 'VIDEO',
        'm_message' => 'video',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
);

//MENCH CONFIG VARIABLES:
$config['n___6404'] = array(12678,12904,11057,11056,12331,12427,3288,12176,4356,12137,4485,4736,11064,11065,13014,13005,7274,11079,11060,13556,13545,11066,7256,13803,12088,13206,11986,6197,12232,10939,13572,12565,12568);
$config['e___6404'] = array(
    12678 => array(
        'm_title' => 'ALGOLIA SEARCH ENABLED (0 OR 1)',
        'm_message' => '1',
        'm_icon' => '',
        'm_profile' => array(3323,6404),
    ),
    12904 => array(
        'm_title' => 'DEFAULT IDEA COVER',
        'm_message' => '//s3foundation.s3-us-west-2.amazonaws.com/a7aea066ff79ab42937c92bf2519bbd7.png',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(6404,7524),
    ),
    11057 => array(
        'm_title' => 'DISCOVER MARKS MAX',
        'm_message' => '89',
        'm_icon' => '',
        'm_profile' => array(6404,4358),
    ),
    11056 => array(
        'm_title' => 'DISCOVER MARKS MIN',
        'm_message' => '-89',
        'm_icon' => '',
        'm_profile' => array(6404,4358),
    ),
    12331 => array(
        'm_title' => 'DISCOVER MIN TIME SHOW',
        'm_message' => '120',
        'm_icon' => '',
        'm_profile' => array(6404),
    ),
    12427 => array(
        'm_title' => 'DISCOVER TIME MINIMUM SECONDS',
        'm_message' => '3',
        'm_icon' => '',
        'm_profile' => array(6404,4356),
    ),
    3288 => array(
        'm_title' => 'EMAIL',
        'm_message' => 'support@mench.com',
        'm_icon' => '<i class="fas fa-envelope-open" aria-hidden="true"></i>',
        'm_profile' => array(13861,13531,6404,4269,6225,4755),
    ),
    12176 => array(
        'm_title' => 'IDEA DEFAULT TIME SECONDS',
        'm_message' => '30',
        'm_icon' => '<i class="fad fa-clock idea" aria-hidden="true"></i>',
        'm_profile' => array(6404),
    ),
    4356 => array(
        'm_title' => 'IDEA DURATION (SECONDS)',
        'm_message' => '7200',
        'm_icon' => '<i class="fas fa-stopwatch idea" aria-hidden="true"></i>',
        'm_profile' => array(10986,13295,6287,12741,11047,7274,6404,12112,10650,6201),
    ),
    12137 => array(
        'm_title' => 'IDEA FEATURE',
        'm_message' => '7766',
        'm_icon' => '<i class="fas fa-star idea" aria-hidden="true"></i>',
        'm_profile' => array(12399,13813,6404,11035,13420,10648,12138,7356,7355,4737),
    ),
    4485 => array(
        'm_title' => 'IDEA NOTES',
        'm_message' => '1000',
        'm_icon' => '<i class="fas fa-comment-plus idea" aria-hidden="true"></i>',
        'm_profile' => array(11054,6768,6404,4527),
    ),
    4736 => array(
        'm_title' => 'IDEA TITLE',
        'm_message' => '100',
        'm_icon' => '<i class="fas fa-h1 idea" aria-hidden="true"></i>',
        'm_profile' => array(13407,13294,12994,6404,12112,10644,6201),
    ),
    11064 => array(
        'm_title' => 'ITEMS PER PAGE',
        'm_message' => '100',
        'm_icon' => '',
        'm_profile' => array(6404),
    ),
    11065 => array(
        'm_title' => 'MAGIC LINK VALID SECONDS',
        'm_message' => '86400',
        'm_icon' => '',
        'm_profile' => array(6404),
    ),
    13014 => array(
        'm_title' => 'MASTER PASSWORD',
        'm_message' => 'fb618a7622654d8fd68f85da02c0ae0b7b8765fffeecedd5c32bda80fc2cf9e8',
        'm_icon' => '<i class="fad fa-key idea" aria-hidden="true"></i>',
        'm_profile' => array(6404),
    ),
    13005 => array(
        'm_title' => 'MAX SOURCES SORTABLE',
        'm_message' => '100',
        'm_icon' => '<i class="fas fa-sort source"></i>',
        'm_profile' => array(6404),
    ),
    7274 => array(
        'm_title' => 'MENCH CRON JOBS',
        'm_message' => '/usr/bin/php /var/www/platform/index.php e plugin',
        'm_icon' => '<i class="fas fa-clock mench-spin" aria-hidden="true"></i>',
        'm_profile' => array(6404,6287,6403,12999),
    ),
    11079 => array(
        'm_title' => 'MENCH TIMEZONE',
        'm_message' => 'America/Los_Angeles',
        'm_icon' => '',
        'm_profile' => array(6404),
    ),
    11060 => array(
        'm_title' => 'MENCH VERSION',
        'm_message' => 'v1.573',
        'm_icon' => '<i class="fad fa-code"></i>',
        'm_profile' => array(6404),
    ),
    13556 => array(
        'm_title' => 'MESSAGE FEATURED CAP WORDS',
        'm_message' => '34',
        'm_icon' => '',
        'm_profile' => array(6404),
    ),
    13545 => array(
        'm_title' => 'MINER GET STARTED IDEA',
        'm_message' => '13436',
        'm_icon' => '',
        'm_profile' => array(6404),
    ),
    11066 => array(
        'm_title' => 'PASSWORD MIN CHARACTERS',
        'm_message' => '6',
        'm_icon' => '',
        'm_profile' => array(6404),
    ),
    7256 => array(
        'm_title' => 'SEARCH MENCH',
        'm_message' => 'Search Existing or Create New...',
        'm_icon' => '<i class="fas fa-search" aria-hidden="true"></i>',
        'm_profile' => array(6404,12678,13357,12701,11035,3323),
    ),
    13803 => array(
        'm_title' => 'SHOW MAX PROFILE',
        'm_message' => '3',
        'm_icon' => '',
        'm_profile' => array(6404),
    ),
    12088 => array(
        'm_title' => 'SHOW TEXT COUNTER THRESHOLD',
        'm_message' => '0.8',
        'm_icon' => '',
        'm_profile' => array(6404),
    ),
    13206 => array(
        'm_title' => 'SOURCE LIST MAX',
        'm_message' => '20',
        'm_icon' => '',
        'm_profile' => array(13446,6404),
    ),
    11986 => array(
        'm_title' => 'SOURCE LIST VISIBLE',
        'm_message' => '5',
        'm_icon' => '',
        'm_profile' => array(13446,6404),
    ),
    6197 => array(
        'm_title' => 'SOURCE TITLE',
        'm_message' => '233',
        'm_icon' => '<i class="fad fa-fingerprint source" aria-hidden="true"></i>',
        'm_profile' => array(13571,13296,13025,6404,12112,12232,10646,5000,4998,4999,6206),
    ),
    12232 => array(
        'm_title' => 'SOURCE TITLE MIN LENGTH',
        'm_message' => '2',
        'm_icon' => '',
        'm_profile' => array(6404),
    ),
    10939 => array(
        'm_title' => 'SUPERPOWER OF IDEATION',
        'm_message' => '/14001',
        'm_icon' => '<i class="fas fa-lightbulb-on idea" aria-hidden="true"></i>',
        'm_profile' => array(13450,11035,6404,10957),
    ),
    13572 => array(
        'm_title' => 'UPLOAD MEDIA',
        'm_message' => '25',
        'm_icon' => '<i class="fas fa-image"></i>',
        'm_profile' => array(6404,11035,13571),
    ),
    12565 => array(
        'm_title' => 'WEIGHT ALGORITHM RATE',
        'm_message' => '89',
        'm_icon' => '',
        'm_profile' => array(12569,6404),
    ),
    12568 => array(
        'm_title' => 'WEIGHT ALGORITHM TRANSACTIONS',
        'm_message' => '1',
        'm_icon' => '',
        'm_profile' => array(12569,6404),
    ),
);

//MENCH MEMORY JAVASCRIPT:
$config['n___11054'] = array(13890,13884,13491,4486,4485,4737,7356,7355,7585,7712,2738,12467,6404,6201,11035,12687,13518,10573,4592,6177,7357,6186);
$config['e___11054'] = array(
    13890 => array(
        'm_title' => 'CUSTOMIZABLE UI',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-send-backward"></i>',
        'm_profile' => array(11054,4758,4527),
    ),
    13884 => array(
        'm_title' => 'DISPLAY MODE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-eclipse"></i>',
        'm_profile' => array(6204,11054,13890,4527,6225),
    ),
    13491 => array(
        'm_title' => 'FONT SIZE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-text-size"></i>',
        'm_profile' => array(11054,13890,6225,4527,6204),
    ),
    4486 => array(
        'm_title' => 'IDEA LINKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link idea" aria-hidden="true"></i>',
        'm_profile' => array(13442,13408,12700,11054,12079,10662,4527),
    ),
    4485 => array(
        'm_title' => 'IDEA NOTES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comment-plus idea" aria-hidden="true"></i>',
        'm_profile' => array(11054,6768,6404,4527),
    ),
    4737 => array(
        'm_title' => 'IDEA STATUS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sliders-h idea" aria-hidden="true"></i>',
        'm_profile' => array(12701,13408,13295,11054,12079,4527,6204,6226,6160,6201),
    ),
    7356 => array(
        'm_title' => 'IDEA STATUS ACTIVE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-check-circle"></i>',
        'm_profile' => array(11054,10891,4527),
    ),
    7355 => array(
        'm_title' => 'IDEA STATUS PUBLIC',
        'm_message' => '',
        'm_icon' => '<i class="far fa-circle" aria-hidden="true"></i>',
        'm_profile' => array(11054,10891,4527),
    ),
    7585 => array(
        'm_title' => 'IDEA TYPES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes idea" aria-hidden="true"></i>',
        'm_profile' => array(12701,13408,13295,11054,12079,6201,4527,6204,10651,6160),
    ),
    7712 => array(
        'm_title' => 'IDEA TYPE SELECT NEXT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-question-circle idea"></i>',
        'm_profile' => array(11054,13028,6287,10893,4527),
    ),
    2738 => array(
        'm_title' => 'MENCH',
        'm_message' => '',
        'm_icon' => '<img src="https://mench.com/img/mench.png" class="mench-spin no-radius">',
        'm_profile' => array(2,1,12891,11054,12041,4527,7312),
    ),
    12467 => array(
        'm_title' => 'MENCH COINS',
        'm_message' => '',
        'm_icon' => '<img src="https://mench.com/img/mench.png" class="mench-spin no-radius">',
        'm_profile' => array(11035,13450,11054,10939,12030,4527),
    ),
    6404 => array(
        'm_title' => 'MENCH CONFIG VARIABLES',
        'm_message' => '',
        'm_icon' => '<i class="far fa-lambda" aria-hidden="true"></i>',
        'm_profile' => array(11054,4527,6403),
    ),
    6201 => array(
        'm_title' => 'MENCH IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-table idea" aria-hidden="true"></i>',
        'm_profile' => array(6212,11054,4527,7735),
    ),
    11035 => array(
        'm_title' => 'MENCH NAVIGATION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-compass" aria-hidden="true"></i>',
        'm_profile' => array(11054,6403,4527),
    ),
    12687 => array(
        'm_title' => 'MENCH RANDOM MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comments-alt" aria-hidden="true"></i>',
        'm_profile' => array(6403,11054,4527),
    ),
    13518 => array(
        'm_title' => 'MONTHLY SUBSCRIPTION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-usd-circle discover"></i>',
        'm_profile' => array(11054,13890,6204,4527,6225,13450),
    ),
    10573 => array(
        'm_title' => 'MY IDEAS MAP',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sitemap idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,11035,11054,4527,10984,11018,4485,4593,7551),
    ),
    4592 => array(
        'm_title' => 'SOURCE LINKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link source" aria-hidden="true"></i>',
        'm_profile' => array(11054,4527,5982),
    ),
    6177 => array(
        'm_title' => 'SOURCE STATUS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sliders-h source" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,12766,4527,11054,6204,5003,6160,6206),
    ),
    7357 => array(
        'm_title' => 'SOURCE STATUS PUBLIC',
        'm_message' => '',
        'm_icon' => '<i class="far fa-globe source" aria-hidden="true"></i>',
        'm_profile' => array(13296,12572,11054,4527),
    ),
    6186 => array(
        'm_title' => 'TRANSACTION STATUS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-sliders-h" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,4527,11054,10677,10656,6204,5865,6160,7304,4341),
    ),
);

//IDEA TYPES:
$config['n___7585'] = array(6677,6683,7637,6684,7231,6907,6914);
$config['e___7585'] = array(
    6677 => array(
        'm_title' => 'READ & NEXT',
        'm_message' => 'Read messages & go next',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12211,13022,12330,7585,6192,4559),
    ),
    6683 => array(
        'm_title' => 'REPLY & NEXT',
        'm_message' => 'Reply with text & go next',
        'm_icon' => '<i class="fas fa-keyboard idea" aria-hidden="true"></i>',
        'm_profile' => array(13022,12955,6144,7585,6192),
    ),
    7637 => array(
        'm_title' => 'UPLOAD & NEXT',
        'm_message' => 'Upload a file & go next',
        'm_icon' => '<i class="fas fa-cloud-upload idea" aria-hidden="true"></i>',
        'm_profile' => array(13022,12955,12117,7585,6192),
    ),
    6684 => array(
        'm_title' => 'SELECT ONE',
        'm_message' => 'Select a single next idea',
        'm_icon' => '<i class="fas fa-check-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12955,12883,12336,12129,7585,7712,6193,6157),
    ),
    7231 => array(
        'm_title' => 'SELECT SOME',
        'm_message' => 'Select 1 or more next idea(s)',
        'm_icon' => '<i class="fas fa-check-square idea" aria-hidden="true"></i>',
        'm_profile' => array(12955,12884,12334,12129,7585,7712,6193,7489),
    ),
    6907 => array(
        'm_title' => 'MEET ANY REQUIREMENT',
        'm_message' => 'Complete by reading one of the next ideas',
        'm_icon' => '<i class="fas fa-cube idea" aria-hidden="true"></i>',
        'm_profile' => array(12330,12883,12700,7486,7485,6140,7585,6193,7309,6997),
    ),
    6914 => array(
        'm_title' => 'MEET ALL REQUIREMENTS',
        'm_message' => 'Complete by reading all next ideas',
        'm_icon' => '<i class="fas fa-cubes idea" aria-hidden="true"></i>',
        'm_profile' => array(12330,12700,7486,7485,6140,7585,6192,7309,6997),
    ),
);

//IDEA MENU:
$config['n___11047'] = array(4356,7276,7264,11049,12733,4341,7279);
$config['e___11047'] = array(
    4356 => array(
        'm_title' => 'IDEA DURATION (SECONDS)',
        'm_message' => '/e/plugin/4356?i__id=',
        'm_icon' => '<i class="fas fa-stopwatch idea" aria-hidden="true"></i>',
        'm_profile' => array(10986,13295,6287,12741,11047,7274,6404,12112,10650,6201),
    ),
    7276 => array(
        'm_title' => 'IDEA CACHED SOURCES',
        'm_message' => '/e/plugin/7276?i__id=',
        'm_icon' => '<i class="fad fa-sync idea mench-spin" aria-hidden="true"></i>',
        'm_profile' => array(6287,12741,11047,7286,7274),
    ),
    7264 => array(
        'm_title' => 'IDEA BIRDS EYE MARKS',
        'm_message' => '/e/plugin/7264?i__id=',
        'm_icon' => '<i class="fad fa-sitemap idea"></i>',
        'm_profile' => array(11047,6287),
    ),
    11049 => array(
        'm_title' => 'IDEA REVIEW JSON',
        'm_message' => '/e/plugin/11049?i__id=',
        'm_icon' => '<i class="fad fa-lambda idea" aria-hidden="true"></i>',
        'm_profile' => array(12741,6287,11047),
    ),
    12733 => array(
        'm_title' => 'DISCOVER REVIEW JSON',
        'm_message' => '/e/plugin/12733?i__id=',
        'm_icon' => '<i class="fad fa-code discover"></i>',
        'm_profile' => array(11047,12741,12701,6287),
    ),
    4341 => array(
        'm_title' => 'MENCH LEDGER',
        'm_message' => '/ledger?any_i__id=',
        'm_icon' => '<i class="fas fa-atlas discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13450,12701,12500,6212,12887,11047,12707,10876,11035,4527,7735),
    ),
    7279 => array(
        'm_title' => 'SYNC SEARCH INDEX',
        'm_message' => '/e/plugin/7279?obj=4535&object__id=',
        'm_icon' => '<i class="fad fa-search"></i>',
        'm_profile' => array(6287,12741,12887,11047,3323,7287),
    ),
);

//SOURCE LINKS:
$config['n___4592'] = array(4259,4257,4261,4260,4319,7657,4230,4255,4318,4256,4258,12827);
$config['e___4592'] = array(
    4259 => array(
        'm_title' => 'AUDIO',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-volume-up source"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4257 => array(
        'm_title' => 'EMBED URL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4758,12822,12605,12524,12403,4593,4592,4537),
    ),
    4261 => array(
        'm_title' => 'FILE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-file-pdf source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4260 => array(
        'm_title' => 'IMAGE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-image source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,6198,11059,10593,4593,4592,4537),
    ),
    4319 => array(
        'm_title' => 'INTEGER',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sort-numeric-down source" aria-hidden="true"></i>',
        'm_profile' => array(4593,4592),
    ),
    7657 => array(
        'm_title' => 'PERCENT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-divide source" aria-hidden="true"></i>',
        'm_profile' => array(4593,4592),
    ),
    4230 => array(
        'm_title' => 'RAW',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-link rotate90 source"></i>',
        'm_profile' => array(4593,4592),
    ),
    4255 => array(
        'm_title' => 'TEXT',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-align-left source"></i>',
        'm_profile' => array(13899,12822,10593,4593,4592),
    ),
    4318 => array(
        'm_title' => 'TIME',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-clock source"></i>',
        'm_profile' => array(4593,4592),
    ),
    4256 => array(
        'm_title' => 'URL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-external-link source" aria-hidden="true"></i>',
        'm_profile' => array(13433,12822,11080,4593,4592,4537),
    ),
    4258 => array(
        'm_title' => 'VIDEO',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    12827 => array(
        'm_title' => 'WORD',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-font source"></i>',
        'm_profile' => array(4593,4592),
    ),
);

//MENCH SOURCES:
$config['n___6206'] = array(6160,6177,6197,6198,13030,6172);
$config['e___6206'] = array(
    6160 => array(
        'm_title' => 'ID',
        'm_message' => 'e__id',
        'm_icon' => '<i class="fas fa-at source" aria-hidden="true"></i>',
        'm_profile' => array(6206),
    ),
    6177 => array(
        'm_title' => 'STATUS',
        'm_message' => 'e__status',
        'm_icon' => '<i class="fas fa-sliders-h source" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,12766,4527,11054,6204,5003,6160,6206),
    ),
    6197 => array(
        'm_title' => 'TITLE',
        'm_message' => 'e__title',
        'm_icon' => '<i class="fad fa-fingerprint source" aria-hidden="true"></i>',
        'm_profile' => array(13571,13296,13025,6404,12112,12232,10646,5000,4998,4999,6206),
    ),
    6198 => array(
        'm_title' => 'ICON',
        'm_message' => 'e__icon',
        'm_icon' => '<i class="fas fa-image-polaroid source"></i>',
        'm_profile' => array(4527,13571,12605,10653,5943,10625,6206),
    ),
    13030 => array(
        'm_title' => 'WEIGHT',
        'm_message' => 'e__weight',
        'm_icon' => '<i class="fas fa-weight source"></i>',
        'm_profile' => array(6206),
    ),
    6172 => array(
        'm_title' => 'METADATA',
        'm_message' => 'e__metadata',
        'm_icon' => '<i class="fas fa-lambda source"></i>',
        'm_profile' => array(7277,4527,6212,6206),
    ),
);

//IDEA NOTES:
$config['n___4485'] = array(4231,12419,4601,7545,13865,4983,10573,12896);
$config['e___4485'] = array(
    4231 => array(
        'm_title' => 'MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comment-lines idea" aria-hidden="true"></i>',
        'm_profile' => array(12994,13550,13294,12359,4485,10593,4986,4603,4593),
    ),
    12419 => array(
        'm_title' => 'COMMENTS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-comments discover" aria-hidden="true"></i>',
        'm_profile' => array(12701,11035,13871,12994,11089,13530,11018,12359,5967,10593,4986,4593,4485),
    ),
    4601 => array(
        'm_title' => 'KEYWORDS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-tags idea" aria-hidden="true"></i>',
        'm_profile' => array(12700,13023,13530,11018,4485,10593,4593),
    ),
    7545 => array(
        'm_title' => 'AWARDS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-file-certificate source" aria-hidden="true"></i>',
        'm_profile' => array(11089,13530,6159,13421,13298,12197,11018,4485,7551,4593),
    ),
    13865 => array(
        'm_title' => 'REQUIREMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shield-check source"></i>',
        'm_profile' => array(4527,11035,13421,13298,7551,13530,4593,11018,11089,4485),
    ),
    4983 => array(
        'm_title' => 'CITATIONS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link source rotate90" aria-hidden="true"></i>',
        'm_profile' => array(12359,7551,13550,13543,13407,4485,12141,10939,12450,12228,4593,10593),
    ),
    10573 => array(
        'm_title' => 'MY IDEAS MAP',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sitemap idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,11035,11054,4527,10984,11018,4485,4593,7551),
    ),
    12896 => array(
        'm_title' => 'SAVED IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bookmark discover"></i>',
        'm_profile' => array(13877,12701,11035,11018,11089,12677,4485,7551,4755,4593),
    ),
);

//AVATARS BASIC:
$config['n___10956'] = array(12286,12287,12288,12234,12233,10965,12236,12235,10979,12295,12294,12293,12300,12301,12299,12237,12238,10978,12314,12315,12316,12240,12239,10963,12241,12242,12207,12244,12243,10966,12245,12246,10976,12248,12247,10962,12249,12250,10975,12252,12251,10982,12253,12254,10970,12256,12255,10972,12257,12258,10969,12260,12259,10960,12439,12262,10981,12264,12263,10968,12265,12266,10974,12268,12267,12206,12269,12270,10958,12272,12271,12231);
$config['e___10956'] = array(
    12286 => array(
        'm_title' => 'BAT BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bat source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12287 => array(
        'm_title' => 'BAT LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-bat source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12288 => array(
        'm_title' => 'BAT MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-bat source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12234 => array(
        'm_title' => 'DOGY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-dog source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12233 => array(
        'm_title' => 'DOGY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-dog source"></i>',
        'm_profile' => array(12279,10956),
    ),
    10965 => array(
        'm_title' => 'DOGY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-dog source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12236 => array(
        'm_title' => 'DONALD BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-duck source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12235 => array(
        'm_title' => 'DONALD LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-duck source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10979 => array(
        'm_title' => 'DONALD MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-duck source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12295 => array(
        'm_title' => 'DOVE BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-dove source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12294 => array(
        'm_title' => 'DOVE LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-dove source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12293 => array(
        'm_title' => 'DOVE MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-dove source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12300 => array(
        'm_title' => 'ELEPHANT BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-elephant source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12301 => array(
        'm_title' => 'ELEPHANT LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-elephant source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12299 => array(
        'm_title' => 'ELEPHANT MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-elephant source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12237 => array(
        'm_title' => 'FISHY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-fish source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12238 => array(
        'm_title' => 'FISHY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-fish source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10978 => array(
        'm_title' => 'FISHY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-fish source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12314 => array(
        'm_title' => 'FROG BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-frog source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12315 => array(
        'm_title' => 'FROG LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-frog source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12316 => array(
        'm_title' => 'FROG MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-frog source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12240 => array(
        'm_title' => 'HIPPOY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-hippo source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12239 => array(
        'm_title' => 'HIPPOY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-hippo source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10963 => array(
        'm_title' => 'HIPPOY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-hippo source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12241 => array(
        'm_title' => 'HONEY BADGER BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-badger-honey source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12242 => array(
        'm_title' => 'HONEY BADGER LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-badger-honey source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12207 => array(
        'm_title' => 'HONEY BADGER MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-badger-honey source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12244 => array(
        'm_title' => 'HONEY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-deer source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12243 => array(
        'm_title' => 'HONEY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-deer source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10966 => array(
        'm_title' => 'HONEY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-deer source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12245 => array(
        'm_title' => 'HORSY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-horse source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12246 => array(
        'm_title' => 'HORSY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-horse source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10976 => array(
        'm_title' => 'HORSY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-horse source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12248 => array(
        'm_title' => 'HUMAN BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-monkey source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12247 => array(
        'm_title' => 'HUMAN LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-monkey source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10962 => array(
        'm_title' => 'HUMAN MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-monkey source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12249 => array(
        'm_title' => 'KIWI BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-kiwi-bird source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12250 => array(
        'm_title' => 'KIWI LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-kiwi-bird source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10975 => array(
        'm_title' => 'KIWI MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-kiwi-bird source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12252 => array(
        'm_title' => 'MIMY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cat source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12251 => array(
        'm_title' => 'MIMY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-cat source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10982 => array(
        'm_title' => 'MIMY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-cat source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12253 => array(
        'm_title' => 'MOMY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cow source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12254 => array(
        'm_title' => 'MOMY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-cow source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10970 => array(
        'm_title' => 'MOMY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-cow source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12256 => array(
        'm_title' => 'NINJA BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-turtle source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12255 => array(
        'm_title' => 'NINJA LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-turtle source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10972 => array(
        'm_title' => 'NINJA MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-turtle source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12257 => array(
        'm_title' => 'PIGGY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-pig source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12258 => array(
        'm_title' => 'PIGGY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-pig source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10969 => array(
        'm_title' => 'PIGGY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-pig source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12260 => array(
        'm_title' => 'ROGER BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-rabbit source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12259 => array(
        'm_title' => 'ROGER LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-rabbit source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10960 => array(
        'm_title' => 'ROGER MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-rabbit source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12439 => array(
        'm_title' => 'RUSSEL BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-crow source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12262 => array(
        'm_title' => 'RUSSEL LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-crow source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10981 => array(
        'm_title' => 'RUSSEL MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-crow source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12264 => array(
        'm_title' => 'SHEEPY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sheep source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12263 => array(
        'm_title' => 'SHEEPY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-sheep source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10968 => array(
        'm_title' => 'SHEEPY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sheep source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12265 => array(
        'm_title' => 'SNAKY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-snake source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12266 => array(
        'm_title' => 'SNAKY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-snake source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10974 => array(
        'm_title' => 'SNAKY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-snake source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12268 => array(
        'm_title' => 'SPIDER BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-spider source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12267 => array(
        'm_title' => 'SPIDER LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-spider source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12206 => array(
        'm_title' => 'SPIDER MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-spider source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12269 => array(
        'm_title' => 'SQUIRRELY BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-squirrel source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12270 => array(
        'm_title' => 'SQUIRRELY LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-squirrel source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    10958 => array(
        'm_title' => 'SQUIRRELY MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-squirrel source"></i>',
        'm_profile' => array(12279,10956),
    ),
    12272 => array(
        'm_title' => 'WHALE BOLD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-whale source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12271 => array(
        'm_title' => 'WHALE LIGHT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-whale source" aria-hidden="true"></i>',
        'm_profile' => array(12279,10956),
    ),
    12231 => array(
        'm_title' => 'WHALE MIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-whale source"></i>',
        'm_profile' => array(12279,10956),
    ),
);

//MENCH:
$config['n___2738'] = array(13450,7146,12024,6403,7254);
$config['e___2738'] = array(
    13450 => array(
        'm_title' => 'GLOSSARY',
        'm_message' => 'A non-profit social ideation network on a mission to spread ideas that matter',
        'm_icon' => '<i class="fas fa-compass"></i>',
        'm_profile' => array(2738),
    ),
    7146 => array(
        'm_title' => 'SOCIAL PROFILES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-share-alt"></i>',
        'm_profile' => array(2738),
    ),
    12024 => array(
        'm_title' => 'MILESTONES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-flag" aria-hidden="true"></i>',
        'm_profile' => array(2738),
    ),
    6403 => array(
        'm_title' => 'PLATFORM',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-code" aria-hidden="true"></i>',
        'm_profile' => array(2738),
    ),
    7254 => array(
        'm_title' => 'SETTINGS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cog" aria-hidden="true"></i>',
        'm_profile' => array(2738),
    ),
);

//IDEA NOTES 2X SOURCES ALLOWED:
$config['n___4986'] = array(12419,4231);
$config['e___4986'] = array(
    12419 => array(
        'm_title' => 'COMMENTS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-comments discover" aria-hidden="true"></i>',
        'm_profile' => array(12701,11035,13871,12994,11089,13530,11018,12359,5967,10593,4986,4593,4485),
    ),
    4231 => array(
        'm_title' => 'MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comment-lines idea" aria-hidden="true"></i>',
        'm_profile' => array(12994,13550,13294,12359,4485,10593,4986,4603,4593),
    ),
);

//THE FIVE LINKS:
$config['n___10692'] = array(4366,4429,4368,4369,4371);
$config['e___10692'] = array(
    4366 => array(
        'm_title' => 'UP',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-triangle source" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,6160,4341),
    ),
    4429 => array(
        'm_title' => 'DOWN',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-triangle rotate180 source" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,6160,4341),
    ),
    4368 => array(
        'm_title' => 'LEFT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-triangle rotate270 idea" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,6202,4341),
    ),
    4369 => array(
        'm_title' => 'RIGHT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-triangle rotate90 idea" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,6202,4341),
    ),
    4371 => array(
        'm_title' => 'REFERENCE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-atlas" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,4367,4341),
    ),
);

//MENCH MEMORY:
$config['n___4527'] = array(13889,13783,10956,12279,13533,13890,11081,7704,5967,12326,6255,12994,12227,7304,7360,7364,7359,13298,13300,13304,10593,12141,12327,12229,13289,12446,6150,13291,13884,13571,13491,13479,4229,12842,4486,12840,12589,13544,11047,6159,4485,7551,4986,12359,4603,6193,4737,7356,12138,7355,12400,12677,12675,11018,13530,13408,13022,12330,7309,7585,7712,12883,12884,12955,13207,6170,2738,12467,6404,12079,6201,4341,4527,11054,11035,6287,12687,13894,6206,12112,10876,6212,7277,6169,13518,6225,12969,10573,12211,3000,12741,12577,13776,4755,13531,13550,13865,12761,13023,13672,4269,6204,13413,6194,6198,12822,13899,11080,4592,12403,11059,4537,12524,4997,12887,6172,12563,6177,7358,12575,7357,12401,11089,12574,13425,10957,12523,10692,6103,6175,12399,6176,6186,6173,4593,13442,6146,13480,13548,12500);
$config['e___4527'] = array(
    13889 => array(
        'm_title' => 'ACCOUNT DEFAULTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-badge-check source"></i>',
        'm_profile' => array(4758,4527),
    ),
    13783 => array(
        'm_title' => 'ANNOUNCEMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-calendar-day discover"></i>',
        'm_profile' => array(4527,7569,12701,11018),
    ),
    10956 => array(
        'm_title' => 'AVATARS BASIC',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-paw source" aria-hidden="true"></i>',
        'm_profile' => array(12289,4527),
    ),
    12279 => array(
        'm_title' => 'AVATARS SUPER',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-paw-claws source" aria-hidden="true"></i>',
        'm_profile' => array(12289,4527),
    ),
    13533 => array(
        'm_title' => 'AVATARS TYPES',
        'm_message' => '',
        'm_icon' => '<i class="far fa-paw-claws source"></i>',
        'm_profile' => array(4527,12289,10939),
    ),
    13890 => array(
        'm_title' => 'CUSTOMIZABLE UI',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-send-backward"></i>',
        'm_profile' => array(11054,4758,4527),
    ),
    11081 => array(
        'm_title' => 'DISCOVER ALL CONNECTIONS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-bezier-curve discover"></i>',
        'm_profile' => array(4527,6771),
    ),
    7704 => array(
        'm_title' => 'DISCOVER ANSWER',
        'm_message' => '',
        'm_icon' => '<i class="far fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(13028,12228,4527),
    ),
    5967 => array(
        'm_title' => 'DISCOVER CARBON COPY',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-envelope-open discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,7569,4755,4593),
    ),
    12326 => array(
        'm_title' => 'DISCOVER EXPANSIONS',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-expand discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,12228),
    ),
    6255 => array(
        'm_title' => 'DISCOVERIES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,13776,13450,11089,12677,11018,12467,12228,4527),
    ),
    12994 => array(
        'm_title' => 'DISCOVER LAYOUT',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-crop-alt discover" aria-hidden="true"></i>',
        'm_profile' => array(13487,4527),
    ),
    12227 => array(
        'm_title' => 'DISCOVER PROGRESS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-walking discover" aria-hidden="true"></i>',
        'm_profile' => array(12228,4527),
    ),
    7304 => array(
        'm_title' => 'DISCOVER STATS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-chart-bar discover"></i>',
        'm_profile' => array(6771,10888,4527),
    ),
    7360 => array(
        'm_title' => 'DISCOVER STATUS ACTIVE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-check-circle" aria-hidden="true"></i>',
        'm_profile' => array(10624,4527),
    ),
    7364 => array(
        'm_title' => 'DISCOVER STATUS INCOMPLETE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-spinner fa-spin" aria-hidden="true"></i>',
        'm_profile' => array(10624,4527),
    ),
    7359 => array(
        'm_title' => 'DISCOVER STATUS PUBLIC',
        'm_message' => '',
        'm_icon' => '<i class="far fa-circle" aria-hidden="true"></i>',
        'm_profile' => array(10624,4527),
    ),
    13298 => array(
        'm_title' => 'DISCOVER TAB HIDE IF ZERO',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle-slash discover"></i>',
        'm_profile' => array(4527,13299),
    ),
    13300 => array(
        'm_title' => 'DISCOVER TAB OPEN BY DEFAULT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-expand discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,13299),
    ),
    13304 => array(
        'm_title' => 'DISCOVER TAB SHOW IF LOGGED-IN',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle discover"></i>',
        'm_profile' => array(4527,13299),
    ),
    10593 => array(
        'm_title' => 'DISCOVER TYPE ADD CONTENT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-file-alt" aria-hidden="true"></i>',
        'm_profile' => array(12144,4527),
    ),
    12141 => array(
        'm_title' => 'DISCOVER TYPE COIN AWARD',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-coin" aria-hidden="true"></i>',
        'm_profile' => array(12144,4527),
    ),
    12327 => array(
        'm_title' => 'DISCOVER UNLOCKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-lock-open discover"></i>',
        'm_profile' => array(4527,12228),
    ),
    12229 => array(
        'm_title' => 'DISCOVERY COMPLETED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,12228),
    ),
    13289 => array(
        'm_title' => 'DISCOVERY CONTROLLER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-rectangle-wide discover"></i>',
        'm_profile' => array(4527,12994),
    ),
    12446 => array(
        'm_title' => 'DISCOVERY LEGEND',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-question-circle discover"></i>',
        'm_profile' => array(4527,6771),
    ),
    6150 => array(
        'm_title' => 'DISCOVERY REMOVED',
        'm_message' => '',
        'm_icon' => '<i class="far fa-bookmark discover" aria-hidden="true"></i>',
        'm_profile' => array(6771,4527),
    ),
    13291 => array(
        'm_title' => 'DISCOVERY TABS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bars discover"></i>',
        'm_profile' => array(10939,4527,12994),
    ),
    13884 => array(
        'm_title' => 'DISPLAY MODE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-eclipse"></i>',
        'm_profile' => array(6204,11054,13890,4527,6225),
    ),
    13571 => array(
        'm_title' => 'EDIT SOURCE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-pen-square"></i>',
        'm_profile' => array(13297,11035,4527,13566),
    ),
    13491 => array(
        'm_title' => 'FONT SIZE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-text-size"></i>',
        'm_profile' => array(11054,13890,6225,4527,6204),
    ),
    13479 => array(
        'm_title' => 'HEADER NAVIGATION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-list" aria-hidden="true"></i>',
        'm_profile' => array(4527,13357),
    ),
    4229 => array(
        'm_title' => 'IDEA LINK CONDITIONAL',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-question-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12842,4486,4527,6410,6283,4593),
    ),
    12842 => array(
        'm_title' => 'IDEA LINK ONE-WAY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-long-arrow-down idea" aria-hidden="true"></i>',
        'm_profile' => array(4527,12841),
    ),
    4486 => array(
        'm_title' => 'IDEA LINKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link idea" aria-hidden="true"></i>',
        'm_profile' => array(13442,13408,12700,11054,12079,10662,4527),
    ),
    12840 => array(
        'm_title' => 'IDEA LINK TWO-WAYS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-exchange rotate90 idea" aria-hidden="true"></i>',
        'm_profile' => array(4527,12841),
    ),
    12589 => array(
        'm_title' => 'IDEA LIST EDITOR',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-list idea" aria-hidden="true"></i>',
        'm_profile' => array(13530,13403,11018,4527,12590),
    ),
    13544 => array(
        'm_title' => 'IDEA MAP STATS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea"></i>',
        'm_profile' => array(6768,4527),
    ),
    11047 => array(
        'm_title' => 'IDEA MENU',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-caret-down" aria-hidden="true"></i>',
        'm_profile' => array(13530,12700,11018,4527,11040),
    ),
    6159 => array(
        'm_title' => 'IDEA METADATA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-lambda idea" aria-hidden="true"></i>',
        'm_profile' => array(7276,7277,4527,6212,11049,6201),
    ),
    4485 => array(
        'm_title' => 'IDEA NOTES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comment-plus idea" aria-hidden="true"></i>',
        'm_profile' => array(11054,6768,6404,4527),
    ),
    7551 => array(
        'm_title' => 'IDEA NOTES 1X SOURCE REQUIRED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-at source" aria-hidden="true"></i>',
        'm_profile' => array(10889,4527),
    ),
    4986 => array(
        'm_title' => 'IDEA NOTES 2X SOURCES ALLOWED',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-at" aria-hidden="true"></i>',
        'm_profile' => array(4527,10889),
    ),
    12359 => array(
        'm_title' => 'IDEA NOTES FILE UPLOADING ALLOWED',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-upload"></i>',
        'm_profile' => array(10889,4527),
    ),
    4603 => array(
        'm_title' => 'IDEA NOTES SORTING ALLOWED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sort" aria-hidden="true"></i>',
        'm_profile' => array(4527,10889),
    ),
    6193 => array(
        'm_title' => 'IDEA OR',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-code-branch rotate180 " aria-hidden="true"></i>',
        'm_profile' => array(10602,4527),
    ),
    4737 => array(
        'm_title' => 'IDEA STATUS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sliders-h idea" aria-hidden="true"></i>',
        'm_profile' => array(12701,13408,13295,11054,12079,4527,6204,6226,6160,6201),
    ),
    7356 => array(
        'm_title' => 'IDEA STATUS ACTIVE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-check-circle"></i>',
        'm_profile' => array(11054,10891,4527),
    ),
    12138 => array(
        'm_title' => 'IDEA STATUS FEATURED',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-search idea" aria-hidden="true"></i>',
        'm_profile' => array(6287,4527,10891),
    ),
    7355 => array(
        'm_title' => 'IDEA STATUS PUBLIC',
        'm_message' => '',
        'm_icon' => '<i class="far fa-circle" aria-hidden="true"></i>',
        'm_profile' => array(11054,10891,4527),
    ),
    12400 => array(
        'm_title' => 'IDEA STATUS SYNC',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sync idea"></i>',
        'm_profile' => array(10891,12732,4527),
    ),
    12677 => array(
        'm_title' => 'IDEA TAB NO MANUAL ADD',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-lock idea"></i>',
        'm_profile' => array(4527,12676),
    ),
    12675 => array(
        'm_title' => 'IDEA TAB OPEN BY DEFAULT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-expand idea" aria-hidden="true"></i>',
        'm_profile' => array(4527,12676),
    ),
    11018 => array(
        'm_title' => 'IDEA TABS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bars idea" aria-hidden="true"></i>',
        'm_profile' => array(13294,4527),
    ),
    13530 => array(
        'm_title' => 'IDEA TAB SHOW EVEN IF ZERO',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea"></i>',
        'm_profile' => array(4527,12676),
    ),
    13408 => array(
        'm_title' => 'IDEA TOOLBAR',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-ellipsis-h idea" aria-hidden="true"></i>',
        'm_profile' => array(4527,12673,13407),
    ),
    13022 => array(
        'm_title' => 'IDEA TYPE ALL NEXT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sitemap idea"></i>',
        'm_profile' => array(4527,10893),
    ),
    12330 => array(
        'm_title' => 'IDEA TYPE COMPLETE IF EMPTY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bolt idea"></i>',
        'm_profile' => array(4527,10893),
    ),
    7309 => array(
        'm_title' => 'IDEA TYPE MEET REQUIREMENT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-cubes idea"></i>',
        'm_profile' => array(10893,4527),
    ),
    7585 => array(
        'm_title' => 'IDEA TYPES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes idea" aria-hidden="true"></i>',
        'm_profile' => array(12701,13408,13295,11054,12079,6201,4527,6204,10651,6160),
    ),
    7712 => array(
        'm_title' => 'IDEA TYPE SELECT NEXT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-question-circle idea"></i>',
        'm_profile' => array(11054,13028,6287,10893,4527),
    ),
    12883 => array(
        'm_title' => 'IDEA TYPE SELECT ONE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check idea"></i>',
        'm_profile' => array(4527,10893),
    ),
    12884 => array(
        'm_title' => 'IDEA TYPE SELECT SOME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-double idea"></i>',
        'm_profile' => array(4527,10893),
    ),
    12955 => array(
        'm_title' => 'IDEA TYPE TAKES COMPLETION TIME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-stopwatch idea"></i>',
        'm_profile' => array(4527,10893),
    ),
    13207 => array(
        'm_title' => 'LEADERBOARD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-medal source"></i>',
        'm_profile' => array(13920,13907,13543,11035,12500,12489,10876,4527),
    ),
    6170 => array(
        'm_title' => 'MAX IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea"></i>',
        'm_profile' => array(13629,4527,6159),
    ),
    2738 => array(
        'm_title' => 'MENCH',
        'm_message' => '',
        'm_icon' => '<img src="https://mench.com/img/mench.png" class="mench-spin no-radius">',
        'm_profile' => array(2,1,12891,11054,12041,4527,7312),
    ),
    12467 => array(
        'm_title' => 'MENCH COINS',
        'm_message' => '',
        'm_icon' => '<img src="https://mench.com/img/mench.png" class="mench-spin no-radius">',
        'm_profile' => array(11035,13450,11054,10939,12030,4527),
    ),
    6404 => array(
        'm_title' => 'MENCH CONFIG VARIABLES',
        'm_message' => '',
        'm_icon' => '<i class="far fa-lambda" aria-hidden="true"></i>',
        'm_profile' => array(11054,4527,6403),
    ),
    12079 => array(
        'm_title' => 'MENCH DROPDOWN MENUS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-caret-down" aria-hidden="true"></i>',
        'm_profile' => array(12829,6403,4527),
    ),
    6201 => array(
        'm_title' => 'MENCH IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-table idea" aria-hidden="true"></i>',
        'm_profile' => array(6212,11054,4527,7735),
    ),
    4341 => array(
        'm_title' => 'MENCH LEDGER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-atlas discover" aria-hidden="true"></i>',
        'm_profile' => array(13920,13450,12701,12500,6212,12887,11047,12707,10876,11035,4527,7735),
    ),
    4527 => array(
        'm_title' => 'MENCH MEMORY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-memory" aria-hidden="true"></i>',
        'm_profile' => array(4755,6403,12741,6287,4527),
    ),
    11054 => array(
        'm_title' => 'MENCH MEMORY JAVASCRIPT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-memory" aria-hidden="true"></i>',
        'm_profile' => array(4755,6403,4527),
    ),
    11035 => array(
        'm_title' => 'MENCH NAVIGATION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-compass" aria-hidden="true"></i>',
        'm_profile' => array(11054,6403,4527),
    ),
    6287 => array(
        'm_title' => 'MENCH PLUGINS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-plug" aria-hidden="true"></i>',
        'm_profile' => array(13920,12500,4367,12701,6405,12699,10876,11035,4527),
    ),
    12687 => array(
        'm_title' => 'MENCH RANDOM MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comments-alt" aria-hidden="true"></i>',
        'm_profile' => array(6403,11054,4527),
    ),
    13894 => array(
        'm_title' => 'MENCH SOCIAL LINKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-share"></i>',
        'm_profile' => array(6403,4593,4527),
    ),
    6206 => array(
        'm_title' => 'MENCH SOURCES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-table source" aria-hidden="true"></i>',
        'm_profile' => array(6212,4527,7735),
    ),
    12112 => array(
        'm_title' => 'MENCH TEXT INPUTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-text" aria-hidden="true"></i>',
        'm_profile' => array(12829,6403,4527),
    ),
    10876 => array(
        'm_title' => 'MENCH URL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link"></i>',
        'm_profile' => array(13672,13023,6403,4527,1326),
    ),
    6212 => array(
        'm_title' => 'MENCH VARIABLES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-subscript" aria-hidden="true"></i>',
        'm_profile' => array(6403,4527),
    ),
    7277 => array(
        'm_title' => 'METADATA CLEAN VARIABLES',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(4527,6287,12741,7287,7274),
    ),
    6169 => array(
        'm_title' => 'MIN IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea"></i>',
        'm_profile' => array(13629,4527,6159),
    ),
    13518 => array(
        'm_title' => 'MONTHLY SUBSCRIPTION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-usd-circle discover"></i>',
        'm_profile' => array(11054,13890,6204,4527,6225,13450),
    ),
    6225 => array(
        'm_title' => 'MY ACCOUNT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cog source" aria-hidden="true"></i>',
        'm_profile' => array(12574,13425,11089,4527),
    ),
    12969 => array(
        'm_title' => 'MY DISCOVERIES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-compass discover" aria-hidden="true"></i>',
        'm_profile' => array(11035,12228,4527),
    ),
    10573 => array(
        'm_title' => 'MY IDEAS MAP',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sitemap idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,11035,11054,4527,10984,11018,4485,4593,7551),
    ),
    12211 => array(
        'm_title' => 'NEXT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chevron-right" aria-hidden="true"></i>',
        'm_profile' => array(4527,13289,11035),
    ),
    3000 => array(
        'm_title' => 'NONFICTION TYPES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes source" aria-hidden="true"></i>',
        'm_profile' => array(12079,4527),
    ),
    12741 => array(
        'm_title' => 'PLUGIN RETURN CODE ONLY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-code"></i>',
        'm_profile' => array(12999,4527),
    ),
    12577 => array(
        'm_title' => 'PORTFOLIO EDITOR UPPERCASE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-text" aria-hidden="true"></i>',
        'm_profile' => array(4527,4758),
    ),
    13776 => array(
        'm_title' => 'PRIMARY COIN',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(4527,7254),
    ),
    4755 => array(
        'm_title' => 'PRIVATE INFORMATION',
        'm_message' => '',
        'm_icon' => '<i class="far fa-eye-slash" aria-hidden="true"></i>',
        'm_profile' => array(11035,12701,4755,6771,4527),
    ),
    13531 => array(
        'm_title' => 'QUICK SHARE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-share-alt"></i>',
        'm_profile' => array(13023,4527),
    ),
    13550 => array(
        'm_title' => 'REFERENCED IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea"></i>',
        'm_profile' => array(11035,4527,12273),
    ),
    13865 => array(
        'm_title' => 'REQUIREMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shield-check source"></i>',
        'm_profile' => array(4527,11035,13421,13298,7551,13530,4593,11018,11089,4485),
    ),
    12761 => array(
        'm_title' => 'SEARCH INDEX',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-search"></i>',
        'm_profile' => array(4527,3323),
    ),
    13023 => array(
        'm_title' => 'SHARE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-share"></i>',
        'm_profile' => array(13291,4527,13024),
    ),
    13672 => array(
        'm_title' => 'SHOW URL',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(4527,7254),
    ),
    4269 => array(
        'm_title' => 'SIGN IN/UP',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sign-in-alt" aria-hidden="true"></i>',
        'm_profile' => array(13479,10876,4527,11035),
    ),
    6204 => array(
        'm_title' => 'SINGLE SELECTABLE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check" aria-hidden="true"></i>',
        'm_profile' => array(4758,4527),
    ),
    13413 => array(
        'm_title' => 'SORTABLE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-arrows"></i>',
        'm_profile' => array(7254,4527),
    ),
    6194 => array(
        'm_title' => 'SOURCE CONNECTORS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link source" aria-hidden="true"></i>',
        'm_profile' => array(12701,4758,4527),
    ),
    6198 => array(
        'm_title' => 'SOURCE ICON',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-image-polaroid source"></i>',
        'm_profile' => array(4527,13571,12605,10653,5943,10625,6206),
    ),
    12822 => array(
        'm_title' => 'SOURCE LINK DISPLAY MESSAGE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-eye source"></i>',
        'm_profile' => array(4527,12821),
    ),
    13899 => array(
        'm_title' => 'SOURCE LINK DISPLAY TOOLTIP-ONLY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comment-alt source"></i>',
        'm_profile' => array(4527,12821),
    ),
    11080 => array(
        'm_title' => 'SOURCE LINK FILE EXTENSIONS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-file source"></i>',
        'm_profile' => array(12821,4527),
    ),
    4592 => array(
        'm_title' => 'SOURCE LINKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link source" aria-hidden="true"></i>',
        'm_profile' => array(11054,4527,5982),
    ),
    12403 => array(
        'm_title' => 'SOURCE LINK TYPE CUSTOM UI',
        'm_message' => '',
        'm_icon' => '<i class="far fa-object-ungroup source" aria-hidden="true"></i>',
        'm_profile' => array(12821,4527),
    ),
    11059 => array(
        'm_title' => 'SOURCE LINK UPLOAD FILE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-file-plus source"></i>',
        'm_profile' => array(12821,4527),
    ),
    4537 => array(
        'm_title' => 'SOURCE LINK URLS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-spider-web source" aria-hidden="true"></i>',
        'm_profile' => array(12821,4527),
    ),
    12524 => array(
        'm_title' => 'SOURCE LINK VISUAL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-film-alt source" aria-hidden="true"></i>',
        'm_profile' => array(12821,4527),
    ),
    4997 => array(
        'm_title' => 'SOURCE LIST EDITOR',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-list source" aria-hidden="true"></i>',
        'm_profile' => array(12574,13429,11035,12703,12590,4527),
    ),
    12887 => array(
        'm_title' => 'SOURCE MENU',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-caret-down" aria-hidden="true"></i>',
        'm_profile' => array(11089,12703,4527,11040),
    ),
    6172 => array(
        'm_title' => 'SOURCE METADATA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-lambda source"></i>',
        'm_profile' => array(7277,4527,6212,6206),
    ),
    12563 => array(
        'm_title' => 'SOURCES FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star source" aria-hidden="true"></i>',
        'm_profile' => array(12399,4527,12575,10654,7358,7357,6177),
    ),
    6177 => array(
        'm_title' => 'SOURCE STATUS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sliders-h source" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,12766,4527,11054,6204,5003,6160,6206),
    ),
    7358 => array(
        'm_title' => 'SOURCE STATUS ACTIVE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-check-circle source" aria-hidden="true"></i>',
        'm_profile' => array(12572,4527),
    ),
    12575 => array(
        'm_title' => 'SOURCE STATUS FEATURED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star source"></i>',
        'm_profile' => array(4527,12572),
    ),
    7357 => array(
        'm_title' => 'SOURCE STATUS PUBLIC',
        'm_message' => '',
        'm_icon' => '<i class="far fa-globe source" aria-hidden="true"></i>',
        'm_profile' => array(13296,12572,11054,4527),
    ),
    12401 => array(
        'm_title' => 'SOURCE STATUS SYNC',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sync source"></i>',
        'm_profile' => array(12572,12732,4527),
    ),
    11089 => array(
        'm_title' => 'SOURCE TABS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bars source" aria-hidden="true"></i>',
        'm_profile' => array(13296,4527),
    ),
    12574 => array(
        'm_title' => 'SOURCE TAB SHOW EVEN IF ZERO',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle source"></i>',
        'm_profile' => array(4527,12573),
    ),
    13425 => array(
        'm_title' => 'SOURCE TAB SHOW IF SOURCE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle source"></i>',
        'm_profile' => array(4527,12573),
    ),
    10957 => array(
        'm_title' => 'SUPERPOWERS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bolt" aria-hidden="true"></i>',
        'm_profile' => array(6225,11035,5007,4527),
    ),
    12523 => array(
        'm_title' => 'SYNC ICONS',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sync source fa-spin" aria-hidden="true"></i>',
        'm_profile' => array(12967,4527),
    ),
    10692 => array(
        'm_title' => 'THE FIVE LINKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bezier-curve"></i>',
        'm_profile' => array(4527,6771),
    ),
    6103 => array(
        'm_title' => 'TRANSACTION METADATA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-lambda"></i>',
        'm_profile' => array(6212,4527,4341),
    ),
    6175 => array(
        'm_title' => 'TRANSACTIONS DRAFT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-spinner-third fa-spin discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,7364,6186,7360),
    ),
    12399 => array(
        'm_title' => 'TRANSACTIONS FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,7360,7359,6186),
    ),
    6176 => array(
        'm_title' => 'TRANSACTIONS PUBLISH',
        'm_message' => '',
        'm_icon' => '<i class="far fa-globe discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,6186,7360,7359),
    ),
    6186 => array(
        'm_title' => 'TRANSACTION STATUS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-sliders-h" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,4527,11054,10677,10656,6204,5865,6160,7304,4341),
    ),
    6173 => array(
        'm_title' => 'TRANSACTIONS UNLINK',
        'm_message' => '',
        'm_icon' => '<i class="far fa-unlink discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,10686,10673,6186),
    ),
    4593 => array(
        'm_title' => 'TRANSACTION TYPE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,6194,6204,11081,10659,6160,4341,4527),
    ),
    13442 => array(
        'm_title' => 'TRANSACTION TYPE UPDATE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes" aria-hidden="true"></i>',
        'm_profile' => array(4527,6212),
    ),
    6146 => array(
        'm_title' => 'UNFINISHED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-times-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(12228,4527),
    ),
    13480 => array(
        'm_title' => 'UNIQUE IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea"></i>',
        'm_profile' => array(12273,6768,4527),
    ),
    13548 => array(
        'm_title' => 'UNIQUE SOURCES',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(4527,12274),
    ),
    12500 => array(
        'm_title' => 'VERTICAL MENU',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-ellipsis-v"></i>',
        'm_profile' => array(13479,12079,12823,4527),
    ),
);

//DISCOVER TYPE ADD CONTENT:
$config['n___10593'] = array(4983,12419,4554,4556,4555,6563,4570,7702,4549,4551,4550,4548,4552,4553,4250,10679,10644,4601,4231,4251,4259,10657,4261,4260,4255,4258,10646);
$config['e___10593'] = array(
    4983 => array(
        'm_title' => 'CITATIONS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link source rotate90" aria-hidden="true"></i>',
        'm_profile' => array(12359,7551,13550,13543,13407,4485,12141,10939,12450,12228,4593,10593),
    ),
    12419 => array(
        'm_title' => 'COMMENTS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-comments discover" aria-hidden="true"></i>',
        'm_profile' => array(12701,11035,13871,12994,11089,13530,11018,12359,5967,10593,4986,4593,4485),
    ),
    4554 => array(
        'm_title' => 'DISCOVER AUDIO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-volume-up"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    4556 => array(
        'm_title' => 'DISCOVER FILE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-file-pdf"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    4555 => array(
        'm_title' => 'DISCOVER IMAGE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-image"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    6563 => array(
        'm_title' => 'DISCOVER QUICK REPLY',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-check"></i>',
        'm_profile' => array(10593,4280,4593,4755),
    ),
    4570 => array(
        'm_title' => 'DISCOVER RECEIVED',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-envelope-open discover" aria-hidden="true"></i>',
        'm_profile' => array(10683,10593,7569,4755,4593),
    ),
    7702 => array(
        'm_title' => 'DISCOVER RECEIVED IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-envelope-open discover" aria-hidden="true"></i>',
        'm_profile' => array(4593,10593,4755,7569),
    ),
    4549 => array(
        'm_title' => 'DISCOVER SENT AUDIO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-volume-up"></i>',
        'm_profile' => array(10593,7653,4755,4593),
    ),
    4551 => array(
        'm_title' => 'DISCOVER SENT FILE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-file-pdf"></i>',
        'm_profile' => array(10593,7653,4755,4593),
    ),
    4550 => array(
        'm_title' => 'DISCOVER SENT IMAGE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-image"></i>',
        'm_profile' => array(10593,7653,4755,4593),
    ),
    4548 => array(
        'm_title' => 'DISCOVER SENT VIDEO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-video"></i>',
        'm_profile' => array(10593,7653,4755,4593),
    ),
    4552 => array(
        'm_title' => 'DISCOVER TEXT',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-align-left"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    4553 => array(
        'm_title' => 'DISCOVER VIDEO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-video"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    4250 => array(
        'm_title' => 'IDEA CREATED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13480,5967,12400,12149,10593,4593),
    ),
    10679 => array(
        'm_title' => 'IDEA NOTES UPDATE CONTENT',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-comment-plus idea" aria-hidden="true"></i>',
        'm_profile' => array(4593,10593),
    ),
    10644 => array(
        'm_title' => 'IDEA UPDATE TITLE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-bullseye-arrow idea" aria-hidden="true"></i>',
        'm_profile' => array(4593,10593),
    ),
    4601 => array(
        'm_title' => 'KEYWORDS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-tags idea" aria-hidden="true"></i>',
        'm_profile' => array(12700,13023,13530,11018,4485,10593,4593),
    ),
    4231 => array(
        'm_title' => 'MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comment-lines idea" aria-hidden="true"></i>',
        'm_profile' => array(12994,13550,13294,12359,4485,10593,4986,4603,4593),
    ),
    4251 => array(
        'm_title' => 'SOURCE CREATED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle source" aria-hidden="true"></i>',
        'm_profile' => array(13548,12401,12149,12141,10593,4593),
    ),
    4259 => array(
        'm_title' => 'SOURCE LINK AUDIO',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-volume-up source"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    10657 => array(
        'm_title' => 'SOURCE LINK CONTENT UPDATE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-comment-plus source" aria-hidden="true"></i>',
        'm_profile' => array(10593,4593),
    ),
    4261 => array(
        'm_title' => 'SOURCE LINK FILE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-file-pdf source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4260 => array(
        'm_title' => 'SOURCE LINK IMAGE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-image source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,6198,11059,10593,4593,4592,4537),
    ),
    4255 => array(
        'm_title' => 'SOURCE LINK TEXT',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-align-left source"></i>',
        'm_profile' => array(13899,12822,10593,4593,4592),
    ),
    4258 => array(
        'm_title' => 'SOURCE LINK VIDEO',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    10646 => array(
        'm_title' => 'SOURCE NAME UPDATE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-fingerprint source"></i>',
        'm_profile' => array(10593,4593),
    ),
);

//IDEA OR:
$config['n___6193'] = array(6684,7231,6907);
$config['e___6193'] = array(
    6684 => array(
        'm_title' => 'SELECT ONE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12955,12883,12336,12129,7585,7712,6193,6157),
    ),
    7231 => array(
        'm_title' => 'SELECT SOME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square idea" aria-hidden="true"></i>',
        'm_profile' => array(12955,12884,12334,12129,7585,7712,6193,7489),
    ),
    6907 => array(
        'm_title' => 'MEET ANY REQUIREMENT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cube idea" aria-hidden="true"></i>',
        'm_profile' => array(12330,12883,12700,7486,7485,6140,7585,6193,7309,6997),
    ),
);

//NONFICTION TYPES:
$config['n___3000'] = array(3005,2998,2997,13218,3147,4446,13350,3192);
$config['e___3000'] = array(
    3005 => array(
        'm_title' => 'BOOKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-book source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    2998 => array(
        'm_title' => 'VIDEOS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle source"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    2997 => array(
        'm_title' => 'ARTICLES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-newspaper source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    13218 => array(
        'm_title' => 'PODCAST EPISODES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-microphone source"></i>',
        'm_profile' => array(12563,3000),
    ),
    3147 => array(
        'm_title' => 'ONLINE COURSES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-presentation source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    4446 => array(
        'm_title' => 'ASSESSMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    13350 => array(
        'm_title' => 'REPORTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-file-chart-line source"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
    3192 => array(
        'm_title' => 'TOOLS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-tools source" aria-hidden="true"></i>',
        'm_profile' => array(12523,12563,3000),
    ),
);

//SOURCE CONNECTORS:
$config['n___6194'] = array(4364,4593);
$config['e___6194'] = array(
    4364 => array(
        'm_title' => 'SOURCE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-user-tag" aria-hidden="true"></i>',
        'm_profile' => array(6194,11081,6160,4341),
    ),
    4593 => array(
        'm_title' => 'TYPE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,6194,6204,11081,10659,6160,4341,4527),
    ),
);

//TRANSACTION METADATA:
$config['n___6103'] = array(4358,4739,4735);
$config['e___6103'] = array(
    4358 => array(
        'm_title' => 'DISCOVERY MARKS',
        'm_message' => 'tr__assessment_points',
        'm_icon' => '<i class="fas fa-comment-alt-check discover" aria-hidden="true"></i>',
        'm_profile' => array(13408,12700,12112,10663,6103,6410),
    ),
    4739 => array(
        'm_title' => 'UNLOCK MAX PERCENTAGE',
        'm_message' => 'tr__conditional_score_max',
        'm_icon' => '<i class="fas fa-temperature-up idea" aria-hidden="true"></i>',
        'm_profile' => array(6103,12112,6402),
    ),
    4735 => array(
        'm_title' => 'UNLOCK MIN PERCENTAGE',
        'm_message' => 'tr__conditional_score_min',
        'm_icon' => '<i class="fas fa-temperature-down idea" aria-hidden="true"></i>',
        'm_profile' => array(6103,12112,6402),
    ),
);

//MENCH LEDGER:
$config['n___4341'] = array(4367,4362,4372,4370,4371,6103,6186,4593,4364,4366,4429,4368,4369);
$config['e___4341'] = array(
    4367 => array(
        'm_title' => 'ID',
        'm_message' => 'x__id',
        'm_icon' => '<i class="fas fa-atlas" aria-hidden="true"></i>',
        'm_profile' => array(6287,4341),
    ),
    4362 => array(
        'm_title' => 'TIME',
        'm_message' => 'x__time',
        'm_icon' => '<i class="fas fa-clock" aria-hidden="true"></i>',
        'm_profile' => array(4341),
    ),
    4372 => array(
        'm_title' => 'MESSAGE',
        'm_message' => 'x__message',
        'm_icon' => '<i class="fas fa-comment-lines" aria-hidden="true"></i>',
        'm_profile' => array(13571,7578,10679,10657,5001,4341),
    ),
    4370 => array(
        'm_title' => 'SORT',
        'm_message' => 'x__sort',
        'm_icon' => '<i class="fas fa-sort" aria-hidden="true"></i>',
        'm_profile' => array(13007,13006,10676,10675,4341),
    ),
    4371 => array(
        'm_title' => 'REFERENCE',
        'm_message' => 'x__reference',
        'm_icon' => '<i class="far fa-atlas" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,4367,4341),
    ),
    6103 => array(
        'm_title' => 'METADATA',
        'm_message' => 'x__metadata',
        'm_icon' => '<i class="fas fa-lambda"></i>',
        'm_profile' => array(6212,4527,4341),
    ),
    6186 => array(
        'm_title' => 'STATUS',
        'm_message' => 'x__status',
        'm_icon' => '<i class="far fa-sliders-h" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,4527,11054,10677,10656,6204,5865,6160,7304,4341),
    ),
    4593 => array(
        'm_title' => 'TYPE',
        'm_message' => 'x__type',
        'm_icon' => '<i class="fas fa-shapes" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,6194,6204,11081,10659,6160,4341,4527),
    ),
    4364 => array(
        'm_title' => 'SOURCE',
        'm_message' => 'x__source',
        'm_icon' => '<i class="fas fa-user-tag" aria-hidden="true"></i>',
        'm_profile' => array(6194,11081,6160,4341),
    ),
    4366 => array(
        'm_title' => 'UP',
        'm_message' => 'x__up',
        'm_icon' => '<i class="fas fa-triangle source" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,6160,4341),
    ),
    4429 => array(
        'm_title' => 'DOWN',
        'm_message' => 'x__down',
        'm_icon' => '<i class="fas fa-triangle rotate180 source" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,6160,4341),
    ),
    4368 => array(
        'm_title' => 'LEFT',
        'm_message' => 'x__left',
        'm_icon' => '<i class="fas fa-triangle rotate270 idea" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,6202,4341),
    ),
    4369 => array(
        'm_title' => 'RIGHT',
        'm_message' => 'x__right',
        'm_icon' => '<i class="fas fa-triangle rotate90 idea" aria-hidden="true"></i>',
        'm_profile' => array(11081,10692,6202,4341),
    ),
);

//MENCH IDEAS:
$config['n___6201'] = array(6202,4737,7585,4736,4356,13029,6159);
$config['e___6201'] = array(
    6202 => array(
        'm_title' => 'ID',
        'm_message' => 'i__id',
        'm_icon' => '<i class="fas fa-hashtag idea" aria-hidden="true"></i>',
        'm_profile' => array(6201),
    ),
    4737 => array(
        'm_title' => 'STATUS',
        'm_message' => 'i__status',
        'm_icon' => '<i class="fas fa-sliders-h idea" aria-hidden="true"></i>',
        'm_profile' => array(12701,13408,13295,11054,12079,4527,6204,6226,6160,6201),
    ),
    7585 => array(
        'm_title' => 'TYPES',
        'm_message' => 'i__type',
        'm_icon' => '<i class="fas fa-shapes idea" aria-hidden="true"></i>',
        'm_profile' => array(12701,13408,13295,11054,12079,6201,4527,6204,10651,6160),
    ),
    4736 => array(
        'm_title' => 'TITLE',
        'm_message' => 'i__title',
        'm_icon' => '<i class="fas fa-h1 idea" aria-hidden="true"></i>',
        'm_profile' => array(13407,13294,12994,6404,12112,10644,6201),
    ),
    4356 => array(
        'm_title' => 'DURATION (SECONDS)',
        'm_message' => 'i__duration',
        'm_icon' => '<i class="fas fa-stopwatch idea" aria-hidden="true"></i>',
        'm_profile' => array(10986,13295,6287,12741,11047,7274,6404,12112,10650,6201),
    ),
    13029 => array(
        'm_title' => 'WEIGHT',
        'm_message' => 'i__weight',
        'm_icon' => '<i class="fas fa-weight idea"></i>',
        'm_profile' => array(6201),
    ),
    6159 => array(
        'm_title' => 'METADATA',
        'm_message' => 'i__metadata',
        'm_icon' => '<i class="fas fa-lambda idea" aria-hidden="true"></i>',
        'm_profile' => array(7276,7277,4527,6212,11049,6201),
    ),
);

//DISCOVER STATS:
$config['n___7304'] = array(6186);
$config['e___7304'] = array(
    6186 => array(
        'm_title' => 'TRANSACTION STATUS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-sliders-h" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,4527,11054,10677,10656,6204,5865,6160,7304,4341),
    ),
);

//SINGLE SELECTABLE:
$config['n___6204'] = array(13884,13491,4737,10602,7585,13518,13158,13172,13167,13166,13153,13174,13171,13152,13162,13156,13157,13155,13173,13170,13164,13160,13168,13165,13169,13159,13163,13161,13154,3290,6177,6186,4593);
$config['e___6204'] = array(
    13884 => array(
        'm_title' => 'DISPLAY MODE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-eclipse"></i>',
        'm_profile' => array(6204,11054,13890,4527,6225),
    ),
    13491 => array(
        'm_title' => 'FONT SIZE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-text-size"></i>',
        'm_profile' => array(11054,13890,6225,4527,6204),
    ),
    4737 => array(
        'm_title' => 'IDEA STATUS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sliders-h idea" aria-hidden="true"></i>',
        'm_profile' => array(12701,13408,13295,11054,12079,4527,6204,6226,6160,6201),
    ),
    10602 => array(
        'm_title' => 'IDEA TYPE GROUPS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-puzzle-piece idea"></i>',
        'm_profile' => array(10893,6204),
    ),
    7585 => array(
        'm_title' => 'IDEA TYPES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes idea" aria-hidden="true"></i>',
        'm_profile' => array(12701,13408,13295,11054,12079,6201,4527,6204,10651,6160),
    ),
    13518 => array(
        'm_title' => 'MONTHLY SUBSCRIPTION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-usd-circle discover"></i>',
        'm_profile' => array(11054,13890,6204,4527,6225,13450),
    ),
    13158 => array(
        'm_title' => 'RATE ANGULAR',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13172 => array(
        'm_title' => 'RATE AWS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13167 => array(
        'm_title' => 'RATE C#',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13166 => array(
        'm_title' => 'RATE C++',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13153 => array(
        'm_title' => 'RATE CSS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13174 => array(
        'm_title' => 'RATE DOCKER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13171 => array(
        'm_title' => 'RATE GIT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13152 => array(
        'm_title' => 'RATE HTML',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13162 => array(
        'm_title' => 'RATE JAVA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13156 => array(
        'm_title' => 'RATE JAVASCRIPT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13157 => array(
        'm_title' => 'RATE JQUERY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13155 => array(
        'm_title' => 'RATE JSON',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13173 => array(
        'm_title' => 'RATE LINUX',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13170 => array(
        'm_title' => 'RATE MONGODB',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13164 => array(
        'm_title' => 'RATE MYSQL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13160 => array(
        'm_title' => 'RATE NODE.JS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13168 => array(
        'm_title' => 'RATE PHP',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13165 => array(
        'm_title' => 'RATE POSTGRESQL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13169 => array(
        'm_title' => 'RATE PYTHON',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13159 => array(
        'm_title' => 'RATE REACT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13163 => array(
        'm_title' => 'RATE SQL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13161 => array(
        'm_title' => 'RATE TYPESCRIPT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    13154 => array(
        'm_title' => 'RATE VUE.JS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star"></i>',
        'm_profile' => array(6204,13151),
    ),
    3290 => array(
        'm_title' => 'SOURCE GENDER',
        'm_message' => '',
        'm_icon' => '<i class="far fa-transgender source" aria-hidden="true"></i>',
        'm_profile' => array(6204),
    ),
    6177 => array(
        'm_title' => 'SOURCE STATUS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sliders-h source" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,12766,4527,11054,6204,5003,6160,6206),
    ),
    6186 => array(
        'm_title' => 'TRANSACTION STATUS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-sliders-h" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,4527,11054,10677,10656,6204,5865,6160,7304,4341),
    ),
    4593 => array(
        'm_title' => 'TRANSACTION TYPE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shapes" aria-hidden="true"></i>',
        'm_profile' => array(13571,12701,6194,6204,11081,10659,6160,4341,4527),
    ),
);

//IDEA TYPE SELECT NEXT:
$config['n___7712'] = array(6684,7231);
$config['e___7712'] = array(
    6684 => array(
        'm_title' => 'ONE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12955,12883,12336,12129,7585,7712,6193,6157),
    ),
    7231 => array(
        'm_title' => 'SOME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square idea" aria-hidden="true"></i>',
        'm_profile' => array(12955,12884,12334,12129,7585,7712,6193,7489),
    ),
);

//DISCOVER ANSWER:
$config['n___7704'] = array(12336,12334,6157,7489);
$config['e___7704'] = array(
    12336 => array(
        'm_title' => 'DISCOVER ANSWER ONE LINK',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(7704,4755,4593,12326,12227),
    ),
    12334 => array(
        'm_title' => 'DISCOVER ANSWER SOME LINK',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square discover" aria-hidden="true"></i>',
        'm_profile' => array(7704,4755,4593,12326,12227),
    ),
    6157 => array(
        'm_title' => 'SOURCE ANSWER ONE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4593,6255,4755),
    ),
    7489 => array(
        'm_title' => 'SOURCE ANSWER SOME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4755,6255,4593),
    ),
);

//IDEA LINK CONDITIONAL:
$config['n___4229'] = array(6140,10664,6997);
$config['e___4229'] = array(
    6140 => array(
        'm_title' => 'DISCOVER UNLOCK CONDITION LINK',
        'm_message' => 'A step that has become available because of the score generated from student answers',
        'm_icon' => '<i class="fad fa-lock-open discover" aria-hidden="true"></i>',
        'm_profile' => array(12326,12227,6410,4229,4755,4593),
    ),
    10664 => array(
        'm_title' => 'IDEA LINK UPDATE SCORE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-bolt idea" aria-hidden="true"></i>',
        'm_profile' => array(4593,4229),
    ),
    6997 => array(
        'm_title' => 'SOURCE UNLOCK CONDITION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(6140,12327,12229,12227,12141,4229,6255,4593,4755),
    ),
);

//DISCOVER CARBON COPY:
$config['n___5967'] = array(12419,4235,12773,4250,12453,12450,4246,7504);
$config['e___5967'] = array(
    12419 => array(
        'm_title' => 'COMMENTS',
        'm_message' => '1',
        'm_icon' => '<i class="far fa-comments discover" aria-hidden="true"></i>',
        'm_profile' => array(12701,11035,13871,12994,11089,13530,11018,12359,5967,10593,4986,4593,4485),
    ),
    4235 => array(
        'm_title' => 'GET STARTED',
        'm_message' => '1',
        'm_icon' => '<i class="fas fa-chevron-right discover" aria-hidden="true"></i>',
        'm_profile' => array(12994,11035,12969,12227,4593,5967,4755),
    ),
    12773 => array(
        'm_title' => 'IDEA APPEND CONTENT',
        'm_message' => '1',
        'm_icon' => '<i class="far fa-plus-circle idea"></i>',
        'm_profile' => array(5967,4755,4593),
    ),
    4250 => array(
        'm_title' => 'IDEA CREATED',
        'm_message' => '1',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13480,5967,12400,12149,10593,4593),
    ),
    12453 => array(
        'm_title' => 'IDEA FEATURE REQUEST',
        'm_message' => '1',
        'm_icon' => '<i class="fad fa-megaphone idea"></i>',
        'm_profile' => array(4755,4593,5967),
    ),
    12450 => array(
        'm_title' => 'IDEA SOURCE REQUEST',
        'm_message' => '1',
        'm_icon' => '<i class="fas fa-plus-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4593,4755,5967),
    ),
    4246 => array(
        'm_title' => 'SOURCE BUG REPORTS',
        'm_message' => '1',
        'm_icon' => '<i class="fad fa-bug source" aria-hidden="true"></i>',
        'm_profile' => array(5967,4755,4593),
    ),
    7504 => array(
        'm_title' => 'SOURCE PENDING MODERATION',
        'm_message' => '1',
        'm_icon' => '<i class="fad fa-comment-exclamation source" aria-hidden="true"></i>',
        'm_profile' => array(5967,4755,4593),
    ),
);

//IDEA NOTES 1X SOURCE REQUIRED:
$config['n___7551'] = array(7545,4983,10573,13865,12896);
$config['e___7551'] = array(
    7545 => array(
        'm_title' => 'AWARDS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-file-certificate source" aria-hidden="true"></i>',
        'm_profile' => array(11089,13530,6159,13421,13298,12197,11018,4485,7551,4593),
    ),
    4983 => array(
        'm_title' => 'CITATIONS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link source rotate90" aria-hidden="true"></i>',
        'm_profile' => array(12359,7551,13550,13543,13407,4485,12141,10939,12450,12228,4593,10593),
    ),
    10573 => array(
        'm_title' => 'MY IDEAS MAP',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sitemap idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,11035,11054,4527,10984,11018,4485,4593,7551),
    ),
    13865 => array(
        'm_title' => 'REQUIREMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shield-check source"></i>',
        'm_profile' => array(4527,11035,13421,13298,7551,13530,4593,11018,11089,4485),
    ),
    12896 => array(
        'm_title' => 'SAVED IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bookmark discover"></i>',
        'm_profile' => array(13877,12701,11035,11018,11089,12677,4485,7551,4755,4593),
    ),
);

//DISCOVERY REMOVED:
$config['n___6150'] = array(7757,6155);
$config['e___6150'] = array(
    7757 => array(
        'm_title' => 'DISCOVER BOOKMARK REMOVED AUTO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-bookmark discover" aria-hidden="true"></i>',
        'm_profile' => array(6150,4755,4593),
    ),
    6155 => array(
        'm_title' => 'REMOVE DISCOVERY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-times"></i>',
        'm_profile' => array(11035,6150,4593,4755),
    ),
);

//IDEA TYPE MEET REQUIREMENT:
$config['n___7309'] = array(6914,6907);
$config['e___7309'] = array(
    6914 => array(
        'm_title' => 'ALL REQUIREMENTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cubes idea" aria-hidden="true"></i>',
        'm_profile' => array(12330,12700,7486,7485,6140,7585,6192,7309,6997),
    ),
    6907 => array(
        'm_title' => 'ANY REQUIREMENT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cube idea" aria-hidden="true"></i>',
        'm_profile' => array(12330,12883,12700,7486,7485,6140,7585,6193,7309,6997),
    ),
);

//MENCH PLUGINS:
$config['n___6287'] = array(12739,12722,12733,7264,7276,13369,4356,7261,12731,12734,7260,7263,11049,7259,12138,7275,12735,7712,7274,4527,7277,13602,12710,12709,12729,12888,7267,13790,12736,12732,13881,7268,7269,12730,12738,12712,12737,7278,12967,7279,4367,12114,12569);
$config['e___6287'] = array(
    12739 => array(
        'm_title' => 'DISCOVER ANALYZE URLS',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(6287),
    ),
    12722 => array(
        'm_title' => 'DISCOVER REVIEW JSON',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(12741,6287),
    ),
    12733 => array(
        'm_title' => 'DISCOVER REVIEW JSON',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-code discover"></i>',
        'm_profile' => array(11047,12741,12701,6287),
    ),
    7264 => array(
        'm_title' => 'IDEA BIRDS EYE MARKS',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sitemap idea"></i>',
        'm_profile' => array(11047,6287),
    ),
    7276 => array(
        'm_title' => 'IDEA CACHED SOURCES',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sync idea mench-spin" aria-hidden="true"></i>',
        'm_profile' => array(6287,12741,11047,7286,7274),
    ),
    13369 => array(
        'm_title' => 'IDEA COVER',
        'm_message' => 'Quickly screenshot an idea cover using Font Awesome icons',
        'm_icon' => '<i class="fas fa-book idea"></i>',
        'm_profile' => array(6287),
    ),
    4356 => array(
        'm_title' => 'IDEA DURATION (SECONDS)',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-stopwatch idea" aria-hidden="true"></i>',
        'm_profile' => array(10986,13295,6287,12741,11047,7274,6404,12112,10650,6201),
    ),
    7261 => array(
        'm_title' => 'IDEA LIST DUPLICATES',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(6287),
    ),
    12731 => array(
        'm_title' => 'IDEA LIST INVALID TITLES',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(6287),
    ),
    12734 => array(
        'm_title' => 'IDEA LIST NEXT/PREVIOUS CROSSOVERS',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(6287),
    ),
    7260 => array(
        'm_title' => 'IDEA LIST ORPHANED',
        'm_message' => '',
        'm_icon' => '👶',
        'm_profile' => array(6287),
    ),
    7263 => array(
        'm_title' => 'IDEA MARKS LIST ALL',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(6287),
    ),
    11049 => array(
        'm_title' => 'IDEA REVIEW JSON',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-lambda idea" aria-hidden="true"></i>',
        'm_profile' => array(12741,6287,11047),
    ),
    7259 => array(
        'm_title' => 'IDEA SEARCH & REPLACE',
        'm_message' => '',
        'm_icon' => '🔍',
        'm_profile' => array(6287),
    ),
    12138 => array(
        'm_title' => 'IDEA STATUS FEATURED',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-search idea" aria-hidden="true"></i>',
        'm_profile' => array(6287,4527,10891),
    ),
    7275 => array(
        'm_title' => 'IDEA SYNC COMMON BASE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sync idea mench-spin"></i>',
        'm_profile' => array(6287,12741,7286,7274),
    ),
    12735 => array(
        'm_title' => 'IDEA SYNC/FIX SOURCES',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(6287),
    ),
    7712 => array(
        'm_title' => 'IDEA TYPE SELECT NEXT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-question-circle idea"></i>',
        'm_profile' => array(11054,13028,6287,10893,4527),
    ),
    7274 => array(
        'm_title' => 'MENCH CRON JOBS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clock mench-spin" aria-hidden="true"></i>',
        'm_profile' => array(6404,6287,6403,12999),
    ),
    4527 => array(
        'm_title' => 'MENCH MEMORY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-memory" aria-hidden="true"></i>',
        'm_profile' => array(4755,6403,12741,6287,4527),
    ),
    7277 => array(
        'm_title' => 'METADATA CLEAN VARIABLES',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(4527,6287,12741,7287,7274),
    ),
    13602 => array(
        'm_title' => 'MONTHLY COIN STATS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-coins"></i>',
        'm_profile' => array(6287),
    ),
    12710 => array(
        'm_title' => 'MY SESSION VARIABLES',
        'm_message' => '',
        'm_icon' => '👤',
        'm_profile' => array(12741,6287),
    ),
    12709 => array(
        'm_title' => 'PHP INFO',
        'm_message' => '',
        'm_icon' => 'ℹ️',
        'm_profile' => array(12741,6287),
    ),
    12729 => array(
        'm_title' => 'PLATFORM COIN STATS',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(6287),
    ),
    12888 => array(
        'm_title' => 'SOURCE EXPLORE EXPERTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-shield-check source"></i>',
        'm_profile' => array(12741,6287,12887),
    ),
    7267 => array(
        'm_title' => 'SOURCE ICON SEARCH',
        'm_message' => '',
        'm_icon' => '🔍',
        'm_profile' => array(12887,6287),
    ),
    13790 => array(
        'm_title' => 'SOURCE IDEA SPREADSHEET',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-table idea"></i>',
        'm_profile' => array(12741,6287),
    ),
    12736 => array(
        'm_title' => 'SOURCE/IDEAS SYNC & FIX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-user source" aria-hidden="true"></i>',
        'm_profile' => array(6287),
    ),
    12732 => array(
        'm_title' => 'SOURCE/IDEA SYNC STATUSES',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(12741,6287),
    ),
    13881 => array(
        'm_title' => 'SOURCE IMPORTER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-download source"></i>',
        'm_profile' => array(6287),
    ),
    7268 => array(
        'm_title' => 'SOURCE LIST DUPLICATES',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(6287),
    ),
    7269 => array(
        'm_title' => 'SOURCE LIST ORPHANED',
        'm_message' => '',
        'm_icon' => '👶',
        'm_profile' => array(6287),
    ),
    12730 => array(
        'm_title' => 'SOURCE NAME SEARCH & REPLACE',
        'm_message' => '',
        'm_icon' => '🔍',
        'm_profile' => array(6287),
    ),
    12738 => array(
        'm_title' => 'SOURCE RANDOM AVATARS',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(6287),
    ),
    12712 => array(
        'm_title' => 'SOURCE REVIEW JSON',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-lambda source" aria-hidden="true"></i>',
        'm_profile' => array(12887,12741,6287),
    ),
    12737 => array(
        'm_title' => 'SOURCE SYNC & FIX LINKS',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(6287),
    ),
    7278 => array(
        'm_title' => 'SYNC GEPHI INDEX',
        'm_message' => '',
        'm_icon' => '',
        'm_profile' => array(6287,12741,7287,7274),
    ),
    12967 => array(
        'm_title' => 'SYNC ICON MODES',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sync source mench-spin" aria-hidden="true"></i>',
        'm_profile' => array(6287,12741,4758,7274),
    ),
    7279 => array(
        'm_title' => 'SYNC SEARCH INDEX',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-search"></i>',
        'm_profile' => array(6287,12741,12887,11047,3323,7287),
    ),
    4367 => array(
        'm_title' => 'TRANSACTION ID',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-atlas" aria-hidden="true"></i>',
        'm_profile' => array(6287,4341),
    ),
    12114 => array(
        'm_title' => 'WEEKLY PAYOUTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sync discover"></i>',
        'm_profile' => array(6225,13450,4593,6287,12741,7274,7569),
    ),
    12569 => array(
        'm_title' => 'WEIGHT ALGORITHM',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-weight"></i>',
        'm_profile' => array(6287,12741,7274),
    ),
);

//DISCOVER STATUS INCOMPLETE:
$config['n___7364'] = array(6175);
$config['e___7364'] = array(
    6175 => array(
        'm_title' => 'TRANSACTIONS DRAFT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-spinner-third fa-spin discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,7364,6186,7360),
    ),
);

//DISCOVER STATUS ACTIVE:
$config['n___7360'] = array(6175,12399,6176);
$config['e___7360'] = array(
    6175 => array(
        'm_title' => 'DRAFT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-spinner-third fa-spin discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,7364,6186,7360),
    ),
    12399 => array(
        'm_title' => 'FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,7360,7359,6186),
    ),
    6176 => array(
        'm_title' => 'PUBLISH',
        'm_message' => '',
        'm_icon' => '<i class="far fa-globe discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,6186,7360,7359),
    ),
);

//DISCOVER STATUS PUBLIC:
$config['n___7359'] = array(12399,6176);
$config['e___7359'] = array(
    12399 => array(
        'm_title' => 'FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,7360,7359,6186),
    ),
    6176 => array(
        'm_title' => 'PUBLISH',
        'm_message' => '',
        'm_icon' => '<i class="far fa-globe discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,6186,7360,7359),
    ),
);

//SOURCE STATUS ACTIVE:
$config['n___7358'] = array(6180,12563,6181);
$config['e___7358'] = array(
    6180 => array(
        'm_title' => 'DRAFT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-spinner fa-spin source" aria-hidden="true"></i>',
        'm_profile' => array(6175,10654,6177,7358),
    ),
    12563 => array(
        'm_title' => 'FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star source" aria-hidden="true"></i>',
        'm_profile' => array(12399,4527,12575,10654,7358,7357,6177),
    ),
    6181 => array(
        'm_title' => 'PUBLISH',
        'm_message' => '',
        'm_icon' => '<i class="far fa-globe source" aria-hidden="true"></i>',
        'm_profile' => array(6176,10654,6177,7358,7357),
    ),
);

//SOURCE STATUS PUBLIC:
$config['n___7357'] = array(12563,6181);
$config['e___7357'] = array(
    12563 => array(
        'm_title' => 'FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star source" aria-hidden="true"></i>',
        'm_profile' => array(12399,4527,12575,10654,7358,7357,6177),
    ),
    6181 => array(
        'm_title' => 'PUBLISH',
        'm_message' => '',
        'm_icon' => '<i class="far fa-globe source" aria-hidden="true"></i>',
        'm_profile' => array(6176,10654,6177,7358,7357),
    ),
);

//IDEA STATUS ACTIVE:
$config['n___7356'] = array(6183,12137,6184);
$config['e___7356'] = array(
    6183 => array(
        'm_title' => 'DRAF',
        'm_message' => '',
        'm_icon' => '<i class="far fa-spinner fa-spin idea" aria-hidden="true"></i>',
        'm_profile' => array(6175,4737,10648,7356),
    ),
    12137 => array(
        'm_title' => 'FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star idea" aria-hidden="true"></i>',
        'm_profile' => array(12399,13813,6404,11035,13420,10648,12138,7356,7355,4737),
    ),
    6184 => array(
        'm_title' => 'PUBLISH',
        'm_message' => '',
        'm_icon' => '<i class="far fa-globe idea" aria-hidden="true"></i>',
        'm_profile' => array(6176,10648,4737,7355,7356),
    ),
);

//IDEA STATUS PUBLIC:
$config['n___7355'] = array(12137,6184);
$config['e___7355'] = array(
    12137 => array(
        'm_title' => 'FEATURE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-star idea" aria-hidden="true"></i>',
        'm_profile' => array(12399,13813,6404,11035,13420,10648,12138,7356,7355,4737),
    ),
    6184 => array(
        'm_title' => 'PUBLISH',
        'm_message' => '',
        'm_icon' => '<i class="far fa-globe idea" aria-hidden="true"></i>',
        'm_profile' => array(6176,10648,4737,7355,7356),
    ),
);

//UNFINISHED:
$config['n___6146'] = array(7492);
$config['e___6146'] = array(
    7492 => array(
        'm_title' => 'DISCOVER TERMINATE',
        'm_message' => 'Logged when users arrive at a locked intent that has no public OR parents or no children, which means there is no way to unlock it.',
        'm_icon' => '<i class="fas fa-times-octagon discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,4755,4593,6146),
    ),
);

//DISCOVERIES:
$config['n___6255'] = array(6157,7489,12117,4559,6144,7485,7486,6997);
$config['e___6255'] = array(
    6157 => array(
        'm_title' => 'ANSWER ONE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4593,6255,4755),
    ),
    7489 => array(
        'm_title' => 'ANSWER SOME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4755,6255,4593),
    ),
    12117 => array(
        'm_title' => 'FILE UPLOAD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cloud-upload discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,4593,4755,6255),
    ),
    4559 => array(
        'm_title' => 'READ MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="far fa-circle discover"></i>',
        'm_profile' => array(12229,12227,12141,6255,4755,4593),
    ),
    6144 => array(
        'm_title' => 'TEXT REPLY',
        'm_message' => '',
        'm_icon' => '<i class="far fa-keyboard discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,6255,4755,4593),
    ),
    7485 => array(
        'm_title' => 'UNLOCK ANSWER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12334,12336,7489,6157,12327,12229,12227,12141,4593,4755,6255),
    ),
    7486 => array(
        'm_title' => 'UNLOCK CHILDREN',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12327,12229,12227,12141,4755,4593,6255),
    ),
    6997 => array(
        'm_title' => 'UNLOCK CONDITION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(6140,12327,12229,12227,12141,4229,6255,4593,4755),
    ),
);

//PRIVATE INFORMATION:
$config['n___4755'] = array(13038,6415,12336,12334,4554,7757,5967,12197,6559,6560,6556,6578,4556,6149,4283,6969,4275,7610,4555,12360,4266,4267,4282,6563,4570,7702,4577,4549,4551,4550,4557,4278,4279,4268,4460,4547,4287,4548,7492,4552,6140,6224,12328,7578,4553,3288,4235,12773,12453,10681,12450,13494,13493,4527,11054,7561,7564,7560,3286,4783,4755,7495,6155,12896,7563,13492,6132,13412,6157,7489,4246,12117,7504,4559,6144,7485,7486,6997,13042,13415,12906,12489,13377);
$config['e___4755'] = array(
    13038 => array(
        'm_title' => '80 DISCOVERIES PER MONTH FOR FREE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-hand-holding-heart discover"></i>',
        'm_profile' => array(13889,13518,4755),
    ),
    6415 => array(
        'm_title' => 'DELETE ALL DISCOVERIES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-trash-alt discover" aria-hidden="true"></i>',
        'm_profile' => array(11035,4593,4755),
    ),
    12336 => array(
        'm_title' => 'DISCOVER ANSWER ONE LINK',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(7704,4755,4593,12326,12227),
    ),
    12334 => array(
        'm_title' => 'DISCOVER ANSWER SOME LINK',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square discover" aria-hidden="true"></i>',
        'm_profile' => array(7704,4755,4593,12326,12227),
    ),
    4554 => array(
        'm_title' => 'DISCOVER AUDIO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-volume-up"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    7757 => array(
        'm_title' => 'DISCOVER BOOKMARK REMOVED AUTO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-bookmark discover" aria-hidden="true"></i>',
        'm_profile' => array(6150,4755,4593),
    ),
    5967 => array(
        'm_title' => 'DISCOVER CARBON COPY',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-envelope-open discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,7569,4755,4593),
    ),
    12197 => array(
        'm_title' => 'DISCOVER CERTIFY USER PROFILE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-user-plus discover" aria-hidden="true"></i>',
        'm_profile' => array(4755,4593),
    ),
    6559 => array(
        'm_title' => 'DISCOVER COMMANDED NEXT',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-wand-magic"></i>',
        'm_profile' => array(4755,4593,6554),
    ),
    6560 => array(
        'm_title' => 'DISCOVER COMMANDED SKIP',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-wand-magic"></i>',
        'm_profile' => array(4755,4593,6554),
    ),
    6556 => array(
        'm_title' => 'DISCOVER COMMANDED STATS',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-wand-magic"></i>',
        'm_profile' => array(4755,4593,6554),
    ),
    6578 => array(
        'm_title' => 'DISCOVER COMMANDED STOP',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-wand-magic"></i>',
        'm_profile' => array(4755,4593,6554),
    ),
    4556 => array(
        'm_title' => 'DISCOVER FILE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-file-pdf"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    6149 => array(
        'm_title' => 'DISCOVER IDEA CONSIDERED',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-search-plus discover" aria-hidden="true"></i>',
        'm_profile' => array(6153,4755,4593),
    ),
    4283 => array(
        'm_title' => 'DISCOVER IDEA LISTED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(6153,4755,4593),
    ),
    6969 => array(
        'm_title' => 'DISCOVER IDEA RECOMMENDED',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-megaphone"></i>',
        'm_profile' => array(4593,4755,6153),
    ),
    4275 => array(
        'm_title' => 'DISCOVER IDEA SEARCH',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-search"></i>',
        'm_profile' => array(6554,4593,4755),
    ),
    7610 => array(
        'm_title' => 'DISCOVER IDEA VIEW',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(4755,4593),
    ),
    4555 => array(
        'm_title' => 'DISCOVER IMAGE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-image"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    12360 => array(
        'm_title' => 'DISCOVER MASS CONTENT UPDATE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-pen discover"></i>',
        'm_profile' => array(6771,4593,4755),
    ),
    4266 => array(
        'm_title' => 'DISCOVER MESSENGER OPT-IN',
        'm_message' => '',
        'm_icon' => '<i class="discover fab fa-facebook-messenger"></i>',
        'm_profile' => array(4755,4593),
    ),
    4267 => array(
        'm_title' => 'DISCOVER MESSENGER REFERRAL',
        'm_message' => '',
        'm_icon' => '<i class="discover fab fa-facebook-messenger"></i>',
        'm_profile' => array(4755,4593),
    ),
    4282 => array(
        'm_title' => 'DISCOVER OPENED PROFILE',
        'm_message' => '',
        'm_icon' => '<i class="discover fas fa-circle"></i>',
        'm_profile' => array(4755,4593),
    ),
    6563 => array(
        'm_title' => 'DISCOVER QUICK REPLY',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-check"></i>',
        'm_profile' => array(10593,4280,4593,4755),
    ),
    4570 => array(
        'm_title' => 'DISCOVER RECEIVED',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-envelope-open discover" aria-hidden="true"></i>',
        'm_profile' => array(10683,10593,7569,4755,4593),
    ),
    7702 => array(
        'm_title' => 'DISCOVER RECEIVED IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-envelope-open discover" aria-hidden="true"></i>',
        'm_profile' => array(4593,10593,4755,7569),
    ),
    4577 => array(
        'm_title' => 'DISCOVER SENT ACCESS',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-user-plus"></i>',
        'm_profile' => array(7654,4755,4593),
    ),
    4549 => array(
        'm_title' => 'DISCOVER SENT AUDIO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-volume-up"></i>',
        'm_profile' => array(10593,7653,4755,4593),
    ),
    4551 => array(
        'm_title' => 'DISCOVER SENT FILE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-file-pdf"></i>',
        'm_profile' => array(10593,7653,4755,4593),
    ),
    4550 => array(
        'm_title' => 'DISCOVER SENT IMAGE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-image"></i>',
        'm_profile' => array(10593,7653,4755,4593),
    ),
    4557 => array(
        'm_title' => 'DISCOVER SENT LOCATION',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-location-circle"></i>',
        'm_profile' => array(7653,4755,4593),
    ),
    4278 => array(
        'm_title' => 'DISCOVER SENT MESSENGER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(7654,4755,4593),
    ),
    4279 => array(
        'm_title' => 'DISCOVER SENT MESSENGER RECEIVED',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-cloud-download"></i>',
        'm_profile' => array(7654,4755,4593),
    ),
    4268 => array(
        'm_title' => 'DISCOVER SENT POSTBACK',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-user-tag"></i>',
        'm_profile' => array(7654,4755,4593),
    ),
    4460 => array(
        'm_title' => 'DISCOVER SENT QUICK REPLY',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-check"></i>',
        'm_profile' => array(7653,4755,4593),
    ),
    4547 => array(
        'm_title' => 'DISCOVER SENT TEXT',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-align-left"></i>',
        'm_profile' => array(7653,4755,4593),
    ),
    4287 => array(
        'm_title' => 'DISCOVER SENT UNKNOWN MESSAGE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-comment-exclamation"></i>',
        'm_profile' => array(7654,4755,4593),
    ),
    4548 => array(
        'm_title' => 'DISCOVER SENT VIDEO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-video"></i>',
        'm_profile' => array(10593,7653,4755,4593),
    ),
    7492 => array(
        'm_title' => 'DISCOVER TERMINATE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-times-octagon discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,4755,4593,6146),
    ),
    4552 => array(
        'm_title' => 'DISCOVER TEXT',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-align-left"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    6140 => array(
        'm_title' => 'DISCOVER UNLOCK CONDITION LINK',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-lock-open discover" aria-hidden="true"></i>',
        'm_profile' => array(12326,12227,6410,4229,4755,4593),
    ),
    6224 => array(
        'm_title' => 'DISCOVER UPDATE ACCOUNT',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sync discover"></i>',
        'm_profile' => array(4755,4593),
    ),
    12328 => array(
        'm_title' => 'DISCOVER UPDATE COMPLETION',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sync discover"></i>',
        'm_profile' => array(4755,4593,6153),
    ),
    7578 => array(
        'm_title' => 'DISCOVER UPDATE PASSWORD',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-key"></i>',
        'm_profile' => array(6153,4755,4593),
    ),
    4553 => array(
        'm_title' => 'DISCOVER VIDEO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-video"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    3288 => array(
        'm_title' => 'EMAIL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-envelope-open" aria-hidden="true"></i>',
        'm_profile' => array(13861,13531,6404,4269,6225,4755),
    ),
    4235 => array(
        'm_title' => 'GET STARTED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chevron-right discover" aria-hidden="true"></i>',
        'm_profile' => array(12994,11035,12969,12227,4593,5967,4755),
    ),
    12773 => array(
        'm_title' => 'IDEA APPEND CONTENT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-plus-circle idea"></i>',
        'm_profile' => array(5967,4755,4593),
    ),
    12453 => array(
        'm_title' => 'IDEA FEATURE REQUEST',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-megaphone idea"></i>',
        'm_profile' => array(4755,4593,5967),
    ),
    10681 => array(
        'm_title' => 'IDEA SORT AUTO',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sort idea" aria-hidden="true"></i>',
        'm_profile' => array(4755,4593),
    ),
    12450 => array(
        'm_title' => 'IDEA SOURCE REQUEST',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-plus-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4593,4755,5967),
    ),
    13494 => array(
        'm_title' => 'LARGE FONT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-h1"></i>',
        'm_profile' => array(4755,13491),
    ),
    13493 => array(
        'm_title' => 'MEDIUM FONT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-h2"></i>',
        'm_profile' => array(13889,4755,13491),
    ),
    4527 => array(
        'm_title' => 'MENCH MEMORY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-memory" aria-hidden="true"></i>',
        'm_profile' => array(4755,6403,12741,6287,4527),
    ),
    11054 => array(
        'm_title' => 'MENCH MEMORY JAVASCRIPT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-memory" aria-hidden="true"></i>',
        'm_profile' => array(4755,6403,4527),
    ),
    7561 => array(
        'm_title' => 'MINER SIGNIN NO IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sign-in discover"></i>',
        'm_profile' => array(12351,4755,4593),
    ),
    7564 => array(
        'm_title' => 'MINER SIGNIN SUCCESS',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sign-in source"></i>',
        'm_profile' => array(12351,4755,4593),
    ),
    7560 => array(
        'm_title' => 'MINER SIGNIN WITH IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sign-in idea" aria-hidden="true"></i>',
        'm_profile' => array(12351,4755,4593),
    ),
    3286 => array(
        'm_title' => 'PASSWORD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-key" aria-hidden="true"></i>',
        'm_profile' => array(13014,4269,6225,7578,4755),
    ),
    4783 => array(
        'm_title' => 'PHONE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-phone"></i>',
        'm_profile' => array(13861,4755),
    ),
    4755 => array(
        'm_title' => 'PRIVATE INFORMATION',
        'm_message' => '',
        'm_icon' => '<i class="far fa-eye-slash" aria-hidden="true"></i>',
        'm_profile' => array(11035,12701,4755,6771,4527),
    ),
    7495 => array(
        'm_title' => 'RECOMMENDED DISCOVERY',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-megaphone discover" aria-hidden="true"></i>',
        'm_profile' => array(12969,12227,4593,4755),
    ),
    6155 => array(
        'm_title' => 'REMOVE DISCOVERY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-times"></i>',
        'm_profile' => array(11035,6150,4593,4755),
    ),
    12896 => array(
        'm_title' => 'SAVED IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bookmark discover"></i>',
        'm_profile' => array(13877,12701,11035,11018,11089,12677,4485,7551,4755,4593),
    ),
    7563 => array(
        'm_title' => 'SIGN-IN MAGIC LINK',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-envelope-open discover" aria-hidden="true"></i>',
        'm_profile' => array(4755,7569,4593),
    ),
    13492 => array(
        'm_title' => 'SMALL FONT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-h3"></i>',
        'm_profile' => array(4755,13491),
    ),
    6132 => array(
        'm_title' => 'SORT DISCOVERY',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-arrows"></i>',
        'm_profile' => array(11035,13413,6153,4593,4755),
    ),
    13412 => array(
        'm_title' => 'SORT IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-arrows"></i>',
        'm_profile' => array(11035,13413,4755,4593),
    ),
    6157 => array(
        'm_title' => 'SOURCE ANSWER ONE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4593,6255,4755),
    ),
    7489 => array(
        'm_title' => 'SOURCE ANSWER SOME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4755,6255,4593),
    ),
    4246 => array(
        'm_title' => 'SOURCE BUG REPORTS',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-bug source" aria-hidden="true"></i>',
        'm_profile' => array(5967,4755,4593),
    ),
    12117 => array(
        'm_title' => 'SOURCE FILE UPLOAD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cloud-upload discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,4593,4755,6255),
    ),
    7504 => array(
        'm_title' => 'SOURCE PENDING MODERATION',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-comment-exclamation source" aria-hidden="true"></i>',
        'm_profile' => array(5967,4755,4593),
    ),
    4559 => array(
        'm_title' => 'SOURCE READ MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="far fa-circle discover"></i>',
        'm_profile' => array(12229,12227,12141,6255,4755,4593),
    ),
    6144 => array(
        'm_title' => 'SOURCE TEXT REPLY',
        'm_message' => '',
        'm_icon' => '<i class="far fa-keyboard discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,6255,4755,4593),
    ),
    7485 => array(
        'm_title' => 'SOURCE UNLOCK ANSWER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12334,12336,7489,6157,12327,12229,12227,12141,4593,4755,6255),
    ),
    7486 => array(
        'm_title' => 'SOURCE UNLOCK CHILDREN',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12327,12229,12227,12141,4755,4593,6255),
    ),
    6997 => array(
        'm_title' => 'SOURCE UNLOCK CONDITION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(6140,12327,12229,12227,12141,4229,6255,4593,4755),
    ),
    13042 => array(
        'm_title' => 'UNLIMITED DISCOVERIES $8 PER MONTH',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-hand-holding-usd discover"></i>',
        'm_profile' => array(13518,4755),
    ),
    13415 => array(
        'm_title' => 'UNLINK IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-unlink"></i>',
        'm_profile' => array(11035,4755,4593),
    ),
    12906 => array(
        'm_title' => 'UNSAVE DISCOVERY',
        'm_message' => '',
        'm_icon' => '<i class="far fa-bookmark discover"></i>',
        'm_profile' => array(12896,11035,4755,4593),
    ),
    12489 => array(
        'm_title' => 'VIEWED LEADERBOARD',
        'm_message' => '',
        'm_icon' => '<i class="far fa-eye source"></i>',
        'm_profile' => array(4755,4593),
    ),
    13377 => array(
        'm_title' => 'WEEKLY NEWSLETTER',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-envelope-open discover" aria-hidden="true"></i>',
        'm_profile' => array(4755,7569),
    ),
);

//TRANSACTION TYPE:
$config['n___4593'] = array(7545,4983,12419,6415,13579,12129,12336,12334,4554,7757,5967,12197,6559,6560,6556,6578,10683,4556,6149,4283,6969,4275,7610,4555,12360,10690,4266,4267,4282,6563,4570,7702,4577,4549,4551,4550,4557,4278,4279,4268,4460,4547,4287,4548,7492,4552,6140,6224,12328,7578,4553,13481,4235,12773,4250,6182,12453,4229,4228,10663,10664,12611,12612,12591,12592,6226,10676,10679,10677,10681,10675,12450,10662,10648,10650,10644,10651,4993,4601,13894,4231,7561,7564,7560,10573,5001,10625,5943,12318,5865,4999,4998,5000,11956,5981,13441,5982,5003,7495,6155,13865,12896,7563,6132,13412,13007,6157,7489,4246,4251,12117,10653,4259,10657,4257,4261,4260,4319,7657,4230,10656,4255,4318,10659,4256,4258,12827,10646,7504,4559,6178,13006,10654,6144,5007,7485,7486,6997,4994,13415,10686,10673,12906,12489,12114);
$config['e___4593'] = array(
    7545 => array(
        'm_title' => 'AWARDS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-file-certificate source" aria-hidden="true"></i>',
        'm_profile' => array(11089,13530,6159,13421,13298,12197,11018,4485,7551,4593),
    ),
    4983 => array(
        'm_title' => 'CITATIONS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-link source rotate90" aria-hidden="true"></i>',
        'm_profile' => array(12359,7551,13550,13543,13407,4485,12141,10939,12450,12228,4593,10593),
    ),
    12419 => array(
        'm_title' => 'COMMENTS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-comments discover" aria-hidden="true"></i>',
        'm_profile' => array(12701,11035,13871,12994,11089,13530,11018,12359,5967,10593,4986,4593,4485),
    ),
    6415 => array(
        'm_title' => 'DELETE ALL DISCOVERIES',
        'm_message' => 'Removes all player read coins so everything is reset to 0% again.',
        'm_icon' => '<i class="fas fa-trash-alt discover" aria-hidden="true"></i>',
        'm_profile' => array(11035,4593,4755),
    ),
    13579 => array(
        'm_title' => 'DELETE NOTE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-trash-alt" aria-hidden="true"></i>',
        'm_profile' => array(4593,11035),
    ),
    12129 => array(
        'm_title' => 'DISCOVER ANSWER ERASED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-times-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(6153,4593),
    ),
    12336 => array(
        'm_title' => 'DISCOVER ANSWER ONE LINK',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(7704,4755,4593,12326,12227),
    ),
    12334 => array(
        'm_title' => 'DISCOVER ANSWER SOME LINK',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square discover" aria-hidden="true"></i>',
        'm_profile' => array(7704,4755,4593,12326,12227),
    ),
    4554 => array(
        'm_title' => 'DISCOVER AUDIO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-volume-up"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    7757 => array(
        'm_title' => 'DISCOVER BOOKMARK REMOVED AUTO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-bookmark discover" aria-hidden="true"></i>',
        'm_profile' => array(6150,4755,4593),
    ),
    5967 => array(
        'm_title' => 'DISCOVER CARBON COPY',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-envelope-open discover" aria-hidden="true"></i>',
        'm_profile' => array(4527,7569,4755,4593),
    ),
    12197 => array(
        'm_title' => 'DISCOVER CERTIFY USER PROFILE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-user-plus discover" aria-hidden="true"></i>',
        'm_profile' => array(4755,4593),
    ),
    6559 => array(
        'm_title' => 'DISCOVER COMMANDED NEXT',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-wand-magic"></i>',
        'm_profile' => array(4755,4593,6554),
    ),
    6560 => array(
        'm_title' => 'DISCOVER COMMANDED SKIP',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-wand-magic"></i>',
        'm_profile' => array(4755,4593,6554),
    ),
    6556 => array(
        'm_title' => 'DISCOVER COMMANDED STATS',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-wand-magic"></i>',
        'm_profile' => array(4755,4593,6554),
    ),
    6578 => array(
        'm_title' => 'DISCOVER COMMANDED STOP',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-wand-magic"></i>',
        'm_profile' => array(4755,4593,6554),
    ),
    10683 => array(
        'm_title' => 'DISCOVER EMAIL',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-envelope-open discover" aria-hidden="true"></i>',
        'm_profile' => array(6153,4593,7654),
    ),
    4556 => array(
        'm_title' => 'DISCOVER FILE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-file-pdf"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    6149 => array(
        'm_title' => 'DISCOVER IDEA CONSIDERED',
        'm_message' => 'When a student chooses to review a given intention from the intentions they have searched or have been recommended after selecting GET STARTED from a mench.com intent landing page.',
        'm_icon' => '<i class="fad fa-search-plus discover" aria-hidden="true"></i>',
        'm_profile' => array(6153,4755,4593),
    ),
    4283 => array(
        'm_title' => 'DISCOVER IDEA LISTED',
        'm_message' => 'Once a student has added an Intention to their Action Plan, this link will be logged every time they access that Action Plan and view its intentions.',
        'm_icon' => '<i class="fas fa-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(6153,4755,4593),
    ),
    6969 => array(
        'm_title' => 'DISCOVER IDEA RECOMMENDED',
        'm_message' => 'Logged every time an intention is recommended to a user by Mench',
        'm_icon' => '<i class="discover fad fa-megaphone"></i>',
        'm_profile' => array(4593,4755,6153),
    ),
    4275 => array(
        'm_title' => 'DISCOVER IDEA SEARCH',
        'm_message' => 'When students invokes the [I want to] command and search for a new intention that they would like to add to their Action Plan.',
        'm_icon' => '<i class="discover fad fa-search"></i>',
        'm_profile' => array(6554,4593,4755),
    ),
    7610 => array(
        'm_title' => 'DISCOVER IDEA VIEW',
        'm_message' => 'When a user viewes the public intent landing page.',
        'm_icon' => '<i class="fas fa-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(4755,4593),
    ),
    4555 => array(
        'm_title' => 'DISCOVER IMAGE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-image"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    12360 => array(
        'm_title' => 'DISCOVER MASS CONTENT UPDATE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-pen discover"></i>',
        'm_profile' => array(6771,4593,4755),
    ),
    10690 => array(
        'm_title' => 'DISCOVER MEDIA UPLOADED',
        'm_message' => 'When a file added by the user is synced to the CDN',
        'm_icon' => '<i class="discover fad fa-upload"></i>',
        'm_profile' => array(4593,6153),
    ),
    4266 => array(
        'm_title' => 'DISCOVER MESSENGER OPT-IN',
        'm_message' => '',
        'm_icon' => '<i class="discover fab fa-facebook-messenger"></i>',
        'm_profile' => array(4755,4593),
    ),
    4267 => array(
        'm_title' => 'DISCOVER MESSENGER REFERRAL',
        'm_message' => '',
        'm_icon' => '<i class="discover fab fa-facebook-messenger"></i>',
        'm_profile' => array(4755,4593),
    ),
    4282 => array(
        'm_title' => 'DISCOVER OPENED PROFILE',
        'm_message' => '',
        'm_icon' => '<i class="discover fas fa-circle"></i>',
        'm_profile' => array(4755,4593),
    ),
    6563 => array(
        'm_title' => 'DISCOVER QUICK REPLY',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-check"></i>',
        'm_profile' => array(10593,4280,4593,4755),
    ),
    4570 => array(
        'm_title' => 'DISCOVER RECEIVED',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-envelope-open discover" aria-hidden="true"></i>',
        'm_profile' => array(10683,10593,7569,4755,4593),
    ),
    7702 => array(
        'm_title' => 'DISCOVER RECEIVED IDEA',
        'm_message' => 'Emails sent to intent subscribers who are looking for updates on an intent.',
        'm_icon' => '<i class="fad fa-envelope-open discover" aria-hidden="true"></i>',
        'm_profile' => array(4593,10593,4755,7569),
    ),
    4577 => array(
        'm_title' => 'DISCOVER SENT ACCESS',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-user-plus"></i>',
        'm_profile' => array(7654,4755,4593),
    ),
    4549 => array(
        'm_title' => 'DISCOVER SENT AUDIO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-volume-up"></i>',
        'm_profile' => array(10593,7653,4755,4593),
    ),
    4551 => array(
        'm_title' => 'DISCOVER SENT FILE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-file-pdf"></i>',
        'm_profile' => array(10593,7653,4755,4593),
    ),
    4550 => array(
        'm_title' => 'DISCOVER SENT IMAGE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-image"></i>',
        'm_profile' => array(10593,7653,4755,4593),
    ),
    4557 => array(
        'm_title' => 'DISCOVER SENT LOCATION',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-location-circle"></i>',
        'm_profile' => array(7653,4755,4593),
    ),
    4278 => array(
        'm_title' => 'DISCOVER SENT MESSENGER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(7654,4755,4593),
    ),
    4279 => array(
        'm_title' => 'DISCOVER SENT MESSENGER RECEIVED',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-cloud-download"></i>',
        'm_profile' => array(7654,4755,4593),
    ),
    4268 => array(
        'm_title' => 'DISCOVER SENT POSTBACK',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-user-tag"></i>',
        'm_profile' => array(7654,4755,4593),
    ),
    4460 => array(
        'm_title' => 'DISCOVER SENT QUICK REPLY',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-check"></i>',
        'm_profile' => array(7653,4755,4593),
    ),
    4547 => array(
        'm_title' => 'DISCOVER SENT TEXT',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-align-left"></i>',
        'm_profile' => array(7653,4755,4593),
    ),
    4287 => array(
        'm_title' => 'DISCOVER SENT UNKNOWN MESSAGE',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-comment-exclamation"></i>',
        'm_profile' => array(7654,4755,4593),
    ),
    4548 => array(
        'm_title' => 'DISCOVER SENT VIDEO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-video"></i>',
        'm_profile' => array(10593,7653,4755,4593),
    ),
    7492 => array(
        'm_title' => 'DISCOVER TERMINATE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-times-octagon discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,4755,4593,6146),
    ),
    4552 => array(
        'm_title' => 'DISCOVER TEXT',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-align-left"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    6140 => array(
        'm_title' => 'DISCOVER UNLOCK CONDITION LINK',
        'm_message' => 'Created when the student responses to OR branches meets the right % points to unlock the pathway to a conditional intent link.',
        'm_icon' => '<i class="fad fa-lock-open discover" aria-hidden="true"></i>',
        'm_profile' => array(12326,12227,6410,4229,4755,4593),
    ),
    6224 => array(
        'm_title' => 'DISCOVER UPDATE ACCOUNT',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sync discover"></i>',
        'm_profile' => array(4755,4593),
    ),
    12328 => array(
        'm_title' => 'DISCOVER UPDATE COMPLETION',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sync discover"></i>',
        'm_profile' => array(4755,4593,6153),
    ),
    7578 => array(
        'm_title' => 'DISCOVER UPDATE PASSWORD',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-key"></i>',
        'm_profile' => array(6153,4755,4593),
    ),
    4553 => array(
        'm_title' => 'DISCOVER VIDEO',
        'm_message' => '',
        'm_icon' => '<i class="discover fad fa-video"></i>',
        'm_profile' => array(10593,4755,4593,4280),
    ),
    13481 => array(
        'm_title' => 'FINANCIAL TRANSACTION',
        'm_message' => 'Records a financial transaction into (+) or out of (-) Mench',
        'm_icon' => '<i class="fas fa-money-bill-wave-alt"></i>',
        'm_profile' => array(4593),
    ),
    4235 => array(
        'm_title' => 'GET STARTED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-chevron-right discover" aria-hidden="true"></i>',
        'm_profile' => array(12994,11035,12969,12227,4593,5967,4755),
    ),
    12773 => array(
        'm_title' => 'IDEA APPEND CONTENT',
        'm_message' => '',
        'm_icon' => '<i class="far fa-plus-circle idea"></i>',
        'm_profile' => array(5967,4755,4593),
    ),
    4250 => array(
        'm_title' => 'IDEA CREATED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(13480,5967,12400,12149,10593,4593),
    ),
    6182 => array(
        'm_title' => 'IDEA DELETE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-trash-alt idea" aria-hidden="true"></i>',
        'm_profile' => array(6173,12400,4593,4737),
    ),
    12453 => array(
        'm_title' => 'IDEA FEATURE REQUEST',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-megaphone idea"></i>',
        'm_profile' => array(4755,4593,5967),
    ),
    4229 => array(
        'm_title' => 'IDEA LINK CONDITIONAL',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-question-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12842,4486,4527,6410,6283,4593),
    ),
    4228 => array(
        'm_title' => 'IDEA LINK FIXED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(12840,4486,6410,4593),
    ),
    10663 => array(
        'm_title' => 'IDEA LINK UPDATE MARKS',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-coin idea" aria-hidden="true"></i>',
        'm_profile' => array(4228,4593),
    ),
    10664 => array(
        'm_title' => 'IDEA LINK UPDATE SCORE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-bolt idea" aria-hidden="true"></i>',
        'm_profile' => array(4593,4229),
    ),
    12611 => array(
        'm_title' => 'IDEA LIST EDITOR PREVIOUS IDEA ADD',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-layer-plus idea" aria-hidden="true"></i>',
        'm_profile' => array(4593,12589),
    ),
    12612 => array(
        'm_title' => 'IDEA LIST EDITOR PREVIOUS IDEA REMOVE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-layer-minus idea" aria-hidden="true"></i>',
        'm_profile' => array(4593,12589),
    ),
    12591 => array(
        'm_title' => 'IDEA LIST EDITOR SOURCE ADD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-plus-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4593,12589),
    ),
    12592 => array(
        'm_title' => 'IDEA LIST EDITOR SOURCE REMOVE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-minus-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4593,12589),
    ),
    6226 => array(
        'm_title' => 'IDEA MASS UPDATE STATUS',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sliders-h idea" aria-hidden="true"></i>',
        'm_profile' => array(4593),
    ),
    10676 => array(
        'm_title' => 'IDEA NOTES SORTED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sort idea" aria-hidden="true"></i>',
        'm_profile' => array(4593),
    ),
    10679 => array(
        'm_title' => 'IDEA NOTES UPDATE CONTENT',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-comment-plus idea" aria-hidden="true"></i>',
        'm_profile' => array(4593,10593),
    ),
    10677 => array(
        'm_title' => 'IDEA NOTES UPDATE STATUS',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sliders-h idea" aria-hidden="true"></i>',
        'm_profile' => array(4593),
    ),
    10681 => array(
        'm_title' => 'IDEA SORT AUTO',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sort idea" aria-hidden="true"></i>',
        'm_profile' => array(4755,4593),
    ),
    10675 => array(
        'm_title' => 'IDEA SORT MANUAL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sort idea" aria-hidden="true"></i>',
        'm_profile' => array(4593),
    ),
    12450 => array(
        'm_title' => 'IDEA SOURCE REQUEST',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-plus-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4593,4755,5967),
    ),
    10662 => array(
        'm_title' => 'IDEA UPDATE LINK',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-hashtag idea" aria-hidden="true"></i>',
        'm_profile' => array(4593),
    ),
    10648 => array(
        'm_title' => 'IDEA UPDATE STATUS',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sliders-h idea" aria-hidden="true"></i>',
        'm_profile' => array(12400,4593),
    ),
    10650 => array(
        'm_title' => 'IDEA UPDATE TIME',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-clock idea" aria-hidden="true"></i>',
        'm_profile' => array(4593),
    ),
    10644 => array(
        'm_title' => 'IDEA UPDATE TITLE',
        'm_message' => 'Logged when trainers update the intent outcome',
        'm_icon' => '<i class="fad fa-bullseye-arrow idea" aria-hidden="true"></i>',
        'm_profile' => array(4593,10593),
    ),
    10651 => array(
        'm_title' => 'IDEA UPDATE TYPE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-shapes idea" aria-hidden="true"></i>',
        'm_profile' => array(4593),
    ),
    4993 => array(
        'm_title' => 'IDEA VIEWED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle idea" aria-hidden="true"></i>',
        'm_profile' => array(4593),
    ),
    4601 => array(
        'm_title' => 'KEYWORDS',
        'm_message' => '',
        'm_icon' => '<i class="far fa-tags idea" aria-hidden="true"></i>',
        'm_profile' => array(12700,13023,13530,11018,4485,10593,4593),
    ),
    13894 => array(
        'm_title' => 'MENCH SOCIAL LINKS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-share"></i>',
        'm_profile' => array(6403,4593,4527),
    ),
    4231 => array(
        'm_title' => 'MESSAGES',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-comment-lines idea" aria-hidden="true"></i>',
        'm_profile' => array(12994,13550,13294,12359,4485,10593,4986,4603,4593),
    ),
    7561 => array(
        'm_title' => 'MINER SIGNIN NO IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sign-in discover"></i>',
        'm_profile' => array(12351,4755,4593),
    ),
    7564 => array(
        'm_title' => 'MINER SIGNIN SUCCESS',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sign-in source"></i>',
        'm_profile' => array(12351,4755,4593),
    ),
    7560 => array(
        'm_title' => 'MINER SIGNIN WITH IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sign-in idea" aria-hidden="true"></i>',
        'm_profile' => array(12351,4755,4593),
    ),
    10573 => array(
        'm_title' => 'MY IDEAS MAP',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sitemap idea" aria-hidden="true"></i>',
        'm_profile' => array(13920,13530,11035,11054,4527,10984,11018,4485,4593,7551),
    ),
    5001 => array(
        'm_title' => 'PORTFOLIO EDITOR CONTENT REPLACE',
        'm_message' => '',
        'm_icon' => '<i class="source fad fa-sticky-note"></i>',
        'm_profile' => array(4997,4593),
    ),
    10625 => array(
        'm_title' => 'PORTFOLIO EDITOR ICON REPLACE',
        'm_message' => '',
        'm_icon' => '<i class="source fad fa-user-circle"></i>',
        'm_profile' => array(4593,4997),
    ),
    5943 => array(
        'm_title' => 'PORTFOLIO EDITOR ICON SET FOR ALL',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-user-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4997,4593),
    ),
    12318 => array(
        'm_title' => 'PORTFOLIO EDITOR ICON SET IF NONE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-user-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4593,4997),
    ),
    5865 => array(
        'm_title' => 'PORTFOLIO EDITOR LINK STATUS',
        'm_message' => '',
        'm_icon' => '<i class="source fad fa-sliders-h" aria-hidden="true"></i>',
        'm_profile' => array(4997,4593),
    ),
    4999 => array(
        'm_title' => 'PORTFOLIO EDITOR NAME POSTFIX',
        'm_message' => '',
        'm_icon' => '<i class="source fad fa-fingerprint"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    4998 => array(
        'm_title' => 'PORTFOLIO EDITOR NAME PREFIX',
        'm_message' => '',
        'm_icon' => '<i class="source fad fa-fingerprint"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    5000 => array(
        'm_title' => 'PORTFOLIO EDITOR NAME REPLACE',
        'm_message' => '',
        'm_icon' => '<i class="source fad fa-fingerprint"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    11956 => array(
        'm_title' => 'PORTFOLIO EDITOR PROFILE COPY @ IF HAS @',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-layer-plus source"></i>',
        'm_profile' => array(12577,4593,4997),
    ),
    5981 => array(
        'm_title' => 'PORTFOLIO EDITOR PROFILE COPY @ IF MISSING',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-layer-plus source"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    13441 => array(
        'm_title' => 'PORTFOLIO EDITOR PROFILE MOVE @ IF MISSING',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-arrow-right source"></i>',
        'm_profile' => array(12577,4593,4997),
    ),
    5982 => array(
        'm_title' => 'PORTFOLIO EDITOR PROFILE REMOVE @ IF THERE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-layer-minus source"></i>',
        'm_profile' => array(12577,4997,4593),
    ),
    5003 => array(
        'm_title' => 'PORTFOLIO EDITOR STATUS',
        'm_message' => '',
        'm_icon' => '<i class="source fad fa-sliders-h"></i>',
        'm_profile' => array(4997,4593),
    ),
    7495 => array(
        'm_title' => 'RECOMMENDED DISCOVERY',
        'm_message' => 'Intention recommended by Mench and added to Action Plan to enable the user to complete their intention',
        'm_icon' => '<i class="fad fa-megaphone discover" aria-hidden="true"></i>',
        'm_profile' => array(12969,12227,4593,4755),
    ),
    6155 => array(
        'm_title' => 'REMOVE DISCOVERY',
        'm_message' => 'Student prematurely removed an intention from their Action Plan without accomplishing it.',
        'm_icon' => '<i class="fas fa-times"></i>',
        'm_profile' => array(11035,6150,4593,4755),
    ),
    13865 => array(
        'm_title' => 'REQUIREMENTS',
        'm_message' => 'If set for an idea, then the user must meet all prerequisites to be able to complete the idea.',
        'm_icon' => '<i class="fas fa-shield-check source"></i>',
        'm_profile' => array(4527,11035,13421,13298,7551,13530,4593,11018,11089,4485),
    ),
    12896 => array(
        'm_title' => 'SAVED IDEAS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-bookmark discover"></i>',
        'm_profile' => array(13877,12701,11035,11018,11089,12677,4485,7551,4755,4593),
    ),
    7563 => array(
        'm_title' => 'SIGN-IN MAGIC LINK',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-envelope-open discover" aria-hidden="true"></i>',
        'm_profile' => array(4755,7569,4593),
    ),
    6132 => array(
        'm_title' => 'SORT DISCOVERY',
        'm_message' => 'Student re-prioritized their top-level intentions to focus on intentions that currently matter the most.',
        'm_icon' => '<i class="fas fa-arrows"></i>',
        'm_profile' => array(11035,13413,6153,4593,4755),
    ),
    13412 => array(
        'm_title' => 'SORT IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-arrows"></i>',
        'm_profile' => array(11035,13413,4755,4593),
    ),
    13007 => array(
        'm_title' => 'SORT SOURCES ALPHABETICALLY',
        'm_message' => 'Removes the sort value of all portfolio sources which would sort it by alphabetical value',
        'm_icon' => '<i class="fas fa-sort-alpha-down source"></i>',
        'm_profile' => array(13422,13429,11035,4593),
    ),
    6157 => array(
        'm_title' => 'SOURCE ANSWER ONE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-circle discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4593,6255,4755),
    ),
    7489 => array(
        'm_title' => 'SOURCE ANSWER SOME',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-check-square discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,7704,4755,6255,4593),
    ),
    4246 => array(
        'm_title' => 'SOURCE BUG REPORTS',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-bug source" aria-hidden="true"></i>',
        'm_profile' => array(5967,4755,4593),
    ),
    4251 => array(
        'm_title' => 'SOURCE CREATED',
        'm_message' => 'Logged when a new entity is created.',
        'm_icon' => '<i class="fas fa-circle source" aria-hidden="true"></i>',
        'm_profile' => array(13548,12401,12149,12141,10593,4593),
    ),
    12117 => array(
        'm_title' => 'SOURCE FILE UPLOAD',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-cloud-upload discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,4593,4755,6255),
    ),
    10653 => array(
        'm_title' => 'SOURCE ICON UPDATE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-user-circle source"></i>',
        'm_profile' => array(4593),
    ),
    4259 => array(
        'm_title' => 'SOURCE LINK AUDIO',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-volume-up source"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    10657 => array(
        'm_title' => 'SOURCE LINK CONTENT UPDATE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-comment-plus source" aria-hidden="true"></i>',
        'm_profile' => array(10593,4593),
    ),
    4257 => array(
        'm_title' => 'SOURCE LINK EMBED URL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4758,12822,12605,12524,12403,4593,4592,4537),
    ),
    4261 => array(
        'm_title' => 'SOURCE LINK FILE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-file-pdf source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    4260 => array(
        'm_title' => 'SOURCE LINK IMAGE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-image source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,6198,11059,10593,4593,4592,4537),
    ),
    4319 => array(
        'm_title' => 'SOURCE LINK INTEGER',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sort-numeric-down source" aria-hidden="true"></i>',
        'm_profile' => array(4593,4592),
    ),
    7657 => array(
        'm_title' => 'SOURCE LINK PERCENT',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-divide source" aria-hidden="true"></i>',
        'm_profile' => array(4593,4592),
    ),
    4230 => array(
        'm_title' => 'SOURCE LINK RAW',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-link rotate90 source"></i>',
        'm_profile' => array(4593,4592),
    ),
    10656 => array(
        'm_title' => 'SOURCE LINK STATUS UPDATE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sliders-h source"></i>',
        'm_profile' => array(4593),
    ),
    4255 => array(
        'm_title' => 'SOURCE LINK TEXT',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-align-left source"></i>',
        'm_profile' => array(13899,12822,10593,4593,4592),
    ),
    4318 => array(
        'm_title' => 'SOURCE LINK TIME',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-clock source"></i>',
        'm_profile' => array(4593,4592),
    ),
    10659 => array(
        'm_title' => 'SOURCE LINK TYPE UPDATE',
        'm_message' => 'Iterations happens automatically based on link content',
        'm_icon' => '<i class="fad fa-plug source"></i>',
        'm_profile' => array(4593),
    ),
    4256 => array(
        'm_title' => 'SOURCE LINK URL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-external-link source" aria-hidden="true"></i>',
        'm_profile' => array(13433,12822,11080,4593,4592,4537),
    ),
    4258 => array(
        'm_title' => 'SOURCE LINK VIDEO',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-play-circle source" aria-hidden="true"></i>',
        'm_profile' => array(12822,12605,12524,11080,11059,10593,4593,4592,4537),
    ),
    12827 => array(
        'm_title' => 'SOURCE LINK WORD',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-font source"></i>',
        'm_profile' => array(4593,4592),
    ),
    10646 => array(
        'm_title' => 'SOURCE NAME UPDATE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-fingerprint source"></i>',
        'm_profile' => array(10593,4593),
    ),
    7504 => array(
        'm_title' => 'SOURCE PENDING MODERATION',
        'm_message' => 'Certain links that match an unknown behavior would require an admin to review and ensure it\'s all good',
        'm_icon' => '<i class="fad fa-comment-exclamation source" aria-hidden="true"></i>',
        'm_profile' => array(5967,4755,4593),
    ),
    4559 => array(
        'm_title' => 'SOURCE READ MESSAGES',
        'm_message' => 'Logged when a student receives the messages of an AND intent that does not have any completion requirements.',
        'm_icon' => '<i class="far fa-circle discover"></i>',
        'm_profile' => array(12229,12227,12141,6255,4755,4593),
    ),
    6178 => array(
        'm_title' => 'SOURCES DELETE',
        'm_message' => '',
        'm_icon' => '<i class="far fa-trash-alt source" aria-hidden="true"></i>',
        'm_profile' => array(6173,4593,12401,6177),
    ),
    13006 => array(
        'm_title' => 'SOURCE SORT MANUAL',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sort source" aria-hidden="true"></i>',
        'm_profile' => array(4593),
    ),
    10654 => array(
        'm_title' => 'SOURCE STATUS UPDATE',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-sliders-h source" aria-hidden="true"></i>',
        'm_profile' => array(12401,4593),
    ),
    6144 => array(
        'm_title' => 'SOURCE TEXT REPLY',
        'm_message' => 'Logged when a student submits the requirements (text, video, etc...) of an AND intent which could not be completed by simply receiving messages.',
        'm_icon' => '<i class="far fa-keyboard discover" aria-hidden="true"></i>',
        'm_profile' => array(12229,12227,12141,6255,4755,4593),
    ),
    5007 => array(
        'm_title' => 'SOURCE TOGGLE SUPERPOWERS',
        'm_message' => '',
        'm_icon' => '<i class="fad fa-bolt source" aria-hidden="true"></i>',
        'm_profile' => array(4593),
    ),
    7485 => array(
        'm_title' => 'SOURCE UNLOCK ANSWER',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12334,12336,7489,6157,12327,12229,12227,12141,4593,4755,6255),
    ),
    7486 => array(
        'm_title' => 'SOURCE UNLOCK CHILDREN',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(12327,12229,12227,12141,4755,4593,6255),
    ),
    6997 => array(
        'm_title' => 'SOURCE UNLOCK CONDITION',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-clipboard-check discover" aria-hidden="true"></i>',
        'm_profile' => array(6140,12327,12229,12227,12141,4229,6255,4593,4755),
    ),
    4994 => array(
        'm_title' => 'SOURCE VIEWED',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-circle source" aria-hidden="true"></i>',
        'm_profile' => array(4593),
    ),
    13415 => array(
        'm_title' => 'UNLINK IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-unlink"></i>',
        'm_profile' => array(11035,4755,4593),
    ),
    10686 => array(
        'm_title' => 'UNLINK IDEA',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-times idea" aria-hidden="true"></i>',
        'm_profile' => array(4593),
    ),
    10673 => array(
        'm_title' => 'UNLINK SOURCE',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-unlink"></i>',
        'm_profile' => array(11035,4593),
    ),
    12906 => array(
        'm_title' => 'UNSAVE DISCOVERY',
        'm_message' => '',
        'm_icon' => '<i class="far fa-bookmark discover"></i>',
        'm_profile' => array(12896,11035,4755,4593),
    ),
    12489 => array(
        'm_title' => 'VIEWED LEADERBOARD',
        'm_message' => '',
        'm_icon' => '<i class="far fa-eye source"></i>',
        'm_profile' => array(4755,4593),
    ),
    12114 => array(
        'm_title' => 'WEEKLY PAYOUTS',
        'm_message' => '',
        'm_icon' => '<i class="fas fa-sync discover"></i>',
        'm_profile' => array(6225,13450,4593,6287,12741,7274,7569),
    ),
);


$config['n___all'] = array(12574,6225,4997,13425,11089,4527,13429,11035,12703,12590,13518,13038,13042,13889,4755,13899,4255,12822,10593,4593,4592,6173,6182,6178,12400,4737,12401,6177,6176,6184,6181,10648,7355,7356,10654,7358,7357,6175,6183,6180,12399,12137,12563,13813,6404,13420,12138,12575,13894,13019,13017,13018,13021,13016,13015,13418,3301,7146,3300,2793,3303,13904,3302,3308,13417,13890,13491,13884,11054,6204,13450,13885,13886,13493,13865,13875,13876,13783,13784,13785,13786,13776,6255,13920,13530,12677,11018,12467,12228,13672,10876,13023,6403,1326,6198,4260,13577,13578,12605,12524,11080,11059,4537,13571,6197,6186,4372,13572,12701,12766,5003,6160,6206,13296,13025,12112,12232,10646,5000,4998,4999,10653,5943,10625,10677,10656,5865,7304,4341,7578,10679,10657,5001,6194,11081,10659,13550,4983,4231,12359,7551,13543,13407,4485,12141,10939,12450,12994,13294,4986,4603,13548,4251,12149,6170,6162,13292,4739,6159,6169,6161,4735,13544,13629,4356,13533,13535,13536,13534,13531,12889,12890,3288,3099,13026,12891,13861,4269,10573,12419,4601,7545,12589,11047,12273,12274,10984,13421,13298,13871,5967,12700,12197,13403,11040,13300,13291,12675,12761,2997,4446,3005,13339,3147,13218,13350,3192,2998,12523,3000,13207,12211,6677,13022,12330,7585,6192,4559,13289,12991,13877,13563,13494,13492,13480,4250,13479,12500,12079,12823,7277,6172,7276,6212,11049,6201,6207,3323,13442,4486,13408,10662,6208,6168,6283,12885,6228,4430,13897,4364,6103,7735,12887,12707,13413,6132,13412,6153,4358,6402,13295,10651,6226,10663,6410,10664,4736,4235,10644,12969,12227,13304,13024,6683,7637,12955,6144,12117,7495,6684,7231,12883,12336,12129,7712,6193,6157,12884,12334,7489,12193,12888,7267,12712,7279,12741,6287,7287,6907,7486,7485,6140,7309,6997,12842,4229,12840,4228,4259,4257,4261,4256,4258,4758,12403,13433,12733,12722,7275,12710,12709,13790,12732,7278,12967,12114,12569,7286,7274,10986,10650,7569,12687,12691,12694,12695,12896,7291,13907,12489,4367,6405,12699,12591,12592,12611,12612,3286,13014,12577,11956,5981,13441,5982,6293,12446,13752,13757,13822,12447,13338,13810,13465,6914,12327,12229,12326,7704,12279,12280,12281,12282,12286,12287,12288,12308,12309,12310,12234,12233,10965,12236,12235,10979,12295,12294,12293,12296,12297,12298,12300,12301,12299,12237,12238,10978,12314,12315,12316,12240,12239,10963,12241,12242,12207,12244,12243,10966,12245,12246,10976,12248,12247,10962,12249,12250,10975,12252,12251,10982,12253,12254,10970,12302,12303,12304,12256,12255,10972,12306,12307,12305,12257,12258,10969,12312,12313,12311,12260,12259,10960,12277,12276,12278,12439,12262,10981,12264,12263,10968,12265,12266,10974,12290,12291,12292,12268,12267,12206,12269,12270,10958,12285,12284,12283,12272,12271,12231,10956,7492,6146,12318,12928,12930,13898,12289,10957,5007,13007,13914,13915,13912,13542,11019,13913,13919,13502,13670,13495,13562,13524,12906,6415,13579,13401,13573,13574,13689,11068,6155,13415,10673,13671,7256,13692,13693,13908,13909,13911,13422,11020,12030,13297,13566,11065,6771,6150,12678,13357,13354,12673,13402,12706,13758,7360,7359,7364,10686,4429,4368,4371,4369,4366,10692,6202,12904,11057,11056,12331,12427,12176,11064,13005,11079,11060,13556,13545,11066,13803,12088,13206,11986,12565,12568,7524,6768,12999,13446,2738,10891,13028,10893,2,1,12041,7312,12572,7264,4319,7657,4230,4318,12827,13030,12024,7254,13487,10888,10624,13299,12144,12841,10889,10602,12676,12829,12821,12573,4554,4556,4555,6563,4570,7702,4549,4551,4550,4548,4552,4553,4280,10683,7653,4362,4370,13006,10676,10675,13029,13158,13172,13167,13166,13153,13174,13171,13152,13162,13156,13157,13155,13173,13170,13164,13160,13168,13165,13169,13159,13163,13161,13154,3290,13151,12773,12453,4246,7504,7757,12739,13369,7261,12731,12734,7260,7263,7259,12735,13602,12729,12736,13881,7268,7269,12730,12738,12737,6559,6560,6556,6578,6149,4283,6969,4275,7610,12360,4266,4267,4282,4577,4557,4278,4279,4268,4460,4547,4287,6224,12328,10681,7561,7564,7560,4783,7563,13377,6554,7654,12351,10690,13481,4993,4994); //694