...
foreach ($this->config->item('players___33292') as $playerid1 => $m1) {
    ...
    foreach ($this->config->item('players___' . $playerid1) as $playerid2 => $m2) {
        ...
        echo '<div class="main__title large_title"><a href="'.view_app_link(4341).'?linkplayertype='.join(',',( $is_link ? $this->config->item('players___' . $playerid2) : array(( $playerid2==12273 ? 4250 : 4251 )) )).'&linkvoid=0" class="card_count_' . $playerid2 . '"><i class="fas fa-yin-yang fa-spin"></i></a></div>';
