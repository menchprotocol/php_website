...
foreach ($this->config->item('players___33292') as $playerid1 => $m1) {
    ...
    if($playerid1==1309754){
        echo '<div class="mid-text-line compact-midline"><span>' . $m1['m__cover'] . ' <a href="'.view_app_link(4341).'?linkvoid=1" class="grey card_count_' . $playerid1 . '"><i class="fas fa-yin-yang fa-spin"></i></a> ' . $m1['m__title'] . '</span></div>';
        //Void Links
        continue;
    ...
    foreach ($this->config->item('players___' . $playerid1) as $playerid2 => $m2) {
        ...
        echo '<div class="main__title large_title"><a href="'.view_app_link(4341).'?linkplayertype='.join(',',( $is_link ? $this->config->item('playerids___' . $playerid2) : array(( $playerid2==12273 ? 4250 : 4251 )) )).'&linkvoid=0" class="card_count_' . $playerid2 . '"><i class="fas fa-yin-yang fa-spin"></i></a></div>';
