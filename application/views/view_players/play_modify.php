...
<div class="container">
    ...
    if(isset($session_en['en_id']) && $session_en['en_id']==$entity['en_id']){
        ...
        echo '<a href="/players/myaccount" class="btn btn-sm btn4536 btn-five inline-block" data-toggle="tooltip" data-placement="bottom" title="'.$en_all_11035[6225]['m_desc'].'">'.$en_all_11035[6225]['m_icon'].' '.$en_all_11035[6225]['m_name'].'</a>';

        echo '<a href="/players/signout" class="btn btn-sm btn4536 btn-five inline-block" data-toggle="tooltip" data-placement="bottom" title="'.$en_all_11035[7291]['m_name'].'">'.$en_all_11035[7291]['m_icon'].'</a>';
    ...
    foreach ($this->config->item('en_all_11088') as $en_id => $m){
        ...
        foreach ($this->config->item('en_all_'.$en_id) as $en_id2 => $m2){
            ...
            if($en_id2==11030){
                ...
                $fetch_11030 = $this->EXCHANGE_model->ln_fetch(array(
                ...
                $fetch_11029 = $this->EXCHANGE_model->ln_fetch(array(
                    'ln_type_entity_id IN (' . join(',', $this->config->item('en_ids_4592')) . ')' => null, //Entity-to-Entity Links
                    'ln_status_entity_id IN (' . join(',', $this->config->item('en_ids_7360')) . ')' => null, //Link Statuses Active
                    'en_status_entity_id IN (' . join(',', $this->config->item('en_ids_7358')) . ')' => null, //Entity Statuses Active
                    'ln_parent_entity_id' => $entity['en_id'],
            ...
            <div class="algolia_search_pad hidden"><span>Search players, create a new player or paste a URL...</span></div>
                ...
                $child_en_filters = $this->EXCHANGE_model->ln_fetch(array(
                    'ln_parent_entity_id' => $entity['en_id'],
                    'ln_type_entity_id IN (' . join(',', $this->config->item('en_ids_4592')) . ')' => null, //Entity-to-Entity Links
                    'ln_status_entity_id IN (' . join(',', $this->config->item('en_ids_7360')) . ')' => null, //Link Statuses Active
                    'en_status_entity_id IN (' . join(',', $this->config->item('en_ids_7358')) . ')' => null, //Entity Statuses Active
                ...
                //COUNT ONLY
                $item_counters = $this->EXCHANGE_model->ln_fetch($blog_note_filters, array(), 1, 0, array(), 'COUNT(ln_id) as totals');
                $counter = $item_counters[0]['totals'];


                //SHOW LASTEST 100
                $this_tab .= '<div id="list-messages" class="list-group">';
                foreach ($this->EXCHANGE_model->ln_fetch($blog_note_filters, array('in_child')) as $blog_note) {
