    ...
    foreach ($this->Chains->read(array(
        ...
        foreach ($hashtag_settings['query_string_filtered'] as $x) {
            ...
            foreach ($children as $down_or) {
                ...
                $html_message .= 'https://' . get_domain('m__message', $x['handleid'], $i['chainhandledomain']) . view_memory(42903, 33286) . $down_or['hashtagstring'] . (!count($discoveries) ? '?handlehandle=' . $x['handlehandle'] . '&time=' . time() . '&hash=' . view_hash(eventreminder . phptime() . $x['handlehandle']) : '') . "\n\n";
