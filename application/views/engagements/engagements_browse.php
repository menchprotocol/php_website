...
$engagement_filters = array(
    'e_parent_c_id' => 'All Engagement Types',
    'e_id' => 'Engagement ID',
    'e_u_id' => 'Entity ID',
    'e_ur_id' => 'Entity Link ID',
    'e_x_id' => 'Entity URL ID',
    'e_i_id' => 'Message ID',
    'e_child_c_id' => 'Intent ID',
    ...
    if(isset($_GET[$key])){
        if($key=='e_u_id'){
            ...
            if(substr_count($_GET[$key],',')>0){
                ...
                $match_columns['(e_child_u_id IN ('.$_GET[$key].') OR e_parent_u_id IN ('.$_GET[$key].'))'] = null;
            } elseif(intval($_GET[$key])>0) {
                $match_columns['(e_child_u_id = '.$_GET[$key].' OR e_parent_u_id = '.$_GET[$key].')'] = null;
            }
        } else {
            if(substr_count($_GET[$key],',')>0){
                //This is multiple IDs:
