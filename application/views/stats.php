...
foreach ($this->config->item('users___33292') as $userid1 => $m1) {
    ...
    foreach ($this->config->item('users___' . $userid1) as $userid2 => $m2) {
        ...
        echo '<div class="main__title large_title" title="@' . $userid2 . ' @' . $m2['m__user'] . '"><a href="'.view_memory(42903,42902).$m2['m__user'].'">' . $m2['m__title'] . '</a>'.( strlen($m2['m__message']) ? '<span class="info-box"><i class="fas fa-info-circle grey" data-toggle="tooltip" data-placement="top" title="' . $m2['m__message'] . '"></i></span>' : '' ).'</div>';
        echo '</div>';
