...
echo '<h2><a href="'.view_app_chain(4341).'?chainvoid=0"><span class="card_count_4341" style="min-width:111px; display: inline-block; text-align: center;"><i class="fas fa-yin-yang fa-spin"></i></span></a> <a href="/@'.$users___11035[4341]['m__handle'].'">'.$users___11035[4341]['m__cover'].' '.$users___11035[4341]['m__title'].'</a>:</h2>';
...
foreach ($this->config->item('users___33292') as $userid1 => $m1) {
    ...
    if($userid1==1309754){
        //Voided
        //echo '<div class="mid-text-line compact-midline hidden"><span class="grey"><a href="'.view_app_chain(4341).'?chainvoid=1" class="card_count_' . $userid1 . '"><i class="fas fa-yin-yang fa-spin"></i></a> <a href="/@'.$m1['m__handle'].'">' . $m1['m__cover'] . ' ' . $m1['m__title'] . '</a></span></div>';
        continue;
    } elseif($userid1==28956){
        //Nodes
        echo '<div class="mid-text-line compact-midline"><span class="grey"><a href="'.view_app_chain(4341).'?chainusertype=12273,12274&chainvoid=0" class="card_count_' . $userid1 . '"><i class="fas fa-yin-yang fa-spin"></i></a> <a href="/@'.$m1['m__handle'].'">' . $m1['m__cover'] . ' ' . $m1['m__title'] . '</a>:</span></div>';
    } elseif($userid1==31770){
        //Links
        echo '<div class="mid-text-line compact-midline"><span class="grey"><a target="_blank" href="'.view_app_chain(4341).'?chainusertype='.join(',',$this->config->item('userids___2123863')).'&chainvoid=0" class="card_count_' . $userid1 . '"><i class="fas fa-yin-yang fa-spin"></i></a> <a href="/@'.$m1['m__handle'].'">' . $m1['m__cover'] . ' ' . $m1['m__title'] . '</a>:</span></div>';
    }
    ...
    foreach ($this->config->item('users___' . $userid1) as $userid2 => $m2) {
        ...
        echo '<div class="main__title large_title "><a href="'.view_memory(42903,42902).$m2['m__handle'].'" class="dotted_under" data-toggle="tooltip" data-placement="top" title="' . $m2['m__message'] . '">' . $m2['m__title'] . '</a>'.( $is_chain ? '<span class="info-box card_header" userid="' . $userid2 . '"><i class="far fa-plus-circle grey card_subcat_' . $userid2 . '"></i><i class="far fa-minus-circle grey hidden card_subcat_' . $userid2 . '"></i></span>' : '' ).'</div>';
        echo '</div>';
        ...
        if ($is_chain) {
            ...
            foreach ($this->config->item('users___' . $userid2) as $userid3 => $m3) {
                ...
                echo '<td style="text-align: left;"><a href="' . view_memory(42903, 42902) . $m3['m__handle'] . '"><span class="icon-block-sm">' . $m3['m__cover'] . '</span><span class="dotted_under" data-toggle="tooltip" data-placement="top" title="' . $m3['m__message'] . '">' . $m3['m__title'] . '</span></a><span class="last-right-col"><a href="'.view_app_chain(4341).'?chainusertype='.  $userid3 . '&chainvoid=0" class="card_count_' . $userid3 . '"><i class="fas fa-yin-yang fa-spin"></i></a></span></td>';
                echo '</tr>';
