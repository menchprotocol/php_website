...
foreach ($this->config->item('users___33292') as $userid1 => $m1) {
    ...
    foreach ($this->config->item('users___' . $userid1) as $userid2 => $m2) {
        ...
        if ($is_chain) {
            echo '<div class="medium_cover card_header">' . $m2['m__cover'] . '</div>';
        } else {
        ...
        if ($is_chain) {
            ...
            foreach ($this->config->item('users___' . $userid2) as $userid3 => $m3) {
                ...
                echo '<td style="text-align: left;"><a href="' . view_memory(42903, 42902) . $m3['m__user'] . '"><span class="icon-block-sm">' . $m3['m__cover'] . '</span><span class="dotted_under" data-toggle="tooltip" data-placement="top" title="' . $m3['m__message'] . '">' . $m3['m__title'] . '</span></a><span class="last-right-col"><a href="'.view_app_chain(4341).'?chainusertype='.  $userid3 . '&chainvoid=0" class="card_count_' . $userid3 . '"><i class="fas fa-yin-yang fa-spin"></i></a></span></td>';
                echo '</tr>';
