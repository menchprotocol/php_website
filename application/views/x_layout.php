...
if(!$top_i__id){
    ...
    foreach($this->config->item('e___13289') as $e__id => $m2) {
        ...
        if($e__id==13877 && $top_i__id && !$in_my_discoveries){
            ...
            if(!$control_btn){
                //NEXT
                $control_btn = '<a class="controller-nav round-btn go-next" href="#" onclick="go_next(\''.$go_next_url.'\')">'.$m2['m__cover'].'</a><span class="nav-title css__title">'.( count($x_completes) ? 'Go Next' : $m2['m__title'] ).'<div class="extra_progress hideIfEmpty"></div></span>';
            }
