...
if(!$in_my_x && !$i_drip_mode){
    ...
    foreach($this->config->item('e___13289') as $e__id => $m) {
        ...
        if($e__id==13877 && $in_my_x && !$in_my_discoveries){
            ...
            //COMMENT
            $control_btn = '<a class="controller-nav round-btn" href="#comment" onclick="load_comments()">'.$m['m__icon'].'</a><span class="nav-counter css__title en-type-counter-12419">'.( count($comments) ? count($comments) : '' ).'</span><span class="nav-title css__title">'.$m['m__title'].'</span>';
