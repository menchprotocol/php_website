...
if(!$top_i__id){
    ...
    foreach($this->config->item('e___13289') as $e__id => $m2) {
        ...
        if($e__id==13877 && $top_i__id && !$in_my_discoveries){
            ...
            if($i_focus['i__type'] == 26560 && !count($x_completes)){
                ...
                if($x__source>0 && count($total_dues)){
                    ...
                    if ($detected_x_type['status'] && in_array($detected_x_type['x__type'], $this->config->item('n___26661'))){
                        ...
                        $control_btn .= '<input type="submit" class="round-btn" style="width:auto !important;" name="pay_now" id="pay_now" value="PAY '.$total_dues[0]['x__message'].'">';
            ...
            if(!$control_btn){
                //NEXT
                $control_btn = '<a class="controller-nav round-btn go-next" href="javascript:void(0);" onclick="go_next(\''.$go_next_url.'\')">'.$m2['m__cover'].'</a><span class="nav-title css__title">'.( count($x_completes) ? 'Go Next' : $m2['m__title'] ).'<div class="extra_progress hideIfEmpty"></div></span>';
            }
