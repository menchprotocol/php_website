...
if($top_i__id) {
    ...
    if ($is_or_idea) {
        ...
        if(isset($_GET['process_pay']) && !count($x_completes)){
            ...
            echo '<td class="table-btn first_btn" style="width:66% !important;"><span class="total_ui css__title">'.($unit_total*$min_allowed).'</span> '.$currency_parts[0].'<b>*</b></td>';
            echo '</tr>';

            echo '</table>';


            echo '<div class="sub_note"><b>*</b>';
            if(!count($this->X_model->fetch(array(
                'x__status IN (' . join(',', $this->config->item('n___7359')) . ')' => null, //PUBLIC
                'x__type IN (' . join(',', $this->config->item('n___13550')) . ')' => null, //SOURCE IDEAS
                'x__right' => $i['i__id'],
