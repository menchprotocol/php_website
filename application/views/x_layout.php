...
foreach($this->X_model->fetch(array(
    ...
    'x__right' => $i_focus['i__id'],
    'x__up > 0' => null,
), array('x__up'), 0, 0, array('e__title' => 'ASC')) as $x){
    ...
    echo '<div style="padding-top: 10px; padding-left: 41px;">'. ( $x['e__id']==30976 /* Hack: Location loads with Google Maps */ ? '<a href="https://www.google.com/maps/search/'.urlencode($x['x__message']).'" target="_blank" style="text-decoration:underline;">'.$x['x__message'].'</a>' : $x['x__message'] ) . '</div>';
    echo '</div>';
