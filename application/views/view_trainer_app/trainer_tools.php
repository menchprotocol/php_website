...
$moderation_tools = array(
    '/trainer_app/trainer_tools/link_words_stats' => 'Link Words Stats',
    '/trainer_app/trainer_tools/in_replace_outcomes' => 'Intent Search/Replace Outcomes',
    '/trainer_app/trainer_tools/in_invalid_outcomes' => 'Intent Invalid Outcomes',
    '/trainer_app/trainer_tools/actionplan_debugger' => 'My Action Plan Debugger',
    '/trainer_app/trainer_tools/en_icon_search' => 'Entity Icon Search',
    '/trainer_app/trainer_tools/moderate_intent_notes' => 'Moderate Intent Notes',
    '/trainer_app/trainer_tools/identical_intent_outcomes' => 'Identical Intent Outcomes',
    '/trainer_app/trainer_tools/identical_entity_names' => 'Identical Entity Names',
    '/trainer_app/trainer_tools/or__children' => 'List OR Intents + Answers',
    '/trainer_app/trainer_tools/orphan_intents' => 'Orphan Intents',
    '/trainer_app/trainer_tools/orphan_entities' => 'Orphan Entities',
    '/trainer_app/trainer_tools/assessment_marks_list_all' => 'Completion Marks List All',
...
$developer_tools = array(
    '/trainer_app/platform_cache' => 'Platform PHP Cache',
    '/trainer_app/my_session' => 'My Session Variables',
    '/trainer_app/php_info' => 'Server PHP Info',
);
...
if(!$action) {
    ...
    if(isset($_GET['update_user_icons'])){
        ...
        $updated = 0;
        foreach($this->Links_model->ln_fetch($base_filters, array('en_child'), 0) as $mench_user){
    ...
    if(count($orphan_ins) > 0){
        ...
        if($command1!='remove_all'){
            ...
            echo '<a class="remove-all hidden maxout" href="/trainer_app/trainer_tools/orphan_intents/remove_all" onclick="">Confirm: <b>Remove All</b> &raquo;</a>';
        }
    ...
    if(count($orphan_ens) > 0){
        ...
        if($command1!='remove_all'){
            ...
            echo '<a class="remove-all hidden maxout" href="/trainer_app/trainer_tools/orphan_entities/remove_all" onclick="">Confirm: <b>Remove All</b> &raquo;</a>';
        }
...
$.post("/intents/in_report_conditional_steps", {
    ...
    if(isset($_POST['test_message'])){
        ...
        echo '<ul class="breadcrumb"><li><a href="/trainer_app/trainer_tools">Trainer Tools</a></li><li><a href="/trainer_app/trainer_tools/'.$action.'">'.$moderation_tools['/trainer_app/trainer_tools/'.$action].'</a></li><li><b>Review Message</b></li></ul>';
