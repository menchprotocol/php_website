...
if(!$action) {
    ...
    foreach($active_ins as $count=>$in){
        ...
        $in_outcome_validation = $this->Intents_model->in_analyze_outcome($in['in_outcome'], $in['in_level_entity_id']);
    ...
    foreach($this->Intents_model->in_fetch(array(
        ...
        echo '<tr class="panel-title down-border">';
        echo '<td style="text-align: left;">'.$in['in_outcome'].'<div style="font-size: 0.7em;">VERB @'.$in['in_verb_entity_id'].' CONNECTION @'.$in['in_level_entity_id'].'</div></td>';
        echo '<td style="text-align: left;">'.$in['in_outcome'].'<div style="font-size: 0.7em;">VERB @'.$in['in_verb_entity_id'].' CONNECTION @'.$in['in_level_entity_id'].'</div></td>';
        echo '<td style="text-align: left;"></td>';
        echo '<td style="text-align: left;"></td>';
        echo '</tr>';
        ...
        if(count($matching_results) < 1){
            ...
            foreach($matching_results as $count=>$in){
                ...
                if($replace_with_is_set){
                    ...
                    $in_outcome_validation = $this->Intents_model->in_analyze_outcome($new_outcome, $in['in_level_entity_id']);
    ...
    foreach ($this->Intents_model->in_fetch(array(
        ...
        foreach($this->Links_model->ln_fetch(array(
            ...
            $qualified_update = ( $child_or['in_type_entity_id']==6677 /* Intent Read-Only */ && in_array($child_or['in_level_entity_id'], $this->config->item('en_ids_7582')) /* Intent Action Plan Addable */ );
