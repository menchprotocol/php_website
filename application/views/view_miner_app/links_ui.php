...
foreach($filters as $key => $value){
    if(!includes_any($key, array('in_status_entity_id', 'in_verb_entity_id', 'in_type_entity_id', 'en_status_entity_id', 'in_scope_entity_id'))){
...
echo '</div></td>';
        ...
        if(isset($_GET['ln_type_entity_id']) && substr_count($_GET['ln_type_entity_id'], ',')>0){
            ...
            if(isset($_GET['ln_creator_entity_id'])) {
                ...
                $all_link_count = 0;
                $select_ui = '';
                foreach ($this->Links_model->ln_fetch($ini_filter, array('en_type'), 0, 0, array('en_name' => 'ASC'), 'COUNT(ln_type_entity_id) as trs_count, SUM(ln_credits) as credits_sum, en_name, ln_type_entity_id', 'ln_type_entity_id, en_name') as $ln) {
                    //Echo drop down:
