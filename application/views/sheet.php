...
if ((count($idea_settings['player_column']) + count($idea_settings['idea_column'])) > $max_limit) {
    ...
    echo '<div class="center-frame hide-subline maxwidth hideIfEmpty remove_first_line">' . view_idea_links($idea_settings['i'], (isset($player_active['playerid']) ? $player_active['playerid'] : 0)) . '</div>';
        ...
        foreach ($idea_settings['idea_column'] as $idea_var) {
            ...
            if (count($discoveries)) {
                ...
                if ($set_linktext) {
                    ...
                    $idea_content .= (isset($_GET['expand']) ? '<p data-placement="top">' . $set_linktext . '</p>' : '<span title="' . $set_linktext . ' [' . $discoveries[0]['linktime'] . ']">ℹ️️</span>');
