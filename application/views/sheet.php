...
if ((count($hashtag_settings['handle_column']) + count($hashtag_settings['hashtag_column'])) > $max_limit) {
    ...
    foreach ($hashtag_settings['query_string_filtered'] as $x) {
        ...
        $body_content .= '<td style="padding-top: 2px;"><span class="icon-block-xs">' . view_cover($x['handlecover'], true) . '</span><a href="' . view_memory(42903, 42902) . $x['handlehandle'] . '" style="font-weight:bold;">' . $x['handlevalue'] . '</a>' . ($this_quantity > 0 ? ' +' . $this_quantity : '') . '</td>';
        ...
        foreach ($hashtag_settings['handle_column'] as $e) {
            ...
            if ($e['handleid'] == 44328) {
                ...
                foreach ($this->Chains->read(array(
                    ...
                    $message_clean = '<a href="' . view_app_chain(44328) . '/' . $target_i['hashtaghashtag'] . '@' . $x['handlehandle'] . '" target="_blank" title="' . $handles___11035[44328]['m__title'] . '"><span class="icon-block-sm">' . $handles___11035[44328]['m__cover'] . '</span></a>';
    ...
    foreach ($hashtag_settings['handle_column'] as $e) {
        ...
        echo '<th id="thhandle_' . $e['handleid'] . '" title="'.(isset($count_totals['e'][$e['handleid']]) ? number_format($count_totals['e'][$e['handleid']], 2) : '').'"><a class="icon-block-xs" href="' . view_memory(42903, 42902) . $e['handlehandle'] . '" target="_blank" title="Open in New Window">' . (isset($count_totals['e'][$e['handleid']]) ? view_number($count_totals['e'][$e['handleid']]) : '0') . '</a><span class="vertical_col">' . view_cover($e['handlecover'], '✔️', ' ') . $e['handlevalue'] . '</span></th>';
    }
    ...
    foreach ($hashtag_settings['hashtag_column'] as $hashtag_var) {
        ...
        echo '<th id="th_hashtag_' . $hashtag_var['hashtagid'] . '"><a class="icon-block-xs" href="' . view_memory(42903, 33286) . $hashtag_var['hashtaghashtag'] . '" target="_blank" title="Open in New Window" ' . ($max_limit ? ($current_x >= $max_limit ? '' : (($current_x / $max_limit) >= 0.5 ? 'isgold' : 'isred')) : '') . '">' . $current_x . ($max_limit ? '/' . $max_limit : '') . '</a><span class="vertical_col">' . (strlen($hashtag_var['chainvalue']) ? $hashtag_var['chainvalue'] : view_hashtag_title($hashtag_var, true)) . '</span></th>';
