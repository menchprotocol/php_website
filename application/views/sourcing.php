...
foreach($this->config->item('players___31916') as $linktype => $m) {
    ...
    $coins_count[$linktype] = view_player_covers($linktype, $focus_e['playerid'], 0, false);
    if(!$coins_count[$linktype] && in_array($linktype, $this->config->item('playerids___12144'))){ continue; }

    $input_content = '';
    if(superpower_unlocked(10939)){

        if(in_array($linktype, $this->config->item('playerids___11028'))){
        ...
        } elseif(in_array($linktype, $this->config->item('playerids___42261'))){
    ...
    if(in_array($linktype, $this->config->item('playerids___42945')) || $coins_count[$linktype]>0){
...
$focus_tab = 0;
foreach($players___focus as $linktype => $m) {
