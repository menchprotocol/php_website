...
<div class="card-body">';
        ...
        echo '<div class="read-topic"><a href="javascript:void(0);" onclick="$(\'.contentTab'.$en_id.'\').toggleClass(\'hidden\')">';
        echo '<span class="icon-block"><i class="far fa-plus-circle contentTab'.$en_id.( $auto_expand_tab ? ' hidden ' : '' ).'"></i>';
        echo '<i class="far fa-minus-circle contentTab'.$en_id.( $auto_expand_tab ? '' : ' hidden ' ).'"></i></span>';
        echo $m['m_name'];
        echo ($counter>0 ? '<span title="'.number_format($counter, 0).'" class="'.superpower_assigned(12701).'">&nbsp;'.echo_number($counter).'</span>' : '').'</a></div>';
