...
if(!isset($_GET['in_id']) || !intval($_GET['in_id'])) {
    ...
    if(count($ins) < 1){
        ...
        view_json(array(
            ...
            'in_user' => array(
                'read_find_next' => $this->READ_model->find_next($_GET['en_id'], $ins[0]),
                'read_completion_progress' => $this->READ_model->completion_progress($_GET['en_id'], $ins[0]),
                'read_completion_marks' => $this->READ_model->completion_marks($_GET['en_id'], $ins[0]),
            ),
        ));
