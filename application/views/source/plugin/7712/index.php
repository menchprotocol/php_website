...
foreach($this->IDEA_model->fetch(array(
    ...
    echo '<div>'.($count+1).') '.view_cache('sources__4737' /* Idea Status */, $in['idea__status']).' '.view_cache('sources__6193' /* OR Ideas */, $in['idea__type']).' <b><a href="/idea/go/'.$in['idea__id'].'">'.view_idea__title($in).'</a></b></div>';
    ...
    foreach($this->READ_model->fetch(array(
        ...
        $all_children++;
        echo '<li>'.view_cache('sources__6186' /* Read Status */, $child_or['read__status']).' '.view_cache('sources__4737' /* Idea Status */, $child_or['idea__status']).' '.view_cache('sources__7585', $child_or['idea__type']).' <a href="/idea/go/'.$child_or['idea__id'].'">'.view_idea__title($child_or).'</a>'.( count($user_steps) > 0 ? ' / Steps: '.count($user_steps) : '' ).'</li>';
    }
    echo '</ul>';
    echo '<hr />';
}
