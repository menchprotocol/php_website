...
foreach($this->IDEA_model->fetch() as $in) {
    ...
    //Scan sources:
    $in_sources = $this->TRANSACTION_model->fetch(array(
    ...
    $in_creators = $this->TRANSACTION_model->fetch(array(
    ...
    if(!$is_deleted && !count($in_sources)){
        ...
        foreach($in_notes as $in_note){
            //Delete this link:
            $stats['note_deleted'] += $this->TRANSACTION_model->update($in_note['ln_id'], array(
