...
$interactions_discoveries_new_last_week = $this->DISCOVER_model->fetch(array(
    'x__time >=' => $last_week_start,
    'x__time <=' => $last_week_end,
), array(), 0, 0, array(), 'COUNT(x__id) as totals');
$interactions_discoveries_last_week = $this->DISCOVER_model->fetch(array(
    'x__time <=' => $last_week_end,
), array(), 0, 0, array(), 'COUNT(x__id) as totals');
...
$html_message .= '<div style="padding-bottom:10px;"><b style="min-width:30px; text-align: center; display: inline-block;">🔵</b><b style="min-width:55px; display: inline-block;">'.( $e_coins_growth_rate >= 0 ? '+' : '-' ).$e_coins_growth_rate.'%</b><span style="min-width:55px; display: inline-block;">(<span title="'.number_format($e_coins_last_week[0]['totals'], 0).'" style="border-bottom:1px dotted #999999;">'.view_number($e_coins_last_week[0]['totals']).'</span>)</span><a href="'.$this->config->item('base_url').'/source" target="_blank" style="color: #0000FF; font-weight:bold; text-decoration:none;">'.$sources__13355[13358]['m_name'].' &raquo;</a></div>';

$html_message .= '<div style="padding-bottom:10px;"><b style="min-width:30px; text-align: center; display: inline-block;">🟡</b><b style="min-width:55px; display: inline-block;">'.( $i_coins_growth_rate >= 0 ? '+' : '-' ).$i_coins_growth_rate.'%</b><span style="min-width:55px; display: inline-block;">(<span title="'.number_format($i_coins_last_week[0]['totals'], 0).'" style="border-bottom:1px dotted #999999;">'.view_number($i_coins_last_week[0]['totals']).'</span>)</span><a href="'.$this->config->item('base_url').'/~" target="_blank" style="color: #ffd500; font-weight:bold; text-decoration:none;">'.$sources__13355[13359]['m_name'].' &raquo;</a></div>';
