...
foreach($this->TREE_model->in_fetch(array(
    'in_status_source_id IN (' . join(',', $this->config->item('en_ids_12138')) . ')' => null, //Tree Status Featured
)) as $in_published){

    //Count Enrolled Users:
    $enrolled_users = $this->READ_model->ln_fetch(array(
        'ln_previous_tree_id' => $in_published['in_id'],
    ...
    $completed_users = $this->READ_model->ln_fetch(array(
        'ln_previous_tree_id' => end($common_steps),
        'ln_type_source_id IN (' . join(',', $this->config->item('en_ids_6255')) . ')' => null, //READ COIN
        'ln_status_source_id IN (' . join(',', $this->config->item('en_ids_7359')) . ')' => null, //Transaction Status Public
    ), array(), 0, 0, array(), 'COUNT(ln_id) as totals');
...
echo '<td style="text-align: left;">ALL TREES</td>';
...
echo '<td style="text-align: left;">TREES</td>';
