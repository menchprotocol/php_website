...
<div class="container">
    ...
    //Content Title
    echo '<div><span class="icon-block icon-white-block">'.$en_all_12762[12772]['m_icon'].'</span><div class="form-group is-empty block-one inline-block"><input type="text" id="content_title" '.( isset($_GET['content_title']) ? ' value="'.$_GET['content_title'].'" ' : '' ).' class="form-control border montserrat doupper source" placeholder="'.$en_all_12762[12772]['m_name'].'"></div></div>';

    //Content Type
    echo '<div class="space-left" style="padding-bottom: 15px;">'.echo_in_dropdown(3000, ( isset($_GET['content_type']) ? $_GET['content_type'] : 3005 /* Books */ ), 'btn-source').'</div>';

    //Content URL
    echo '<div><span class="icon-block icon-white-block">'.$en_all_12762[12763]['m_icon'].'</span><div class="form-group is-empty block-one inline-block"><input type="url" id="ln_content" '.( isset($_GET['ln_content']) ? ' value="'.$_GET['ln_content'].'" ' : '' ).' class="form-control border" placeholder="'.$en_all_12762[12763]['m_name'].'"></div></div>';
