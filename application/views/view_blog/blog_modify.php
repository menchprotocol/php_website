...
if(!$can_manage){
    echo '<div class="alert alert-info"><i class="fas fa-exclamation-triangle yellow"></i> You are not a blog author. <a href="/blog/in_become_author/'.$in['in_id'].'" class="inline-block montserrat '.superpower_active(10984).'">BECOME AUTHOR</a></div>';
}
...
foreach ($this->config->item('en_all_11021') as $en_id => $m){
    ...
    if($col_num==1){
        ...
        if($can_manage){
            echo '<textarea onkeyup="show_save_button()" class="form-control" id="new_blog_title" placeholder="'.$en_all_6201[4736]['m_name'].'" style="margin-bottom: 5px;">'.$in['in_outcome'].'</textarea>';
        } else {
