...
foreach($this->config->item('e___'.$tab_group) as $x__type => $m){
    ...
    echo '<li class="nav-item '.( count($superpower_actives) ? superpower_active(end($superpower_actives)) : '' ).'"><a class="nav-x tab-nav-'.$tab_group.' tab-head-'.$x__type.' '.( $default_active ? ' active ' : '' ).'" href="javascript:void(0);" onclick="loadtab('.$tab_group.','.$x__type.')" data-toggle="tooltip" data-placement="top" title="'.$m['m_title'].( strlen($m['m_message']) ? ': '.$m['m_message'] : '' ).'"><span class="'.extract_icon_color($m['m_icon']).'">'.$m['m_icon'].( is_null($counter) ? '' : ' <span class="en-type-counter-'.$x__type.'">'.$counter.'</span>' ).'<span class="show-max-active">&nbsp;'.$m['m_title'].'</span></span>'.( $stats_append ? '<span class="show-max-active second-stats">'.$stats_append.'</span>' : '' ).'</a></li>';
