        ...
        $(element_focus + ' .add-input').on('autocomplete:selected', function (event, suggestion, dataset) {
            ...
            templates: {
                ...
                header: function (data) {
                    if (!data.isEmpty) {
                        return '<a href="javascript:void(0);" onclick="e_only_add(0, '+note_type_id+');" class="suggestion montserrat"><span class="icon-block"><i class="far fa-plus-circle add-plus source"></i></span><b class="source">' + data.query.toUpperCase() + '</b></a>';
                    }
                },
                empty: function (data) {
                    return '<a href="javascript:void(0);" onclick="e_only_add(0, '+note_type_id+');" class="suggestion montserrat"><span class="icon-block"><i class="far fa-plus-circle add-plus source"></i></span><b class="source">' + data.query.toUpperCase() + '</b></a>';
    ...
    if (r == true) {
        ...
        $.post("/i/i_remove", {
            ...
            if (data.status) {
                ...
                //Delete from UI:
                $('.i__tr_' + x__id).html('<span style="color:#111111;"><i class="fas fa-trash-alt"></i></span>');
...
function i_add(i_x_id, is_parent, i_x_child_id) {
    ...
    add_to_list(sort_list_id, sort_handler, '<div id="tempLoader" class="list-group-item montserrat no-side-padding"><span class="icon-block"><i class="fas fa-yin-yang fa-spin idea"></i></span>' + js_view_12687(12695) +  '</div>');
