...
<div class="wizard-box" id="price_selection">
    ...
    <div class="radio pricing_block">
        <label>
            ...
            <b id="p_name_1"><i class="fa <?= $r_statuses[0]['s_mini_icon'] ?>" aria-hidden="true"></i> <?= $r_statuses[0]['s_name'] ?> for <?= echo_price($admission,1) ?></b>
    ...
    <?php if($admission['b_p2_max_seats']>0){ ?>
        ...
        <div class="radio pricing_block">
            <label>
                ...
                <b id="p_name_2"><i class="fa <?= $r_statuses[1]['s_mini_icon'] ?>" aria-hidden="true"></i> <?= $r_statuses[1]['s_name'] ?> for <?= echo_price($admission,2) ?></b> <b class="badge"><?= ( $classroom_available ? $classroom_available . ' Seat' . show_s($classroom_available).' Remaining' : 'SOLD OUT' ) ?></b>
                <p><?= nl2br($r_statuses[1]['s_desc']) ?></p>
            </label>
        </div>
        ...
        <?php if($admission['b_p3_rate']>0){ ?>
            ...
            <div class="radio pricing_block">
                <label>
                    ...
                    <b id="p_name_3"><i class="fa fa-handshake-o" aria-hidden="true"></i> <?= $r_statuses[1]['s_name'] ?> + 50 Minutes of Tutoring for <?= echo_price($admission,3) ?></b>
                    <p style="margin: 0; padding: 0 0 5px 0;">Students also receive a 50 Minute 1-on-1 video call for progress review and personalized feedback.</p>
                    <p>Ideal for students with less experience on this subject matter.</p>
                </label>
            </div>
