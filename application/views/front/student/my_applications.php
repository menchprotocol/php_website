...
if(count($admissions)>0 && is_array($admissions)){
    ...
    foreach($admissions as $admission){
        ...
        if($admission['ru_upfront_pay']>0 && $admission['ru_status']<4 && $admission['ru_status']>=0 && isset($_GET['pay_ru_id']) && intval($_GET['pay_ru_id'])==intval($admission['ru_id'])){
            ?>
            <script>
                $( document ).ready(function() {
            ...
            <form action="https://www.paypal.com/cgi-bin/webscr" id="paypal_<?= $admission['ru_id'] ?>" method="post" target="_top" style="display:none;">
                ...
                <input type="hidden" name="amount" value="<?= $admission['ru_upfront_pay'] ?>">
                <input type="hidden" name="currency_code" value="USD">
                <input type="hidden" name="button_subtype" value="services">
                <input type="hidden" name="no_note" value="1">
                <input type="hidden" name="no_shipping" value="1">
                <input type="hidden" name="rm" value="1">
                <input type="hidden" name="return" value="https://mench.com/my/applications?status=1&purchase_value=<?= $admission['ru_upfront_pay'] ?>&message=<?= urlencode('Payment received.'); ?>&u_key=<?= $u_key ?>&u_id=<?= $u_id ?>">
