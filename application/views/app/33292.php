    ...
    foreach($this->config->item('e___'.$e__id1) as $e__id => $m) {
        ...
        foreach($this->config->item('e___'.$cat_id) as $e__id2 => $m2) {
            ...
            if($sub_counter[0]['totals'] > 0){
                ...
                $inner_stats .= '<tr><td style="text-align: right;" width="34%">'.number_format($sub_counter[0]['totals'], 0).'</td><td style="text-align: left;" data-toggle="tooltip" data-placement="top" title="'.$m2['m__message'].' ('.number_format(($sub_counter[0]['totals']/$this_count*100),2).'% Of Total)"><span class="icon-block">'.$m2['m__cover'].'</span>'.$m2['m__title'].( strlen($m2['m__message']) ? ' <i class="far fa-info-circle" ></i>' : '' ).'</td></tr>';
