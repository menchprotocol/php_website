...
foreach($this->config->item('e___33292') as $e__id1 => $m1) {
    ...
    foreach($this->config->item('e___'.$e__id1) as $e__id2 => $m2) {
        ...
        foreach($this->config->item('e___'.map_primary_links($e__id2)) as $e__id3 => $m3) {
            ...
            echo '<td style="text-align: left;">'.$placeholder.'<span class="icon-block-xxs">'.$m3['m__cover'].'</span>'.$m3['m__title'].'<span class="last-right-col"><b class="card_count_'.$e__id3.'"><i class="far fa-yin-yang fa-spin"></i></b></span><span class="second-right-col mench-points " title="'.$e___11035[42225]['m__title'].'">'.( isset($e___42225[$e__id3]['m__message']) ? $e___42225[$e__id3]['m__message'].'<span class="icon-block-xs">'.$e___11035[42225]['m__cover'].'</span>' : '' ).'</span></td>';
            echo '</tr>';
