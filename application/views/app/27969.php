...
function build_item($e__id, $i__id, $s__title, $s__cover, $link, $desc = null, $small_text = null){
    ...
    <div class="d-flex justify-content-between">
      <h4><b><span class="icon-block-lg" style="margin-right: 5px;">'.view_cover(($e__id>0 ? 12274 : 12273),$s__cover).'</span>'.$s__title.'</b></h4>
...
if(isset($_GET['e__id'])){
    ...
    foreach($this->X_model->fetch(array(
        ...
        foreach(view_coins_e(12273, $header['e__id'], 1) as $ref_i){
            if(i_is_startable($ref_i)){
                ...
                foreach($this->X_model->fetch(array(
                    ...
                    $small_text .= '<div class="key-ref css__title"><span class="icon-block-img">'.view_cover(12274,$key_references['e__cover']).'</span>&nbsp;'.$key_references['e__title'].'</div>';
                }
        ...
        if($list_body){
            //Add this to the UI:
            $ui .= '<div class="css__title mid-title grey"><span class="icon-block">'.view_cover(12274,$header['e__cover']).'</span>'.$header['e__title'].'</div>';
