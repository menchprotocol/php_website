    ...
    foreach($this->I_model->fetch(array(
        ...
        if(isset($_POST) && count($_POST)){
            ...
            if (strlen($_POST['x_write'])<5 || !substr_count($_POST['x_write'] , ' ')) {
                echo '<div class="alert alert-danger" role="alert"><span class="icon-block"><i class="far fa-exclamation-circle"></i></span>Enter Your First & Last Name</div>';
            } elseif (!isset($_POST['DigitalSignAgreement']) || !intval($_POST['DigitalSignAgreement'])) {
                echo '<div class="alert alert-danger" role="alert"><span class="icon-block"><i class="far fa-exclamation-circle"></i></span>You must agree to be legally bound by this document.</div>';
            } elseif (!filter_var($_POST['x_email'], FILTER_VALIDATE_EMAIL)) {
                echo '<div class="alert alert-danger" role="alert"><span class="icon-block"><i class="far fa-exclamation-circle"></i></span>Enter a valid Email Address</div>';
            } elseif (strlen(intval($_POST['x_phone']))<10) {
                echo '<div class="alert alert-danger" role="alert"><span class="icon-block"><i class="far fa-exclamation-circle"></i></span>Enter a valid Phone Number</div>';
            } else {
                ...
                foreach($map_users as $map_user){
                    ...
                    $this->X_model->create(array(
                        'x__player' => $map_user['e__id'],
                        'x__type' => 32603,
                        'x__previous' => $i_sign['i__id'],
                    ));
                }

                echo '<div class="alert alert-success" role="alert"><span class="icon-block"><i class="far fa-check-circle"></i></span> Waver signed for "'.$_POST['x_write'].'".<br />Show your ID at the door to enter'.( isset($map_user['e__id']) ? '<a href="'.view_memory(42903,42902).$map_user['e__handle'].'" style="text-decoration: none;">.</a>' : '.' ).'</div>';
...
if(!$found_i){
    echo '<div class="alert alert-danger" role="alert"><span class="icon-block"><i class="far fa-exclamation-circle"></i></span>Missign Agreement Idea ID</div>';
}