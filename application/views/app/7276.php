...
if(isset($_GET['i__id'])){
    ...
    //All Orphans:
    $query_filters = array(
        ' NOT EXISTS (SELECT 1 FROM table__x WHERE i__id=x__right AND x__type IN (' . join(',', $this->config->item('n___4486')) /* IDEA LINKS */ . ') AND x__status IN ('.join(',', $this->config->item('n___7360')) /* ACTIVE */.')) ' => null,
