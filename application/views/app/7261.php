...
$q = $this->db->query('select in1.* from table__i in1 where (select count(*) from table__i in2 where in2.i__title = in1.i__title AND in2.i__privacy IN (' . join(',', $this->config->item('n___31871')) . ')) > 1 AND in1.i__privacy IN (' . join(',', $this->config->item('n___31871')) . ') ORDER BY in1.i__title ASC');
