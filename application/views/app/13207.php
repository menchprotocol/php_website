...
foreach($this->config->item('e___13207') as $e__id => $m) {
    ...
    $e_list = $this->X_model->fetch(array(
        ...
        ' EXISTS (SELECT 1 FROM mench__x WHERE e__id=x__down AND x__up IN (' . ( in_array($e__id, $this->config->item('n___14598')) ? join(',', $this->config->item('n___'.$e__id)) : $e__id ) . ') AND x__type IN (' . join(',', $this->config->item('n___4592')) . ') AND x__status IN ('.join(',', $this->config->item('n___7359')) /* PUBLIC */.')) ' => null,
