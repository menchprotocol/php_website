                ...
                if(($time_starts - intval($e___42216[$i['link_up']]['m__message'])) < time()){
                    ...
                    foreach($this->Mench_ledger->fetch(array(
                        ...
                        $html_message = 'This is a friendly reminder about an upcoming event you signed up for:'.
                            ...
                            "\n".'https://'.get_domain('m__message', $x['e__id'], $user_website).view__app_link(42216).'?link_id='.$x['link_id'].'&e__handle='.$x['e__handle'].'&e__time='.time().'&e__hash='.view__hash(time().$x['e__handle']);
    ...
    foreach ($this->Mench_ledger->fetch(array(
        ...
        foreach ($list_settings['query_string_filtered'] as $x) {
            ...
            foreach ($children as $down_or) {
                ...
                $html_message .= 'https://' . get_domain('m__message', $x['e__id'], $i['link_domain']) . view__memory(42903,33286) . $down_or['i__hashtag'] . (!count($discoveries) ? '?e__handle=' . $x['e__handle'] . '&e__time='.time().'&e__hash=' . view__hash(time().$x['e__handle']) : '') . "\n\n";
