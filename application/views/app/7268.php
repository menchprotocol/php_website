...
if(isset($_GET['e__id'])){
    ...
    //Find by name:
    $q = $this->db->query('select en1.* from table__e en1 where (select count(*) from table__e en2 where en2.e__title = en1.e__title AND en2.e__status IN (' . join(',', $this->config->item('n___7358')) . ')) > 1 AND en1.e__status IN (' . join(',', $this->config->item('n___7358')) . ') ORDER BY en1.e__title ASC');
    ...
    if(count($duplicates) > 0){
        ...
        foreach($duplicates as $en) {
            ...
            echo '<span data-toggle="tooltip" data-placement="right" title="'.$e___6177[$en['e__status']]['m__title'].': '.$e___6177[$en['e__status']]['m__message'].'">' . $e___6177[$en['e__status']]['m__cover'] . '</span> <a href="/@' . $en['e__id'] . '"><b>' . $en['e__title'] . '</b></a> @' . $en['e__id'] . '<br />';
        }
