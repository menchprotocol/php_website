                ...
                <table class="three-menus">
                    <tr>
                        ...
                        foreach($this->config->item('en_all_2738') as $en_id => $m){
                            ...
                            echo '<td valign="bottom"><a class="'.$handle.' border-'.$handle.( $this->uri->segment(1)==$handle ? ' background-'.$handle: null ).'" href="/'.$handle.(intval($this->uri->segment(2) && isset($session_en['en_id']) && (($this->uri->segment(1)=='blog' && $en_id==6205) || ($this->uri->segment(1)=='read' && $en_id==4535))) ? '/'.$this->uri->segment(2) : '' ).'">' . $m['m_icon'] . '<span class="mn_name montserrat">' . $m['m_name'] . '</span> <span class="inline-block">'.( $play_logged_in ? '' : ' <span class="current_count mono"><i class="far fa-yin-yang fa-spin"></i></span>' ).' '.( $play_logged_in ? trim(one_two_explode('',' ', $session_en['en_name'])) : '<span class="mono mini-hidden">'.$m['m_desc'].'</span>' ).'</span></a></td>';
