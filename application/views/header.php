                ...
                <table class="three-menus">
                    <tr>
                        ...
                        foreach($this->config->item('en_all_2738') as $en_id => $m){
                            ...
                            echo '<td><a class="'.$identifier.' border-'.$identifier.( $this->uri->segment(1)==$handle || $this->uri->segment(1)==$identifier || ( $en_id==6205 && !$handle && is_numeric($this->uri->segment(1)) ) ? ' focustab ': '' ).'" href="/'.$identifier.( $en_id==4536 ? '/'.$session_en['en_id'] : '' ).'">' . ( $en_id==4536 ? '<span class="parent-icon icon-block">'.$session_en['en_icon'].'</span><span class="mn_name montserrat">'.one_two_explode('',' ',$session_en['en_name']).'</span>' : '<span class="parent-icon icon-block">'.$m['m_icon'].'</span><span class="current_count mn_name montserrat"><i class="far fa-yin-yang fa-spin"></i></span><span class="montserrat">&nbsp;' . $m['m_name'] . '</span>' ) .'</a></td>'; //show-max
