...
<div class="container">
    ...
    foreach ($this->config->item('en_all_11088') as $en_id => $m){
        ...
        foreach ($this->config->item('en_all_'.$en_id) as $en_id2 => $m2){
            ...
            echo '<li class="nav-item '.require_superpower(array_intersect($this->config->item('en_ids_10957'), $m2['m_parents'])).'"><a class="nav-link tab-nav-'.$en_id.' tab-head-'.$en_id2.' '.( $default_active ? ' active ' : '' ).'" href="javascript:void(0);" onclick="loadtab('.$en_id.','.$en_id2.')" data-toggle="tooltip" data-placement="top" title="'.( $show_tab_names ? '' : $m2['m_name'] ).' ['.print_r(array_intersect($this->config->item('en_ids_10957'), $m2['m_parents']), true).']">'.$m2['m_icon'].( is_null($counter) ? '' : ' <span class="counter-'.$en_id2.'">'.echo_number($counter).'</span>' ).( $show_tab_names ? ' '.$m2['m_name'] : '' ).'</a></li>';
