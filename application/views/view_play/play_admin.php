...
$moderation_tools = array(
    '/play/play_admin/link_words_stats' => 'Link Words Stats',
    ...
    '/play/play_admin/en_icon_search' => 'Entity Icon Search',
    '/play/play_admin/moderate_intent_notes' => 'Moderate Blog Notes',
    '/play/play_admin/identical_intent_outcomes' => 'Identical Blog Titles',
    '/play/play_admin/identical_entity_names' => 'Identical Entity Names',
    '/play/play_admin/or__children' => 'List OR Blogs + Answers',
    '/play/play_admin/orphan_intents' => 'Orphan Blogs',
    '/play/play_admin/orphan_entities' => 'Orphan Entities',
...
if(!$action) {
    ...
    if(count($orphan_ins) > 0){
        ...
        if($command1!='remove_all'){
            ...
            echo '<a class="remove-all hidden maxout" href="/play/play_admin/orphan_intents/remove_all" onclick="">Confirm: <b>Remove All</b> &raquo;</a>';
        }
    ...
    if(count($orphan_ens) > 0){
        ...
        foreach ($orphan_ens  as $count => $orphan_en) {
            ...
            if($command1=='remove_all'){
                ...
                $this->PLAY_model->en_update($orphan_en['en_id'], array(
                    'en_status_player_id' => 6178, /* Entity Removed */
                ), true, $session_en['en_id']);
        ...
        if($command1!='remove_all'){
            ...
            echo '<a class="remove-all hidden maxout" href="/play/play_admin/orphan_entities/remove_all" onclick="">Confirm: <b>Remove All</b> &raquo;</a>';
        }
    ...
    if(isset($_GET['search_for']) && strlen($_GET['search_for'])>0){
        ...
        if(count($matching_results) > 0){
            ...
            foreach($matching_results as $count=>$en){
                ...
                echo '<td style="text-align: left;">'.echo_en_cache('en_all_6177' /* Entity Statuses */, $en['en_status_player_id'], true, 'right').' <span class="icon-block">'.echo_en_icon($en['en_icon']).'</span><a href="/play/'.$en['en_id'].'">'.$en['en_name'].'</a></td>';
                echo '</tr>';
