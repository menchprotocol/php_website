...
<script src="/js/custom/en_source_wizard.js?v=v<?= config_value(11060).time() ?>"
    ...
    <div class="url-parsed hidden">
        ...
        <?php for($contributor_num = 1; $contributor_num <= 5; $contributor_num++){ ?>
            ...
            <span class="inline-block en_role_<?= $contributor_num ?> hidden">
                <input style="padding-left:3px;" type="text" id="auth_role_<?= $contributor_num ?>" class="form-control border" data-toggle="tooltip" data-placement="top" title="Define the role of this person/organization" placeholder="Contributor's Role..." maxlength="<?= config_value(11073) ?>" value="" />
            </span>
            ...
            <div class="form-group explain_expert_<?= $contributor_num ?> label-floating is-empty hidden inline-box" style="margin:0px 19px 10px 10px;">
                ...
                <div class="below-counter">[<span id="char_count_<?= $contributor_num ?>">0</span>/<?= config_value(11073) ?>]</div>
        ...
        foreach($this->config->item('en_all_3000') as $en_id => $m ){
            ...
            <div class="extra_info_'.$en_id.' hidden" style="display:'.( in_array(6805 , $m['m_parents']) ? 'block' : 'none' ).';">
                ...
                <div class="below-counter">[<span id="char_count_'.$en_id.'">0</span>/'.config_value(11073).']</div>
            </div>
