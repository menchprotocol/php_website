...
foreach($this->Mench_ledger->fetch(array(
    ...
    if(count($previous_x) && $previous_x['link_type']==$x['link_type']){
        ...
        if(($previous_x['link_left']>0 || $previous_x['link_right']>0) && $previous_x['link_left']==$x['link_left'] && $previous_x['link_right']==$x['link_right'] && (($previous_x['link_text']==$x['link_text']) || !strlen($previous_x['link_text']) || !strlen($x['link_text']))){
            ...
            $this->db->query("DELETE FROM mench_ledger WHERE link_id=".( strlen($previous_x['link_text']) ? $x['link_id'] : $previous_x['link_id'] ).";");
            ...
            echo '<tr '.( strlen($previous_x['link_text']) ? '' : 'style="background-color: #EFEFEF;"' ).'><td>ID '.$previous_x['link_id'].'</td><td>TP '.$previous_x['link_type'].'</td><td>PL '.$previous_x['link_player'].'</td><td> </td><td> </td><td>UP '.$previous_x['link_up'].'</td><td>DW '.$previous_x['link_down'].'</td><td>'.$previous_x['link_text'].'</td></tr>';
            echo '<tr '.( strlen($x['link_text']) ? '' : 'style="background-color: #EFEFEF;"' ).'><td>ID '.$x['link_id'].'</td><td>TP '.$x['link_type'].'</td><td>PL '.$x['link_player'].'</td><td> </td><td> </td><td>UP '.$x['link_up'].'</td><td>DW '.$x['link_down'].'</td><td>'.$x['link_text'].'</td></tr>';
