...
if(!isset($_GET['handleterm']) || !strlen($_GET['handleterm']) || !$_GET['handleterm'] || $_GET['handleterm']=='0'){
    
    echo '<h1>'.$handles___6287[27004]['m__title'].'</h1>';
    foreach($this->Handles->tree(11029, $handle_session['handleid'], array(27004)) as $e){
        echo '<div><a href="'.view_app_chain(27004).view_memory(42903,42902).$e['handleterm'].'" class="main__title">'.$e['handlename'].'</a></div>';
    }
    ...
    echo '<h2>'.$es[0]['handlename'].' @'.$es[0]['handleterm'].'</h2>';
    ...
    foreach($hashtag_query as $i){
        ...
        foreach($this->Chains->read(array(
            ...
            $chain_content .= '<td>'.( count($es) ? '<span class="icon-block-sm e_cover_micro">'.view_cover($es[0]['handlecover'],true).'</span><a href="'.view_memory(42903,42902).$es[0]['handleterm'].'" style="font-weight:bold; display: inline-block;">'.$es[0]['handlename'].'</a> ' : '' ).$chainvalue['first_name'].' '.$chainvalue['last_name'].'</td>';
        ...
        $sale_type_content .= '<td>'.( $total_sales>0 ? '<a href="javascript:void(0)" onclick="$(\'.chains_'.$i['hashtagid'].'\').toggleClass(\'hidden\');" style="font-weight:bold;">'.view_hashtag_title($i).'</a>' : view_hashtag_title($i) ).' <a href="'.view_memory(42903,33286).$i['hashtagterm'].'"><i class="far fa-cog" style="font-size:1em !important;"></i></a></td>';
    ...
    if(count($other_es)){
        ...
        foreach($other_es as $other_e){
            ...
            $otherhandle_content .= '<td><span class="icon-block e_cover_micro">'.view_cover($other_e['handlecover'],true).'</span><a href="'.view_memory(42903,42902).$other_e['handleterm'].'" style="font-weight:bold; display: inline-block;">'.$other_e['handlename'].'</a></td>';
            $otherhandle_content .= '<td style="text-align: right;" class="advance_columns hidden">&nbsp;</td>';
            $otherhandle_content .= '<td style="text-align: right;" class="advance_columns hidden">&nbsp;</td>';
