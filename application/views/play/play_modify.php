...
<div class="container">
    ...
    //NAME
    echo '<h1 class="inline-block montserrat '.extract_icon_color($player['en_icon']).'"><span class="icon-block-lg en-icon en_ui_icon_'.$player['en_id'].'">'.echo_en_icon($player['en_icon']).'</span> <span class="en_name_'.$player['en_id'].'">'.$player['en_name'].'</span></h1>';
    ...
    foreach ($this->config->item('en_all_11088') as $en_id => $m){
        ...
        foreach ($this->config->item('en_all_'.$en_id) as $en_id2 => $m2){
            ...
            <div class="algolia_search_pad hidden"><b class="montserrat"><span class="icon-block"><i class="fas fa-search-plus blue"></i></span>Search players, create a new player or paste URL...</b></div>
                ...
                if (count($child_en_filters) > 0 && $child_en_filters[0]['totals'] < $player['en__child_count']) {
                    ...
                    $tab_content .= '<li class="nav-item"><a href="#" onclick="en_filter_status(-1)" class="nav-link u-status-filter active u-status--1" data-toggle="tooltip" data-placement="top" title="View all players"><i class="fas fa-at"></i><span class="hide-small"> All</span> [<span class="counter-11029">' . $player['en__child_count'] . '</span>]</a></li>';
