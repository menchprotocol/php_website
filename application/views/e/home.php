<div class="container">
    ...
    foreach($this->config->item('e___13207') as $e__id => $m) {
        ...
        if(in_array($e__id, $this->config->item('e___n_13365'))){
            ...
            $e_list = $this->X_model->fetch(array(
                ...
                ' EXISTS (SELECT 1 FROM mench__x WHERE e__id=x__down AND x__up IN (' . join(',', $this->config->item('e___n_'.$e__id)) . ') AND x__type IN (' . join(',', $this->config->item('e___n_4592')) . ') AND x__status IN ('.join(',', $this->config->item('e___n_7359')) /* PUBLIC */.')) ' => null,
        ...
        } elseif(in_array($e__id, $this->config->item('e___n_13439'))){
