...
<div class="container">
    ...
    foreach($profiles as $count => $e_profile) {
        ...
        if(!$hide && $count==$show_max){
            $hide = true;
            echo '<div class="load-more montserrat list-group-item itemsource no-left-padding see_all_11030"><a href="javascript:void(0);" onclick="$(\'.see_all_11030\').toggleClass(\'hidden\')"><span class="icon-block"><i class="far fa-search-plus source"></i></span><b class="montserrat source">FULL PROFILE<span class="'.superpower_active(13422).'"> ['.count($profiles).']</span></b></a></div>';
            ...
            if($counter>0){
                ...
                foreach($i_notes_query as $count => $i_notes) {
                    ...
                    if($x__type==12896){
                        ...
                        //Saved IDEA
                        $focus_tab .= view_i_x($i_notes, -1 /* Unlocked */, true, null, true);


                    } else {
                        $focus_tab .= view_i_x($i_notes, -1 /* Unlocked */, true); //Inputs when it used to use view_i() : 0, false, false, $i_notes['x__message'], null, false
                    }
