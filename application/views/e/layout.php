...
<script src="/application/views/e/layout.js?v=<?= config_var(11060) ?>" type="text/javascript"></script>
    ...
    //SOURCE NAME
    echo '<div style="padding: 8px 0; margin-top:13px;" class="itemsource">'.view_input_text(6197, $e['e__title'], $e['e__id'], ($source_of_e && in_array($e['e__status'], $this->config->item('n___7358'))), 0, true, '<span class="e_ui_icon_'.$e['e__id'].'">'.view_e__icon($e['e__icon']).'</span>', extract_icon_color($e['e__icon'])).'</div>';
    ...
    foreach($this->config->item('e___11089') as $x__type => $m) {
        ...
        if(in_array(11040 , $m['m_profile'])){
            ...
            if($source_is_e){
                ...
                $focus_tab .= '<div class="headline" style="margin:21px 0;"><span class="icon-block">'.$e___11035[13813]['m_icon'].'</span>'.$e___11035[13813]['m_title'].'</div>';
...
<div class="card-body">';
        ...
        $tab_nav .= '<li class="nav-item"><a class="nav-x tab-nav-11089 tab-head-'.$x__type.' '.( $default_active ? ' active ' : '' ).extract_icon_color($m['m_icon']).'" href="javascript:void(0);" onclick="loadtab(11089, '.$x__type.')">'.$m['m_icon'].( is_null($counter) ? '' : ' <span class="en-type-counter-'.$x__type.'">'.view_number($counter).'</span>' ).'<span class="show-max-active">&nbsp;'.$m['m_title'].'</span></a></li>';
