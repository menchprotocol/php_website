...
$target_hashtaghashtag = (count($target_i) && $chainhandlecreator ? $target_i['hashtaghashtag'] : null);
$at_starting_point = $target_hashtaghashtag==$focus_i['hashtaghashtag'];

//Breadcrump for logged in users NOT at the starting point...
$breadcrum_content = null;
if ($chainhandlecreator && !$at_starting_point) {

    $previous = $this->Chains->previoushashtag($chainhandlecreator, $target_hashtaghashtag, $focus_i['hashtagid']);
    if (count($previous)) {
        ...
        foreach ($previous as $followings_i) {
            ...
            $breadcrum_content .= '<a href="' . view_memory(42903, 30795) . $target_hashtaghashtag . '/' . ($followings_i['hashtaghashtag'] == $target_hashtaghashtag ? 'start' : $followings_i['hashtaghashtag']) . '">' . view_hashtag_title($followings_i, true) . '</a>';
            ...
            if (count($query_subset) >= 2) {
                ...
                foreach ($query_subset as $hashtag_subset) {
                    ...
                    if (count($this->Chains->read(array(
                        ...
                        $breadcrum_content .= '<a href="' . view_memory(42903, 30795) . $target_hashtaghashtag . '/' . $hashtag_subset['hashtaghashtag'] . '" class="dropdown-item ' . (in_array($hashtag_subset['hashtagid'], $main_branch) ? ' active ' : '') . '">' . view_hashtag_title($hashtag_subset, true) . '</a>';
                    } else {
                        //Locked
