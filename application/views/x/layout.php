...
//HEADER
echo '<div style="position: relative; display: block; padding: 8px 0; margin-top:13px;">' . $progress_bar . '<h1 class="block-one"><span class="icon-block top-icon '.( $show_percentage ? '' : ' thin-top ' ).'">'.view_icon_i_x( $completion_rate['completion_percentage'], $i_focus, 13757 /* Current Idea */ ).'</span><span class="title-block-lg '.( $show_percentage ? ' title-block-squeeze ' : '' ).'">' . view_i_title($i_focus) . '</span></h1>'.'</div>';
...
if(!count($sitemap_items) && $in_my_x){
    if($i_completion_percentage<100){
        ...
        //Not Yet Completed
        echo '<div class="alert no-margin"><span class="icon-block"><i class="fas fa-lock"></i></span><span class="title-block">Navigation will unlock once 100% done. You\'re '.$i_completion_percentage.'% done. Tap <i class="fas fa-step-forward black"></i> (below) to continue.</span></div>';
...
if($in_my_x){
    ...
    foreach($this->config->item('e___13289') as $e__id => $m) {
        ...
        if($e__id==12896 && count($sitemap_items)){
            ...
            $control_btn = '<a class="grey-bg" href="javascript:void(0);" onclick="x_save('.$i_focus['i__id'].')" title="'.$m['m_title'].'"><span class="controller-nav toggle_saved '.( $is_saved ? '' : 'hidden' ).'">'.$e___11035[12896]['m_icon'].'</span><span class="controller-nav toggle_saved '.( $is_saved ? 'hidden' : '' ).'">'.$e___11035[12906]['m_icon'].'</span></a>';

        } elseif($e__id==12991 && !$i_completed && count($sitemap_items)){

            //GO BACK
            $control_btn = '<a class="controller-nav grey-bg" href="'.( isset($_GET['previous_x']) && $_GET['previous_x']>0 ? '/'.$_GET['previous_x'] : ( $previous_level_id > 0 ? '/x/x_previous/'.$previous_level_id.'/'.$i_focus['i__id'] : home_url() ) ).'" title="'.$m['m_title'].'">'.$m['m_icon'].'</a>';
