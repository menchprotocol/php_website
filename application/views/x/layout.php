...
foreach($this->config->item('e___'.$tab_group) as $x__type => $m){
    ...
    $tab_pills .= '<li class="nav-item"><a '.$href.' class="nav-x tab-nav-'.$tab_group.' tab-head-'.$x__type.' '.( $default_active ? ' active ' : '' ).extract_icon_color($m['m_icon']).'" title="'.$m['m_title'].( $m['m_message'] ? ' '.$m['m_message'] : '' ).'" data-toggle="tooltip" data-placement="top">&nbsp;'.$m['m_icon'].'&nbsp;'.( $pre_fix ? '<span class="show-max-active">'.$pre_fix.'</span>' : '' ).( !$counter ? '' : '<span class="en-type-counter-'.$x__type.'">'.$counter.'</span>&nbsp;' ).'</a></li>';
