...
if(isset($bootcamp['runs']) && count($bootcamp['runs'])>0){
    ...
    foreach($bootcamp['runs'] as $cohort){
        echo '<a href="/console/'.$bootcamp['c_id'].'/cohorts/'.$cohort['r_id'].'" class="list-group-item"><span class="pull-right"><span class="label label-primary"><i class="fa fa-chevron-right" aria-hidden="true"></i></span></span>';
            ...
            if($cohort['r_pace_id']>0){
                echo '<i class="fa fa-clock-o" aria-hidden="true"></i> '.$r_pace_options[$cohort['r_pace_id']]['p_hours'].' &nbsp; ';
            }
