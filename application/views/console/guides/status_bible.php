...
foreach($core_objects as $object_id=>$co){
    ...
    if($statuses){
        ...
        foreach($statuses as $intval=>$status){
            echo '<p style="padding-left:10px;"><span style="width:30px; display:inline-block;">'.$intval.'</span>'.status_bible($object_id,$intval,0,'right').( isset($status['s_desc']) ? ' '.$status['s_desc'] : '').'</p>';
        }
    }
	echo '<br />';
}
