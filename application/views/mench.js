...
function load_editor(){
    ...
    $('.in_quick_search').on('autocomplete:selected', function (event, suggestion, dataset) {
        ...
        templates: {
            ...
            empty: function (data) {
                return '<div class="not-found"><i class="fad fa-exclamation-triangle"></i> No Notes Found</div>';
            },
        }
    }]);
}
...
function go_to_read(in_id){
    ...
    if( parseInt($('.dropd_4737_'+in_id+'_0').attr('selected-val')) in js_en_all_7355 ){
        ...
        //No, give them option:
        var r = confirm("You can only read this note once its published. Navigate to reading list?");
        if (r == true) {
...
function echo_js_suggestion(alg_obj){
    ...
    var obj_type = ( parseInt(alg_obj.alg_obj_is_in) ? 'note' : 'source' );
    var is_published = ( parseInt(alg_obj.alg_obj_status) in ( parseInt(alg_obj.alg_obj_is_in) ? js_en_all_7355 : js_en_all_7357 ));
    var obj_icon = ( parseInt(alg_obj.alg_obj_is_in) ? '<i class="fas fa-circle '+( js_session_superpowers_assigned.includes(10939) ? 'note' : 'read' )+'"></i>' : alg_obj.alg_obj_icon );
...
$(document).ready(function () {
    ...
    $("#mench_search").on('autocomplete:selected', function (event, suggestion, dataset) {
        ...
        if (parseInt(suggestion.alg_obj_is_in)==1) {
            window.location = "/" + ( js_session_superpowers_assigned.includes(10939) ? 'note/' : '' ) + suggestion.alg_obj_id;
        } else {
                ...
                if (( search_only_in || search_only_source ) && !isNaN($("#mench_search").val().substr(1)) ) {
                    ...
                    if(js_pl_id > 0){
                        ...
                        if(search_only_source || search_only_in){
                            ...
                            //Guest can search notes only by default as they start typing;
                ...
                empty: function (data) {
                    if(validURL(data.query)){
                        ...
                        if(isNaN($("#mench_search").val().substr(1))){
                            return '<div class="not-found"><span class="icon-block"><i class="fad fa-exclamation-triangle"></i></span>No NOTE found</div>';
                        }
                    } else if($("#mench_search").val().charAt(0)=='@'){
                        if(isNaN($("#mench_search").val().substr(1))) {
