    ...
    if(parseInt(js_en_all_6404[12678]['m_desc'])){
        $('.en_quick_search').on('autocomplete:selected', function (event, suggestion, dataset) {
            ...
            templates: {
                ...
                empty: function (data) {
                    return '<div class="not-found"><i class="fad fa-exclamation-circle"></i> No Sources Found</div>';
                },
            }
        }]);
        ...
        $('.in_quick_search').on('autocomplete:selected', function (event, suggestion, dataset) {
            ...
            templates: {
                ...
                empty: function (data) {
                    return '<div class="not-found"><i class="fad fa-exclamation-circle"></i> No Ideas Found</div>';
                },
            }
        }]);
    }
...
function discover_in_history(tab_group_id, note_in_id, owner_en_id, last_loaded_ln_id){
    ...
    $.post("/discover/discover_in_history/"+tab_group_id+"/"+note_in_id+"/"+owner_en_id+"/"+last_loaded_ln_id, { }, function (data) {
        if (data.status) {
            ...
            $(load_class).html('<b class="discover"><span class="icon-block"><i class="fad fa-exclamation-circle"></i></span>' + data.message + '</b>');
        }
                    ...
                    empty: function (data) {
                        if(validURL(data.query)){
                            ...
                            if(isNaN($("#mench_search").val().substr(1))){
                                return '<div class="not-found"><span class="icon-block"><i class="fad fa-exclamation-circle"></i></span>No IDEA found</div>';
                            }
                        } else if($("#mench_search").val().charAt(0)=='@'){
                            if(isNaN($("#mench_search").val().substr(1))) {
                                return '<div class="not-found"><span class="icon-block"><i class="fad fa-exclamation-circle"></i></span>No SOURCE found</div>';
                            }
                        } else {
                            return '<div class="not-found suggestion"><span class="icon-block"><i class="fad fa-exclamation-circle"></i></span>No results found</div>';
                        }
                    },
                }
            }
...
function en_fetch_canonical_url(query_string, not_found){
    ...
    return ( not_found ? '<div class="not-found"><i class="fad fa-exclamation-circle"></i> URL not found</div>' : '');
}
