...
function load_leaderboard(){
    //Show loading icon:
    $('#load_leaderboard').html('<div><span class="icon-block"><i class="far fa-yin-yang fa-spin play"></i></span></div>');
                    ...
                    if(js_pl_id > 0){
                        ...
                        if(search_only_play || search_only_idea){
                            ...
                            if(search_only_play && js_session_superpowers_assigned.includes(10967)){
                                ...
                                search_filters += ' ( alg_obj_is_in = '+( search_only_idea ? '1' : '0' )+' AND ( _tags:is_featured OR _tags:alg_author_' + js_pl_id + ' )) ';
                            }
                        ...
                        if(search_only_play || search_only_idea){
                            ...
                            search_filters += ' ( alg_obj_is_in = '+( search_only_idea ? '1' : '0' )+' AND _tags:is_featured ) ';
...
var update_coin_counter = function( ) {
    ...
    $.post("/play/update_coin_counter", { }, function (data) {
        ...
        //PLAY
        if(data.play_count != $('td.play .current_count').html()){
            $('td.play .current_count').html(data.play_count).fadeOut(fadeout_speed).fadeIn(fadeout_speed);
        }

        //READ
        if(data.read_count != $('td.read .current_count').html()){
            $('td.read .current_count').html(data.read_count).fadeOut(fadeout_speed).fadeIn(fadeout_speed);
        }

        //IDEA
        if(data.idea_count != $('td.idea .current_count').html()){
            $('td.idea .current_count').html(data.idea_count).fadeOut(fadeout_speed).fadeIn(fadeout_speed);
        }
