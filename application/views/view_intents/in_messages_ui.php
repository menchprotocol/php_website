...
<div id="intent_messages<?= $in_id ?>">
    ...
    foreach ($en_all_4485 as $tr_type_entity_id => $m) {
        ...
        if(in_array(4985, $en_all_4485[$tr_type_entity_id]['m_parents'])){
            echo '<span class="' . fn___echo_advance() . '"><i class="fas fa-hashtag"></i> <span data-toggle="tooltip" class="underdot" title="You can reference up to 1 parent intent using the # sign" data-placement="bottom">Intent Reference</span> &nbsp;</span>';
        }
        ...
        if(count($completion_requirements) == 1){
            ...
            //No Requirements:
            echo '<span class="' . fn___echo_advance() . '"><i class="fas fa-asterisk"></i> <span data-toggle="tooltip" title="You can add any message type including text, URLs, videos, audios, etc..." data-placement="bottom" class="underdot">Supports all formats</span></span>';
        }
...
<div style="margin-top:-7px;">
    ...
    ///firstname
    echo '<a href="javascript:fn___add_first_name();" class="textarea_buttons ' . fn___echo_advance() . ' remove_loading" style="float:right;" data-toggle="tooltip" title="Replaced with master\'s First Name for a more personal message." data-placement="left"><i class="fas fa-fingerprint"></i> /firstname</a>';
