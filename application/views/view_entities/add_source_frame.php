    ...
    <div class="url-parsed hidden">
        ...
        <?php for($contributor_num = 1; $contributor_num <= 5; $contributor_num++){ ?>
            ...
            <span class="inline-block en_role_<?= $contributor_num ?> hidden">
                <input style="padding-left:3px;" type="text" id="auth_role_<?= $contributor_num ?>" class="form-control border" data-toggle="tooltip" data-placement="top" title="Define the role of this person/organization" placeholder="Contributor's Role..." maxlength="<?= $this->config->item('tr_content_max_length') ?>" value="" />
            </span>
            ...
            <div class="form-group explain_expert_<?= $contributor_num ?> label-floating is-empty hidden inline-box" style="margin:0px 19px 10px 10px;">
                ...
                <div class="below-counter">[<span id="char_count_<?= $contributor_num ?>">0</span>/<?= $this->config->item('tr_content_max_length') ?>]</div>
        ...
        foreach($this->config->item('en_all_3000') as $en_id => $m ){
            ...
            <div class="extra_info_'.$en_id.' hidden" style="display:'.( in_array(4255 , $m['m_parents']) ? 'block' : 'none' ).';">
                <span class="mini-header" style="margin: 0 0 2px 30px;">'.(  in_array(4255 , $m['m_parents']) ? $en_all_4255[$en_id]['m_desc'] : '' ).':</span>
