...
foreach($cron_jobs as $cron_job){
    if(strlen($cron_job['linktext'])){
        echo str_pad($cron_job['linktext'], $longest_time, " ", STR_PAD_RIGHT) . '/usr/bin/php /var/www/platform/index.php controller load '.str_pad($cron_job['playerid'], $longest_id, " ", STR_PAD_RIGHT).' #'.$cron_job['playertext']."\n";
    }
}
echo '</textarea>';