...
foreach($cron_jobs as $cron_job){
    if(strlen($cron_job['link_text'])){
        echo str_pad($cron_job['link_text'], $longest_time, " ", STR_PAD_RIGHT) . ' cronjobs.php' .view__memory(6404,7274).' '.str_pad($cron_job['e__id'], $longest_id, " ", STR_PAD_RIGHT).' #'.$cron_job['e__title']."\n";
    }
}
echo '</textarea>';