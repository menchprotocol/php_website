...
echo '<div class="container">';
    ...
    echo '<h1 class="inline-block ispink"><span class="icon-block-lg en-icon"><i class="fas fa-atlas ispink"></i></span> READ HISTORY</h1>';
...
echo '</div></td>';
    ...
    if(isset($_GET['ln_type_player_id']) && substr_count($_GET['ln_type_player_id'], ',')>0){
        ...
        if(isset($_GET['ln_creator_player_id'])) {
            ...
            $all_link_count = 0;
            $select_ui = '';
            foreach ($this->READ_model->ln_fetch($ini_filter, array('en_type'), 0, 0, array('en_name' => 'ASC'), 'COUNT(ln_type_player_id) as total_count, SUM(ABS(ln_words)) as total_words, en_name, ln_type_player_id', 'ln_type_player_id, en_name') as $ln) {
                //Echo drop down:
