    ...
    if($is_home_page){
        ...
        if ($idea_stats['ideas_average']) {
            ...
            if($diff_idea || $diff_time){
                echo '<p class="space-content">Depending on your answers it would take '.( $diff_time ? view_time_hours($idea_stats['duration_min']).' - ' : '' ).view_time_hours($idea_stats['duration_max']).' to read '.( $diff_idea ? 'between ' : '' ).'<span class="inline-block">'.( $diff_idea ? number_format($idea_stats['ideas_min'], 0).' - ' : '' ).number_format($idea_stats['ideas_max'], 0).'</span> key idea'.view__s($idea_stats['ideas_max']).':</p>';
            }
