...
//READ TITLE
echo '<h1 class="block-one" '.( !$recipient_en['en_id'] ? ' style="padding-top: 21px;" ' : '' ).'><span class="icon-block top-icon">'.( $completion_rate['completion_percentage']==100 ? '<i class="fas fa-circle read"></i>' : '<i class="fas fa-circle idea"></i>' ).'</span><span class="title-block-lg">' . echo_in_title($in) . '</span></h1>';
...
if(!$read_in_home){
    ...
    if($all_child_featured){
        ...
        if ($has_time || $has_idea || $chapters) {
            ...
            echo '<div class="read-topic"><a href="javascript:void(0);" onclick="$(\'.contentTabIdeas\').toggleClass(\'hidden\')" class="doupper"><span class="icon-block"><i class="far fa-plus-circle contentTabIdeas"></i><i class="far fa-minus-circle contentTabIdeas hidden"></i></span>'.( $has_idea ? $metadata['in__metadata_max_steps'].' Idea'.echo__s($metadata['in__metadata_max_steps']) : '' ).( $chapters && ( !isset($metadata['in__metadata_max_steps']) || $chapters<$metadata['in__metadata_max_steps'] ) ? ( $has_idea ? ' Across ' : '').$chapters.( $has_idea ? ' Topic' : ' Idea' ).echo__s($chapters) : '').( $has_time || $chapters ? ( $has_idea ? ' <i class="fas fa-clock"></i> ' : '' ).echo_time_range($metadata) : '' ).'</a></div>';
