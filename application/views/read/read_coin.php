    ...
    if($is_home_page){
        ...
        if ($idea_stats['ideas_average']) {
            ...
            if($diff_idea || $diff_time){
                echo '<p class="space-content">It takes '.( $diff_time ? 'about '.view_time_hours($idea_stats['duration_min']).' - ' : '' ).view_time_hours($idea_stats['duration_max']).' to read '.( $diff_idea ? number_format($idea_stats['ideas_min'], 0).' - ' : '' ).number_format($idea_stats['ideas_max'], 0).' key idea'.view__s($idea_stats['ideas_max']).' based on your choices along the way:</p>';
            }
