...
foreach ($this->Chains->read($filters, array('chainhashtagoutput'), 0) as $expires) {
    ...
    foreach ($this->Chains->read(array(
        ...
        if (!count($answer_completed) && intval($expires['chainvalue']) > 0 && $seconds_left <= 0) {
            ...
            if ($deleted) {
                $chains_deleted++;
                echo '<div style="padding-left: 21px;">' . $chains_deleted . ') <a href="' . view_memory(42903, 42902) . $x_progress['handlestring'] . '">' . $x_progress['handlename'] . '</a>: ' . $x_progress['chaintime'] . ' ? ' . $x_progress['chainvalue'] . ' / <a href="' . view_app_chain(12722) . '?chainid=' . $x_progress['chainid'] . '">' . $x_progress['chainid'] . ' / Answer: ' . count($answer_completed) . '</a> ' . (!count($answer_completed) ? ($seconds_left <= 0 ? ' DELETE ' : '[' . $seconds_left . '] SEcs left') : '') . ' (' . intval($expires['chainvalue']) . '+' . $buffer_time . '-' . time() . '-' . strtotime($x_progress['chaintime']) . ' = ' . $seconds_left . ')</div>';
            }
