...
class Idea extends CI_Controller {
    ...
    function in_update_text(){
        ...
        if (!$session_en) {
            ...
            if(!count($ins)){
                ...
                return echo_json(array(
                    'status' => 0,
                    'message' => $en_all_12112[$_POST['cache_en_id']]['m_name'].' must be between '.config_var(12427).' - '.config_var(12113).' Seconds. If larger, break ideas down into into smaller ideas.',
                    'original_val' => $ins[0]['in_read_time'],
                ));
    ...
    function in_READ_BOOKMARKS(){
        ...
        foreach($actionplan_users as $apu){
            ...
            $item_ui .= '&nbsp;<a href="/oii?ln_owner_play_id='.$apu['en_id'].'" data-toggle="tooltip" data-placement="top" title="Full User History"><i class="fas fa-link"></i></a>';
