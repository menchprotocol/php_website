...
class Miner_app extends CI_Controller
{
    ...
    function extra_stats_intents(){
        ...
        foreach($in_verbs as $count => $verb){
            ...
            echo '<td style="text-align: right;"><a href="/links?ln_type_entity_id=4250&in_status_entity_id=' . join(',', $this->config->item('en_ids_7356')) . '&in_verb_entity_id='.$verb['in_verb_entity_id'].'">'.number_format($verb['totals'],0).'</a><i class="fas fa-info-circle icon-block" data-toggle="tooltip" title="Intent'.echo__s($verb['totals']).' starting with '.$verb['en_name'].'" data-placement="top"></i></td>';
            echo '</tr>';
        ...
        foreach ($this->config->item('en_all_4737') as $en_id => $m) {
            ...
            echo '<td style="text-align: right;" class="'.( $en_id==6182 ? 'is-removed' : '' ).'">' . '<a href="/links?in_status_entity_id=' . $en_id . '&ln_type_entity_id=4250">' . number_format($objects_count[0]['totals'],0) . '</a>' . '<i class="fas fa-info-circle icon-block" data-toggle="tooltip" title="'. $m['m_desc'] . '" data-placement="top"></i>' . '</td>';
            echo '</tr>';
    ...
    function extra_stats_entities(){
        ...
        foreach ($this->config->item('en_all_3000') as $en_id => $m) {
            ...
            foreach($this->config->item('en_all_7358') /* Entity Active Statuses */ as $en_status_entity_id => $m_status){
                ...
                //Display row:
                $expert_source_statuses .= '<td style="text-align: right;"'.( $en_status_entity_id != 6181 /* Entity Featured */ && 0 ? ' class="' . advance_mode() . '"' : '' ).'><a href="/entities/' . $en_id .'#status-'.$en_status_entity_id.'">'.number_format($source_count,0).'</a><i class="fas fa-info-circle icon-block" data-toggle="tooltip" title="'  . $m['m_name'] . ' that are ' . $en_all_6177[$en_status_entity_id]['m_desc'] . '" data-placement="top"></i></td>';
        ...
        foreach($this->config->item('en_all_7358') /* Entity Active Statuses */ as $en_status_entity_id => $m_status){
            echo '<td style="text-align: right;" '.( $en_status_entity_id != 6181 && 0 /* Entity Featured */ ? ' class="' . advance_mode() . '"' : '' ).'>' . number_format($total_total_counts[$en_status_entity_id], 0) . '<i class="fas fa-info-circle icon-block" data-toggle="tooltip" title="'.$en_all_6177[$en_status_entity_id]['m_name'] . ' '.$en_all_7303[3000]['m_name'].'" data-placement="top"></i>' . '</td>';
        }
        echo '</tr>';
        ...
        foreach ($this->config->item('en_all_6177') as $en_id => $m) {
            ...
            echo '<tr>';
            echo '<td style="text-align: left;"><span class="icon-block">' . $m['m_icon'] . '</span><a href="/entities/'.$en_id.'">' . $m['m_name'] . '</a></td>';
            echo '<td style="text-align: right;" class="'.( $en_id==6178 ? 'is-removed' : '' ).'">' . '<a href="/links?en_status_entity_id=' . $en_id . '&ln_type_entity_id=4251">' . number_format($objects_count[0]['totals'], 0) . '</a>' .'<i class="fas fa-info-circle icon-block" data-toggle="tooltip" title="'.$m['m_desc'] . '" data-placement="top"></i>' . '</td>';
            echo '</tr>';
    ...
    function load_leaderboard($user_group_en_id, $time_group_en_id){
        ...
        if(count($leaderboard_ens) > 0){
            foreach ($leaderboard_ens as $count=>$ln) {
                ...
                echo '<td style="text-align: right;"><a href="/links?ln_creator_entity_id='.$ln['ln_creator_entity_id'].( $start_date ? '&start_range='.$start_date : $start_date ).'">'.number_format($ln['credits_sum'], 0).'</a><i class="fas fa-info-circle icon-block" data-toggle="tooltip" title="'.$ln['en_name'].' credits for '.number_format($ln['links_count'],0).' links averaging '.round(($ln['credits_sum']/$ln['links_count']),1).' credits/link" data-placement="top"></i></td>';
                echo '</tr>';
    ...
    function extra_stats_links(){
        ...
        foreach ($this->config->item('en_all_6186') as $en_id => $m) {
            ...
            echo '<i class="fas fa-info-circle icon-block" data-toggle="tooltip" title="' . $m['m_desc'] . '" data-placement="top"></i>';
            echo '</td>';
            echo '</tr>';
        }
