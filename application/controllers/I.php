...
class I extends CI_Controller {
    ...
    function i_layout($i__id){
        ...
        if(!$member_e){
            if($is_public){
                ...
                return redirect_message(home_url(), '<div class="msg alert alert-danger" role="alert"><span class="icon-block"><i class="fas fa-exclamation-circle"></i></span>IDEA #' . $i__id . ' is not published yet.</div>', true);
            }
        }
    ...
    function i_set_dropdown(){
        ...
        if($_POST['x__id'] > 0){
            ...
            ), $member_e['e__id'], end($x_update_types));
            ...
            if($_POST['element_id']==4737){
                ...
                if(!in_array($_POST['new_e__id'], $this->config->item('n___7356'))){
                    ...
                    $this->I_model->remove($_POST['i__id'] , $member_e['e__id']);
