        ...
        if(count($blog_coins) > 0){
            foreach ($blog_coins as $count=>$ln) {
                ...
                if($count==$show_max){
                    ...
                    echo '<tr class="see_more_who"><td colspan="3"><span class="parent-icon icon-block-sm"><i class="fas fa-search-plus"></i></span><a href="javascript:void(0);" onclick="$(\'.see_more_who\').toggleClass(\'hidden\')"><b>SEE TOP '.$load_max.'</b></a></td></tr>';
                }
                if($ln['total_words'] < 1){
                    continue;
                }
