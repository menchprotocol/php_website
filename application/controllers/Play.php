...
class Play extends CI_Controller
{
    ...
    function add_11158(){
        ...
        $ln_player_play_id = 1; //Shervin as Developer for logging all READS
            ...
            foreach ($this->READ_model->ln_fetch(array(
                ...
                foreach($unique_authors as $author_handler){
                    ...
                    if(!count($already_added)){
                        ...
                        //Create new Player:
                        $added_en = $this->PLAY_model->en_verify_create($author_handler, $ln_player_play_id, 6181, random_player_avatar());
  ...
  __typename
        ...
        */
        ...
        foreach ($blog_player_coins as $count=>$ln) {
            ...
            if($count==$show_max){
                ...
                echo '<tr class="see_more_who"><td colspan="3"><span class="icon-block"><i class="far fa-search-plus play"></i></span><a href="javascript:void(0);" onclick="$(\'.see_more_who\').toggleClass(\'hidden\')"><b class="montserrat play" style="text-decoration: none !important;">TOP '.$load_max.' PLAYERS</b></a></td></tr>';
            ...
            //PLAY
            echo '<td class="play fixedColumns MENCHcolumn1"><span class="icon-block icon_en_'.$ln['en_id'].'">'.echo_en_icon($ln['en_icon']).'</span>'.'<a href="/play/'.$ln['en_id'].'" class="montserrat play title-block en_name_full_'.$ln['en_id'].'">'.$ln['en_name'].'</a>'.echo_rank($count+1).'</td>';
        ...
        if($show_readers > 0){
            ...
            foreach ($read_coins as $count=>$ln) {
                ...
                //PLAY
                echo '<td class="play fixedColumns MENCHcolumn1"><span class="icon-block">'.echo_en_icon($ln['en_icon']).'</span><a href="/play/'.$ln['en_id'].'" class="montserrat play title-block en_name_full_'.$ln['en_id'].'">'.$ln['en_name'].'</a>'.echo_rank($count+1).'</td>';
    ...
    function sign_reset_password_apply()
    {
        ...
        if (!isset($_POST['ln_id']) || intval($_POST['ln_id']) < 1 || !isset($_POST['input_email']) || strlen($_POST['input_email']) < 1 || !isset($_POST['input_password'])) {
            ...
            //Validate user:
            $ens = $this->PLAY_model->en_fetch(array(
                'en_id' => $validate_links[0]['ln_player_play_id'],
            ));
            if(count($ens) < 1){
                return echo_json(array(
                    'status' => 0,
    ...
    function sign_create_account(){
        ...
        */
        ...
        $this->READ_model->ln_create(array(
            ...
            'ln_player_play_id' => $user_en['en']['en_id'],
    ...
    function magicemail(){
        ...
        $reset_link = $this->READ_model->ln_create(array(
            'ln_type_play_id' => 7563, //User Signin Magic Link Email
            'ln_content' => $_POST['input_email'],
            'ln_player_play_id' => $user_emails[0]['en_id'], //User making request
            'ln_parent_blog_id' => intval($_POST['referrer_in_id']),
        ));
