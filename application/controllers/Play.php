        ...
        */
        ...
        if(count($blog_coins) > 0){
            foreach ($blog_coins as $count=>$ln) {
                ...
                //PLAY
                echo '<td><span class="parent-icon icon-block">'.echo_en_icon($ln).'</span><b class="montserrat blue">'.( $session_en ? '<a href="/play/'.$ln['en_id'].'" class="blue">'.$first_name.'</a>' : $first_name ).'</b></td>';
                ...
                //BLOG
                echo '<td>'.( $session_en ? '<a href="/read/history?ln_status_entity_id='.join(',', $this->config->item('en_ids_7359')) /* Link Statuses Public */.'&ln_type_entity_id='.join(',', $this->config->item('en_ids_10589')).'&ln_creator_entity_id='.$ln['en_id'].( $start_date ? '&start_range='.$start_date : $start_date ).'" class="montserrat blog"><span class="parent-icon icon-block">'.$en_all_2738[4535]['m_icon'].'</span>'.echo_number($ln['total_words']).'</a>' : '<span class="montserrat blog"><span class="parent-icon icon-block">'.$en_all_2738[4535]['m_icon'].'</span>'.echo_number($ln['total_words']).'</span> '.echo_rank($count+1)  ).'</td>';
                echo '</tr>';
            ...
            foreach (array(
                ...
                echo '<td valign="bottom" style="width:33%"><span class="'.$handle.' border-'.$handle.'"><span class="parent-icon icon-block">' . $en_all_2738[$en_id]['m_icon'] . '</span><span class="montserrat current_count">'.$current_count.'</span> '.( $en_id==4536 ? '<span class="montserrat">PLAYERS</span>' : '' ).'</span></td>';
            }
            echo '</tr>';
