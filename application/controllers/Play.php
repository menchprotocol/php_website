...
class Play extends CI_Controller
{
    ...
    function play_modify($en_id)
    {
        ...
        if ($en_id == 0) {
            return redirect_message('/play/' . $session_en['en_id']);
        }
    ...
    function leaderboard($choose_10591){
        ...
        $filters = array(
            ...
            'ln_type_entity_id IN (' . join(',', $this->config->item('en_ids_'.$choose_10591)) . ')' => null, //PLAY DIRECTIONS
        ...
        */
        ...
        if(count($leaderboard_ens) > 0){
            ...
            if(!$start_date){
                ...
                //Fetch stats:
                $word_stats = $this->Links_model->ln_fetch(array(
                    'ln_type_entity_id IN (' . join(',', $this->config->item('en_ids_'.$choose_10591)) . ')' => null, //PLAY DIRECTIONS
                    'ln_status_entity_id IN (' . join(',', $this->config->item('en_ids_7359')) . ')' => null, //Link Statuses Public
                ), array(), 0, 0, array(), 'SUM(ln_words) as total_words');

                echo '<tr>';
                echo '<td style="text-align: left;"><span class="parent-icon icon-block"><i class="fas fa-asterisk"></i></span>ALL '.$en_all_10591[$choose_10591]['m_name'].'</td>';
                echo '<td style="text-align: right;"><a href="/links?ln_status_entity_id='.join(',', $this->config->item('en_ids_7359')) /* Link Statuses Public */.'&ln_type_entity_id='.join(',', $this->config->item('en_ids_'.$choose_10591)).( $start_date ? '&start_range='.$start_date : $start_date ).'" class="mono">'.number_format(abs($word_stats[0]['total_words']), 0).'</a></td>';
                echo '</tr>';
    ...
    function en_add_source_paste_url()
    {
        ...
        $url_entity = $this->Entities_model->en_sync_url($_POST['input_url']);
    ...
    function en_load_next_page()
    {
        ...
        $child_entities_count = $this->Links_model->ln_fetch($filters, array('en_child'), 0, 0, array(), 'COUNT(ln_id) as totals');
    ...
    function en_add_or_link()
    {
        ...
        if (intval($_POST['en_existing_id']) > 0) {
            ...
            if (filter_var($_POST['en_new_string'], FILTER_VALIDATE_URL)) {
                ...
                $url_entity = $this->Entities_model->en_sync_url($_POST['en_new_string']);
                if (!$url_entity['status']) {
                ...
                if($url_entity['url_is_root']){
                    ...
                    $domain_entity = $this->Entities_model->en_sync_domain($_POST['en_new_string'], $session_en['en_id']);
                ...
                //Create entity:
                $added_en = $this->Entities_model->en_verify_create($_POST['en_new_string'], $session_en['en_id']);
                if(!$added_en['status']){
        ...
        $ens_latest = $this->Entities_model->en_fetch(array(
    ...
    function en_modify_save()
    {
        ...
        if (!in_array($en_update['en_status_entity_id'], $this->config->item('en_ids_7358') /* Entity Statuses Active */) && !($en_update['en_status_entity_id'] == $ens[0]['en_status_entity_id'])) {
            ...
            if (strlen($_POST['en_merge']) > 0) {
                ...
                if ($merger_en_id < 1) {
                    ...
                    $merged_ens = $this->Entities_model->en_fetch(array(
                        'en_id' => $merger_en_id,
                        'en_status_entity_id IN (' . join(',', $this->config->item('en_ids_7358')) . ')' => null, //Entity Statuses Active
                    ));
            ...
            $links_adjusted = $this->Entities_model->en_unlink($_POST['en_id'], $session_en['en_id'], $merger_en_id);
        ...
        //Now update the DB:
        $this->Entities_model->en_update(intval($_POST['en_id']), $en_update, true, $session_en['en_id']);
    ...
    function en_fetch_canonical_url(){
        ...
        //Fetch URL:
        $url_entity = $this->Entities_model->en_sync_url($_POST['search_url']);
    ...
    function en_add_source_process()
    {
        ...
        for ($contributor_num = 1; $contributor_num <= 5; $contributor_num++) {
            ...
            if ($ln_en_link_id > 0) {
                ...
                $referenced_ens = $this->Entities_model->en_fetch(array(
                    'en_status_entity_id IN (' . join(',', $this->config->item('en_ids_7358')) . ')' => null, //Entity Statuses Active
                    'en_id' => $ln_en_link_id,
                ));
                ...
                //First analyze URL:
                $contributor_url_entity = $this->Entities_model->en_sync_url($_POST['ref_url_' . $contributor_num]);
