...
class X extends CI_Controller
{
    ...
    function x_type_preview()
    {
        ...
        $has_preview = preview_x__message($_POST['x__message'], $detected_x_type['x__type'], null, true);

        return view_json(array(
            'status' => 1,
            'x__type_preview' => '<b class="css__title">' . $e___4592[$detected_x_type['x__type']]['m__cover'] . ' ' . $e___4592[$detected_x_type['x__type']]['m__title'] . '</b>',
            'x__message_preview' => ( $has_preview && in_array($detected_x_type['x__type'], $this->config->item('n___12524')) ? '<span class="paddingup">' . $has_preview . '</span>' : '' ),
