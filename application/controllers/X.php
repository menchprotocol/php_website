...
class X extends CI_Controller
{
    ...
    function x_load(){
        ...
        if($total_items_loaded > 0){
            ...
            if($page_num==1){
                ...
                $message .= '<div class="left-margin montserrat x-info">'.number_format($x_count[0]['total_count'] , 0).' Transactions index <span class="montserrat source inline-block">'.$e___12467[12273]['m_icon'].' '.number_format($this->config->item('s___12273'), 0).' '.$e___12467[12273]['m_name'].'</span> from <span class="montserrat idea inline-block">'.$e___12467[12274]['m_icon'].' '.number_format($this->config->item('s___12274'), 0).' '.$e___12467[12274]['m_name'].'</span> with <span class="montserrat discover inline-block">'.$e___12467[6255]['m_icon'].' '.number_format($this->config->item('s___6255'), 0).' '.$e___12467[6255]['m_name'].'</span>. Here are the last '.$total_items_loaded.' transactions:</div>';
            } else {
                //Subsequent messages:
    ...
    function x_upload()
    {
        ...
        $cdn_status = upload_to_cdn($temp_local, $session_e['e__id'], $_FILES[$_POST['upload_type']], true, $is[0]['i__title'].' BY '.$session_e['e__title']);
