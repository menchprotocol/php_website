...
class Intents extends CI_Controller
{
    ...
    function in_report_conditional_steps(){
        ...
        return echo_json(array(
            'status' => 1,
            'message' => '<h3>'.$en_all_7585[$ins[0]['in_completion_method_entity_id']]['m_icon'].' '.$en_all_4737[$ins[0]['in_status_entity_id']]['m_icon'].' '.echo_in_outcome($ins[0]['in_outcome'], false, true).'</h3>'.echo_in_answer_scores($_POST['starting_in'], $_POST['depth_levels'], $_POST['depth_levels'], $ins[0]['in_completion_method_entity_id']),
        ));
    ...
    function in_modify_save()
    {
        ...
        if (!$session_en) {
            ...
            return echo_json(array(
                'status' => 0,
                'message' => 'Missing in_level_entity_id',
            ));
        } elseif (count($ins) < 1) {
            return echo_json(array(
                'status' => 0,
        ...
        $in_outcome_validation = $this->Intents_model->in_validate_outcome($_POST['in_outcome'], $_POST['in_level_entity_id']);
