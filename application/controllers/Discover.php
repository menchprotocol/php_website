...
class Discover extends CI_Controller
{
    ...
    function x_set_text(){
        ...
        if (!$session_source) {
            ...
            $this->MAP_model->update($_POST['object__id'], array(
            ...
            //All good, go ahead and update:
            $this->SOURCE_model->update($sources[0]['e__id'], array(
            ...
            if(!count($reads)){
                ...
                //All good, go ahead and update:
                $this->DISCOVER_model->update($_POST['object__id'], array(
    ...
    function x_previous($previous_level_id, $i__id){
        ...
        if(!count($this->MAP_model->fetch(array(
            'i__id' => $current_i__id,
    ...
    function x_clear_coins(){
        ...
        $progress_links = $this->DISCOVER_model->fetch(array(
            'x__status IN (' . join(',', $this->config->item('sources_id_7360')) . ')' => null, //ACTIVE
        ...
        if(count($progress_links) > 0){
            ...
            //Log link:
            $clear_all_link = $this->DISCOVER_model->create(array(
                'x__message' => $message,
                'x__type' => 6415,
                'x__player' => $session_source['e__id'],
            ));
