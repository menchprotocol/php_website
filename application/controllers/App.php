...
class App extends CI_Controller
{
    ...
    function load($app_playerid = 14563 /* Error if none provided */, $focus_handle = 0, $focus_hashtag = 0, $target_hashtag = 0)
    {
        ...
        if (!isset($_SERVER['REMOTE_ADDR']) || $_SERVER['REMOTE_ADDR'] != '73.15.62.97') {
            die('We will be back up shortly... IP ');
        }
        ...
        if (strlen($_GET['ideahashtag'])) {
            ...
            if ($app_playerid == 33286 && $focus_i && $focus_i['ideahashtag'] !== $_GET['ideahashtag']) {
                ...
                return redirect_message(view_memory(42903, 33286) . $focus_i['ideahashtag']);
            }
        }
        ...
        if (isset($_GET['playerhandle']) && strlen($_GET['playerhandle'])) {
            ...
            if ($app_playerid == 42902 && $focus_e && $focus_e['playerhandle'] !== $_GET['playerhandle']) {
                //Adjust URL Case Sensitive:
                return redirect_message(view_memory(42903, 42902) . $focus_e['playerhandle']);
            }
        }
        ...
        if ($memory_detected && !in_array($app_playerid, $this->config->item('playerids___6287'))) {
            //Invalid App:
            return redirect_message(view_memory(42903, 42902) . $players___6287[$app_playerid]['m__handle'], '<div class="alert alert-danger" role="alert">@' . $players___6287[$app_playerid]['m__handle'] . ' Is not an APP, yet 🤔</div>');
            ...
            if (in_array($app_playerid, $this->config->item('playerids___42905')) && !$focus_e) {
                return redirect_message(home_url(), '<div class="alert alert-danger" role="alert"><span class="icon-block"><i class="far fa-exclamation-circle"></i></span>Error: @' . $_GET['playerhandle'] . ' is not a valid Player handle.</div>');
                ...
                if ($_GET['hash'] == view_hash($_GET['time'] . $focus_e['playerhandle'])) {
                    ...
                    if ($focus_i) {
                        if (idea_is_startable($focus_i)) {
                            ...
                            $this->Ledger->mark_complete(idea_discovery_link($focus_i), $focus_e['playerid'], ($target_i ? $target_i['ideaid'] : 0), $focus_i);
                            $this->Ledger->mark_complete(29393, $focus_e['playerid'], ($target_i ? $target_i['ideaid'] : 0), $focus_i);
                    ...
                    if (!$player_e) {
                        $session_data = $this->Nodeplayers->activate_session($player_e, true);
                    }
        ...
        if ($player_http_request && $memory_detected) {
            ...
            if ($player_e && in_array($app_playerid, $this->config->item('playerids___14639'))) {
                //Should redirect them:
                return redirect_message(view_memory(42903, 42902) . $player_e['playerhandle']);
            } elseif (!$player_e && in_array($app_playerid, $this->config->item('playerids___14740'))) {
                //Should redirect them:
            ...
            } elseif (!$skip_idea_privacy_check && $focus_i && !$access_level_i) {
                $missing_access = 'Error: You Cannot Access Focus #' . $focus_i['ideahashtag'] . ' due to Privacy Settings.';
            } elseif (!$skip_idea_privacy_check && $target_i && !$target_access_level_i) {
                ...
                if (!isset($_GET['reset_cache'])) {
                    ...
                    foreach ($this->Ledger->fetch(array(
                        ...
                        if (strtotime($latest_cache['linktime']) <= (time() - view_memory(6404, 14599))) {
                            //Its expired, void it:
                            $this->Ledger->update($latest_cache['linkid'], array());
                        } else {
        ...
        if ($new_cache) {
            $cache_x = $this->Ledger->create(array(
                'linkdomain' => website_setting(0),
                'linktype' => 44179, //Triggered
                'linkup' => 14599, //Cache App
                'linkdown' => $app_playerid,
        ...
        if ($app_playerid == 30795 && $target_i && $focus_i && $player_e && $target_i['ideahashtag'] == $focus_i['ideahashtag']) {
            ...
            } elseif (!$this->Ledger->i_has_started($player_e['playerid'], $target_i['ideahashtag'])) {

                //Not yet started, add to their starting point:
                $completion_status = $this->Ledger->mark_complete(4235, $player_e['playerid'], 0, $target_i);

                //Now return next idea:
                $next__url = $this->Ledger->find_next($player_e['playerid'], $target_i['ideahashtag'], $target_i);

                if ($next__url) {
                    //Go Next:
                    return redirect_message(view_memory(42903, 30795) . $target_i['ideahashtag'] . '/' . $next__url);
                }
    ...
    function i_void()
    {
        ...
        $links_removed = $this->Nodeideas->remove($o__id, $player_e['playerid'], $migrate_s__id);
    ...
    function e_void()
    {
        ...
        if (strlen($migrate_s__handle) > 0) {
            $valid_handle = $this->Nodeplayers->fetch(array(
                'LOWER(playerhandle)' => strtolower($migrate_s__handle),
            ));
            if (!count($valid_handle)) {
                return array(
                    'status' => 0,
                    'message' => '@' . $migrate_s__handle . ' is an Invalid Player Handle!',
                );
            } elseif ($valid_handle[0]['playerid'] == $o__id) {
                return array(
                    'status' => 0,
                    'message' => 'You cannot migrate this Player to itself! Choose a different Player to migrate.',
                );
            }
        ...
        if ($o__id == $focus__id) {
            ...
            if (!$deletion_redirect) {
                foreach ($this->Nodeplayers->fetch(array('playerid' => $o__id)) as $e2) {
                    $deletion_redirect = view_memory(42903, 42902) . e2['playerhandle'];
                }
            }
        ...
        //Delete all transactions:
        $links_removed = $this->Nodeplayers->remove($o__id, $player_e['playerid'], $migrate_s__id);

        //Remove:
        $status = $this->Nodeplayers->update($o__id, array(), true, $player_e['playerid']);
    ...
    function i_editor_save()
    {
        ...
        if (is_null($is[0]['ideatext'])) {
            ...
            if (strlen($_POST['save_ideatext']) && !count($_POST['uploaded_media']) && !substr_count($_POST['save_ideatext'], "\n") && intval($_POST['save_linktype']) && (intval($_POST['next_ideaid']) || intval($_POST['previous_ideaid']))) {
                ...
                if ($all_hashtags && count($idea_references) && $_POST['save_linktype'] > 0) {
                    ...
                    foreach ($this->Nodeideas->fetch(array(
                        ...
                        foreach ($idea_references as $reference_i) {
                            if (intval($_POST['next_ideaid']) > 0) {
                                $status = $this->Nodeideas->i_link($focus_i, $_POST['save_linktype'], $reference_i, $player_e['playerid']);
                            } elseif (intval($_POST['previous_ideaid']) > 0) {
                                $status = $this->Nodeideas->i_link($reference_i, $_POST['save_linktype'], $focus_i, $player_e['playerid']);
                            }
                            if (!$status['status']) {
            ...
            $this->Nodeideas->update($is[0]['ideaid'], array(
        ...
        if ($_POST['save_ideaid'] > 0) {
            for ($p = 1; $p <= view_memory(6404, 42206); $p++) {
                ...
                //Fetch the current value:
                if ($d_linkid > 0) {
                    $values = $this->Ledger->fetch(array(
                        'linkid' => $d_linkid,
                    ));
                }
                ...
                if (!strlen($dynamic_value)) {
                    ...
                    if (count($values) && $dynamic_playerid != 11035 /* HACK: Summary are key links that should not be removed */) {
                        $this->Ledger->update($values[0]['linkid'], array(), $player_e['playerid']);
                    }
                    ...
                    //Update Link:
                    $this->Ledger->update($values[0]['linkid'], array(
                        'linktext' => $dynamic_value,
                    ), $player_e['playerid']);
        ...
        if (strlen($_POST['save_ideahashtag']) && $is[0]['ideahashtag'] !== trim($_POST['save_ideahashtag'])) {
            ...
            foreach ($this->Ledger->fetch(array(
                ...
                'linktype IN (' . join(',', $this->config->item('playerids___42341')) . ')' => null, //Idea References
            ), array('linkright')) as $ref) {
                view_sync_links(str_replace('#' . $is[0]['ideahashtag'], '#' . trim($_POST['save_ideahashtag']), $ref['ideatext']), true, $ref['ideaid']);
            }
        ...
        if ($_POST['save_linkid'] > 0 && $_POST['save_linktext'] != 'IGNORE_INPUT') {
            ...
            foreach ($this->Ledger->fetch(array(
                ...
                if ($this_x['linktext'] != trim($_POST['save_linktext'])) {
                    $this->Ledger->update($this_x['linkid'], array(
        ...
        //Update Links based on save_ideatext / Sync Idea Synonym & Player References links:
        $view_sync_links = view_sync_links($_POST['save_ideatext'], true, $is[0]['ideaid']);
    ...
    function i_copy()
    {
        ...
        return view_json($this->Nodeideas->recursive_clone(intval($_POST['ideaid']), intval($_POST['do_recursive']), $player_e['playerid']));
    ...
    function i_load_cover()
    {
        ...
        if (!isset($_POST['ideaid']) || !isset($_POST['linktype']) || !isset($_POST['first_segment']) || !isset($_POST['counter'])) {
            ...
            if (in_array($_POST['linktype'], $this->config->item('playerids___42376')) && !access_level_i(null, $_POST['ideaid'])) {
                ...
                $ui = '';
                $listed_items = 0;
                if (in_array($_POST['linktype'], $this->config->item('playerids___42261')) || in_array($_POST['linktype'], $this->config->item('playerids___42284'))) {
                    ...
                    foreach (view_idea_covers($_POST['linktype'], $_POST['ideaid'], 1, false) as $player_e) {
                        if (isset($player_e['playerid'])) {
                            $ui .= view_card(view_memory(42903, 42902) . $player_e['playerhandle'], $current_playerhandle && $player_e['playerhandle'] == $current_playerhandle, $player_e['linktype'], view_cover($player_e['playercover'], true), $player_e['playertext'], $player_e['linktext']);
                            $listed_items++;
                        }
                    }

                } elseif (in_array($_POST['linktype'], $this->config->item('playerids___11020'))) {
                    ...
                    foreach (view_idea_covers($_POST['linktype'], $_POST['ideaid'], 1, false) as $next_i) {
                        if (isset($next_i['ideaid'])) {
                            $ui .= view_card($target_disccovery . view_memory(42903, 33286) . $next_i['ideahashtag'], $next_i['ideahashtag'] == $current_ideahashtag, $next_i['linktype'], (in_array($next_i['ideatype'], $this->config->item('playerids___32172')) ? $players___4737[$next_i['ideatype']]['m__cover'] : ''), view_idea_title($next_i, true), $next_i['linktext']);
                            $listed_items++;
                        }
                    }
                ...
                if ($listed_items < $_POST['counter']) {
                    ...
                    foreach ($this->Nodeideas->fetch(array(
                        ...
                        $ui .= view_more($target_disccovery . view_memory(42903, 33286) . $i['ideahashtag'], false, '&nbsp;', '&nbsp;', 'View All');
                    }
                }
    ...
    function i_sort_load()
    {
        ...
        } elseif (!isset($_POST['linktype']) || !in_array($_POST['linktype'], $this->config->item('playerids___4603'))) {
            return view_json(array(
                'status' => 0,
        ...
        foreach ($_POST['new_x_order'] as $linknumber => $linkid) {
            if (intval($linkid) > 0 && intval($linknumber) > 0) {
                ...
                if ($this->Ledger->update(intval($linkid), array(
    ...
    function view_idea_body()
    {
        ...
        if (!isset($_POST['ideaid']) || intval($_POST['ideaid']) < 1 || !isset($_POST['counter']) || !isset($_POST['linktype']) || intval($_POST['linktype']) < 1) {
            ...
            echo view_idea_body($_POST['linktype'], $_POST['counter'], $_POST['ideaid']);
        }
    }
    ...
    function view_player_body()
    {
        ...
        if (!isset($_POST['playerid']) || intval($_POST['playerid']) < 1 || !isset($_POST['linktype']) || intval($_POST['linktype']) < 1) {
            ...
            echo view_player_body($_POST['linktype'], $_POST['counter'], $_POST['playerid'], $_POST['js_request_uri']);
        }
    }
    ...
    function player_load_cover()
    {
        ...
        if (!isset($_POST['playerid']) || !isset($_POST['linktype']) || !isset($_POST['first_segment']) || !isset($_POST['counter'])) {
            ...
            if (in_array($_POST['linktype'], $this->config->item('playerids___42376')) && !access_level_player(null, $_POST['playerid'])) {
                ...
                if (in_array($_POST['linktype'], $this->config->item('playerids___11028'))) {

                    //SOURCES
                    $current_playerhandle = view_valid_handle_player($_POST['first_segment']);
                    $players___4593 = $this->config->item('players___4593'); //Transaction Types

                    foreach (view_player_covers($_POST['linktype'], $_POST['playerid'], 1, false) as $player_e) {
                        if (isset($player_e['playerid'])) {
                            $ui .= view_card(view_memory(42903, 42902) . $player_e['playerhandle'], $player_e['playerhandle'] == $current_playerhandle, $player_e['linktype'], view_cover($player_e['playercover'], true), $player_e['playertext'], $player_e['linktext']);
                            $listed_items++;
                        }
                    }

                } elseif (in_array($_POST['linktype'], $this->config->item('playerids___42261')) || in_array($_POST['linktype'], $this->config->item('playerids___42284'))) {
                    ...
                    foreach (view_player_covers($_POST['linktype'], $_POST['playerid'], 1, false) as $next_i) {
                        if (isset($next_i['ideaid'])) {
                            $ui .= view_card($target_disccovery . view_memory(42903, 33286) . $next_i['ideahashtag'], $next_i['ideahashtag'] == $current_ideahashtag, $next_i['linktype'], (in_array($next_i['ideatype'], $this->config->item('playerids___32172')) ? $players___4737[$next_i['ideatype']]['m__cover'] : ''), view_idea_title($next_i, true), $next_i['linktext']);
                            $listed_items++;
                        }
        ...
        if (!$player_e) {
            ...
            if (count($es) < 1) {
                ...
                foreach ($_POST['new_linknumber'] as $rank => $linkid) {
                    $this->Ledger->update($linkid, array(
    ...
    function link_unlink()
    {
        ...
        //Archive Transaction:
        $this->Ledger->update($_POST['linkid'], array(), $player_e['playerid']);
    ...
    function e_copy()
    {
        ...
        //Create:
        $added_e = $this->Nodeplayers->create($_POST['copy_player_title'], $player_e['playerid'], $fetch_o[0]['playercover']);
        if (!$added_e['status']) {
        ...
        foreach ($this->Ledger->fetch(array(
            'linkup' => $_POST['playerid'],
            'linktype IN (' . join(',', $this->config->item('playerids___41303')) . ')' => null, //Clone Player Links
        ), array(), 0) as $x) {
    ...
    function new_idea()
    {
        ...
        if (!$_POST['link_ideaid'] && view_valid_handle_i($_POST['new_ideatext'])) {
            foreach ($this->Nodeideas->fetch(array(
                'LOWER(ideahashtag)' => strtolower(view_valid_handle_i($_POST['new_ideatext'])),
            )) as $i) {
        ...
        if ($_POST['link_ideaid'] > 0) {
            ...
            $x_i = $this->Nodeideas->fetch(array(
        ...
        return view_json($this->Nodeideas->create_or_link($_POST['focus_card'], $_POST['linktype'], trim($_POST['new_ideatext']), $member_e['playerid'], $_POST['focus_id'], $_POST['link_ideaid']));
    ...
    function new_player()
    {
        ...
        } elseif (!isset($_POST['player_existing_id']) || !isset($_POST['player_new_string']) || (intval($_POST['player_existing_id']) < 1 && strlen($_POST['player_new_string']) < 1)) {
            return view_json(array(
                'status' => 0,
        ...
        if ($adding_to_i) {
            ...
            //Validate Idea:
            $fetch_o = $this->Nodeideas->fetch(array(
        ...
        $is_upwards = in_array($_POST['linktype'], $this->config->item('playerids___14686'));

        if (!intval($_POST['player_existing_id']) && view_valid_handle_player($_POST['player_new_string'])) {
            foreach ($this->Nodeplayers->fetch(array(
                'LOWER(playerhandle)' => strtolower(substr($_POST['player_new_string'], 1)),
            )) as $e) {
                $_POST['player_existing_id'] = $e['playerid'];
            }
        }
        $adding_to_existing = (intval($_POST['player_existing_id']) > 0);
        ...
        if ($adding_to_existing) {
            ...
            $es = $this->Nodeplayers->fetch(array(
                'playerid' => $_POST['player_existing_id'],
            ));

            if (count($es) < 1) {
                return view_json(array(
                    'status' => 0,
                    'message' => 'Player @' . $_POST['player_existing_id'] . ' is not active',
                ));
            }
            ...
            $added_e = $this->Nodeplayers->create($_POST['player_new_string'], $player_e['playerid']);
            if (!$added_e['status']) {
                //We had an error, return it:
                return view_json($added_e);
            } else {
        ...
        //We need to check to ensure this is not a duplicate transaction if adding an existing Player:
        $ur2 = array();

        if ($adding_to_i) {

            //Add Reference:
            $ur2 = $this->Ledger->create(array(
                'linkcreator' => $player_e['playerid'],
                'linktype' => 4983, //Co-Author
                'linkup' => $focus_e['playerid'],
        ...
        return view_json(array(
            'status' => 1,
            'player_new_echo' => view_card_player($_POST['linktype'], array_merge($focus_e, $ur2), null),
        ));
    ...
    function e_editor_load()
    {
        ...
        } elseif (!access_level_player($es[0]['playerhandle'], 0, $es[0])) {
            return view_json(array(
                'status' => 0,
        ...
        foreach ($this->Ledger->fetch(array(
            'linkup IN (' . join(',', $this->config->item('playerids___42178')) . ')' => null, //Dynamic Players
            'linkdown' => $es[0]['playerid'],
            'linktype IN (' . join(',', $this->config->item('playerids___32292')) . ')' => null, //SOURCE LINKS
        ), array('linkup'), 0, 0, sort_by(42178)) as $player_group) {
            ...
            foreach ($this->Ledger->fetch(array(
                'linkdown' => $player_group['playerid'],
                'linkup IN (' . join(',', $this->config->item('playerids___42145')) . ')' => null, //Dynamic Input Templates
                'linktype IN (' . join(',', $this->config->item('playerids___32292')) . ')' => null, //SOURCE LINKS
            ), array('linkup'), 0, 0, $order_42145) as $player_template) {
                ...
                if (!is_array($this->config->item('players___' . $player_template['playerid']))) {
                    ...
                    $this->Ledger->create(array(
                        'linktype' => 44179, //Triggered
                        'linkup' => 4246, //Platform Bug Reports
                        'linkdown' => $player_template['playerid'],
                        'linktext' => 'player_editor_load() ERROR: @' . $player_template['playerid'] . ' is NOT in memory cache',
                    ));
                    continue;
    ...
    function e_editor_save()
    {
        ...
        //Update:
        $this->Nodeplayers->update($es[0]['playerid'], array(
        ...
        if ($_POST['save_playerid'] == $player_e['playerid']) {
            $this->Nodeplayers->activate_session($es[0], true);
        }
    ...
    function e_select_apply()
    {
            ...
            if (isset($_POST['selected_playerid']) && intval($_POST['selected_playerid']) > 0) {
                foreach ($this->Ledger->fetch(array(
                    ...
                    if (count($this->Ledger->fetch(array(
                        ...
                        $total_sent = $this->Ledger->send_idea_mass_dm(array($player_e), $i, website_setting(0), false);
        ...
        if (!$_POST['enable_mulitiselect'] || $_POST['was_previously_selected']) {
            ...
            foreach ($this->Ledger->fetch($query_filters, array('linkdown'), 0, 0) as $answer_e) {
                $stats['total']++;
            ...
            if ($_POST['down_playerid']) {
                $delete_query = $this->Ledger->fetch(array(
            ...
            foreach ($delete_query as $delete) {
                ...
                $this->Ledger->update($delete['linkid'], array(), $player_e['playerid']);
            }
        ...
        if ((!$_POST['enable_mulitiselect'] && $is_required) || !$_POST['was_previously_selected']) {
            if ($_POST['down_playerid']) {
                ...
                if (!count($this->Ledger->fetch(array(
                    'linktype IN (' . join(',', $this->config->item('playerids___31919')) . ')' => null, //IDEA AUTHOR
                    'linkup' => $_POST['selected_playerid'],
                    'linkright' => $_POST['right_ideaid'],
                )))) {
                    $stats['added']++;
        ...
        if ($_POST['down_playerid'] && count($player_e)) {
            $this->Nodeplayers->activate_session($player_e, true);
        }
    ...
    function e_contact_auth()
    {
        ...
        foreach ($this->Ledger->fetch(array(
            ...
            if (strtotime($sent_key['linktime']) <= (time() - 86400)) {
                //Expired
                $this->Ledger->update($sent_key['linkid'], array(), $_POST['account_id']); //Code Verified
                break;
            }
            ...
            if (strlen($session_key) && $key_parts[1] == md5($session_key . $_POST['input_code'])) {
                //Void access code:
                $is_authenticated = $this->Ledger->update($sent_key['linkid'], array(), $_POST['account_id']); //Code Verified
            }
        }
        if (!$is_authenticated) {
            return view_json(array(
        ...
        if ($_POST['account_id'] > 0) {
            ...
            $this->Nodeplayers->activate_session($es[0]);
            ...
            $player_result = $this->Nodeplayers->add_member(strstr($acc_email, '@', true), $acc_email, (!$is_email ? $_POST['account_email_phone'] : ''));
    ...
    function e_toggle_player()
    {
        ...
        if (!$player_e) {
            ...
            $already_added = $this->Ledger->fetch(array(
                'linkup' => $_POST['playerid'],
            ...
            if (count($already_added)) {
                ...
                if (intval($_POST['require_writing'])) {
                    ...
                    if (strlen($_POST['written_answer']) && trim($_POST['written_answer']) != $already_added[0]['linktext']) {
                        $this->Ledger->update($already_added[0]['linkid'], array(
                            'linktext' => $_POST['written_answer'],
                        ), $player_e['playerid']);
                    } elseif (!strlen($_POST['written_answer'])) {
                        $this->Ledger->update($already_added[0]['linkid'], array(), $player_e['playerid']);
                    }
    ...
    function e_verify_contact()
    {
        ...
        if (intval($_POST['sign_ideaid']) > 0) {
            //Fetch the idea:
            $referrer_i = $this->Nodeideas->fetch(array(
                'ideaid' => $_POST['sign_ideaid'],
            ));
        } else {
            $referrer_i = array();
    ...
    function x_view_load_page()
    {
        ...
        if ($_POST['focus__node'] == 12274) {
            ...
            //SOURCE
            $focus_es = $this->Nodeplayers->fetch(array(
                'playerid' => $_POST['focus__id'],
            ));
            $focus_e = $focus_es[0];

            foreach (view_player_covers($_POST['linktype'], $_POST['focus__id'], $_POST['current_page']) as $s) {
                if (in_array($_POST['linktype'], $this->config->item('playerids___11028'))) {
                    echo view_card_player($_POST['linktype'], $s);
                    $success = true;
                } else if ($_POST['linktype'] == 6255 || in_array($_POST['linktype'], $this->config->item('playerids___42284')) || in_array($_POST['linktype'], $this->config->item('playerids___42261')) || in_array($_POST['linktype'], $this->config->item('playerids___11020'))) {
                    echo view_card_i($_POST['linktype'], $s, $previous_i, null, $focus_e['playerid']);
                    $success = true;
                }
            }
            ...
            //IDEA
            $previous_is = $this->Nodeideas->fetch(array(
                'ideaid' => $_POST['focus__id'],
            ));
            $previous_i = $previous_is[0];

            foreach (view_idea_covers($_POST['linktype'], $_POST['focus__id'], $_POST['current_page']) as $s) {
                if (in_array($_POST['linktype'], $this->config->item('playerids___11020'))) {
                    echo view_card_i($_POST['linktype'], $s, $previous_i, null, $focus_e['playerid']);
                    $success = true;
                } else if ($_POST['linktype'] == 6255 || in_array($_POST['linktype'], $this->config->item('playerids___42261')) || in_array($_POST['linktype'], $this->config->item('playerids___42284')) || in_array($_POST['linktype'], $this->config->item('playerids___11028'))) {
                    echo view_card_player($_POST['linktype'], $s);
                    $success = true;
                }
            }
    ...
    function go_next()
    {
        ...
        foreach ($this->Nodeideas->fetch(array(
            ...
            $completion_status = $this->Ledger->mark_complete(idea_discovery_link($focus_i, $trying_to_skip), $player_e['playerid'], $_POST['target_ideaid'], $focus_i, $_POST['focus_idea_data'], array(
            ...
            foreach ($_POST['next_idea_data'] as $index => $next_idea_data) {
                ...
                foreach ($this->Nodeideas->fetch(array(
                    ...
                    $some_input_required = count($this->Ledger->fetch(array(
                        'linktype IN (' . join(',', $this->config->item('playerids___42991')) . ')' => null, //Active Writes
                        'linkright' => $idea_next['linkright'],
                        'linkup IN (' . join(',', $this->config->item('playerids___43050')) . ')' => null, //Input Required Ideas
                    )));
                    ...
                    if (!($idea_required && $trying_to_skip)) {
                        //Try to complete:
                        $completion_status = $this->Ledger->mark_complete(idea_discovery_link($idea_next, $trying_to_skip), $player_e['playerid'], $_POST['target_ideaid'], $idea_next, $next_idea_data, array(
            ...
            if (!$idea_redirect_url) {
                $find_next = $this->Ledger->find_next($player_e['playerid'], $_POST['target_ideahashtag'], $focus_i);
            }
    ...
    function x_update_instant_select()
    {
        ...
        if ($first_letter == '@' && strlen($_POST['migrate_s__handle']) > 1) {
            if (!count($this->Nodeplayers->fetch(array(
            ...
            if (!count($this->Nodeideas->fetch(array(
        ...
        if (is_array($_POST['o__id'])) {
            ...
            foreach ($_POST['o__id'] as $o__id) {
                array_push($mass_result, $this->Ledger->x_update_instant_select($_POST['focus__id'], $o__id, $_POST['element_id'], $_POST['new_playerid'], $_POST['migrate_s__handle'], $_POST['linkid']));
            }
            return view_json($mass_result);
        } else {
            return view_json($this->Ledger->x_update_instant_select($_POST['focus__id'], $_POST['o__id'], $_POST['element_id'], $_POST['new_playerid'], $_POST['migrate_s__handle'], $_POST['linkid']));
        }
    ...
    function x_4341()
    {
        ...
        $x = $this->Ledger->fetch($query_filters, $joined_by, view_memory(6404, 11064), $query_offset);
        $x_count = $this->Ledger->fetch($query_filters, $joined_by, 0, 0, array(), 'COUNT(linkid) as total_count');
    ...
    function refresh_wallet()
    {
        ...
        if ($has_handle) {
            ...
            $recursive_down_ids = $this->Nodeideas->recursive_down_ids($is[0], 'ALL');

            //List stats:
            $miscstats .= '<div>Tree Ideas: ' . number_format(count($recursive_down_ids['recursive_idea_ids']), 0) . '</div>';
        ...
        //Count transactions:
        $return_array = array();
        foreach ($this->config->item('players___33292') as $linktype1 => $m1) { //Gameplay
            $level1_total = 0;
            foreach ($this->config->item('players___' . $linktype1) as $linktype2 => $m2) { //Nodes/Links

                $player_pinned = player_pinned($linktype2, true);
                $level2_total = 0;
                if (!is_array($this->config->item('players___' . $player_pinned)) || !count($this->config->item('players___' . $player_pinned))) {
                    continue;
                }
                foreach ($this->config->item('players___' . $player_pinned) as $linktype3 => $m3) { //Player/Idea/Discovery
                    ...
                    if ($linktype2 == 12273) {
                        ...
                        if ($has_handle) {
                            ...
                            //See stats for this idea:
                            $sub_counter = $this->Nodeideas->fetch(array(
                                'ideaid IN (' . join(',', $recursive_down_ids['recursive_idea_ids']) . ')' => null,
                            ), 0, 0, array(), 'COUNT(ideaid) as totals');

                        } else {

                            $sub_counter = $this->Nodeideas->fetch(array(), 0, 0, array(), 'COUNT(ideaid) as totals');
                        ...
                        if ($has_handle) {
                            ...
                            $sub_counter = $this->Ledger->fetch(array(
                                'linktype IN (' . join(',', $this->config->item('playerids___33602')) . ')' => null, //Idea/Player Links Active
                                'linkright IN (' . join(',', $recursive_down_ids['recursive_idea_ids']) . ')' => null,
                            ), array('linkup'), 0, 0, array(), 'COUNT(linkid) as totals');

                        } else {

                            $sub_counter = $this->Nodeplayers->fetch(array(), 0, 0, array(), 'COUNT(playerid) as totals');
                        ...
                        if ($has_handle) {
                            ...
                            $sub_counter = $this->Ledger->fetch(array(
                                'linktype' => $linktype3,
                                '( linkleft IN (' . join(',', $recursive_down_ids['recursive_idea_ids']) . ') OR linkright IN (' . join(',', $recursive_down_ids['recursive_idea_ids']) . '))' => null,
                            ), array(), 0, 0, array(), 'COUNT(linkid) as totals');
