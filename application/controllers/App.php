...
class App extends CI_Controller
{
    ...
    function load($app_e__id = 14563 /* Error if none provided */){
        ...
        if($member_e && in_array($app_e__id, $this->config->item('n___14639'))){
            ...
            //Should redirect them:
            return redirect_message('/-4269?url='.urlencode($_SERVER['REQUEST_URI']), '<div class="msg alert alert-danger" role="alert"><span class="icon-block"><i class="fas fa-exclamation-circle"></i></span>App @'.$app_e__id.' is for members only</div>', true);
        }
        ...
        if(!$ui){
            ...
            if($memory_detected && !in_array($app_e__id, $this->config->item('n___14597'))){
                $ui .= '<h1>' . $e___6287[$app_e__id]['m__cover'] . ' ' . $e___6287[$app_e__id]['m__title'] . '</h1>';
