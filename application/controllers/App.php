...
class App extends CI_Controller
{
    ...
    function i_editor_load()
    {
        ...
        $this->Mench_ledger->create(array(
            'link_player' => $player_e['e__id'],
            'link_type' => 14576, //MODAL VIEWED
            'link_up' => 31911, //Edit Idea
            'link_right' => $i__id,
            'link_reference' => $_POST['link_id'],
            'link_metadata' => $return_array,
        ));
            ...
            if(in_array($_POST['link_type'], $this->config->item('n___42376')) && !access_level_i(null, $_POST['i__id'])){
                ...
                if(in_array($_POST['link_type'], $this->config->item('n___42261')) || in_array($_POST['link_type'], $this->config->item('n___42284'))){
                    ...
                    foreach(view__i_covers($_POST['link_type'], $_POST['i__id'], 1, false) as $e_e) {
                        if(isset($e_e['e__id'])){
                            $ui .= view__card(view__memory(42903,42902).$e_e['e__handle'], $current_e__handle && $e_e['e__handle']==$current_e__handle, $e_e['link_type'], $e_e['e__privacy'], view__cover($e_e['e__cover'], true), $e_e['e__title'], $e_e['link_text']);
                            $listed_items++;
                        }
                    }
                    ...
                    foreach(view__i_covers($_POST['link_type'], $_POST['i__id'], 1, false) as $next_i) {
                        if(isset($next_i['i__id'])){
                            $ui .= view__card($target_disccovery.view__memory(42903,33286).$next_i['i__hashtag'], $next_i['i__hashtag']==$current_i__hashtag, $next_i['link_type'], null, ( in_array($next_i['i__type'], $this->config->item('n___32172')) ? $e___4737[$next_i['i__type']]['m__cover'] : '' ), view__i_title($next_i, true), $next_i['link_text']);
                            $listed_items++;
                        }
                    }
            ...
            if(in_array($_POST['link_type'], $this->config->item('n___42376')) && !access_level_e(null, $_POST['e__id'])){
                ...
                if(in_array($_POST['link_type'], $this->config->item('n___11028'))){
                    ...
                    foreach(view__e_covers($_POST['link_type'], $_POST['e__id'], 1, false) as $e_e) {
                        if(isset($e_e['e__id'])){
                            $ui .= view__card(view__memory(42903,42902).$e_e['e__handle'], $e_e['e__handle']==$current_e__handle, $e_e['link_type'], $e_e['e__privacy'], view__cover($e_e['e__cover'], true), $e_e['e__title'], $e_e['link_text']);
                            $listed_items++;
                        }
                    }
    ...
    function e_delete(){
        ...
        //Archive Transaction:
        $this->Mench_ledger->update($_POST['link_id'], array(
            'link_privacy' => 6173,
    ...
    function e_copy(){
        ...
        foreach($this->Mench_ledger->fetch(array(
            ...
            if(!count($this->Mench_ledger->fetch(array(
                ...
                'link_reference' => $x['link_reference'],
                'link_privacy' => $x['link_privacy'],
                'link_metadata' => $x['link_metadata'],
            )))){
                $this->Mench_ledger->create(array(
                    'link_player' => $player_e['e__id'],
    ...
    function e_editor_load()
    {
        ...
        foreach($this->Mench_ledger->fetch(array(
            'link_down' => $_POST['e__id'],
            'link_type' => 10653, //Source Cover Update
            'link_privacy IN (' . join(',', $this->config->item('n___7360')) . ')' => null, //ACTIVE
        ), array(), 0, 0, array('link_id' => 'DESC')) as $x) {
            $link_metadata = unserialize($x['link_metadata']);
            if(strlen($link_metadata['before'])){
                $cover = ( substr_count($link_metadata['before'], 'class="') ? one_two_explode('class="','"',$link_metadata['before']) : $link_metadata['before'] );
    ...
    function e_contact_auth(){
        ...
        foreach($this->Mench_ledger->fetch(array(
            ...
            'link_privacy' => 6175, //Still Pending
            'link_text' => $_POST['account_email_phone'],
        ), array(), 1, 0, array('link_id' => 'DESC')) as $sent_key){

            $link_metadata = unserialize($sent_key['link_metadata']);
            $session_key = $this->session->userdata('session_key');

            if(strlen($session_key) && $link_metadata['hash_code']==md5($session_key.$_POST['input_code'])){

                //Complete access code:
                $is_authenticated = $this->Mench_ledger->update($sent_key['link_id'], array(
                    'link_privacy' => 6176, //Published
    ...
    function e_verify_contact(){
        ...
        if($valid_email) {
            ...
            $this->Mench_ledger->create(array(
                ...
                'link_type' => 32078, //Sign In Key
                'link_privacy' => 6175, //Pending until used (if used)
                'link_text' => $_POST['account_email_phone'],
                'link_metadata' => array(
    ...
    function x_mass_apply_preview()
    {
        ...
        $player_e = superpower_unlocked(null, 0, $this->player_e);
        $this->Mench_ledger->create(array(
            'link_player' => ( isset($player_e['e__id']) ? $player_e['e__id'] : 0 ),
            'link_type' => 14576, //MODAL VIEWED
            'link_up' => $_POST['apply_id'],
            'link_down' => ( $_POST['apply_id']==4997 ? $_POST['s__id'] : 0 ),
            'link_right' => ( $_POST['apply_id']==12589 ? $_POST['s__id'] : 0 ),
        ));
        ...
        if(count($x)>0){
            ...
            foreach($x as $x) {
                ...
                if($player_e && strlen($x['link_text'])>0 && strlen($_POST['link_text_find'])>0 && strlen($_POST['link_text_replace'])>0 && substr_count($x['link_text'], $_POST['link_text_find'])>0){
                    ...
                    ), $player_e['e__id'], 12360, update_description($x['link_text'], $new_content));
