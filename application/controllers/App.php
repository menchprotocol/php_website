...
class App extends CI_Controller
{
    ...
    function load($app_e__id = 14563 /* Error if none provided */){
        ...
        if($memory_detected && !in_array($app_e__id, $this->config->item('n___6287'))){
            return redirect_message('/@'.$app_e__id, '<div class="msg alert alert-danger" role="alert"><span class="icon-block">'.view_cover($es[0]['e__cover']).'</span>'.$es[0]['e__title'].' Is not an APP, yet 🤫</div>');
        }
