...
class App extends CI_Controller
{
    ...
    function load($app_e__id = 14563 /* Error if none provided */, $focus_handle = 0, $focus_hashtag = 0, $target_hashtag = 0)
    {
                ...
                if ($_GET['e__hash'] == view__hash($_GET['e__time'] . $focus_e['e__handle'])) {
                    ...
                    if ($focus_i) {
                        if (i_startable($focus_i)) {
                            ...
                            $this->Ledger->mark_complete(i__discovery_link($focus_i), $focus_e['e__id'], ($target_i ? $target_i['i__id'] : 0), $focus_i);
                            $this->Ledger->mark_complete(29393, $focus_e['e__id'], ($target_i ? $target_i['i__id'] : 0), $focus_i);
                    ...
                    if (!$player_e) {
                        $session_data = $this->Sources->activate_session($player_e, true);
                    }
        ...
        if ($new_cache) {
            $cache_x = $this->Ledger->write(array(
                'x__player' => $x__player,
                'x__type' => 14599, //Cache App
                'x__following' => $app_e__id,
                'x__message' => $ui,
        ...
        if ($app_e__id == 30795 && $target_i && $focus_i && $player_e && $target_i['i__hashtag'] == $focus_i['i__hashtag']) {
            ...
            if (!i_startable($target_i)) {
            ...
            } elseif (!$this->Ledger->i_has_started($player_e['e__id'], $target_i['i__hashtag'])) {
                ...
                //Mark as complete:
                $this->Ledger->mark_complete(i__discovery_link($target_i), $player_e['e__id'], $target_i['i__id'], $target_i);

                //Now return next idea:
                $next__url = $this->Ledger->find_next($player_e['e__id'], $target_i['i__hashtag'], $target_i);
                if ($next__url) {
                    //Go Next:
    ...
    function i_editor_save()
    {
        ...
        if ($is[0]['i__privacy'] == 42636) {
                    ...
                    foreach ($this->Ideas->read(array(
                        ...
                        foreach ($i_references as $reference_i) {
                            if (intval($_POST['next_i__id']) > 0) {
                                $status = $this->Ideas->i_link($focus_i, $_POST['save_x__type'], $reference_i, $player_e['e__id']);
                            } elseif (intval($_POST['previous_i__id']) > 0) {
                                $status = $this->Ideas->i_link($reference_i, $_POST['save_x__type'], $focus_i, $player_e['e__id']);
                            }
                            if (!$status['status']) {
            ...
            $this->Ideas->edit($is[0]['i__id'], array(
            ...
            for ($p = 1; $p <= view_memory(6404, 42206); $p++) {
                ...
                if (!strlen($dynamic_value)) {
                    ...
                    if (count($values) && $dynamic_e__id != 11035 /* HACK: Summary are key links that should not be removed */) {
                        $this->Ledger->edit($values[0]['x__id'], array(
        ...
        if (strlen($_POST['save_i__hashtag']) && $is[0]['i__hashtag'] !== trim($_POST['save_i__hashtag'])) {
            ...
            foreach ($this->Ledger->read(array(
                ...
                'x__type IN (' . njoin(42341) . ')' => null, //Idea References
        ...
        if ($_POST['save_x__id'] > 0 && $_POST['save_x__message'] != 'IGNORE_INPUT') {
            ...
            foreach ($this->Ledger->read(array(
                ...
                if ($this_x['x__message'] != trim($_POST['save_x__message'])) {
                    $this->Ledger->edit($this_x['x__id'], array(
    ...
    function i_copy()
    {
        ...
        return view_json($this->Ideas->recursive_clone(intval($_POST['i__id']), intval($_POST['do_recursive']), $player_e['e__id']));
    ...
    function i_sort_load()
    {
        ...
        foreach ($_POST['new_x_order'] as $x__weight => $x__id) {
            if (intval($x__id) > 0 && intval($x__weight) > 0) {
                ...
                if ($this->Ledger->edit(intval($x__id), array(
                    'x__weight' => $x__weight,
                ), $player_e['e__id'], 4603)) {
                    $updated++;
                }
    ...
    function e_delete()
    {
        ...
        //Archive Transaction:
        $this->Ledger->edit($_POST['x__id'], array(
            'x__privacy' => 6173,
    ...
    function e_copy()
    {
        ...
        //Create:
        $added_e = $this->Sources->verify_write($_POST['copy_source_title'], $player_e['e__id'], $fetch_o[0]['e__cover']);
        if (!$added_e['status']) {
        ...
        foreach ($this->Ledger->read(array(
            'x__following' => $_POST['e__id'],
            'x__type IN (' . njoin(41303) . ')' => null, //Clone Source Links
        ...
        //ideas:
        foreach ($this->Ledger->read(array(
            'x__type IN (' . njoin(41302) . ')' => null, //Clone Idea Source Links
            'x__following' => $_POST['e__id'],
        ), array('x__follower'), 0) as $x) {
            if (!count($this->Ledger->read(array(
                'x__type' => $x['x__type'],
    ...
    function i__add()
    {
        ...
        return view_json($this->Ideas->create_or_link($_POST['focus_card'], $_POST['x__type'], trim($_POST['new_i__message']), $member_e['e__id'], $_POST['focus_id'], $_POST['link_i__id']));
    ...
    function e__add()
    {
        ...
        if ($adding_to_existing) {
            ...
            $added_e = $this->Sources->verify_write($_POST['e_new_string'], $player_e['e__id']);
            if (!$added_e['status']) {
                //We had an error, return it:
                return view_json($added_e);
            } else {
    ...
    function e_editor_load()
    {
        ...
        foreach ($this->Ledger->read(array(
            'x__following IN (' . njoin(42178) . ')' => null, //Dynamic sources
            ...
            foreach ($this->Ledger->read(array(
                ...
                'x__following IN (' . njoin(42145) . ')' => null, //Dynamic Input Templates
                'x__type IN (' . njoin(32292) . ')' => null, //SOURCE LINKS
        ...
        if (!array_intersect($scanned_sources, $this->config->item('n___42885'))) {
            foreach ($this->Sources->read(array(
                'e__id IN (' . njoin(42776) . ')' => null, //Universal Dynamic Inputs
            )) as $selected_e) {
    ...
    function e_editor_save()
    {
        ...
        //Update:
        $this->Sources->edit($es[0]['e__id'], array(
        ...
        if ($_POST['save_e__id'] == $player_e['e__id']) {
            $this->Sources->activate_session($es[0], true);
        }
    ...
    function e_select_apply()
    {
            ...
            if (isset($_POST['selected_e__id']) && intval($_POST['selected_e__id']) > 0) {
                foreach ($this->Ledger->read(array(
                    ...
                    if (count($this->Ledger->read(array(
                        ...
                        $total_sent = $this->Ledger->send_i_mass_dm(array($player_e), $i, website_setting(0), false);
        ...
        if (!$_POST['enable_mulitiselect'] || $_POST['was_previously_selected']) {
            ...
            foreach ($this->Ledger->read($query_filters, array('x__follower'), 0, 0) as $answer_e) {
                $stats['total']++;
            ...
            if ($_POST['down_e__id']) {
                $delete_query = $this->Ledger->read(array(
        ...
        if ($_POST['down_e__id'] && count($player_e)) {
            $this->Sources->activate_session($player_e, true);
        }
    ...
    function e_contact_auth()
    {
        ...
        foreach ($this->Ledger->read(array(
            ...
            if (strlen($session_key) && $x__metadata['hash_code'] == md5($session_key . $_POST['input_code'])) {
                ...
                $is_authenticated = $this->Ledger->edit($sent_key['x__id'], array(
        ...
        if ($_POST['account_id'] > 0) {
            ...
            $this->Sources->activate_session($es[0]);
            ...
            $player_result = $this->Sources->add_member($_POST['new_username'], ($is_email ? $_POST['account_email_phone'] : $_POST['new_account_email']), (!$is_email ? $_POST['account_email_phone'] : ''));
    ...
    function e_toggle_e()
    {
        ...
        if (!$player_e) {
            ...
            $already_added = $this->Ledger->read(array(
                'x__following' => $_POST['e__id'],
            ...
            if (count($already_added)) {
                ...
                if (intval($_POST['require_writing'])) {
                    ...
                    if (strlen($_POST['written_answer']) && trim($_POST['written_answer']) != $already_added[0]['x__message']) {
                        $this->Ledger->edit($already_added[0]['x__id'], array(
    ...
    function x_mass_apply_preview()
    {
        ...
        if (!isset($_POST['apply_id']) || !isset($_POST['s__id'])) {
            ...
            if ($_POST['apply_id'] == 4997) {
                ...
                //idea list:
                $is_next = $this->Ledger->read(array(
    ...
    function go_next()
    {
        ...
        foreach ($this->Ideas->read(array(
            ...
            if ($input__selection) {
                ...
                foreach ($this->Ledger->read(array(
                    'i__type NOT IN (' . njoin(41055) . ')' => null, //Ignore paid answers since they cannot be removed!
                    ...
                    $this->Ledger->edit($x_selection['x__id'], array(
                        'x__privacy' => 6173, //Transaction Deleted
                    ...
                    foreach ($this->Ledger->read(array(
                        ...
                        $this->Ledger->edit($x_discovery['x__id'], array(
                            'x__privacy' => 6173, //Transaction Deleted
                        ), $player_e['e__id'], 12129 /* DISCOVERY ANSWER DELETED */);
            ...
            $completion_status = $this->Ledger->mark_complete(i__discovery_link($focus_i, $trying_to_skip), $player_e['e__id'], $_POST['target_i__id'], $focus_i, $_POST['focus_i_data'], array(
            ...
            foreach ($_POST['next_i_data'] as $index => $next_i_data) {
                ...
                foreach ($this->Ideas->read(array(
                    ...
                    if (in_array($i_next['i__type'], $this->config->item('n___43039')) || (!strlen($next_i_data['i__text']) && !count($next_i_data['uploaded_media']) && count($this->Ledger->read(array(
                                'i__type IN (' . njoin(43050) . ')' => null, //Input Required ideas
                                'x__type IN (' . njoin(42267) . ')' => null, //IDEA LINKS
                    ...
                    if (!($i_required && $trying_to_skip)) {
                        //Try to complete:
                        $completion_status = $this->Ledger->mark_complete(i__discovery_link($i_next, $trying_to_skip), $player_e['e__id'], $_POST['target_i__id'], $i_next, $next_i_data, array(
            ...
            if (!$i_redirect_url) {
                $find_next = $this->Ledger->find_next($player_e['e__id'], $_POST['target_i__hashtag'], $focus_i);
            }
    ...
    function x_update_instant_select()
    {
        ...
        if (is_array($_POST['o__id'])) {
            ...
            foreach ($_POST['o__id'] as $o__id) {
                array_push($mass_result, $this->Ledger->x_update_instant_select($_POST['focus__id'], $o__id, $_POST['element_id'], $_POST['new_e__id'], $_POST['migrate_s__handle'], $_POST['x__id']));
            }
            return view_json($mass_result);
        } else {
            return view_json($this->Ledger->x_update_instant_select($_POST['focus__id'], $_POST['o__id'], $_POST['element_id'], $_POST['new_e__id'], $_POST['migrate_s__handle'], $_POST['x__id']));
        }
    ...
    function x_4341()
    {
        ...
        $x = $this->Ledger->read($query_filters, $joined_by, view_memory(6404, 11064), $query_offset);
        $x_count = $this->Ledger->read($query_filters, $joined_by, 0, 0, array(), 'COUNT(x__id) as total_count');
    ...
    function x__refresh_gameplay()
    {
        ...
        if ($has_handle) {
            ...
            $recursive_down_ids = $this->Ideas->recursive_down_ids($is[0], 'ALL');
                ...
                foreach ($this->config->item('e___' . $e_pinned) as $x__type3 => $m3) { //Source/Idea/Discovery
                    ...
                    if ($x__type2 == 12273) {
                        ...
                        if ($has_handle) {
                            ...
                            $sub_counter = $this->Ledger->read(array(
                                'x__type IN (' . njoin(33602) . ')' => null, //Idea/Source Links Active
                        ...
                        if ($has_handle) {
                            ...
                            $sub_counter = $this->Sources->read(array(
                                'e__privacy' => $x__type3,
