...
function load_link_list(link_filters, link_join_by, page_num){
    //Show spinner:
    $('#link_page_'+page_num).html('<div style="margin:20px 0 100px 0;"><i class="fas fa-yin-yang fa-spin"></i> Loading...</div>').hide().fadeIn();
...
function load_link_connections(ln_id,load_main) {
    ...
    $('.link_connections_link_'+ln_id).html('<span><i class="fas fa-yin-yang fa-spin"></i> Loading...</span>').hide().fadeIn();
