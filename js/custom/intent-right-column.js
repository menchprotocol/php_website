...
function in_modify_load(in_id, ln_id) {
    ...
    $.post("/intents/in_load_data", {
        ...
        if (!data.status) {
            ...
            $('#in_engagement_level_entity_id').val(data.in.in_engagement_level_entity_id); //Set the status before it gets changed by miners
...
function in_modify_save() {
    ...
    var modify_data = {
        ...
        in_engagement_level_entity_id: parseInt($('#in_engagement_level_entity_id').val()),
    ...
    $.post("/intents/in_modify_save", modify_data, function (data) {
        ...
        if (!data.status) {
            ...
            if (data.remove_from_ui) {
                ...
                $('.in_engagement_level_entity_id_' + modify_data['in_id']).html('<span data-toggle="tooltip" data-placement="right" title="'+ js_en_all_7596[modify_data['in_engagement_level_entity_id']]['m_name'] + ': '+ js_en_all_7596[modify_data['in_engagement_level_entity_id']]['m_desc'] + '">'+ js_en_all_7596[modify_data['in_engagement_level_entity_id']]['m_icon'] +'</span>');
